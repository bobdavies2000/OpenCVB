...
int main(int argc, char * argv[]) try
{
    ...
    window app(width, height, "RealSense Align (Advanced) Example"); // Simple window handling
    ...
    int nextFunction = -1;
    bool heartBeat = true;
    bool optionsChanged = true;
    bool clickFlag = false;
    bool moveUpdated = false;
    bool displayDst0 = false;
    bool displayDst1 = false;
    cppTask* cPtr = 0;
    Rect textRect = Rect(0, 0, 200, 30);
    ...
    while (app) // Application still alive?
    {
        ...
        // new code for the CPP_IncludeOnly interface.
        if (frameCount % 30 == 0) heartBeat = true; else heartBeat = false;
        if (frameCount % 10 == 0)
        {
            cppTask_Close(cPtr);
            nextFunction++;
            if (nextFunction == 5) nextFunction = 0;
            heartBeat = true; // start new function on the beat.
            stringstream labelBuffer(512);
            stringstream buffer(512);
                
            // to customize, present these options in the user interface and reflect their value here.
            cPtr = (cppTask *) cppTask_Open(nextFunction, rows, cols, heartBeat, 0.5, 1, 16, 1, 32, 6000, 40, false, 5,
                                            true, 7, optionsChanged, 0, 10, 0, 0, clickFlag, 0, 0, 0, moveUpdated, 0, 0, 0, 0,
                                            displayDst0, displayDst1);
            optionsChanged = false;
        }

        // depthRGB is not captured in this Realsense configuration so just invert RGB.
        Mat depthRGB = Mat(rows, cols, CV_8UC3, (int*)other_frame.get_data()).clone();
        cvtColor(depthRGB, depthRGB, COLOR_BGR2RGB);
        cppTask_depthRGB(cPtr, (int*)depthRGB.data, rows, cols);


        Point3f accRadians = Point3f(0, 0, 0); // placeholder until the IMU is read...

        auto imagePtr = cppTask_RunCPP(cPtr, (int*)other_frame.get_data(), 3, 
                                            frameCount, rows, cols, accRadians.x, accRadians.y, accRadians.z,
                                            optionsChanged, heartBeat, displayDst0, displayDst1, 0.5f);
            
        Mat color = Mat(rows, cols, CV_8UC3, (int*)imagePtr, Mat::AUTO_STEP);
        color(textRect).setTo(Scalar(255,255,255));
        putText(color, functionNames[nextFunction], Point(10, 20), cPtr->font, 0.5, 0);
        
        imshow("color", color);
        frameCount++;

        // End of new code for CPP_IncludeOnly interface.
