...
Namespace VBClasses
    Public Class NR_Correlation_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim row = atask.mouseMovePoint.Y
            ...
            atask.pcSplit(0).CopyTo(dataX, mask)
            atask.pcSplit(1).CopyTo(dataY, mask)
            atask.pcSplit(2).CopyTo(dataZ, mask)

            Dim row1 = dataX.Row(row)
            Dim row2 = dataZ.Row(row)
            dst2.Line(New cv.Point(0, row), New cv.Point(dst2.Width, row), cv.Scalar.Yellow, atask.lineWidth + 1)
            ...
            If plotX.Count > 0 Then
                ...
                For i = 0 To plotX.Count - 1
                    ...
                    DrawCircle(dst3, New cv.Point(x, y), atask.DotSize, cv.Scalar.Yellow)
                Next
