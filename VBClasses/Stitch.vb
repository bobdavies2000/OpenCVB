...
Namespace VBClasses
    ...
    Public Class Stitch_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim mats As New List(Of Mat)
            dst2 = src.Clone()
            For i = 0 To options.imageCount - 1
                Dim x1 = CInt(task.msRNG.Next(0, src.Width - options.width))
                Dim x2 = CInt(task.msRNG.Next(0, src.Height - options.height))
                Dim rect = New cv.Rect(x1, x2, options.width, options.height)
                Rectangle(dst2, rect, Scalar.red, 2)
                mats.Add(src(rect).Clone())
            Next

            If sticherObj IsNot Nothing Then sticherObj = Stitcher.Create(Stitcher.Mode.Scans)
            Dim pano As New Mat
            ...
            Else
                If status = Stitcher.Status.ErrorNeedMoreImgs Then SetTrueText("Need more images", 3)
            End If
        End Sub
