...
Namespace VBClasses
    Public Class PCdiff_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If standalone Then src = tsk.pcSplit(2)
            ...
            Dim r3 = New cv.Rect(1, 1, tsk.cols - 2, tsk.rows - 2)
    ...
    Public Class NR_PCdiff_Edges : Inherits TaskParent
        ...
        Public Sub New()
            ...
            tsk.gOptions.DebugSlider.Maximum = 2
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            index = tsk.gOptions.DebugSlider.Value
            pcDiff.Run(tsk.pcSplit(index))
    ...
    Public Class PCdiff_Points : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            filter.Run(src)
            tsk.pcSplit = filter.dst3.Split()

            dst2.SetTo(0)
            Dim countInf As Integer
            Dim delta = filter.pcDiff.options1.pixelDiffThreshold / 1000
            For y = 0 To tsk.pcSplit(2).Rows - 1
                Dim slice = tsk.pcSplit(2).Row(y)
                ...
                For x = 0 To slice.Cols - 2
                    ...
                    If val <> 0 Then
                        If Math.Abs(val - lastVal) > delta Then
                            tsk.pcSplit(2).Set(Of Single)(y, x, 0) ' neighbors cannot have diff > delta.
                            dst2.Set(Of Byte)(y, x, 255)
                        End If
                    End If
                    lastVal = val
            ...
            tsk.noDepthMask = tsk.pcSplit(2).InRange(0, 0)
