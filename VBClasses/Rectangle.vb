...
Namespace VBClasses
    ...
    Public Class Rectangle_Intersection : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If standaloneTest() Then
                If taskAlg.heartBeat Then
                    rotatedCheck.Enabled = taskAlg.toggleOn
    ...
    Public Class Rectangle_Fit : Inherits TaskParent
        Public Sub New()
            If standalone Then taskAlg.drawRect = New cv.Rect(25, 25, 25, 35)
            desc = "Fit a rectangle into dst2 that maximizes the width or height of the rectangle"
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If taskAlg.optionsChanged Then dst2.SetTo(0)

            If src.Width = dst2.Width Then dst1 = src(taskAlg.drawRect) Else dst1 = src
