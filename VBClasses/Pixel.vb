...
Namespace PixelViewer
    Public Class Pixel_Viewer : Inherits TaskParent
        ...
        Public dst0Input As New cv.Mat(taskAlg.workRes, cv.MatType.CV_8UC3, 0)
        Public dst1Input As New cv.Mat(taskAlg.workRes, cv.MatType.CV_8UC3, 0)
        Public dst2Input As New cv.Mat(taskAlg.workRes, cv.MatType.CV_8UC3, 0)
        Public dst3Input As New cv.Mat(taskAlg.workRes, cv.MatType.CV_8UC3, 0)
        Enum displayTypes
            noType = -1
            type8uC3 = 0
            type8u = 1
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If standaloneTest() Then
                taskAlg.dstList(0) = dst0Input
                taskAlg.dstList(1) = dst1Input
                taskAlg.dstList(2) = dst2Input
                taskAlg.dstList(3) = dst3Input
            End If

            Dim dst = Choose(taskAlg.mousePicTag + 1, taskAlg.dstList(0), taskAlg.dstList(1), dst2Input, dst3Input)
            ...
            viewerForm.Text = "Pixel Viewer for " + Choose(taskAlg.mousePicTag + 1, "Color", "RGB Depth", "dst2", "dst3") + " " + formatType
            ...
            If viewerForm.mousePoint <> newPoint Then
                taskAlg.mouseMovePoint += viewerForm.mousePoint
                taskAlg.mouseMovePointUpdated = True
                viewerForm.mousePoint = New cv.Point
            End If
            If taskAlg.mouseMovePoint.X Or taskAlg.mouseMovePoint.Y Then
                Dim x As Integer = If(taskAlg.mouseMovePoint.X >= drWidth, taskAlg.mouseMovePoint.X - drWidth - 1, 0)
                Dim y As Integer = If(taskAlg.mouseMovePoint.Y >= drHeight, taskAlg.mouseMovePoint.Y - drHeight - 1, 0)
                If taskAlg.mouseMovePoint.X >= drWidth Then x += 2
                If taskAlg.mouseMovePoint.Y >= drHeight Then y += 2
                mouseLoc = New cv.Point(x, y)
            End If

            taskAlg.pixelViewerRect = New cv.Rect(0, 0, -1, -1)
            taskAlg.pixelViewTag = taskAlg.mousePicTag
            ...
            Dim imgText = ""
            Dim ClickPoint = New cv.Point(taskAlg.clickPoint.X - dw.X, taskAlg.clickPoint.Y - dw.Y)
            Select Case displayType
                ...
                Case displayTypes.type8u
                    ...
                    For y = 0 To img.Height - 1
                        imgText += "r" + Format(dw.Y + y, "000") + "   "
                        For x = 0 To img.Width - 1
                            If (taskAlg.toggleOn And y = ClickPoint.Y) And (x = ClickPoint.X - 1 Or x = ClickPoint.X) Then
...
Namespace VBClasses
    ...
    Public Class Pixel_Measure : Inherits TaskParent
        Public Sub New()
            Dim maxZ = taskAlg.MaxZmeters * 1000
        ...
        Public Function Compute(mmDist As Single) As Single
            Dim halfLineInMeters = Math.Tan(0.0174533 * taskAlg.hFov / 2) * mmDist
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If standaloneTest() Then
                If taskAlg.heartBeat Then
                    Dim w = 25, h = 25
                    If taskAlg.drawRect <> New cv.Rect Then
                        random.range = taskAlg.drawRect
                    Else
    ...
    Public Class Pixel_Unstable : Inherits TaskParent
        ...
        Public Sub New()
            taskAlg.featureOptions.ColorDiffSlider.Value = 2
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            unstable.Add(dst3)
            If unstable.Count > taskAlg.frameHistoryCount Then unstable.RemoveAt(0)
    ...
    Public Class Pixel_Zoom : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskAlg.mouseMovePoint <> newPoint Then mousePoint = taskAlg.mouseMovePoint
    ...
    Public Class Pixel_SubPixel : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskAlg.mouseMovePoint <> newPoint Then zoom.mousePoint = taskAlg.mouseMovePoint
            Dim width As Double = src.Width / zoomFactor
            Dim height As Double = src.Height / zoomFactor
    ...
    Public Class Pixel_NeighborsHorizontal : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To pt1.Count - 1
                vbc.DrawLine(dst2, pt1(i), pt2(i), cv.Scalar.Yellow)
            Next
    ...
    Public Class Pixel_NeighborsPatchNeighbors : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            End If
            cv.Cv2.Merge(taskAlg.pcSplit, dst3)
        End Sub
    End Class
    ...
    Public Class Pixel_MapLeftRight : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim tmp As cv.Mat = taskAlg.rightView
            If tmp.Channels = 3 Then tmp = tmp.CvtColor(cv.ColorConversionCodes.BGR2GRAY)
    ...
    Public Class Pixel_MapDistance : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            End If
            Dim tmp As cv.Mat = taskAlg.leftView
            If tmp.Channels = 3 Then tmp = tmp.CvtColor(cv.ColorConversionCodes.BGR2GRAY)
            cv.Cv2.ApplyColorMap(tmp, dst2, myColorMap)

            tmp = taskAlg.rightView
            If tmp.Channels = 3 Then tmp = tmp.CvtColor(cv.ColorConversionCodes.BGR2GRAY)
    ...
    Public Class Pixel_Display : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskAlg.heartBeat Then
                ...
                cv.Cv2.MeanStdDev(src(taskAlg.drawRect), mean, stdev)
    ...
    Public Class Pixel_ColorGuess : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskAlg.leftView.Channels = 1 Then
                cv.Cv2.ApplyColorMap(taskAlg.leftView, dst2, myColorMap)
                cv.Cv2.ApplyColorMap(taskAlg.rightView, dst3, myColorMap)
            Else
                cv.Cv2.ApplyColorMap(taskAlg.leftView.CvtColor(cv.ColorConversionCodes.BGR2GRAY), dst2, myColorMap)
                cv.Cv2.ApplyColorMap(taskAlg.rightView.CvtColor(cv.ColorConversionCodes.BGR2GRAY), dst3, myColorMap)
            End If
        End Sub
    End Class
