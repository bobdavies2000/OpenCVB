...
Public Class Pixel_Viewer : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        If standaloneTest() Then
            sharedResults.images.dstList(0) = task.color.Clone
            sharedResults.images.dstList(1) = task.depthRGB.Clone
            sharedResults.images.dstList(2) = New cv.Mat(task.workRes, cv.MatType.CV_8UC3, cv.Scalar.All(0))
            sharedResults.images.dstList(3) = New cv.Mat(task.workRes, cv.MatType.CV_8UC3, cv.Scalar.All(0))
        End If

        Dim dst = Choose(task.mousePicTag + 1, sharedResults.images.dstList(0), sharedResults.images.dstList(1),
                                               sharedResults.images.dstList(2), sharedResults.images.dstList(3))
