...
Namespace PixelViewer
    Public Class Pixel_Viewer : Inherits TaskParent
        ...
        Public dst0Input As New cv.Mat(taskA.workRes, cv.MatType.CV_8UC3, 0)
        Public dst1Input As New cv.Mat(taskA.workRes, cv.MatType.CV_8UC3, 0)
        Public dst2Input As New cv.Mat(taskA.workRes, cv.MatType.CV_8UC3, 0)
        Public dst3Input As New cv.Mat(taskA.workRes, cv.MatType.CV_8UC3, 0)
        Enum displayTypes
            noType = -1
            type8uC3 = 0
            type8u = 1
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If standaloneTest() Then
                taskA.dstList(0) = dst0Input
                taskA.dstList(1) = dst1Input
                taskA.dstList(2) = dst2Input
                taskA.dstList(3) = dst3Input
            End If

            Dim dst = Choose(taskA.mousePicTag + 1, taskA.dstList(0), taskA.dstList(1), dst2Input, dst3Input)
            ...
            viewerForm.Text = "Pixel Viewer for " + Choose(taskA.mousePicTag + 1, "Color", "RGB Depth", "dst2", "dst3") + " " + formatType
            ...
            If viewerForm.mousePoint <> newPoint Then
                taskA.mouseMovePoint += viewerForm.mousePoint
                taskA.mouseMovePointUpdated = True
                viewerForm.mousePoint = New cv.Point
            End If
            If taskA.mouseMovePoint.X Or taskA.mouseMovePoint.Y Then
                Dim x As Integer = If(taskA.mouseMovePoint.X >= drWidth, taskA.mouseMovePoint.X - drWidth - 1, 0)
                Dim y As Integer = If(taskA.mouseMovePoint.Y >= drHeight, taskA.mouseMovePoint.Y - drHeight - 1, 0)
                If taskA.mouseMovePoint.X >= drWidth Then x += 2
                If taskA.mouseMovePoint.Y >= drHeight Then y += 2
                mouseLoc = New cv.Point(x, y)
            End If

            taskA.pixelViewerRect = New cv.Rect(0, 0, -1, -1)
            taskA.pixelViewTag = taskA.mousePicTag
            ...
            Dim imgText = ""
            Dim ClickPoint = New cv.Point(taskA.clickPoint.X - dw.X, taskA.clickPoint.Y - dw.Y)
            Select Case displayType
                ...
                Case displayTypes.type8u
                    ...
                    For y = 0 To img.Height - 1
                        imgText += "r" + Format(dw.Y + y, "000") + "   "
                        For x = 0 To img.Width - 1
                            If (taskA.toggleOn And y = ClickPoint.Y) And (x = ClickPoint.X - 1 Or x = ClickPoint.X) Then
...
Namespace VBClasses
    ...
    Public Class Pixel_Measure : Inherits TaskParent
        Public Sub New()
            Dim maxZ = taskA.MaxZmeters * 1000
        ...
        Public Function Compute(mmDist As Single) As Single
            Dim halfLineInMeters = Math.Tan(0.0174533 * taskA.hFov / 2) * mmDist
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If standaloneTest() Then
                If taskA.heartBeat Then
                    Dim w = 25, h = 25
                    If taskA.drawRect <> New cv.Rect Then
                        random.range = taskA.drawRect
                    Else
    ...
    Public Class NR_Pixel_Unstable : Inherits TaskParent
        ...
        Public Sub New()
            taskA.featureOptions.ColorDiffSlider.Value = 2
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            unstable.Add(dst3)
            If unstable.Count > taskA.frameHistoryCount Then unstable.RemoveAt(0)
    ...
    Public Class Pixel_Zoom : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskA.mouseMovePoint <> newPoint Then mousePoint = taskA.mouseMovePoint
    ...
    Public Class Pixel_SubPixel : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskA.mouseMovePoint <> newPoint Then zoom.mousePoint = taskA.mouseMovePoint
            Dim width As Double = src.Width / zoomFactor
            Dim height As Double = src.Height / zoomFactor
    ...
    Public Class Pixel_Vector3D : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2.SetTo(0)
            For Each rc In redC.rcList
                taskA.color(rc.rect).CopyTo(dst2(rc.rect), rc.mask)
    ...
    Public Class NR_Pixel_MapLeftRight : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim tmp As cv.Mat = taskA.rightView
    ...
    Public Class NR_Pixel_MapDistance : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            End If
            Dim tmp As cv.Mat = taskA.leftView
            If tmp.Channels = 3 Then tmp = tmp.CvtColor(cv.ColorConversionCodes.BGR2GRAY)
    ...
    Public Class NR_Pixel_Display : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskA.heartBeat Then
                ...
                cv.Cv2.MeanStdDev(src(taskA.drawRect), mean, stdev)
    ...
    Public Class NR_Pixel_ColorGuess : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskA.leftView.Channels = 1 Then
                cv.Cv2.ApplyColorMap(taskA.leftView, dst2, myColorMap)
                cv.Cv2.ApplyColorMap(taskA.rightView, dst3, myColorMap)
            Else
                cv.Cv2.ApplyColorMap(taskA.leftView.CvtColor(cv.ColorConversionCodes.BGR2GRAY), dst2, myColorMap)
                cv.Cv2.ApplyColorMap(taskA.rightView.CvtColor(cv.ColorConversionCodes.BGR2GRAY), dst3, myColorMap)
            End If
        End Sub
    End Class
