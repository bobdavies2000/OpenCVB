...
Namespace VBClasses
    Public Class Bin4Way_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            Static index As Integer = atask.gridMap.Get(Of Integer)(atask.clickPoint.Y, atask.clickPoint.X)
            index = atask.gridMap.Get(Of Integer)(atask.clickPoint.Y, atask.clickPoint.X)
            Dim grSave = If(index < atask.gridRects.Count, atask.gridRects(index), New cv.Rect)

            If atask.optionsChanged Then index = 0
            ...
            Dim counts(3, atask.gridRects.Count) As Integer
            ...
            For i = 0 To counts.GetUpperBound(0)
                For j = 0 To atask.gridRects.Count - 1
                    Dim gr = atask.gridRects(j)
                    ...
                    means(j).Add(atask.gray(gr).Mean(tmp)(0))
            ...
            Dim bump = 3
            Dim ratio = dst2.Height / atask.gridRects(0).Height
            For i = 0 To matList.Count - 1
            ...
            dst1.Rectangle(grSave, white, atask.lineWidth)
            atask.color.Rectangle(grSave, white, atask.lineWidth)
        End Sub
    End Class
    ...
    Public Class NR_Bin4Way_Unstable1 : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst3 = diff.dst2
            If atask.heartBeat Then labels(3) = "There are " + CStr(dst3.CountNonZero) + " unstable pixels"
        End Sub
    End Class
    ...
    Public Class Bin4Way_UnstableEdges : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If atask.gOptions.DebugCheckBox.Checked = False Then dst3.SetTo(0, dst1)
        End Sub
    End Class
    ...
    Public Class NR_Bin4Way_UnstablePixels : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To pts.Count - 1 Step 2
                Dim val = atask.gray.Get(Of Byte)(pts(i + 1), pts(i))
            ...
            If atask.heartBeat Then labels(3) = "There are " + CStr(dst2.CountNonZero) + " unstable pixels"
        End Sub
    End Class
    ...
    Public Class NR_Bin4Way_SplitValley : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If atask.heartBeat Then valley.Run(atask.gray)

            mats.mat(0) = atask.gray.InRange(0, valley.valleys(1) - 1)
            mats.mat(1) = atask.gray.InRange(valley.valleys(1), valley.valleys(2) - 1)
            mats.mat(2) = atask.gray.InRange(valley.valleys(2), valley.valleys(3) - 1)
            mats.mat(3) = atask.gray.InRange(valley.valleys(3), 255)
    ...
    Public Class NR_Bin4Way_SplitGaps : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = Math.Min(mats.mat.Count, unstable.gapValues.Count) - 1 To 0 Step -1
                mats.mat(i) = atask.gray.InRange(unstable.gapValues(i), lastVal)
            ...
            dst2 = mats.dst2
            dst3 = mats.dst3
            If atask.heartBeat Then labels(1) = "There are " + CStr(dst1.CountNonZero) + " unstable pixels"
        End Sub
    End Class
    ...
    Public Class NR_Bin4Way_RegionsLeftRight : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            binaryRight.Run(atask.rightView)
    ...
    Public Class NR_Bin4Way_Regions1 : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim topColor = cv.Cv2.Mean(atask.gray, mask)(0)
            Dim botColor = cv.Cv2.Mean(atask.gray, Not mask)(0)
            mats.mat(0) = atask.gray.InRange(0, botColor)
            mats.mat(1) = atask.gray.InRange(botColor, midColor)
            mats.mat(2) = atask.gray.InRange(midColor, topColor)
            mats.mat(3) = atask.gray.InRange(topColor, 255)
    ...
    Public Class Bin4Way_SplitMean : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If atask.heartBeat Then
                ...
                topColor = cv.Cv2.Mean(atask.gray, mask)(0)
                botColor = cv.Cv2.Mean(atask.gray, Not mask)(0)
            End If
