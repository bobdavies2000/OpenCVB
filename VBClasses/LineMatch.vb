...
Namespace VBClasses
    Public Class LineMatch_Basics : Inherits TaskParent
        Public lpListLast As List(Of lpData) = New List(Of lpData)(tsk.lines.lpList)
        Public lpList As New List(Of lpData)
        Public lpMatches As New List(Of lpData)
        Dim slices As New LineMatch_Slices
        Public lpMapLast = tsk.lines.dst1.Clone
        Public Sub New()
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2.SetTo(0)
            dst3.SetTo(0)
            For i = 0 To Math.Min(tsk.lines.lpList.Count, slices.xSlices.Count) - 1
                ...
                For Each sliceSet In {slices.xSlices(i), slices.ySlices(i)}
                    ...
                    If angleDelta.Count > 0 Then
                        ...
                        If minAngleDelta < 5 Then ' within 5 degrees of the original line's angle
                            ...
                            dst2.Line(lp.p1, lp.p2, color, tsk.lineWidth + 2, tsk.lineType)
                            dst3.Line(lpMatch.p1, lpMatch.p2, color, tsk.lineWidth + 2, tsk.lineType)
                            lpList.Add(lp)
    ...
    Public Class NR_LineMatch_Tester : Inherits TaskParent
        ...
        Public Sub New()
            tsk.gOptions.DebugCheckBox.Checked = True
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Static lpListTest As New List(Of lpData)(tsk.lines.lpList)
            Static lpMapTest = tsk.lines.dst1.Clone
            If tsk.optionsChanged Or tsk.gOptions.DebugCheckBox.Checked Then
                lpListTest = New List(Of lpData)(tsk.lines.lpList)
                lpMapTest = tsk.lines.dst1.Clone
                srcLast = src.Clone
            End If

            ' tsk.gOptions.DebugCheckBox.Checked = tsk.heartBeatLT
            ...
            For i = 0 To match.lpList.Count - 1
                ...
                dst3.Line(lp.p1, lp.p2, color, tsk.lineWidth + 2, tsk.lineType)
                dst3.Line(lp.p1, lp.p2, white, tsk.lineWidth, tsk.lineType)
            Next
    ...
    Public Class LineMatch_Slices : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            Static lpMapLast As cv.Mat = tsk.lines.dst1.Clone

            dst2.SetTo(0)
            xSlices.Clear()
            ySlices.Clear()
            For i = 0 To Math.Min(tsk.lines.lpList.Count, 5) - 1
                Dim lp = tsk.lines.lpList(i)
                Dim color = tsk.scalarColors(lp.index + 1)
                dst2.Line(lp.p1, lp.p2, color, tsk.lineWidth + 2, tsk.lineType)
                ...
                dst2.Line(ptMinX, ptMaxX, white, tsk.lineWidth)
                ...
                dst2.Line(ptMinY, ptMaxY, white, tsk.lineWidth)

                ySlices.Add(SliceY.ToList)
            Next

            lpMapLast = tsk.lines.dst1.Clone
        End Sub
    End Class
