    ...
    Public Module Structures
        ...
        Public Class fPolyData
            ...
            Public Function computeCurrLengths() As Single
                ...
                For i = 0 To currPoly.Count - 1
                    d = currPoly(i).DistanceTo(currPoly((i + 1) Mod atask.polyCount))
                    currLength.Add(d)
                Next
            ...
            Public Function prevmp() As lpData
                Return New lpData(prevPoly(polyPrevSideIndex), prevPoly((polyPrevSideIndex + 1) Mod atask.polyCount))
            End Function
            Public Function currmp() As lpData
                If polyPrevSideIndex >= currPoly.Count - 1 Then polyPrevSideIndex = 0
                Return New lpData(currPoly(polyPrevSideIndex), currPoly((polyPrevSideIndex + 1) Mod atask.polyCount))
            End Function
            Public Sub jitterTest(dst As cv.Mat, parent As Object) ' return true if there is nothing to change
                ...
                Dim polymp = currmp()
                parent.DrawLine(jitterCheck, polymp.p1, polymp.p2, 255, atask.lineWidth)
        ...
        Public Class lpData
            ...
            Public Function perpendicularPoints(pt As cv.Point2f) As lpData
                ...
                Dim xShift = atask.brickSize * Math.Cos(angleRadians)
                Dim yShift = atask.brickSize * Math.Sin(angleRadians)
                Dim p1 = New cv.Point(pt.X + xShift, pt.Y + yShift)
                Dim p2 = New cv.Point(pt.X - xShift, pt.Y - yShift)
                If p1.X < 0 Then p1.X = 0
                If p1.X >= atask.color.Width Then p1.X = atask.color.Width - 1
                If p1.Y < 0 Then p1.Y = 0
                If p1.Y >= atask.color.Height Then p1.Y = atask.color.Height - 1
                If p2.X < 0 Then p2.X = 0
                If p2.X >= atask.color.Width Then p2.X = atask.color.Width - 1
                If p2.Y < 0 Then p2.Y = 0
                If p2.Y >= atask.color.Height Then p2.Y = atask.color.Height - 1
            ...
            Public Shared Function validatePoint(pt As cv.Point2f) As cv.Point2f
                ...
                If pt.X > atask.color.Width - 1 Then pt.X = atask.color.Width - 1
                If pt.Y < 0 Then pt.Y = 0
                If pt.Y > atask.color.Height - 1 Then pt.Y = atask.color.Height - 1
                Return pt
            End Function
            Sub New(_p1 As cv.Point2f, _p2 As cv.Point2f)
                ...
                p1GridIndex = atask.gridMap.Get(Of Integer)(p1.Y, p1.X)
                p2GridIndex = atask.gridMap.Get(Of Integer)(p2.Y, p2.X)
                color = atask.scalarColors(p1GridIndex Mod 255)

                pVec1 = atask.pointCloud.Get(Of cv.Vec3f)(p1.Y, p1.X)
                If Single.IsNaN(pVec1(0)) Or pVec1(2) = 0 Then
                    Dim r = atask.gridRects(p1GridIndex)
                    pVec1 = New cv.Vec3f(0, 0, atask.pcSplit(2)(r).Mean(atask.depthmask(r)).Item(0))
                End If

                pVec2 = atask.pointCloud.Get(Of cv.Vec3f)(p2.Y, p2.X)
                If Single.IsNaN(pVec2(0)) Or pVec2(2) = 0 Then
                    Dim r = atask.gridRects(p2GridIndex)
                    pVec2 = New cv.Vec3f(0, 0, atask.pcSplit(2)(r).Mean(atask.depthmask(r)).Item(0))
                End If
                ...
                If p1.X <> p2.X Then
                    ...
                    If p1.Y = p2.Y Then
                        ...
                        pE2 = New cv.Point2f(atask.workRes.Width - 1, p1.Y)
                    Else
                        Dim x1 = -b / slope
                        Dim x2 = (atask.workRes.Height - b) / slope
                        Dim y1 = b
                        Dim y2 = slope * atask.workRes.Width + b

                        Dim pts As New List(Of cv.Point2f)
                        If x1 >= 0 And x1 <= atask.workRes.Width Then pts.Add(New cv.Point2f(x1, 0))
                        If x2 >= 0 And x2 <= atask.workRes.Width Then pts.Add(New cv.Point2f(x2, atask.workRes.Height - 1))
                        If y1 >= 0 And y1 <= atask.workRes.Height Then pts.Add(New cv.Point2f(0, y1))
                        If y2 >= 0 And y2 <= atask.workRes.Height Then pts.Add(New cv.Point2f(atask.workRes.Width - 1, y2))
                        pE1 = pts(0)
                        If pts.Count < 2 Then
                            If CInt(x2) >= atask.workRes.Width Then pts.Add(New cv.Point2f(CInt(x2), atask.workRes.Height - 1))
                            If CInt(y2) >= atask.workRes.Height Then pts.Add(New cv.Point2f(atask.workRes.Width - 1, CInt(y2)))
                        End If
                        pE2 = pts(1)
                    End If
                Else
                    pE1 = New cv.Point2f(p1.X, 0)
                    pE2 = New cv.Point2f(p1.X, atask.workRes.Height - 1)
                End If
                ptCenter = New cv.Point2f((p1.X + p2.X) / 2, (p1.Y + p2.Y) / 2)

                Dim bpRow = atask.bricksPerRow - 1
                Dim bpCol = atask.bricksPerCol - 1
                If pE1.Y = 0 Then indexVTop = pE1.X / atask.workRes.Width * bpRow
                If pE1.Y = atask.workRes.Height - 1 Then indexVBot = pE1.X / atask.workRes.Width * bpRow

                If pE2.Y = 0 Then indexVTop = pE2.X / atask.workRes.Width * bpRow
                If pE2.Y = atask.workRes.Height - 1 Then indexVBot = pE2.X / atask.workRes.Width * bpRow

                If pE1.X = 0 Then indexHLeft = pE1.Y / atask.workRes.Height * bpCol
                If pE1.X = atask.workRes.Width - 1 Then indexHRight = pE1.Y / atask.workRes.Height * bpCol

                If pE2.X = 0 Then indexHLeft = pE2.Y / atask.workRes.Height * bpCol
                If pE2.X = atask.workRes.Width - 1 Then indexHRight = pE2.Y / atask.workRes.Height * bpCol
            ...
            Public Function displayCell(ByRef dst As cv.Mat) As String
                ...
                For Each lp In atask.lines.lpList
                    dst.Line(lp.p1, lp.
                             p2, white, atask.lineWidth, cv.LineTypes.Link8)
                    dst.Circle(lp.ptCenter, atask.DotSize, atask.highlight, -1)
                Next

                dst.Line(atask.lpD.p1, atask.lpD.p2, atask.highlight, atask.lineWidth + 1, atask.lineType)
            ...
            Public Sub New(_mask As cv.Mat, _rect As cv.Rect, _index As Integer, Optional minContours As Integer = 3)
                ...
                If contour.Count >= minContours Then
                    ...
                    If contour.Count >= 3 Then
                        ...
                        gridIndex = atask.gridMap.Get(Of Integer)(rect.TopLeft.Y + contour(0).Y,
                                                          rect.TopLeft.X + contour(0).X) Mod 255
                    Else
                        gridIndex = atask.gridMap.Get(Of Integer)(rect.TopLeft.Y, rect.TopLeft.X) Mod 255
                    End If
                    buildMaxDist()

                    color = atask.vecColors(index)
                    pixels = mask.CountNonZero
                    depth = atask.pcSplit(2)(rect).Mean(atask.depthmask(rect))(0)
                End If
            End Sub
