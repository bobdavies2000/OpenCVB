...
Public Class OEX_CalcBackProject_Demo1 : Inherits TaskParent
    ...
    Public Overrides sub RunAlg(src As cv.Mat)
        ...
        Dim hsv As cv.Mat = algTask.color.CvtColor(cv.ColorConversionCodes.BGR2HSV)
        cv.Cv2.CalcHist({hsv}, {0}, New cv.Mat, histogram, 1, {algTask.histogramBins}, ranges)
        ...
        Dim binW = dst2.Width / algTask.histogramBins
        ...
        Next
        If algTask.heartBeat Then labels(3) = $"The max value below is {peakValue}"
    End Sub
End Class
...
Public Class OEX_CalcBackProject_Demo2 : Inherits TaskParent
    ...
    Public Sub New()
        ...
        algTask.gOptions.setHistogramBins(6)
    ...
    Public Overrides sub RunAlg(src As cv.Mat)
        ...
        If algTask.ClickPoint <> newPoint Then
            ...
            count = cv.Cv2.FloodFill(dst2, mask2, algTask.ClickPoint, 255, Nothing, bounds, bounds, flags)
        ...
        Dim hsv As cv.Mat = algTask.color.CvtColor(cv.ColorConversionCodes.BGR2HSV)
        cv.Cv2.CalcHist({hsv}, {0, 1}, New cv.Mat, histogram, 2, {algTask.histogramBins, algTask.histogramBins}, ranges)
        classCount = histogram.CountNonZero
...
Public Class OEX_delaunay2 : Inherits TaskParent
    ...
    Public Sub locatePoint(img As cv.Mat, subdiv As cv.Subdiv2D, pt As cv.Point, activeColor As cv.Scalar)
        ...
        If e0 > 0 Then
            ...
            Do
                ...
                If subdiv.EdgeOrg(e, org) > 0 And subdiv.EdgeDst(e, dst) > 0 Then
                    img.Line(org, dst, activeColor, algTask.lineWidth + 3, algTask.lineType, 0)
                End If
        ...
        DrawCircle(img, pt, algTask.DotSize, activeColor)
    End Sub
    Public Overrides sub RunAlg(src As cv.Mat)
        If algTask.quarterBeat Then
            ...
            Else
                ...
                For i = 0 To facets.Count - 1
                    ...
                    Dim color = algTask.vecColors(i Mod 256)
                    dst3.FillConvexPoly(ifacet, color, 8, 0)

                    ifacets(0) = ifacet
                    cv.Cv2.Polylines(dst3, ifacets, True, New cv.Vec3b, algTask.lineWidth, algTask.lineType)
...
Public Class OEX_MeanShift : Inherits TaskParent
    ...
    Public Overrides sub RunAlg(src As cv.Mat)
        ...
        If algTask.optionsChanged Then
            ...
            cv.Cv2.CalcHist({hsv(roi)}, {0}, New cv.Mat, histogram, 1, {algTask.histogramBins}, ranges)
            histogram = histogram.Normalize(0, 255, cv.NormTypes.MinMax)
        End If
...
Public Class OEX_Filter2D : Inherits TaskParent
    ...
    Public Overrides sub RunAlg(src As cv.Mat)
        ...
        If algTask.heartBeat Then ind += 1
...
Public Class OEX_FitEllipse : Inherits TaskParent
    ...
    Public Sub New()
        Dim fileInputName As New FileInfo(algTask.homeDir + "opencv/samples/data/ellipses.jpg")
