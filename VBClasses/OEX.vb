...
Namespace VBClasses
    Public Class OEX_CalcBackProject_Demo1 : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim hsv As cv.Mat = tsk.color.CvtColor(cv.ColorConversionCodes.BGR2HSV)
            cv.Cv2.CalcHist({hsv}, {0}, New cv.Mat, histogram, 1, {tsk.histogramBins}, ranges)
            ...
            Dim binW = dst2.Width / tsk.histogramBins
            ...
            Next
            If tsk.heartBeat Then labels(3) = $"The max value below is {peakValue}"
        End Sub
    End Class
    ...
    Public Class NR_OEX_CalcBackProject_Demo2 : Inherits TaskParent
        ...
        Public Sub New()
            ...
            tsk.gOptions.setHistogramBins(6)
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If tsk.clickPoint <> newPoint Then
                ...
                count = cv.Cv2.FloodFill(dst2, mask2, tsk.clickPoint, 255, Nothing, bounds, bounds, flags)
            ...
            Dim hsv As cv.Mat = tsk.color.CvtColor(cv.ColorConversionCodes.BGR2HSV)
            cv.Cv2.CalcHist({hsv}, {0, 1}, New cv.Mat, histogram, 2, {tsk.histogramBins, tsk.histogramBins}, ranges)
            classCount = histogram.CountNonZero
    ...
    Public Class NR_OEX_BasicLinearTransformsTrackBar : Inherits TaskParent
        Dim options As New Options_BrightnessContrast
        Public Sub New()
    ...
    Public Class NR_OEX_delaunay2 : Inherits TaskParent
        ...
        Public Sub locatePoint(img As cv.Mat, subdiv As cv.Subdiv2D, pt As cv.Point, activeColor As cv.Scalar)
            ...
            If e0 > 0 Then
                ...
                Do
                    ...
                    If subdiv.EdgeOrg(e, org) > 0 And subdiv.EdgeDst(e, dst) > 0 Then
                        img.Line(org, dst, activeColor, tsk.lineWidth + 3, tsk.lineType, 0)
                    End If
            ...
            DrawCircle(img, pt, tsk.DotSize, activeColor)
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If tsk.quarterBeat Then
                ...
                Else
                    ...
                    For i = 0 To facets.Count - 1
                        ...
                        Dim color = tsk.vecColors(i Mod 256)
                        dst3.FillConvexPoly(ifacet, color, 8, 0)

                        ifacets(0) = ifacet
                        cv.Cv2.Polylines(dst3, ifacets, True, New cv.Vec3b, tsk.lineWidth, tsk.lineType)
    ...
    Public Class NR_OEX_MeanShift : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            Dim roi = If(tsk.drawRect.Width > 0, tsk.drawRect, New cv.Rect(0, 0, dst2.Width, dst2.Height))
            ...
            If tsk.optionsChanged Then
                ...
                cv.Cv2.CalcHist({hsv(roi)}, {0}, New cv.Mat, histogram, 1, {tsk.histogramBins}, ranges)
                histogram = histogram.Normalize(0, 255, cv.NormTypes.MinMax)
            End If
    ...
    Public Class NR_OEX_Threshold_Inrange : Inherits TaskParent
    ...
    Public Class NR_OEX_FitEllipse : Inherits TaskParent
        ...
        Public Sub New()
            Dim fileInputName As New FileInfo(tsk.homeDir + "opencv/samples/data/ellipses.jpg")
