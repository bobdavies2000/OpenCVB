...
Namespace VBClasses
    ...
    Public Class XR_OEX_delaunay2 : Inherits TaskParent
        ...
        End Sub
        Public Shared Sub locatePoint(img As Mat, subdiv As Subdiv2D, pt As cv.Point, activeColor As Scalar)
            Dim e0 As Integer = 0
    ...
    Public Class XR_OEX_MeanShift : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If trackWindow.Width <> 0 Then
                ...
                Rectangle(src, trackWindow, white, task.lineWidth, task.lineType)
            End If
        End Sub
    End Class
    ...
    Public Class XR_OEX_PointPolygon_demo : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim vert As New List(Of cv.Point)({New Point2f(3 * r / 2 + dst2.Width / 4, 1.34 * r),
                                               New Point2f(1 * r + dst2.Width / 4, 2 * r),
                                               New Point2f(3 * r / 2 + dst2.Width / 4, 2.866 * r),
                                               New Point2f(5 * r / 2 + dst2.Width / 4, 2.866 * r),
                                               New Point2f(3 * r + dst2.Width / 4, 2 * r),
                                               New Point2f(5 * r / 2 + dst2.Width / 4, 1.34 * r)})
            dst2.SetTo(0)
    ...
    Public Class XR_OEX_Filter2D : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim kernel As New Mat(kernelSize, kernelSize, MatType.CV_32F, Scalar.All(1 / (kernelSize * kernelSize)))
