...
Namespace VBClasses
    ...
    Public Class NR_Reduction_HeatMapLines1 : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim vecArray = atask.lines.getRawVecs(heat.dst2.CvtColor(cv.ColorConversionCodes.BGR2GRAY))
            Dim lplist = Line_Basics.getRawLines(vecArray)
            setupTop.Run(heat.dst2)
            dst2 = setupTop.dst2

            vecArray = atask.lines.getRawVecs(heat.dst3.CvtColor(cv.ColorConversionCodes.BGR2GRAY))
    ...
    Public Class NR_Reduction_XYZ : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            SetTrueText("atask.PointCloud (or 32fc3 input) has been reduced and is in dst3")
        End Sub
    End Class
    ...
    Public Class Reduction_PointCloud : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If src.Type <> cv.MatType.CV_32FC3 Then src = atask.pcSplit(2)

            src *= 255 / atask.MaxZmeters
