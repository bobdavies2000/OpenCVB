...
Namespace VBClasses
    Public Class GridRect_Basics : Inherits TaskParent
        ...
        Public Sub New()
            atask.gOptions.GridSlider.Value = dst2.Width \ 40 ' arbitrary but the goal is to get a reasonable (< 500) number of gr's.
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To stdevList.Count - 1
                ...
                Dim depthCheck = atask.noDepthMask(gr)
            ...
            If atask.heartBeat Then
                labels(2) = CStr(rects.Count) + " of " + CStr(atask.gridRects.Count) + " gr's had above average standard deviation (average = " +
    ...
    Public Class NR_GridRect_ColorSplit : Inherits TaskParent
        ...
        Public Sub New()
            devGrid.maskVal = 255
            atask.gOptions.GridSlider.Value = dst2.Width \ 40 ' arbitrary but the goal is to get a reasonable (< 500) number of roi's.
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To gather.stdevList.Count - 1
                ...
                If gather.stdevList(i) >= gather.stdevAverage Then
                    ...
                    If corr < atask.fCorrThreshold Then SetTrueText(Format(corr, fmt1), gr.TopLeft)
                    If corr < atask.fCorrThreshold Then motionCount += 1
                End If
            Next
            ...
            labels(2) = CStr(gather.rects.Count) + " of " + CStr(atask.gridRects.Count) + " gr's had above average standard deviation."
    ...
    Public Class GridRect_LowStdev : Inherits TaskParent
        ...
        Public Sub New()
            desc = "Isolate the gr's with low stdev"
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            dst1 = If(src.Channels() <> 1, src.CvtColor(cv.ColorConversionCodes.BGR2GRAY), src.Clone)
            gather.Run(dst1)
            ...
            For i = 0 To gather.stdevList.Count - 1
                ...
                End If
            Next
            If atask.heartBeat Then labels = {"", "", CStr(atask.gridRects.Count - gather.rects.Count) + " gr's had low standard deviation",
    ...
    Public Class NR_GridRect_LowStdevCorrelation : Inherits TaskParent
        ...
        Public Sub New()
            desc = "Display the correlation coefficients for gr's with low standard deviation."
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            options.Run()
            ...
            End If
            For i = 0 To saveRects.Count - 1
                If saveCorrs(i) < atask.fCorrThreshold Then SetTrueText(Format(saveCorrs(i), fmt2), saveRects(i).TopLeft)
                If saveCorrs(i) < atask.fCorrThreshold Then SetTrueText(Format(saveStdev(i), fmt2), saveRects(i).TopLeft, 3)
            Next
    ...
    Public Class NR_GridRect_LRClick : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst0 = src.Clone
            dst3 = If(atask.rightView.Channels() <> 3, atask.rightView.CvtColor(cv.ColorConversionCodes.GRAY2BGR), atask.rightView.Clone)
            src = src.CvtColor(cv.ColorConversionCodes.BGR2GRAY)
            If atask.rightView.Channels() <> 1 Then atask.rightView = atask.rightView.CvtColor(cv.ColorConversionCodes.BGR2GRAY)
            ...
            If atask.mouseClickFlag Then setClickPoint(atask.clickPoint, atask.mousePicTag)
            If ClickPoint = newPoint Then setClickPoint(gather.rects(gather.rects.Count / 2).TopLeft, 2)
            Dim gridIndex As Integer = atask.gridMap.Get(Of Integer)(ClickPoint.Y, ClickPoint.X)
            Dim gr = atask.gridRects(gridIndex)
            ...
            For j = 0 To gr.X - 1
                ...
                cv.Cv2.MatchTemplate(src(gr), atask.rightView(r), correlationMat, cv.TemplateMatchModes.CCoeffNormed)
            ...
            Else
                ...
                If maxCorr < atask.fCorrThreshold Then
                    SetTrueText("Correlation " + Format(maxCorr, fmt3) + " is less than " + Format(atask.fCorrThreshold, fmt1), 1)
                Else
                    ...
                    End If
                    dst3.Rectangle(rectRight, atask.highlight, atask.lineWidth)
                    dst0.Rectangle(gr, atask.highlight, atask.lineWidth)
                    dst1.SetTo(0)
                    DrawCircle(dst1, gr.TopLeft, atask.DotSize, atask.highlight)
                    DrawCircle(dst1, rectRight.TopLeft, atask.DotSize + 2, atask.highlight)
                    Dim pt = New cv.Point(rectRight.X, gr.Y + 5)
                    SetTrueText(CStr(offset) + " pixel offset" + vbCrLf + "Larger = Right", pt, 1)
                    SetTrueText(strOut, 1)
                    labels(3) = "Corresponding gr highlighted in yellow.  Average stdev = " + Format(gather.stdevAverage, fmt3)
                End If
            End If
        End Sub
    End Class
    ...
    Public Class NR_GridRect_LRAll : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each roi In gather.rects
                ...
                cv.Cv2.MatchTemplate(src(roi), atask.rightView(r), correlationMat, cv.TemplateMatchModes.CCoeffNormed)
                Dim mm = vbc.GetMinMax(correlationMat)
                If mm.maxVal >= atask.fCorrThreshold Then sortedRects.Add(mm.maxVal, New cv.Rect(mm.maxLoc.X, roi.Y, roi.Width, roi.Height))
            Next
            labels(2) = CStr(sortedRects.Count) + " roi's had left/right correlation higher than " + Format(atask.fCorrThreshold, fmt3)

            For Each roi In sortedRects.Values
                dst3.Rectangle(roi, atask.highlight, atask.lineWidth)
            Next
        End Sub
    End Class
