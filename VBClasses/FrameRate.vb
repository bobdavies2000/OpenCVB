...
Namespace VBClasses
    Public Class FrameRate_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            Static lastImages() As cv.Mat = {taskA.color.Clone, taskA.leftView.Clone,
                                             taskA.rightView.Clone, taskA.depthRGB.Clone}
            For i = 0 To frameCounts.Count - 1
                mats.mat(i) = Choose(i + 1, taskA.color, taskA.leftView, taskA.rightView, taskA.depthRGB).clone()
            ...
            If taskA.heartBeat Then
                ...
                For i = 0 To frameCounts.Count - 1
                    ...
                    strOut += Format(frameCounts(i), fmt0) + vbTab + " frameCount = " + CStr(taskA.frameCount) + vbCrLf
                Next
            End If
            SetTrueText(strOut, 3)
            mats.Run(emptyMat)
            dst2 = mats.dst2
            lastImages = {taskA.color.Clone, taskA.leftView.Clone, taskA.rightView.Clone, taskA.depthRGB.Clone}
        End Sub
    End Class
