...
Namespace VBClasses
    Public Class Edge_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If saveSelection <> taskAlg.edgeMethod Then
                saveSelection = taskAlg.edgeMethod
                Select Case taskAlg.edgeMethod
            ...
            dst2 = edges.dst2
            labels(2) = traceName + " - selection = " + taskAlg.edgeMethod
        End Sub
    End Class
    ...
    Public Class Edge_Preserving : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            End If
            If options.recurseCheck Then
                cv.Cv2.EdgePreservingFilter(taskAlg.depthRGB, dst3, cv.EdgePreservingMethods.RecursFilter, options.EP_Sigma_s, options.EP_Sigma_r)
            Else
                cv.Cv2.EdgePreservingFilter(taskAlg.depthRGB, dst3, cv.EdgePreservingMethods.NormconvFilter, options.EP_Sigma_s, options.EP_Sigma_r)
            End If
        End Sub
    End Class
    ...
    Public Class Edge_RandomForest_CPP : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskAlg.frameCount < 100 Then SetTrueText("On the first call only, it takes a few seconds to load the randomForest model.", New cv.Point(10, 100))

            ' why not do this in the constructor?  Because the message is held up by the lengthy process of loading the model.
            If taskAlg.frameCount = 5 Then
                Dim modelInfo = New FileInfo(taskAlg.homeDir + "Data/model.yml.gz")
                cPtr = Edge_RandomForest_Open(modelInfo.FullName)
            End If
            If taskAlg.frameCount > 5 Then ' the first images are skipped so the message above can be displayed.
    ...
    Public Class Edge_Consistent : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If taskAlg.optionsChanged Then saveFrames = New List(Of cv.Mat)

            edges.Run(src.Clone)

            Dim tmp = If(edges.dst2.Channels() = 1, edges.dst2.Clone, edges.dst2.CvtColor(cv.ColorConversionCodes.BGR2GRAY))
            saveFrames.Add(tmp)
            If saveFrames.Count > taskAlg.frameHistoryCount Then saveFrames.RemoveAt(0)
    ...
    Public Class Edge_SobelLRBinarized : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If taskAlg.mouseClickFlag Then taskAlg.mouseClickFlag = False ' preempt use of quadrants.

            edges.Run(taskAlg.leftView)
            dst2 = If(standaloneTest(), ShowAddweighted(edges.dst2, taskAlg.leftView, labels(2)), edges.dst2)

            edges.Run(taskAlg.rightView)
            dst3 = If(standaloneTest(), ShowAddweighted(edges.dst2, taskAlg.rightView, labels(3)), edges.dst2)
        End Sub
    End Class
    ...
    Public Class Edge_Matching : Inherits TaskParent
        ...
        Public Sub New()
            taskAlg.gOptions.GridSlider.Value = 16
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim maxLocs(taskAlg.gridRects.Count - 1) As Integer
            ...
            If options.overlayChecked Then
                dst2.SetTo(255, taskAlg.gridMask)
                dst3.SetTo(255, taskAlg.gridMask)
            End If
            ...
            Else
                ...
                If options.clearChecked Then
                    redRects.Clear()
                    taskAlg.gridROIclicked = 0
                    options.clearChecked = False
                End If
                If taskAlg.gridROIclicked Then
                    If redRects.Contains(taskAlg.gridROIclicked) = False Then redRects.Add(taskAlg.gridROIclicked)
    ...
    Public Class Edge_SobelLR : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            sobel.Run(taskAlg.rightView)
            dst3 = sobel.dst2.Clone()

            sobel.Run(taskAlg.leftView)
            dst2 = sobel.dst2
        End Sub
    End Class
    ...
    Public Class Edge_SobelCustomLeftRight : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            custom.Run(taskAlg.leftView)
            dst0 = custom.dst2.Clone
            dst1 = custom.dst3.Clone

            custom.Run(taskAlg.rightView)
            dst2 = custom.dst2
            dst3 = custom.dst3
        End Sub
    End Class
    ...
    Public Class Edge_RedCloud : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            canny.Run(taskAlg.redList.dst2)
    ...
    Public Class Edge_DiffX_CPP : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            segments.Run(taskAlg.pcSplit(0))
    ...
    Public Class Edge_DiffY_CPP : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            segments.Run(taskAlg.pcSplit(1))
            src = segments.dst1 ' the byte version of the segmented image.
    ...
    Public Class Edge_DiffZ_CPP : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            segments.Run(taskAlg.pcSplit(2))
            src = segments.dst1 ' the byte version of the segmented image.
    ...
    Public Class Edge_Sweep : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskAlg.heartBeatLT Then
                Dim index = taskAlg.featureOptions.EdgeMethods.SelectedIndex + 1
                If index >= taskAlg.featureOptions.EdgeMethods.Items.Count Then index = 0
                taskAlg.featureOptions.EdgeMethods.SelectedIndex = index
            End If

            edges.Run(src)
            dst2 = edges.dst2

            strOut = "Current edge algorithm is " + taskAlg.featureOptions.EdgeMethods.SelectedText
            labels(2) = strOut
    ...
    Public Class Edge_MeanSubtraction : Inherits TaskParent
        ...
        Public Sub New()
            desc = "Use canny on the left image of the meanSubtraction taskAlg algorithm."
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            LRMeanSub.Run(src)
            ...
            If taskAlg.optionsChanged Then dst2 = canny.dst2.Clone Else canny.dst2.CopyTo(dst2, taskAlg.motionMask)
        End Sub
    End Class
    ...
    Public Class Edge_SobelQT : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            dst0 = taskAlg.grayStable.Sobel(cv.MatType.CV_32F, 1, 0, 3)
            dst1 = taskAlg.grayStable.Sobel(cv.MatType.CV_32F, 0, 1, 3)
            Dim diagonal = taskAlg.grayStable.Sobel(cv.MatType.CV_32F, 1, 1, 3)
    ...
    Public Class Edge_NoDepth : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst3.SetTo(0)
            dst2.CopyTo(dst3, taskAlg.noDepthMask)

            dst1.SetTo(0)
            dst2.CopyTo(dst1, taskAlg.depthMask)
        End Sub
    End Class
    ...
    Public Class Edge_Stability : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To gEdges.featureRects.Count - 1
                ...
                pops.Add(pop)
                popSorted.Add(pop, i)
                dst2.Rectangle(roi, 255, taskAlg.lineWidth)
            Next
            ...
            Dim gSize = taskAlg.brickSize
            Dim pt = New cv.Point(gEdges.featureRects(index).X + gSize / 2, gEdges.featureRects(index).Y + gSize / 2)
            dst2.Circle(pt, gSize * 1.5, 255, taskAlg.lineWidth * 2)

            dst3.SetTo(0)
            dst3.Circle(pt, gSize * 1.5, 255, taskAlg.lineWidth * 2)
            Dim count As Integer
            For Each index In popSorted.Values
                dst3.Rectangle(gEdges.featureRects(index), white, taskAlg.lineWidth)
                count += 1
    ...
    Public Class Edge_LeftRightDepth : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim lp = New lpData(New cv.Point(0, taskAlg.mouseMovePoint.Y),
                            New cv.Point(dst2.Width, taskAlg.mouseMovePoint.Y))
            ...
            For i = 0 To tmp.Rows - 1
                ...
                If depth > 0 Then
                    ptRight.Add(pt)
                    SetTrueText(Format(depth, fmt1), New cv.Point(pt.X, taskAlg.mouseMovePoint.Y), 1)
                End If
            Next
    ...
    Public Class Edge_LeftRightBrick : Inherits TaskParent
        ...
        Public Sub New()
            ...
            taskAlg.gOptions.GridSlider.Value *= 2
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            labels(2) = CStr(count) + " (of " + CStr(taskAlg.bricks.brickList.Count) +
    ...
    Public Class Edge_CannyLeftRight : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            canny.Run(taskAlg.leftView)
            dst2 = canny.dst2.Clone

            canny.Run(taskAlg.rightView)
            dst3 = canny.dst2
        End Sub
    End Class
    ...
    Public Class Edge_MotionFrames : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            labels(2) = "Accumulated edges over " + CStr(taskAlg.frameHistoryCount) + " frames."
        End Sub
    End Class
End Namespace