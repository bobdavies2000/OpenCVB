...
Namespace VBClasses
    ...
    Public Class NR_BackProject_Reduction : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2 = bProject.dst2
            dst3 = bProject.dst3
            labels(2) = "Reduction = " + CStr(reduction.options.simpleReductionValue) + " and bins = " + CStr(tsk.histogramBins)
        End Sub
    End Class
    ...
    Public Class NR_BackProject_PointCloud : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.CalcBackProject({tsk.pointCloud}, {0, 2}, dst0, mask, hist.rangesX)
            mask.ConvertTo(mask, cv.MatType.CV_8U)
            tsk.pointCloud.CopyTo(dst2, mask)

            cv.Cv2.CalcBackProject({tsk.pointCloud}, {1, 2}, dst1, mask, hist.rangesY)
            mask.ConvertTo(mask, cv.MatType.CV_8U)
            tsk.pointCloud.CopyTo(dst3, mask)
        End Sub
    End Class
    ...
    Public Class BackProject_Side : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.CalcBackProject({tsk.pointCloud}, tsk.channelsSide, histSide.histogram, dst3, tsk.rangesSide)
    ...
    Public Class BackProject_Top : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.CalcBackProject({tsk.pointCloud}, tsk.channelsTop, histTop.histogram, dst1, tsk.rangesTop)
    ...
    Public Class NR_BackProject_Horizontal : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            bpTop.Run(src)
            tsk.pointCloud.SetTo(0, bpTop.dst3)
    ...
    Public Class BackProject_SoloTop : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.CalcBackProject({tsk.pointCloud}, tsk.channelsTop, histTop.histogram, dst3, tsk.rangesTop)
            dst3 = dst3.Threshold(0, 255, cv.ThresholdTypes.Binary).ConvertScaleAbs
        End Sub
    End Class
    ...
    Public Class BackProject_Image : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If tsk.kalman.kInput.Length <> 2 Then ReDim tsk.kalman.kInput(2 - 1)
            tsk.kalman.kInput(0) = hist.mm.minVal
            tsk.kalman.kInput(1) = hist.mm.maxVal
            tsk.kalman.Run(emptyMat)
            hist.mm.minVal = Math.Min(tsk.kalman.kOutput(0), tsk.kalman.kOutput(1))
            hist.mm.maxVal = Math.Max(tsk.kalman.kOutput(0), tsk.kalman.kOutput(1))

            Dim totalPixels = dst2.Total ' assume we are including zeros.
            If hist.plotHist.removeZeroEntry Then totalPixels = tsk.gray.CountNonZero

            Dim brickWidth = dst2.Width / tsk.histogramBins
            Dim incr = (hist.mm.maxVal - hist.mm.minVal) / tsk.histogramBins
            Dim histIndex = Math.Floor(tsk.mouseMovePoint.X / brickWidth)

            Dim minRange = New cv.Scalar(histIndex * incr)
            Dim maxRange = New cv.Scalar((histIndex + 1) * incr + 1)
            If histIndex + 1 = tsk.histogramBins Then
                minRange = New cv.Scalar(254)
                maxRange = New cv.Scalar(255)
            End If
            If useInrange Then
                If histIndex = 0 And hist.plotHist.removeZeroEntry Then
                    mask = New cv.Mat(tsk.grayStable.Size(), cv.MatType.CV_8U, cv.Scalar.All(0))
                Else
                    mask = tsk.grayStable.InRange(minRange, maxRange)
                End If
            Else
                Dim bRange = New cv.Rangef(minRange(0), maxRange(0))
                Dim ranges() = New cv.Rangef() {bRange}
                cv.Cv2.CalcBackProject({tsk.grayStable}, {0}, hist.histogram, mask, ranges)
            End If
            dst3 = src
            ...
            dst2.Rectangle(New cv.Rect(CInt(histIndex * brickWidth), 0, brickWidth, dst2.Height), cv.Scalar.Yellow, tsk.lineWidth)
        End Sub
    End Class
    ...
    Public Class NR_BackProject_MeterByMeter : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If tsk.heartBeat Then
                ...
                histogram = cv.Mat.FromPixelData(tsk.histogramBins, 1, cv.MatType.CV_32F, histData.ToArray)
            End If
            Dim ranges() = New cv.Rangef() {New cv.Rangef(0, tsk.histogramBins)}
            cv.Cv2.CalcBackProject({tsk.pcSplit(2)}, {0}, histogram, dst2, ranges)

            dst2.SetTo(0, tsk.noDepthMask)
            dst3 = PaletteFull(dst2.ConvertScaleAbs)
            labels(2) = "CV_8U backprojection up to " + CStr(tsk.histogramBins) + " meters."
        End Sub
    End Class
    ...
    Public Class NR_BackProject_MaskLines : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim vecArray = tsk.lines.getRawVecs(masks.mask)
            Dim lpList = Line_Basics.getRawLines(vecArray)

            For Each lp In lpList
                Dim val = masks.dst3.Get(Of Byte)(lp.p1.Y, lp.p1.X)
                If val = 255 Then dst2.Line(lp.p1, lp.p2, white, tsk.lineWidth, tsk.lineWidth)
            Next
            dst3.SetTo(cv.Scalar.Yellow, masks.mask)
            dst3.SetTo(tsk.highlight, dst1)
        End Sub
    End Class
    ...
    Public Class BackProject_Masks : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim brickWidth = dst2.Width / tsk.histogramBins
            histIndex = Math.Floor(tsk.mouseMovePoint.X / brickWidth)

            dst3 = tsk.color.Clone
            dst1 = maskDetect(tsk.gray, histIndex)
    ...
    Public Class NR_BackProject_MaskList : Inherits TaskParent
        ...
        Public Sub New()
            ...
            tsk.gOptions.setHistogramBins(40)
            tsk.gOptions.DebugSlider.Minimum = 0
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            Dim bins = If(tsk.histogramBins <= 255, tsk.histogramBins - 1, 255)
            Dim incr = 255 / bins
            If bins <> tsk.gOptions.DebugSlider.Maximum Then
                tsk.gOptions.DebugSlider.Value = 0
                tsk.gOptions.DebugSlider.Maximum = bins
            End If
            ...
            If standalone Then
                ...
                If tsk.heartBeat Then
                    ...
                    tsk.gOptions.DebugSlider.Value = depthIndex
                End If
            End If
            histList.Clear()
            histogramList.Clear()
            inputMatList.Clear()
            histS.ranges = {New cv.Rangef(0 - 0.01, tsk.MaxZmeters + 0.01)}
            For i = 0 To bins - 2
                Dim minVal = i * incr
                Dim maxVal = (i + 1) * incr
                histS.inputOnlyMask = tsk.gray.InRange(minVal, maxVal)
                histS.Run(tsk.pcSplit(2))
            ...
            Next
            Dim index = Math.Min(bins, tsk.gOptions.DebugSlider.Value)
            If index >= inputMatList.Count Then index = inputMatList.Count - 1
            Dim tmp = inputMatList(index)
            If tsk.heartBeat Then strOut = CStr(tmp.CountNonZero) + " mask pixels between " + CStr(incr * index) + " and " +
                                        CStr(incr * (index + 1)) + " from " + CStr(tsk.pcSplit(2).CountNonZero) + " depth pixels"
    ...
    Public Class NR_BackProject_FullOld : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim histogram As New cv.Mat
            cv.Cv2.CalcHist({tsk.grayStable}, {0}, New cv.Mat, histogram, 1, {classCount}, ranges)
    ...
    Public Class NR_BackProject_InRangeDepthTest : Inherits TaskParent
        Public classCount As Integer
        Public Sub New()
            tsk.gOptions.setHistogramBins(4)
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim binSize = tsk.MaxZmeters / tsk.histogramBins
            ...
            Else
                dst2 = tsk.pcSplit(2).InRange(minRange, maxRange).ConvertScaleAbs
                If maxRange >= tsk.MaxZmeters Then dst2 = dst2 Or tsk.maxDepthMask
            End If
            ...
            If tsk.heartBeatLT And tsk.frameCount > 1 Then index += 1
            If maxRange > tsk.MaxZmeters Then index = 0
        End Sub
    End Class
    ...
    Public Class NR_BackProject_InRangeDepth : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 1 To classCount
                ...
                dst1 = tsk.pcSplit(2).InRange(minRange, maxRange).ConvertScaleAbs * i
                If maxRange >= tsk.MaxZmeters Then dst1 = dst1 Or tsk.maxDepthMask
            ...
            dst3 = PaletteFull(dst2)
            labels(3) = "Below are the " + CStr(tsk.histogramBins) + " classes of depth data."
        End Sub
    End Class
    ...
    Public Class BackProject_FullDepth : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            src.SetTo(tsk.MaxZmeters, tsk.maxDepthMask)
            plotHist.Run(src)
            dst1 = plotHist.dst2
            ...
            If standaloneTest() Then
                Static index As Integer
                If tsk.heartBeatLT Then index += 1
    ...
    Public Class NR_BackProject_DepthSlider : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim index = Math.Abs(tsk.gOptions.DebugSlider.Value)
