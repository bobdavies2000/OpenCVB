...
Namespace VBClasses
    Public Class BackProject_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim incr = (hist.mm.maxVal - hist.mm.minVal) / taskAlg.histogramBins
            Dim histIndex = Math.Floor(taskAlg.mouseMovePoint.X / brickWidth)

            minRange = New cv.Scalar(histIndex * incr)
            maxRange = New cv.Scalar((histIndex + 1) * incr)
            If histIndex + 1 = taskAlg.histogramBins Then maxRange = New cv.Scalar(255)

            '     Dim ranges() = New cv.Rangef() {New cv.Rangef(minRange, maxRange)}
            '     cv.Cv2.CalcBackProject({taskAlg.gray}, {0}, histK.hist.histogram, dst0, ranges)
            ...
            dst2.Rectangle(New cv.Rect(CInt(histIndex) * brickWidth, 0, brickWidth, dst2.Height), cv.Scalar.Yellow, taskAlg.lineWidth)
        End Sub
    End Class
    ...
    Public Class BackProject_Reduction : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2 = bProject.dst2
            dst3 = bProject.dst3
            labels(2) = "Reduction = " + CStr(reduction.options.simpleReductionValue) + " and bins = " + CStr(taskAlg.histogramBins)
        End Sub
    End Class
    ...
    Public Class BackProject_FeatureLess : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2 = bProject.dst2
            dst3 = bProject.dst3
            labels(2) = "Bins = " + CStr(taskAlg.histogramBins)
        End Sub
    End Class
    ...
    Public Class BackProject_PointCloud : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.CalcBackProject({taskAlg.pointCloud}, {0, 2}, dst0, mask, hist.rangesX)
            mask.ConvertTo(mask, cv.MatType.CV_8U)
            taskAlg.pointCloud.CopyTo(dst2, mask)

            cv.Cv2.CalcBackProject({taskAlg.pointCloud}, {1, 2}, dst1, mask, hist.rangesY)
            mask.ConvertTo(mask, cv.MatType.CV_8U)
            taskAlg.pointCloud.CopyTo(dst3, mask)
        End Sub
    End Class
    ...
    Public Class BackProject_FullEqualized : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            equalize.Run(taskAlg.grayStable)
    ...
    Public Class BackProject_Side : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.CalcBackProject({taskAlg.pointCloud}, taskAlg.channelsSide, histSide.histogram, dst3, taskAlg.rangesSide)
    ...
    Public Class BackProject_Top : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.CalcBackProject({taskAlg.pointCloud}, taskAlg.channelsTop, histTop.histogram, dst1, taskAlg.rangesTop)
    ...
    Public Class BackProject_Horizontal : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            bpTop.Run(src)
            taskAlg.pointCloud.SetTo(0, bpTop.dst3)
    ...
    Public Class BackProject_SoloTop : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.CalcBackProject({taskAlg.pointCloud}, taskAlg.channelsTop, histTop.histogram, dst3, taskAlg.rangesTop)
            dst3 = dst3.Threshold(0, 255, cv.ThresholdTypes.Binary).ConvertScaleAbs
        End Sub
    End Class
    ...
    Public Class BackProject_Image : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskAlg.kalman.kInput.Length <> 2 Then ReDim taskAlg.kalman.kInput(2 - 1)
            taskAlg.kalman.kInput(0) = hist.mm.minVal
            taskAlg.kalman.kInput(1) = hist.mm.maxVal
            taskAlg.kalman.Run(emptyMat)
            hist.mm.minVal = Math.Min(taskAlg.kalman.kOutput(0), taskAlg.kalman.kOutput(1))
            hist.mm.maxVal = Math.Max(taskAlg.kalman.kOutput(0), taskAlg.kalman.kOutput(1))

            Dim totalPixels = dst2.Total ' assume we are including zeros.
            If hist.plotHist.removeZeroEntry Then totalPixels = taskAlg.gray.CountNonZero
            ...
            If histIndex + 1 = taskAlg.histogramBins Then
                minRange = New cv.Scalar(254)
                maxRange = New cv.Scalar(255)
            End If
            If useInrange Then
                If histIndex = 0 And hist.plotHist.removeZeroEntry Then
                    mask = New cv.Mat(taskAlg.grayStable.Size(), cv.MatType.CV_8U, cv.Scalar.All(0))
                Else
                    mask = taskAlg.grayStable.InRange(minRange, maxRange)
                End If
            Else
                Dim bRange = New cv.Rangef(minRange(0), maxRange(0))
                Dim ranges() = New cv.Rangef() {bRange}
                cv.Cv2.CalcBackProject({taskAlg.grayStable}, {0}, hist.histogram, mask, ranges)
            End If
            dst3 = src
    ...
    Public Class BackProject_MeterByMeter : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If taskAlg.heartBeat Then
                ...
                histogram = cv.Mat.FromPixelData(taskAlg.histogramBins, 1, cv.MatType.CV_32F, histData.ToArray)
            End If
            Dim ranges() = New cv.Rangef() {New cv.Rangef(0, taskAlg.histogramBins)}
            cv.Cv2.CalcBackProject({taskAlg.pcSplit(2)}, {0}, histogram, dst2, ranges)

            dst2.SetTo(0, taskAlg.noDepthMask)
            dst3 = PaletteFull(dst2.ConvertScaleAbs)
            labels(2) = "CV_8U backprojection up to " + CStr(taskAlg.histogramBins) + " meters."
        End Sub
    End Class
    ...
    Public Class BackProject_MaskLines : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each lp In rawLines.lpList
                ...
                If val = 255 Then dst2.Line(lp.p1, lp.p2, white, taskAlg.lineWidth, taskAlg.lineWidth)
            Next
            dst3.SetTo(cv.Scalar.Yellow, masks.mask)
            dst3.SetTo(taskAlg.highlight, dst1)
        End Sub
    End Class
    ...
    Public Class BackProject_Masks : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim brickWidth = dst2.Width / taskAlg.histogramBins
            histIndex = Math.Floor(taskAlg.mouseMovePoint.X / brickWidth)

            dst3 = taskAlg.color.Clone
            dst1 = maskDetect(taskAlg.gray, histIndex)
    ...
    Public Class BackProject_MaskList : Inherits TaskParent
        ...
        Public Sub New()
            ...
            taskAlg.gOptions.setHistogramBins(40)
            taskAlg.gOptions.DebugSlider.Minimum = 0
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            Dim bins = If(taskAlg.histogramBins <= 255, taskAlg.histogramBins - 1, 255)
            Dim incr = 255 / bins
            If bins <> taskAlg.gOptions.DebugSlider.Maximum Then
                taskAlg.gOptions.DebugSlider.Value = 0
                taskAlg.gOptions.DebugSlider.Maximum = bins
            End If
            ...
            If standalone Then
                ...
                If taskAlg.heartBeat Then
                    ...
                    taskAlg.gOptions.DebugSlider.Value = depthIndex
                End If
            End If
            histList.Clear()
            histogramList.Clear()
            inputMatList.Clear()
            histS.ranges = {New cv.Rangef(0 - 0.01, taskAlg.MaxZmeters + 0.01)}
            For i = 0 To bins - 2
                Dim minVal = i * incr
                Dim maxVal = (i + 1) * incr
                histS.inputOnlyMask = taskAlg.gray.InRange(minVal, maxVal)
                histS.Run(taskAlg.pcSplit(2))
            ...
            Next
            Dim index = Math.Min(bins, taskAlg.gOptions.DebugSlider.Value)
            If index >= inputMatList.Count Then index = inputMatList.Count - 1
            Dim tmp = inputMatList(index)
            If taskAlg.heartBeat Then strOut = CStr(tmp.CountNonZero) + " mask pixels between " + CStr(incr * index) + " and " +
                                        CStr(incr * (index + 1)) + " from " + CStr(taskAlg.pcSplit(2).CountNonZero) + " depth pixels"
    ...
    Public Class BackProject_FullOld : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            classCount = taskAlg.histogramBins

            Dim histogram As New cv.Mat
            cv.Cv2.CalcHist({taskAlg.grayStable}, {0}, New cv.Mat, histogram, 1, {classCount}, ranges)
    ...
    Public Class BackProject_InRangeDepthTest : Inherits TaskParent
        Public classCount As Integer
        Public Sub New()
            taskAlg.gOptions.setHistogramBins(4)
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim binSize = taskAlg.MaxZmeters / taskAlg.histogramBins
            ...
            Else
                dst2 = taskAlg.pcSplit(2).InRange(minRange, maxRange).ConvertScaleAbs
                If maxRange >= taskAlg.MaxZmeters Then dst2 = dst2 Or taskAlg.maxDepthMask
            End If
            ...
            If taskAlg.heartBeatLT And taskAlg.frameCount > 1 Then index += 1
            If maxRange > taskAlg.MaxZmeters Then index = 0
        End Sub
    End Class
    ...
    Public Class BackProject_InRangeDepth : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 1 To classCount
                ...
                dst1 = taskAlg.pcSplit(2).InRange(minRange, maxRange).ConvertScaleAbs * i
                If maxRange >= taskAlg.MaxZmeters Then dst1 = dst1 Or taskAlg.maxDepthMask
            ...
            dst3 = PaletteFull(dst2)
            labels(3) = "Below are the " + CStr(taskAlg.histogramBins) + " classes of depth data."
        End Sub
    End Class
    ...
    Public Class BackProject_Full : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If standaloneTest() Then
                If taskAlg.heartBeatLT Then index += 1
    ...
    Public Class BackProject_FullDepth : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If src.Type <> cv.MatType.CV_32F Then src = taskAlg.pcSplit(2).Clone
            classCount = taskAlg.histogramBins + 1
    ...
    Public Class BackProject_DepthSlider : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim index = Math.Abs(taskAlg.gOptions.DebugSlider.Value)
