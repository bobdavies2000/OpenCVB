...
Namespace VBClasses
    ...
    Public Class Blob_RenderBlobs : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            input.Run(src)
            dst2 = input.dst2
            If taskA.frameCount Mod input.updateFrequency = 0 Then
                Dim binary = taskA.gray.Threshold(0, 255, cv.ThresholdTypes.Otsu Or cv.ThresholdTypes.Binary)
                ...
                For Each b In cc.Blobs.Skip(1)
                    dst2.Rectangle(b.Rect, cv.Scalar.Red, taskA.lineWidth + 1, taskA.lineType)
                Next
                ...
                DrawCircle(dst3, New cv.Point(maxBlob.Centroid.X, maxBlob.Centroid.Y), taskA.DotSize + 3, cv.Scalar.Blue)
                DrawCircle(dst3, New cv.Point(maxBlob.Centroid.X, maxBlob.Centroid.Y), taskA.DotSize, cv.Scalar.Yellow)
            End If
        End Sub
    End Class
