...
Namespace VBClasses
    Public Class Corners_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            Dim featurePoints = New List(Of cv.Point)(tsk.featurePoints)
            fast.Run(src)

            Dim threshold = tsk.featurePoints.Count / 10

            dst2 = src
            dst3.SetTo(0)
            Dim newPts As New List(Of cv.Point)
            Dim new2f As New List(Of cv.Point2f)
            For i = 0 To tsk.featurePoints.Count - 1
                Dim pt = tsk.featurePoints(i)
            ...
            tsk.featurePoints = If(newPts.Count <= threshold, tsk.featurePoints, New List(Of cv.Point)(newPts))
            features = If(new2f.Count <= threshold, fast.features, New List(Of cv.Point2f)(new2f))
            labels(2) = Format(tsk.featurePoints.Count, "000") + " identified FAST stable points - slider adjusts threshold"
        End Sub
    End Class
    ...
    Public Class Corners_Core : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2 = src.Clone
            Dim kpoints() As cv.KeyPoint = cv.Cv2.FAST(tsk.gray, options.FASTthreshold, options.useNonMax)

            features.Clear()
            tsk.featurePoints.Clear()
            For Each kp As cv.KeyPoint In kpoints
                tsk.featurePoints.Add(New cv.Point(kp.Pt.X, kp.Pt.Y))
                features.Add(kp.Pt)
            Next
            ...
            If standaloneTest() Then
                dst3.SetTo(0)
                For Each kp As cv.KeyPoint In kpoints
                    DrawCircle(dst2, kp.Pt, tsk.DotSize, cv.Scalar.Yellow)
    ...
    Public Class Corners_Harris : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            mc = New cv.Mat(tsk.gray.Size(), cv.MatType.CV_32FC1, 0)
            dst2 = New cv.Mat(tsk.gray.Size(), cv.MatType.CV_8U, cv.Scalar.All(0))
            cv.Cv2.CornerEigenValsAndVecs(tsk.gray, dst2, options.blockSize, options.aperture, cv.BorderTypes.Default)

            For y = 0 To tsk.gray.Rows - 1
                For x = 0 To tsk.gray.Cols - 1
            ...
            For y = 0 To tsk.gray.Rows - 1
                For x = 0 To tsk.gray.Cols - 1
                    If mc.Get(Of Single)(y, x) > mm.minVal + (mm.maxVal - mm.minVal) * options.quality / options.qualityMax Then
                        DrawCircle(dst2, New cv.Point(x, y), tsk.DotSize, tsk.highlight)
                        count += 1
                    End If
                Next
            Next
    ...
    Public Class NR_Corners_PreCornerDetect : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.PreCornerDetect(tsk.gray, prob, options.kernelSize)

            cv.Cv2.Normalize(prob, prob, 0, 255, cv.NormTypes.MinMax)
            prob.ConvertTo(tsk.gray, cv.MatType.CV_8U)
            median.Run(tsk.gray.Clone())
            dst2 = tsk.gray.CvtColor(cv.ColorConversionCodes.GRAY2BGR)
            dst3 = tsk.gray.Threshold(160, 255, cv.ThresholdTypes.BinaryInv).CvtColor(cv.ColorConversionCodes.GRAY2BGR)
    ...
    Public Class Corners_ShiTomasi_CPP : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim data(tsk.gray.Total - 1) As Byte
            Dim handle = GCHandle.Alloc(data, GCHandleType.Pinned)
            Marshal.Copy(tsk.gray.Data, data, 0, data.Length)
    ...
    Public Class NR_Corners_BasicsCentroid : Inherits TaskParent
        ...
        Public Sub New()
            ...
            ReDim tsk.kalman.kInput(1) ' 2 elements - cv.point
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If m.M00 > 500 Then ' if more than x pixels are present (avoiding a zero area!)
                ...
                DrawCircle(dst2, New cv.Point(tsk.kalman.kOutput(0), tsk.kalman.kOutput(1)), 10, cv.Scalar.Red)
            End If
        End Sub
    End Class
    ...
    Public Class NR_Corners_BasicsCentroids : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            fast.Run(src)
            ReDim fastCenters(tsk.gridRects.Count - 1)
            For i = 0 To tsk.gridRects.Count - 1
                Dim gr = tsk.gridRects(i)
                Dim tmp = fast.dst3(gr).FindNonZero()
                If tmp.Rows > 0 Then
                    Dim mean = tmp.Mean()
                    fastCenters(i) = New cv.Point2f(gr.X + mean(0), gr.Y + mean(1))
                End If
            Next

            For i = 0 To fastCenters.Count - 1
                DrawCircle(dst2, fastCenters(i), tsk.DotSize, cv.Scalar.Yellow)
            Next
    ...
    Public Class NR_Corners_Harris_CPP : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim dataSrc(tsk.gray.Total - 1) As Byte
            Marshal.Copy(tsk.gray.Data, dataSrc, 0, dataSrc.Length)
            ...
            dst3 = ShowAddweighted(dst2.CvtColor(cv.ColorConversionCodes.GRAY2BGR), tsk.color, labels(3))
        End Sub
    ...
    Public Class Corners_HarrisDetector_CPP : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim dataSrc(tsk.gray.Total) As Byte
            Marshal.Copy(tsk.gray.Data, dataSrc, 0, dataSrc.Length)
            ...
            If ptCount > 1 Then
                ...
                For i = 0 To ptCount - 1
                    ...
                    DrawCircle(dst2, features(i), tsk.DotSize, cv.Scalar.Yellow)
                Next
            End If
        End Sub
        Public Overloads Sub Dispose() Implements IDisposable.Dispose
    ...
    Public Class NR_Corners_SubPix : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.CornerSubPix(tsk.gray, fast.features, New cv.Size(options.subpixSize, options.subpixSize),
