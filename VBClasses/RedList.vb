    ...
    Public Class NR_RedList_Features : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Select Case options.selection
                Case 0
                    Dim pt = rc.maxDist
                    dst2.Circle(pt, atask.DotSize, atask.highlight, -1, cv.LineTypes.AntiAlias)
                    labels(3) = "maxDist Is at (" + CStr(pt.X) + ", " + CStr(pt.Y) + ")"
                Case 1
                    dst3(rc.rect).SetTo(vbNearFar((rc.depth) / atask.MaxZmeters), rc.mask)
                    labels(3) = "rc.depth Is highlighted in dst2"
                    labels(3) = "Mean depth for the cell Is " + Format(rc.depth, fmt3)
                Case 2
                    cv.Cv2.MatchTemplate(atask.pcSplit(0)(rc.rect), atask.pcSplit(2)(rc.rect), correlationMat, cv.TemplateMatchModes.CCoeffNormed, rc.mask)
                ...
                Case 3
                    cv.Cv2.MatchTemplate(atask.pcSplit(1)(rc.rect), atask.pcSplit(2)(rc.rect), correlationMat, cv.TemplateMatchModes.CCoeffNormed, rc.mask)
    ...
    Public Class NR_RedList_CellDepthHistogram : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If atask.heartBeat Then
                Dim depth As cv.Mat = atask.pcSplit(2)(atask.oldrcD.rect)
                depth.SetTo(0, atask.noDepthMask(atask.oldrcD.rect))
                plot.minRange = 0
