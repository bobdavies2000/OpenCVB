...
Namespace VBClasses
    ...
    Public Class Random_Basics3D : Inherits TaskParent
        ...
        Public ranges() As Single = {0, dst2.Width, 0, dst2.Height, 0, taskA.MaxZmeters}
        Public Sub New()
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If standaloneTest() Then
                dst2.SetTo(0)
                For Each pt In PointList
                    DrawCircle(dst2, New cv.Point2f(pt.X, pt.Y), taskA.DotSize, cv.Scalar.Yellow)
                Next
            End If
    ...
    Public Class Random_Basics4D : Inherits TaskParent
        ...
        Public ranges() As Single = {0, dst2.Width, 0, dst2.Height, 0, taskA.MaxZmeters, 0, taskA.MaxZmeters}
        Dim options As New Options_Random
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If standaloneTest() Then
                ...
                For Each v In PointList
                    DrawCircle(dst2, New cv.Point2f(v(0), v(1)), taskA.DotSize, cv.Scalar.Yellow)
                Next
            End If
    ...
    Public Class NR_Random_LUTMask : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If taskA.heartBeat Or taskA.frameCount < 10 Then
                random.Run(src)
    ...
    Public Class NR_Random_KalmanPoints : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If refreshPoints Then
                ...
                If targetSet.Count * 2 <> taskA.kalman.kInput.Length Then ReDim taskA.kalman.kInput(targetSet.Count * 2 - 1)

            End If

            For i = 0 To targetSet.Count - 1
                Dim pt As cv.Point = targetSet(i)
                taskA.kalman.kInput(i * 2) = pt.X
                taskA.kalman.kInput(i * 2 + 1) = pt.Y
            Next

            taskA.kalman.Run(emptyMat)
            For i = 0 To taskA.kalman.kOutput.Count - 1 Step 2
                currSet(i / 2) = New cv.Point(taskA.kalman.kOutput(i), taskA.kalman.kOutput(i + 1))
            Next

            dst2.SetTo(0)
            For i = 0 To currSet.Count - 1
                DrawCircle(dst2, currSet(i), taskA.DotSize + 2, cv.Scalar.Yellow)
                DrawCircle(dst2, targetSet(i), taskA.DotSize + 2, cv.Scalar.Red)
            Next
    ...
    Public Class Random_Clusters : Inherits TaskParent
        ...
        Public Sub New()
            taskA.scalarColors(0) = cv.Scalar.Yellow
            taskA.scalarColors(1) = cv.Scalar.Blue
            taskA.scalarColors(2) = cv.Scalar.Red
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To options.numClusters - 1
                ...
                For j = 0 To options.numPoints - 1
                    ...
                    DrawCircle(dst2, pt, taskA.DotSize, taskA.scalarColors(i Mod 256))
