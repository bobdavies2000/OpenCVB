...
Namespace VBClasses
    ...
    Public Class Random_Basics3D : Inherits TaskParent
        ...
        Public ranges() As Single = {0, dst2.Width, 0, dst2.Height, 0, taskAlg.MaxZmeters}
        Public Sub New()
    ...
    Public Class Random_Basics4D : Inherits TaskParent
        ...
        Public ranges() As Single = {0, dst2.Width, 0, dst2.Height, 0, taskAlg.MaxZmeters, 0, taskAlg.MaxZmeters}
        Dim options As New Options_Random
            ...
            If taskAlg.paused = False Then
                ...
                If standaloneTest() Then
                    ...
                    For Each v In PointList
                        DrawCircle(dst2, New cv.Point2f(v(0), v(1)), taskAlg.DotSize, cv.Scalar.Yellow)
                    Next
                End If
    ...
    Public Class Random_LUTMask : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If taskAlg.heartBeat Or taskAlg.frameCount < 10 Then
                random.Run(src)
    ...
    Public Class Random_KalmanPoints : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If refreshPoints Then
                ...
                If targetSet.Count * 2 <> taskAlg.kalman.kInput.Length Then ReDim taskAlg.kalman.kInput(targetSet.Count * 2 - 1)

            End If

            For i = 0 To targetSet.Count - 1
                Dim pt As cv.Point = targetSet(i)
                taskAlg.kalman.kInput(i * 2) = pt.X
                taskAlg.kalman.kInput(i * 2 + 1) = pt.Y
            Next

            taskAlg.kalman.Run(emptyMat)
            For i = 0 To taskAlg.kalman.kOutput.Count - 1 Step 2
                currSet(i / 2) = New cv.Point(taskAlg.kalman.kOutput(i), taskAlg.kalman.kOutput(i + 1))
            Next

            dst2.SetTo(0)
            For i = 0 To currSet.Count - 1
                DrawCircle(dst2, currSet(i), taskAlg.DotSize + 2, cv.Scalar.Yellow)
                DrawCircle(dst2, targetSet(i), taskAlg.DotSize + 2, cv.Scalar.Red)
            Next
    ...
    Public Class Random_Clusters : Inherits TaskParent
        ...
        Public Sub New()
            taskAlg.scalarColors(0) = cv.Scalar.Yellow
            taskAlg.scalarColors(1) = cv.Scalar.Blue
            taskAlg.scalarColors(2) = cv.Scalar.Red
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To options.numClusters - 1
                ...
                For j = 0 To options.numPoints - 1
                    ...
                    DrawCircle(dst2, pt, taskAlg.DotSize, taskAlg.scalarColors(i Mod 256))
