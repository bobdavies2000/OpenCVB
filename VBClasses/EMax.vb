...
Public Class EMax_Basics : Inherits TaskParent
    ...
    Public eSamples As New List(Of Point2f)
    Public dimension = 2 ' point2f for the basics...
    Public regionCount As Integer
    Public centers As New List(Of Point2f)
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        If eLabels.Count = 0 Or useInputClusters Then
            ...
            eSamples = New List(Of Point2f)(emaxInput.eSamples)
            regionCount = emaxInput.regionCount
        End If
        If centers.Count = 0 Then centers = New List(Of Point2f)(emaxInput.centers)
        ...
        dst1 = Mat.FromPixelData(dst3.Rows, dst3.Cols, MatType.CV_32S, imagePtr).Clone
        dst1.ConvertTo(dst0, MatType.CV_8U)

        palette.colors.Clear()
        Dim newLabels(regionCount) As Vec3b
        For i = 0 To eLabels.Count - 1
            ...
            Dim c = palette.originalColorMap.Get(Of Vec3b)(0, original Mod 256)
            If newLabels.Contains(c) = False And newLabel <= regionCount Then newLabels(newLabel) = c
        Next
        palette.colors = New List(Of Vec3b)(newLabels)
        palette.Run(dst0)
        dst2 = palette.dst2
        centers = New List(Of Point2f)(emaxInput.centers)
    End Sub
...
Public Class XR_EMax_Centers : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        emax.Run(src)
        dst2 = emax.dst2
        Static lastCenters As New List(Of Point2f)(emax.centers)
        ...
        Circle(dst2, emax.centers(i), task.DotSize + 1, task.highlight, -1, task.lineType)
            ...
            Circle(dst2, lastCenters(i), task.DotSize + 2, Scalar.Black, -1, task.lineType)
            End If
        Next
        lastCenters = New List(Of Point2f)(emax.centers)
    End Sub
End Class
...
Public Class EMax_InputClusters : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        Dim samples = New Mat(regionCount * options.samplesPerRegion, 2, MatType.CV_32F).Reshape(2, 0)
        Dim eLabelMat = New Mat(regionCount * options.samplesPerRegion, 1, MatType.CV_32S)
        ...
        For i = 0 To regionCount - 1
            ...
            Randn(tmp, New Scalar(grid.gridWidth / 2, grid.gridHeight / 2),
...
' https://docs.opencvb.org/3.0-beta/modules/ml/doc/expectation_maximization.html
' https://github.com/opencv/opencv/blob/master/samples/cpp/em.cpp
Public Class XR_EMax_VB_Failing : Inherits TaskParent
    Implements IDisposable
    Public emaxInput As New EMax_InputClusters
    Public eLabels As New List(Of Integer)
    Public eSamples As New List(Of Point2f)
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        If em_model Is Nothing Then em_model = EM.Create()
        ...
        Dim samples = Mat.FromPixelData(eSamples.Count, 2, MatType.CV_32FC1, eSamples.ToArray)
        Dim eLabelsMat = Mat.FromPixelData(eLabels.Count, 1, MatType.CV_32S, eLabels.ToArray)
        em_model.TrainEM(samples, Nothing, eLabelsMat, Nothing)

        ' now classify every image pixel based on the samples.
        Dim sample As Mat = Mat.FromPixelData(1, 2, MatType.CV_32FC1, 0)  ' tried doubles but it fails as well...
...
Public Class XR_EMax_PointTracker : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        knn.queries = New List(Of Point2f)(emax.centers)
        knn.Run(src)
...
Public Class XR_EMax_RandomClusters : Inherits TaskParent
