...
Namespace VBClasses
    Public Class BackProject2D_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If standaloneTest() Then DrawRect(dst2, gRect, white)
            ...
            Else
                ...
                hist2d.histogram(gRect).CopyTo(histogram(gRect))
            End If
            cv.Cv2.CalcBackProject({colorFmt.dst2}, hist2d.channels, histogram, dst0, hist2d.ranges)

            Dim bpCount = hist2d.histogram(gRect).CountNonZero
    ...
    Public Class NR_BackProject2D_RowCol : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim gRect = task.gridRects(task.gridMap.Get(Of Integer)(task.mouseMovePoint.Y,
                                                                 task.mouseMovePoint.X))
            Dim rect As cv.Rect
            If options.backProjectRow Then
                rect = New cv.Rect(0, gRect.Y, dst2.Width, gRect.Height)
            Else
                rect = New cv.Rect(gRect.X, 0, gRect.Width, dst2.Height)
            End If
