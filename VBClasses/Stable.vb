...
Namespace VBClasses
    Public Class Stable_Basics : Inherits TaskParent
        ...
        Public Sub New()
            desc = "Use atask.lines.lplist(0) to find the angle needed to stabilize the image."
        End Sub
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If atask.heartBeat Then lpLast = lp
    ...
    Public Class Stable_BasicsCount : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If atask.features.Count > 0 Then
                basics.facetGen.inputPoints = New List(Of cv.Point2f)(atask.features)
            Else
                bPoint.Run(src)
    ...
    Public Class NR_Stable_Lines : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each lp In atask.lines.lpList
                ...
                vbc.DrawLine(dst1, lp.p1, lp.p2, atask.highlight)
            Next
            basics.Run(src)
            dst2 = basics.dst2
            dst3 = basics.dst3
            ...
            labels(3) = CStr(atask.lines.lpList.Count) + " line end points were found and " + CStr(basics.ptList.Count) + " were stable"
        End Sub
    End Class
    ...
    Public Class NR_Stable_FAST : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Next
            labels(2) = basics.labels(2)
            labels(3) = CStr(atask.features.Count) + " features were found and " + CStr(basics.ptList.Count) + " were stable"
        End Sub
    End Class
