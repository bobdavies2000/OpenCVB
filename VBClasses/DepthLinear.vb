...
Namespace VBClasses
    Public Class DepthLinear_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.Merge({dst2, dst3, taskAlg.pcSplit(2)}, cloud)
        End Sub
    End Class
    ...
    Public Class DepthLinear_Visualize : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To taskAlg.pcSplit.Count - 1
                pc = taskAlg.pcSplit(i)(roi)

                ' toggle between the pixel to the right or below
                If taskAlg.toggleOn Then
                    r1 = New cv.Rect(0, 0, taskAlg.cols - 1, taskAlg.rows)
                    r2 = New cv.Rect(1, 0, r1.Width, r1.Height)
                Else
                    r1 = New cv.Rect(0, 0, taskAlg.cols, taskAlg.rows - 1)
            ...
            SetTrueText("Lower left is a mask showing where depth is" + vbCrLf + "within " +
                     ...
                     New cv.Point(taskAlg.cols / 2 + 5, taskAlg.rows / 2 + 5))
        End Sub
    End Class
    ...
    Public Class DepthLinear_Input : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            pc = taskAlg.pcSplit(options.dimension)(roi)
    ...
    Public Class DepthLinear_Slices : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim pt = taskAlg.mouseMovePoint
            If standalone And taskAlg.mouseMovePoint = newPoint Then
                pt = New cv.Point(dst2.Width / 2, dst2.Height / 2)
            End If

            Dim rowCol As cv.Mat, p1 As cv.Point, p2 As cv.Point
            If options.dimension = 1 Or (options.dimension = 2 And options.zy) Then
                rowCol = taskAlg.pcSplit(options.dimension).Col(pt.X).Clone
                ...
                plotSLR.plot.minY = -taskAlg.yRange
                plotSLR.plot.maxY = taskAlg.yRange
            Else
                rowCol = taskAlg.pcSplit(options.dimension).Row(pt.Y)
                p1 = New cv.Point(0, pt.Y)
                p2 = New cv.Point(dst2.Width, pt.Y)
                plotSLR.plot.minY = -taskAlg.xRange
                plotSLR.plot.maxY = taskAlg.xRange
            End If
            taskAlg.depthRGB.Line(p1, p2, taskAlg.highlight, taskAlg.lineWidth)
            ...
            Else
                ...
                dst2 = plotSLR.dst2
                dst3 = plotSLR.dst3
                If taskAlg.heartBeat Then labels(2) = plotSLR.plot.labels(2)
            End If
        End Sub
    End Class
    ...
    Public Class DepthLinear_ImageX : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For y = 0 To dst2.Height - 1
                Dim rowCol As cv.Mat = taskAlg.pcSplit(0).Row(y)
                ...
                Next
            Next
            cv.Cv2.Merge({dst2, taskAlg.pcSplit(1), taskAlg.pcSplit(2)}, dst3)
    ...
    Public Class DepthLinear_ImageY : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For x = 0 To dst2.Width - 1
                Dim rowCol = taskAlg.pcSplit(1).Col(x).Clone
                rowCol = cv.Mat.FromPixelData(1, rowCol.Rows, cv.MatType.CV_32FC1, rowCol.Data)
                ...
                Next
            Next
            cv.Cv2.Merge({taskAlg.pcSplit(0), dst2, taskAlg.pcSplit(2)}, dst3)
            dst3.SetTo(0, taskAlg.noDepthMask)
        End Sub
    End Class
End Namespace