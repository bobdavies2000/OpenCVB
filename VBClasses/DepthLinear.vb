...
Namespace VBClasses
    Public Class NR_DepthLinear_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.Merge({dst2, dst3, tsk.pcSplit(2)}, cloud)
        End Sub
    End Class
    ...
    Public Class NR_DepthLinear_Visualize : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To tsk.pcSplit.Count - 1
                pc = tsk.pcSplit(i)(roi)

                ' toggle between the pixel to the right or below
                If tsk.toggleOn Then
                    r1 = New cv.Rect(0, 0, tsk.cols - 1, tsk.rows)
                    r2 = New cv.Rect(1, 0, r1.Width, r1.Height)
                Else
                    r1 = New cv.Rect(0, 0, tsk.cols, tsk.rows - 1)
            ...
            SetTrueText("Lower left is a mask showing where depth is" + vbCrLf + "within " +
                     ...
                     New cv.Point(tsk.cols / 2 + 5, tsk.rows / 2 + 5))
        End Sub
    End Class
    ...
    Public Class DepthLinear_Input : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            pc = tsk.pcSplit(options.dimension)(roi)
    ...
    Public Class NR_DepthLinear_Slices : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim pt = tsk.mouseMovePoint
            If standalone And tsk.mouseMovePoint = newPoint Then
                pt = New cv.Point(dst2.Width / 2, dst2.Height / 2)
            End If

            Dim rowCol As cv.Mat, p1 As cv.Point, p2 As cv.Point
            If options.dimension = 1 Or (options.dimension = 2 And options.zy) Then
                rowCol = tsk.pcSplit(options.dimension).Col(pt.X).Clone
                ...
                plotSLR.plot.minY = -tsk.yRange
                plotSLR.plot.maxY = tsk.yRange
            Else
                rowCol = tsk.pcSplit(options.dimension).Row(pt.Y)
                p1 = New cv.Point(0, pt.Y)
                p2 = New cv.Point(dst2.Width, pt.Y)
                plotSLR.plot.minY = -tsk.xRange
                plotSLR.plot.maxY = tsk.xRange
            End If
            tsk.depthRGB.Line(p1, p2, tsk.highlight, tsk.lineWidth)
            ...
            Else
                ...
                dst2 = plotSLR.dst2
                dst3 = plotSLR.dst3
                If tsk.heartBeat Then labels(2) = plotSLR.plot.labels(2)
            End If
        End Sub
    End Class
    ...
    Public Class NR_DepthLinear_ImageX : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For y = 0 To dst2.Height - 1
                Dim rowCol As cv.Mat = tsk.pcSplit(0).Row(y)
                ...
                Next
            Next
            cv.Cv2.Merge({dst2, tsk.pcSplit(1), tsk.pcSplit(2)}, dst3)
    ...
    Public Class NR_DepthLinear_ImageY : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For x = 0 To dst2.Width - 1
                Dim rowCol = tsk.pcSplit(1).Col(x).Clone
                rowCol = cv.Mat.FromPixelData(1, rowCol.Rows, cv.MatType.CV_32FC1, rowCol.Data)
                ...
                Next
            Next
            cv.Cv2.Merge({tsk.pcSplit(0), dst2, tsk.pcSplit(2)}, dst3)
            dst3.SetTo(0, tsk.noDepthMask)
        End Sub
    End Class
End Namespace