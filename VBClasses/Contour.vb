...
Namespace VBClasses
    Public Class Contour_Basics : Inherits TaskParent
        ...
        Public Function selectContour() As contourData
            ...
            Dim id = contourMap.Get(Of Integer)(taskAlg.clickPoint.Y, taskAlg.clickPoint.X)
            For Each taskAlg.contourD In contourList
                If id = taskAlg.contourD.ID Then Exit For
            Next

            For Each tour In contourList
                If tour.ID = id Then Exit For
            Next
            taskAlg.color(tour.rect).SetTo(cv.Scalar.White, tour.mask)
            Return taskAlg.contourD
        End Function
        Public Shared Function buildContours(input As cv.Mat) As cv.Point()()
            ...
            If options.retrievalMode = cv.RetrievalModes.FloodFill Then
                Dim dst As New cv.Mat(taskAlg.workRes, cv.MatType.CV_8U, 0)
    ...
    Public Class Contour_Features : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each pt In taskAlg.feat.features
                DrawCircle(dst2, pt)
                DrawCircle(dst3, pt)
                Dim rect = taskAlg.gridRects(taskAlg.gridMap.Get(Of Integer)(pt.Y, pt.X))
            ...
            labels(2) = "There are " + CStr(contours.contourList.Count) + " contours and " +
                    CStr(taskAlg.feat.features.Count) + " features."
        End Sub
    End Class
    ...
    Public Class Contour_LineRGB : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each lp In taskAlg.lines.lpList
                dst2.Line(lp.p1, lp.p2, taskAlg.highlight, taskAlg.lineWidth, taskAlg.lineWidth)
            Next
        End Sub
    End Class
    ...
    Public Class Contour_RemoveLines : Inherits TaskParent
        ...
        Public Sub New()
            ...
            image = cv.Cv2.ImRead(taskAlg.homeDir + "Data/invoice.jpg")
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            tour = scaleTour(tour)
            For i = 0 To tour.Count - 1
                cv.Cv2.DrawContours(dst2, tour, i, cv.Scalar.Black, taskAlg.lineWidth)
            Next
            ...
            tour = scaleTour(tour)
            For i = 0 To tour.Count - 1
                cv.Cv2.DrawContours(dst3, tour, i, cv.Scalar.Black, taskAlg.lineWidth)
            Next
        End Sub
    End Class
    ...
    Public Class Contour_SidePoints : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If sides.corners.Count > 0 And taskAlg.heartBeat Then
                ...
                If rc.contour.Count > 0 Then
                    ...
                    dst3.Line(ptLeft, ptRight, white, taskAlg.lineWidth, taskAlg.lineWidth)
                    dst3.Line(ptTop, ptBot, white, taskAlg.lineWidth, taskAlg.lineWidth)
                End If
            ...
            For Each pt In rc.contour
                ...
                If ptList.Contains(ptStr) Then
                    ...
                    If pct > 0.1 And pct < 0.9 Then
                        selfInt = True
                        DrawCircle(dst3, pt, taskAlg.DotSize, cv.Scalar.Red)
                    End If
                End If
                ptList.Add(ptStr)
            Next
    ...
    Public Class Contour_Compare : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim tmp = taskAlg.oldrcD.mask.Clone
            ...
            dst3.SetTo(0)
            cv.Cv2.DrawContours(dst3(taskAlg.oldrcD.rect), allContours, -1, cv.Scalar.Yellow)
        End Sub
    End Class
    ...
    Public Class Contour_Smoothing : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            DrawTour(dst3(rc.rect), bestContour, white, taskAlg.lineWidth + 3)

            Dim approxContour = ContourBuild(rc.mask, options.ApproximationMode)
            DrawTour(dst3(rc.rect), approxContour, cv.Scalar.Red)

            If taskAlg.heartBeat Then labels(2) = "Contour points count reduced from " + CStr(bestContour.Count) +
            ...
            If taskAlg.heartBeat Then
                ...
                For Each p1 In random.PointList
                    For Each p2 In random.PointList
                        dst2.Line(p1, p2, white, taskAlg.lineWidth, taskAlg.lineWidth)
                    Next
                Next
            End If
    ...
    Public Class Contour_General : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Else
                If src.Type = cv.MatType.CV_8U Then dst2 = src Else dst2 = taskAlg.grayStable
            End If
    ...
    Public Class Contour_InfoDepth : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            taskAlg.contourD = contours.selectContour()

            strOut = vbCrLf + vbCrLf
            Dim index = contours.contourList.IndexOf(taskAlg.contourD)
            strOut += "Index = " + CStr(index) + vbCrLf
            strOut += "Depth = " + Format(taskAlg.contourD.depth, fmt1) + vbCrLf
            strOut += "Range (m) = " + Format(taskAlg.contourD.mm.range, fmt1) + vbCrLf
            strOut += "Number of pixels in the mask: " + CStr(taskAlg.contourD.pixels) + vbCrLf

            DrawRect(dst2, taskAlg.contourD.rect)
            DrawCircle(dst2, taskAlg.contourD.maxDist)
    ...
    Public Class Contour_Isolate : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst1.SetTo(0)
            Dim indexD = contours.contourList.IndexOf(taskAlg.contourD)
            For Each contour In contours.contourList
    ...
    Public Class Contour_Hulls : Inherits TaskParent
        ...
        Public Function getSelectedHull() As contourData
            Dim id = contourMap.Get(Of Integer)(taskAlg.ClickPoint.Y, taskAlg.ClickPoint.X)
            For Each taskAlg.contourD In contours.contourList
                If id = taskAlg.contourD.ID Then Exit For
            Next
            Return taskAlg.contourD
        End Function
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskAlg.heartBeat Then labels(3) = CStr(contourList.Count) + " hulls"
        End Sub
    End Class
    ...
    Public Class Contour_EdgePoints : Inherits TaskParent
        ...
        Public Sub New()
            ...
            plot.max = rangeVal
            plot.min = -rangeVal
            taskAlg.gOptions.DebugSlider.Value = 50
        ...
        Public Shared Function EdgePointOffset(ptIn As cv.Point, offset As Integer) As cv.Point
            ...
            If pt.X = 0 Then pt.X += offset
            If pt.Y = 0 Then pt.Y += offset
            If pt.X = taskAlg.workRes.Width - 1 Then pt.X -= offset
            If pt.Y = taskAlg.workRes.Height - 1 Then pt.Y -= offset
            If pt.X = taskAlg.workRes.Width - 1 Then pt.X -= offset
            If pt.Y = taskAlg.workRes.Height - 1 Then pt.Y -= offset
            Return pt
        End Function
        Public Overrides Sub RunAlg(src As cv.Mat)
            contours.Run(src)
            dst0 = dst1.Clone
            dst2 = taskAlg.lines.dst3
            labels(2) = taskAlg.lines.labels(2)

            Dim ptList As New List(Of cv.Point)
            dst1.SetTo(0)
            Dim maxLines = 3 ' taskAlg.gOptions.DebugSlider.Value * 2
            For Each contour In contours.contourList
                Dim index = contours.contourList.IndexOf(contour)
    ...
    Public Class Contour_Info : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst0 = src
            dst0(taskAlg.contourD.rect).SetTo(white, taskAlg.contourD.mask)

            dst2.Rectangle(taskAlg.contourD.rect, taskAlg.highlight, taskAlg.lineWidth)
    ...
    Public Class Contour_Sort : Inherits TaskParent
        Public allContours As cv.Point()()
        Public contourList As New List(Of contourData)
        Public contourMap As New cv.Mat(taskAlg.workRes, cv.MatType.CV_32S, 0)
        Public Sub New()
            dst1 = New cv.Mat(dst1.Size, cv.MatType.CV_8U, 0)
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim tourMat As New cv.Mat(taskAlg.workRes, cv.MatType.CV_8U, 0)
            For Each ptArray In allContours
                ...
                If tour.pixels > taskAlg.color.Total * 3 / 4 Then Continue For ' toss this contour - it covers everything...
                ...
                tour.depth = taskAlg.pcSplit(2)(tour.rect).Mean(taskAlg.depthMask(tour.rect))(0)
                tour.mm = GetMinMax(taskAlg.pcSplit(2)(tour.rect), tour.mask)
                tour.maxDist = GetMaxDistContour(tour)
                tour.ID = taskAlg.gridMap.Get(Of Integer)(tour.maxDist.Y, tour.maxDist.X)
    ...
    Public Class Contour_SortNew : Inherits TaskParent
        ...
        Public rcMap As New cv.Mat(taskAlg.workRes, cv.MatType.CV_32S, 0)
        Dim edgeline As New EdgeLine_Basics
        Public Sub New()
            dst1 = New cv.Mat(dst1.Size, cv.MatType.CV_8U, 0)
            desc = "Sort the contours by size and prepare the contour map"
    ...
    Public Class Contour_BasicsOld : Inherits TaskParent
        ...
        Public Shared Function selectContour(contours As Contour_Basics_List) As contourData
            Dim tour As New contourData
            Dim id = contours.contourMap.Get(Of Integer)(taskAlg.ClickPoint.Y, taskAlg.ClickPoint.X)
            For Each taskAlg.contourD In contours.contourList
                If id = taskAlg.contourD.ID Then Exit For
            Next
