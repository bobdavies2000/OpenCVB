...
Namespace VBClasses
    ...
    Public Class NR_Transform_Affine3D : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If atask.testAllRunning Then
                If atask.frameCount = 30 Then options.firstCheck = True
                If atask.frameCount = 60 Then options.secondCheck = True
            End If

            If options.firstCheck Then
                pc1 = atask.pointCloud.Clone()
                options.firstCheck = False
                output += "First point cloud captured" + vbCrLf
            End If

            If options.secondCheck Then
                pc2 = atask.pointCloud.Clone()
    ...
    Public Class NR_Transform_Rotate : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            DrawCircle(dst2, imageCenter, atask.DotSize * 2, cv.Scalar.Yellow)
            DrawCircle(dst2, imageCenter, atask.DotSize, cv.Scalar.Blue)
        End Sub
    End Class
End Namespace