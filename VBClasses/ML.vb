...
Namespace VBClasses
    Public Class ML_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskAlg.heartBeat Or buildEveryPass Then
    ...
    Public Class ML_DepthFromColor : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            mats.mat(1) = taskAlg.noDepthMask.Clone
            ...
            Dim depth = taskAlg.pcSplit(2).Resize(color32f.Size())

            Dim mask = depth.Threshold(taskAlg.MaxZmeters, 255, cv.ThresholdTypes.Binary)
            mask.ConvertTo(mask, cv.MatType.CV_8U)
            mats.mat(2) = mask.Resize(src.Size())

            depth.SetTo(taskAlg.MaxZmeters, Not mask)
            ...
            mats.Run(emptyMat)
            dst2 = mats.dst2
            labels(2) = "prediction, shadow, Depth Mask < " + CStr(taskAlg.MaxZmeters) + ", Learn Input"
            dst3 = mats.dst3
        End Sub
        Public Sub Close()
    ...
    Public Class ML_DepthFromXYColor : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            mats.mat(0) = taskAlg.noDepthMask.CvtColor(cv.ColorConversionCodes.GRAY2BGR)
            ...
            Dim colorROI As New cv.Rect(0, 0, resizer.newSize.Width, resizer.newSize.Height)
            resizer.dst2.ConvertTo(color32f, cv.MatType.CV_32FC3)
            Dim shadowSmall = taskAlg.noDepthMask.Resize(color32f.Size())
            color32f.SetTo(cv.Scalar.Black, shadowSmall) ' where depth is unknown, set to black (so we don't learn anything invalid, i.e. good color but missing depth.
            Dim depth32f = taskAlg.pcSplit(2).Resize(color32f.Size())

            Dim mask = depth32f.Threshold(taskAlg.MaxZmeters, taskAlg.MaxZmeters, cv.ThresholdTypes.BinaryInv)
            ...
            mask = Not mask
            depth32f.SetTo(taskAlg.MaxZmeters, mask)
            ...
            mats.Run(emptyMat)
            dst3 = mats.dst2
            labels(3) = "shadow, empty, Depth Mask < " + CStr(taskAlg.MaxZmeters) + ", Learn Input"
        End Sub
        Public Sub Close()
            If rtree IsNot Nothing Then rtree.Dispose()
        End Sub
    ...
    Public Class ML_Color2Depth : Inherits TaskParent
        ...
        Dim rtree As RTrees
        Public Sub New()
            taskAlg.featureOptions.Color8USource.SelectedItem() = "Bin4Way_Regions"
            desc = "Prepare a grid of color and depth data."
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            color8U.Run(src)
            dst2 = color8U.dst3
            labels(2) = "Output of Color8U_Basics running " + taskAlg.featureOptions.Color8USource.Text
            ...
            For i = 0 To taskAlg.gridRects.Count - 1
                ...
                If taskAlg.noDepthMask(roi).CountNonZero > 0 Then
                    roiPredict.Add(roi)
                    predictList.Add(mls)
                Else
                    mlInput.Add(mls)
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For y = 0 To regions.rcMapX.Height - 1
                For x = 0 To regions.rcMapX.Width - 1
                    Dim zVal = taskAlg.pcSplit(2).Get(Of Single)(y, x)
