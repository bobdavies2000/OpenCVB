...
Namespace VBClasses
    Public Class Cluster_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2 = src.Clone
            If standalone Then ptInput = taskAlg.featurePoints
            ...
            For Each group In clusters
                For i = 0 To group.Value.Count - 1
                    For j = 0 To group.Value.Count - 1
                        dst2.Line(group.Value(i), group.Value(j), white, taskAlg.lineWidth, taskAlg.lineWidth)
                    Next
                Next
            Next
            dst3.SetTo(0)
    ...
    Public Class Cluster_Hulls : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cluster.ptInput = taskAlg.featurePoints
            cluster.Run(src)
            dst2 = cluster.dst2
            dst3 = cluster.dst3
            ...
            For Each group In cluster.clusters
                ...
                ElseIf hullPoints.Count = 2 Then
                    dst3.Line(hullPoints(0), hullPoints(1), white, taskAlg.lineWidth, taskAlg.lineWidth)
                Else
                    DrawCircle(dst3, hullPoints(0), taskAlg.DotSize, taskAlg.highlight)
                End If

                hulls.Add(hull)
                If (hull.Count > 0) Then DrawTour(dst3, hull, white, taskAlg.lineWidth)
            Next
