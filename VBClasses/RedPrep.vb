...
Namespace VBClasses
    Public Class RedPrep_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            src.ConvertTo(pc32S, cv.MatType.CV_32SC3, 1000 / taskAlg.reductionTarget)
            Dim split = pc32S.Split()

            dst2.SetTo(0)
            Dim saveNoDepth As cv.Mat = Nothing
            If src.Size <> taskAlg.workRes Then
                saveNoDepth = taskAlg.noDepthMask.Clone
                taskAlg.noDepthMask = taskAlg.noDepthMask.Resize(src.Size)
            End If
            If options.PrepX Then
                prepEdges.Run(reduceChan(split(0), taskAlg.noDepthMask))
            ...
            If options.PrepY Then
                prepEdges.Run(reduceChan(split(1), taskAlg.noDepthMask))
                dst2 = dst2 Or prepEdges.dst3
            End If

            If options.PrepZ Then
                prepEdges.Run(reduceChan(split(2), taskAlg.noDepthMask))
                dst2 = dst2 Or prepEdges.dst3
            End If
            ...
            If src.Size <> taskAlg.workRes Then taskAlg.noDepthMask = saveNoDepth.Clone
            labels(2) = "Using reduction factor = " + CStr(taskAlg.reductionTarget)
        End Sub
    End Class
    ...
    Public Class RedPrep_Depth : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim inputX(taskAlg.pcSplit(0).Total * taskAlg.pcSplit(0).ElemSize - 1) As Byte
            Dim inputY(taskAlg.pcSplit(1).Total * taskAlg.pcSplit(1).ElemSize - 1) As Byte

            Marshal.Copy(taskAlg.pcSplit(0).Data, inputX, 0, inputX.Length)
            Marshal.Copy(taskAlg.pcSplit(1).Data, inputY, 0, inputY.Length)
            ...
            Dim imagePtr = PrepXY_Run(cPtr, handleX.AddrOfPinnedObject(), handleY.AddrOfPinnedObject(), src.Rows, src.Cols,
                                  taskAlg.xRange, taskAlg.yRange, taskAlg.histogramBins)
            handleX.Free()
            handleY.Free()
    ...
    Public Class RedPrep_VB : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To 1
                Select Case i
                    Case 0 ' X Reduction
                        ...
                        ranges = New cv.Rangef() {New cv.Rangef(-taskAlg.xRange, taskAlg.xRange)}
                    Case 1 ' Y Reduction
                        dst1 = taskAlg.pcSplit(1)
                        ranges = New cv.Rangef() {New cv.Rangef(-taskAlg.yRange, taskAlg.yRange)}
                End Select

                cv.Cv2.CalcHist({dst1}, {0}, taskAlg.depthMask, histogram, 1, {taskAlg.histogramBins}, ranges)
            ...
            labels(2) = CStr(taskAlg.histogramBins * 2 - zeroCount) + " depth regions mapped (control with histogram bins.)"
        End Sub
    End Class
    ...
    Public Class RedPrep_ReductionChoices : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim reduceAmt = taskAlg.reductionTarget
            taskAlg.pcSplit(0).ConvertTo(split(0), cv.MatType.CV_32S, 1000 / reduceAmt)
            taskAlg.pcSplit(1).ConvertTo(split(1), cv.MatType.CV_32S, 1000 / reduceAmt)
            taskAlg.pcSplit(2).ConvertTo(split(2), cv.MatType.CV_32S, 1000 / reduceAmt)
    ...
    Public Class RedPrep_Edges_CPP : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If src.Size <> taskAlg.noDepthMask.Size Then
                dst3.SetTo(255, taskAlg.noDepthMask.Resize(src.Size))
                dst2 = dst2.Resize(src.Size)
                dst2.SetTo(0, dst3)
            Else
                dst3.SetTo(255, taskAlg.noDepthMask)
                dst2.SetTo(0, dst3)
            End If
        End Sub
        Public Sub Close()
    ...
    Public Class RedPrep_CloudAndColor : Inherits TaskParent
        ...
        Private Function reduceChan(chan As cv.Mat) As cv.Mat
            ...
            chan = (chan - mm.minVal) * 255 / (mm.maxVal - mm.minVal)
            chan.ConvertTo(chan, cv.MatType.CV_8U)
            chan.SetTo(0, taskAlg.noDepthMask)
            Return chan
        End Function
        Public Overrides Sub RunAlg(src As cv.Mat)
            options.Run()
            ...
            Dim pc32S As New cv.Mat
            taskAlg.pointCloud.ConvertTo(pc32S, cv.MatType.CV_32SC3, 1000 / taskAlg.reductionTarget)
            Dim split = pc32S.Split()
            ...
            dst3 = edges.dst2
            dst3.CopyTo(dst2, taskAlg.noDepthMask)

            dst2.Rectangle(New cv.Rect(0, 0, dst2.Width - 1, dst2.Height - 1), 255, taskAlg.lineWidth)
            labels(2) = "Using reduction factor = " + CStr(taskAlg.reductionTarget)
        End Sub
    End Class
    ...
    Public Class XO_RedList_LikelyFlatSurfaces : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each rc In taskAlg.redList.oldrclist
                If rc.depth >= taskAlg.MaxZmeters Then Continue For
    ...
    Public Class XO_RedList_Motion : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If taskAlg.motionRect.Width = 0 Then Exit Sub ' full image stable means nothing needs to be done...
            runRedList(src, labels(2))
            ...
            dst1.SetTo(0)
            taskAlg.redList.oldrclist.RemoveAt(0)
            ...
            For Each rc In taskAlg.redList.oldrclist
                tmp = taskAlg.motionMask(rc.rect) And rc.mask
            ...
            Next
            labels(3) = CStr(count) + " of " + CStr(taskAlg.redList.oldrclist.Count) + " redCloud cells had motion." +
            ...
            rcLastList = New List(Of oldrcData)(taskAlg.redList.oldrclist)
    ...
    Public Class XO_RedList_OnlyColorHist3D : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            labels(3) = taskAlg.redList.labels(2)
        End Sub
    End Class
    ...
    Public Class XO_RedList_OnlyColorAlt : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim lastMap As cv.Mat = taskAlg.redList.rcMap.Clone
            ...
            For Each rc In taskAlg.redList.oldrclist
                ...
                If taskAlg.redList.rcMap.Get(Of Byte)(rc.maxDist.Y, rc.maxDist.X) = 0 Then
                    rc.index = taskAlg.redList.oldrclist.Count
                    newCells.Add(rc)
            ...
            taskAlg.redList.oldrclist = New List(Of oldrcData)(newCells)
            labels(3) = CStr(taskAlg.redList.oldrclist.Count) + " cells were identified."
            labels(2) = taskAlg.redList.labels(3) + " " + CStr(unmatched) + " cells were not matched to previous frame."

            If taskAlg.redList.oldrclist.Count > 0 Then dst2 = PaletteFull(lastMap)
        End Sub
    End Class
End Namespace