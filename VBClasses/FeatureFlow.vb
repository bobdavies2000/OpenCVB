...
Namespace VBClasses
    Public Class FeatureFlow_Basics : Inherits TaskParent
        ...
        Public Sub New()
            taskAlg.gOptions.MaxDepthBar.Value = 20
        ...
        Public Sub buildCorrelations(prevFeatures As List(Of cv.Point), currFeatures As List(Of cv.Point))
            ...
            For Each p1 In prevFeatures
                Dim rect = ValidateRect(New cv.Rect(p1.X - pad, p1.Y - pad, taskAlg.brickSize, taskAlg.brickSize))
                Dim correlations As New List(Of Single)
                For Each p2 In currFeatures
                    Dim r = ValidateRect(New cv.Rect(p2.X - pad, p2.Y - pad, Math.Min(rect.Width, taskAlg.brickSize),
                                                                             Math.Min(taskAlg.brickSize, rect.Height)))
                ...
                If maxCorrelation >= taskAlg.fCorrThreshold Then
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst3 = If(taskAlg.firstPass, src.Clone, dst2.Clone)
            Static prevFeatures As New List(Of cv.Point)(taskAlg.featurePoints)
            buildCorrelations(prevFeatures, taskAlg.featurePoints)
            ...
            Next
            prevFeatures = New List(Of cv.Point)(taskAlg.featurePoints)
        End Sub
    End Class
    ...
    Public Class FeatureFlow_LucasKanade : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If src.Channels() = 3 Then src = taskAlg.grayStable
            Static lastGray As cv.Mat = taskAlg.grayStable.Clone
            features = taskAlg.features
            ...
            For i = 0 To status.Rows - 1
                If status.Get(Of Byte)(i, 0) Then
                    ...
                    If length < 30 Then
                        ...
                        dst2.Line(pt1, pt2, taskAlg.highlight, taskAlg.lineWidth + taskAlg.lineWidth, taskAlg.lineType)
                        DrawCircle(dst3, pt1, taskAlg.DotSize + 3, white)
                        DrawCircle(dst3, pt2, taskAlg.DotSize + 1, cv.Scalar.Red)
                    End If
                End If
            Next
            labels(2) = "Matched " + CStr(features.Count) + " points "

            If taskAlg.heartBeat Then lastGray = src.Clone()
            lastGray = src.Clone()
        End Sub
    End Class
    ...
    Public Class FeatureFlow_LeftRight1 : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            pyrLeft.Run(taskAlg.leftView)
            pyrRight.Run(taskAlg.rightView)
    ...
    Public Class FeatureFlow_LeftRightHist : Inherits TaskParent
        ...
        Public Function displayFeatures(dst As cv.Mat, features As List(Of cv.Point)) As cv.Mat
            For Each pt In features
                DrawCircle(dst, pt, taskAlg.DotSize, taskAlg.highlight)
            Next
            Return dst
        End Function
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2 = displayFeatures(taskAlg.leftView.Clone, leftFeatures)
            dst3 = displayFeatures(taskAlg.rightView.Clone, rightFeatures)

            leftHist.Add(tmpLeft)
            rightHist.Add(tmpRight)
            Dim threshold = Math.Min(taskAlg.frameHistoryCount, leftHist.Count)

            If leftHist.Count >= taskAlg.frameHistoryCount Then leftHist.RemoveAt(0)
            If rightHist.Count >= taskAlg.frameHistoryCount Then rightHist.RemoveAt(0)
