...
Namespace VBClasses
    Public Class NR_FeatureFlow_Basics : Inherits TaskParent
        ...
        Public Sub buildCorrelations(prevFeatures As List(Of cv.Point), currFeatures As List(Of cv.Point))
            ...
            For Each p1 In prevFeatures
                Dim rect = ValidateRect(New cv.Rect(p1.X - pad, p1.Y - pad, atask.brickSize, atask.brickSize))
                Dim correlations As New List(Of Single)
                For Each p2 In currFeatures
                    Dim r = ValidateRect(New cv.Rect(p2.X - pad, p2.Y - pad, Math.Min(rect.Width, atask.brickSize),
                                                                             Math.Min(atask.brickSize, rect.Height)))
                ...
                If maxCorrelation >= atask.fCorrThreshold Then
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst3 = If(atask.firstPass, src.Clone, dst2.Clone)
            Static prevFeatures As New List(Of cv.Point)(atask.featurePoints)
            buildCorrelations(prevFeatures, atask.featurePoints)
            ...
            Next
            prevFeatures = New List(Of cv.Point)(atask.featurePoints)
        End Sub
    End Class
    ...
    Public Class FeatureFlow_LucasKanade : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If src.Channels() = 3 Then src = atask.grayStable
            Static lastGray As cv.Mat = atask.grayStable.Clone
            features = atask.features
            ...
            For i = 0 To status.Rows - 1
                If status.Get(Of Byte)(i, 0) Then
                    ...
                    If length < 30 Then
                        ...
                        dst2.Line(pt1, pt2, atask.highlight, atask.lineWidth + atask.lineWidth, atask.lineType)
                        DrawCircle(dst3, pt1, atask.DotSize + 3, white)
                        DrawCircle(dst3, pt2, atask.DotSize + 1, cv.Scalar.Red)
                    End If
                End If
            Next
            labels(2) = "Matched " + CStr(features.Count) + " points "

            If atask.heartBeat Then lastGray = src.Clone()
            lastGray = src.Clone()
        End Sub
    End Class
    ...
    Public Class NR_FeatureFlow_LeftRight1 : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            pyrLeft.Run(atask.leftView)
            pyrRight.Run(atask.rightView)
    ...
    Public Class FeatureFlow_LeftRightHist : Inherits TaskParent
        ...
        Public Function displayFeatures(dst As cv.Mat, features As List(Of cv.Point)) As cv.Mat
            For Each pt In features
                DrawCircle(dst, pt, atask.DotSize, atask.highlight)
            Next
            Return dst
        End Function
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2 = displayFeatures(atask.leftView.Clone, leftFeatures)
            dst3 = displayFeatures(atask.rightView.Clone, rightFeatures)

            leftHist.Add(tmpLeft)
            rightHist.Add(tmpRight)
            Dim threshold = Math.Min(atask.frameHistoryCount, leftHist.Count)

            If leftHist.Count >= atask.frameHistoryCount Then leftHist.RemoveAt(0)
            If rightHist.Count >= atask.frameHistoryCount Then rightHist.RemoveAt(0)
