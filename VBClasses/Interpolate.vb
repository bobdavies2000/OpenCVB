...
Namespace VBClasses
    ...
    Public Class Interpolate_Kalman : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If atask.optionsChanged Then
                ReDim atask.kalman.kInput(dst2.Width * dst2.Height - 1)
                myFrameCount = 1
                updatedFrames = 0
            End If
            ...
            Marshal.Copy(tmp32f.Data, atask.kalman.kInput, 0, atask.kalman.kInput.Length)
            atask.kalman.Run(emptyMat)

            Dim results(atask.kalman.kInput.Length - 1) As Byte
            For i = 0 To atask.kalman.kOutput.Length - 1
                Dim val = atask.kalman.kOutput(i)
                If Single.IsNaN(val) Then val = 255
                If val < 0 Then val = 0
                If val > 255 Then val = 255
                results(i) = val
            Next
            Marshal.Copy(results, 0, dst2.Data, atask.kalman.kOutput.Length)
    ...
    Public Class NR_Interpolate_Lines : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Next
            labels(3) = "There were " + CStr(atask.lines.lpList.Count) + " lines found"
