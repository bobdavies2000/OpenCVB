...
Namespace VBClasses
    ...
    Public Class Interpolate_Kalman : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskA.optionsChanged Then
                ReDim taskA.kalman.kInput(dst2.Width * dst2.Height - 1)
                myFrameCount = 1
                updatedFrames = 0
            End If
            ...
            Marshal.Copy(tmp32f.Data, taskA.kalman.kInput, 0, taskA.kalman.kInput.Length)
            taskA.kalman.Run(emptyMat)

            Dim results(taskA.kalman.kInput.Length - 1) As Byte
            For i = 0 To taskA.kalman.kOutput.Length - 1
                Dim val = taskA.kalman.kOutput(i)
                If Single.IsNaN(val) Then val = 255
                If val < 0 Then val = 0
                If val > 255 Then val = 255
                results(i) = val
            Next
            Marshal.Copy(results, 0, dst2.Data, taskA.kalman.kOutput.Length)
    ...
    Public Class NR_Interpolate_Lines : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Next
            labels(3) = "There were " + CStr(taskA.lines.lpList.Count) + " lines found"
