...
Namespace VBClasses
    ...
    Public Class Interpolate_Kalman : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If tsk.optionsChanged Then
                ReDim tsk.kalman.kInput(dst2.Width * dst2.Height - 1)
                myFrameCount = 1
                updatedFrames = 0
            End If
            ...
            Marshal.Copy(tmp32f.Data, tsk.kalman.kInput, 0, tsk.kalman.kInput.Length)
            tsk.kalman.Run(emptyMat)

            Dim results(tsk.kalman.kInput.Length - 1) As Byte
            For i = 0 To tsk.kalman.kOutput.Length - 1
                Dim val = tsk.kalman.kOutput(i)
                If Single.IsNaN(val) Then val = 255
                If val < 0 Then val = 0
                If val > 255 Then val = 255
                results(i) = val
            Next
            Marshal.Copy(results, 0, dst2.Data, tsk.kalman.kOutput.Length)
    ...
    Public Class NR_Interpolate_Lines : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Next
            labels(3) = "There were " + CStr(tsk.lines.lpList.Count) + " lines found"
