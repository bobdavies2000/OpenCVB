...
Namespace VBClasses
    Public Class Filter_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each cb In atask.featureOptions.colorCheckbox
                If cb.Checked Then
                    Select Case cb.Text
                        Case "Original"
                            dst2 = atask.color
            ...
            labels(2) = "Color input to all algorithms - " + atask.featureOptions.colorCheckbox(filterIndex).Text
    ...
    Public Class Filter_BasicsGray : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            dst2 = src
            For Each cb In atask.featureOptions.grayCheckbox
                If cb.Checked Then
                    Select Case cb.Text
                        Case "Original"
                        Case "Blur_Basics"
            ...
            labels(2) = "Grayscale input to all algorithms - " + atask.featureOptions.grayCheckbox(filterIndex).Text
            If filterIndex > 0 Then
                filters(filterIndex).run(dst2)
                dst2 = filters(filterIndex).dst2
    ...
    Public Class NR_Filter_Normalized2D : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            options.Run()
            Dim kernelSize As Integer = If(standaloneTest(), (atask.frameCount Mod 20) + 1, options.kernelSize)
    ...
    Public Class NR_Filter_Equalize : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If src.Channels = 1 Then cv.Cv2.EqualizeHist(src, dst2) Else cv.Cv2.EqualizeHist(atask.grayStable, dst2)
        End Sub
    End Class
End Namespace