...
Namespace VBClasses
    Public Class Profile_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To rc.contour.Count - 1
                Dim pt = rc.contour(i)
                Dim vec = atask.pointCloud(rc.rect).Get(Of cv.Point3f)(pt.Y, pt.X)
            ...
            corners3D.Add(atask.pointCloud.Get(Of cv.Point3f)(rc.rect.Y + rc.contour(0).Y, rc.rect.X + rc.contour(0).X))
            ...
            For i As Integer = 0 To 6 - 1
                ...
                If ptList.Count > 0 Then
                    ...
                    corners3D.Add(atask.pointCloud(rc.rect).Get(Of cv.Point3f)(pt.Y, pt.X))
                End If
            Next

            For i = 0 To corners.Count - 1
                DrawCircle(dst3, corners(i), atask.DotSize + 2, cornerColors(i))
            Next

            If atask.heartBeat Then
                strOut = "X     " + vbTab + "Y     " + vbTab + "Z " + vbTab + "units=meters" + vbCrLf
                Dim w = atask.brickSize
                For i = 0 To corners.Count - 1
    ...
    Public Class Profile_Rotation : Inherits TaskParent
        ...
        Public Sub New()
            If standalone Then atask.gOptions.setGravityUsage(False)
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If standaloneTest() Then
                ...
                strOut += atask.gravityMatrix.strOut + vbCrLf
                strOut += vbCrLf + "New gMatrix from sliders" + vbCrLf
                strOut += IMU_GMatrix.gMatrixToStr(atask.gravityMatrix.gMatrix) + vbCrLf + vbCrLf
    ...
    Public Class NR_Profile_Derivative : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If rc.index > 0 Then
                For i = 0 To rc.contour.Count - 1
                    ...
                    Else
                        color = white
                    End If
                    DrawCircle(dst3, pt, atask.DotSize, color)

                    If sides.cornersRaw.Contains(rc.contour(i)) Then
                        Dim index = sides.cornersRaw.IndexOf(rc.contour(i))
                        DrawCircle(dst1, pt, atask.DotSize + 5, white)
                        DrawCircle(dst1, pt, atask.DotSize + 3, sides.cornerColors(index))
            ...
            For i = 0 To sides.corners.Count - 1
                ...
                DrawCircle(dst1, sides.corners(i), atask.DotSize, color)
            Next
    ...
    Public Class Profile_ConcentrationTop : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            ySlider.Value += 1
            rotate.Run(src)
            Dim output = (vecMat.Reshape(1, vecMat.Rows * vecMat.Cols) * atask.gravityMatrix.gMatrix).ToMat  ' <<<<<<<<<<<<<<<<<<<<<<< this is the XYZ-axis rotation...
    ...
    Public Class NR_Profile_Kalman : Inherits TaskParent
        ...
        Public Sub New()
            ...
            ReDim atask.kalman.kInput(12 - 1)
            If standalone Then atask.gOptions.displayDst1.Checked = True
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If atask.kalman.kInput.Count <> sides.corners.Count * 2 Then ReDim atask.kalman.kInput(sides.corners.Count * 2 - 1)
            For i = 0 To sides.corners.Count - 1
                atask.kalman.kInput(i * 2) = sides.corners(i).X
                atask.kalman.kInput(i * 2 + 1) = sides.corners(i).Y
            Next
            ...
            If rc.index > 0 Then
                ...
                DrawTour(dst3(rc.rect), rc.contour, cv.Scalar.Yellow)
                For i = 0 To sides.corners.Count - 1
                    Dim pt = New cv.Point(CInt(atask.kalman.kOutput(i * 2)), CInt(atask.kalman.kOutput(i * 2 + 1)))
                    DrawCircle(dst3, pt, atask.DotSize + 2, sides.cornerColors(i))
                Next
            End If
