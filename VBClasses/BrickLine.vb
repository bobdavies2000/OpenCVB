...
Namespace VBClasses
    Public Class BrickLine_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            atask.featList.Clear()
            For Each index In edgeSorted.Values
                If featList(index).Count > 0 Then atask.featList.Add(featList(index))
            Next

            Dim edgeIndex = Math.Abs(atask.gOptions.DebugSlider.Value)
            If edgeIndex <> 0 And edgeIndex < atask.featList.Count Then
                For Each index In atask.featList(edgeIndex)
                    Dim gr = atask.bricks.brickList(index)
                    dst2.Rectangle(gr.rect, atask.highlight, atask.lineWidth)
                Next
            End If

            For i = 0 To atask.featList.Count - 1
                If i <> Math.Abs(atask.gOptions.DebugSlider.Value) Then Continue For
                Dim depthSorted As New SortedList(Of Single, Integer)(New compareAllowIdenticalSingle)
                For Each index In atask.featList(i)
                    Dim gr = atask.bricks.brickList(index)
                ...
                For Each ele In depthSorted
                    If Math.Abs(ele.Key - lastDepth) > atask.depthDiffMeters Then
                        Dim gr = atask.bricks.brickList(ele.Value)
                        dst2.Rectangle(gr.rect, red, atask.lineWidth + 1)
                    End If
                    lastDepth = ele.Key
                Next
            Next
            labels(3) = CStr(atask.featList.Count) + " features are present in the input lines or edges"
        End Sub
    End Class
    ...
    Public Class BrickLine_Edges : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            findCells.Run(atask.grayStable)
            dst2 = findCells.dst2
            dst3 = findCells.dst3
        End Sub
    End Class
    ...
    Public Class BrickLine_DepthGap : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To atask.featList.Count - 1
                If atask.featList(i).Count = 0 Then Exit For
                Dim depthSorted As New SortedList(Of Single, Integer)(New compareAllowIdenticalSingle)
                For Each index In atask.featList(i)
                    Dim gr = atask.bricks.brickList(index)
                    depthSorted.Add(gr.depth, index)
                ...
                Dim lastDepth = depthSorted.ElementAt(0).Key
                For Each ele In depthSorted
                    If Math.Abs(ele.Key - lastDepth) > atask.depthDiffMeters Then gapCells.Add(atask.bricks.brickList(ele.Value).index)
                    lastDepth = ele.Key
                Next
            Next
            ...
            If atask.heartBeat Then
                ...
                Dim debugMode = atask.gOptions.DebugSlider.Value <> 0
                For i = 0 To gapCells.Count - 1
                    If debugMode Then If i <> Math.Abs(atask.gOptions.DebugSlider.Value) Then Continue For
                    Dim gr = atask.bricks.brickList(gapCells(i))
                    dst2.Rectangle(gr.rect, atask.highlight, atask.lineWidth)
                    If i = Math.Abs(atask.gOptions.DebugSlider.Value) Then
    ...
    Public Class NR_BrickLine_DepthGaps : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each lp In atask.lines.lpList
                dst1.Line(lp.p1, lp.p2, lp.index, atask.lineWidth, cv.LineTypes.Link8)
            Next
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If standaloneTest() Then
                For Each index In edges
                    DrawRect(dst2, atask.gridRects(index), white)
                Next
                For Each index In noEdges
                    DrawRect(dst3, atask.gridRects(index), white)
                Next
            End If
    ...
    Public Class BrickLine_LeftRight : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each index In leftEdges
                DrawRect(mats.mat(2), atask.gridRects(index), white)
            Next
            ...
            For Each index In rightEdges
                DrawRect(mats.mat(3), atask.gridRects(index), white)
            Next
            ...
            For Each index In leftEdges
                ...
                gr.rect = atask.gridRects(index)

                ' too close to the edges of the image
                If atask.gridNabeRects(index).Width + gr.rect.X + atask.brickSize * 2 > dst2.Width Then Continue For
                If atask.gridNabeRects(index).Height + gr.rect.Y + atask.brickSize * 2 > dst2.Height Then Continue For

                gr.lRect = gr.rect
                gr.depth = atask.pcSplit(2)(gr.rect).Mean()(0)
                If gr.depth > 0 Then
                    gr.rRect = gr.rect
                    ...
                    cv.Cv2.MatchTemplate(atask.leftView(gr.lRect), atask.rightView(gr.rRect), correlationMat,
            ...
            labels(3) = CStr(bestBricks.Count) + " bricks had lines and correlation >" + Format(atask.fCorrThreshold, fmt2) + ") or " +
                  Format(bestBricks.Count / atask.gridRects.Count, "00%") + " of all the bricks"
        End Sub
    End Class
End Namespace