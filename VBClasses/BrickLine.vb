        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To task.featList.Count - 1
                ...
                For Each index In task.featList(i)
                    Dim gRect = bricks.brickList(index)
                    depthSorted.Add(gRect.depth, index)
                Next
                ...
                For Each ele In depthSorted
                    If Math.Abs(ele.Key - lastDepth) > task.depthDiffMeters Then
                        Dim gRect = bricks.brickList(ele.Value)
                        dst2.Rectangle(gRect.rect, red, task.lineWidth + 1)
                    End If
                    lastDepth = ele.Key
                Next
            Next
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If task.heartBeat Then
                ...
                For i = 0 To gapCells.Count - 1
                    ...
                    Dim gRect = bricks.brickList(gapCells(i))
                    dst2.Rectangle(gRect.rect, task.highlight, task.lineWidth)
                    If i = Math.Abs(task.gOptions.DebugSlider.Value) Then
                        SetTrueText(Format(gRect.depth, fmt1), gRect.rect.BottomRight)
                    End If
                Next
            End If
            labels(3) = CStr(task.featList.Count) + " features are present in the input lines or edges"
    ...
    Public Class BrickLine_LeftRight : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each index In leftEdges
                ...
                gRect.rect = task.gridRects(index)

                ' too close to the edges of the image
                If task.gridNabeRects(index).Width + gRect.rect.X + task.brickEdgeLen * 2 > dst2.Width Then Continue For
                If task.gridNabeRects(index).Height + gRect.rect.Y + task.brickEdgeLen * 2 > dst2.Height Then Continue For

                gRect.lRect = gRect.rect
                gRect.depth = task.pcSplit(2)(gRect.rect).Mean()(0)
                If gRect.depth > 0 Then
                    ...
                    If gRect.rRect.X < 0 Or gRect.rRect.X + gRect.rRect.Width >= dst2.Width Then Continue For

                    cv.Cv2.MatchTemplate(task.leftView(gRect.lRect), task.rightView(gRect.rRect), correlationMat,
                                     cv.TemplateMatchModes.CCoeffNormed)

                    gRect.correlation = correlationMat.Get(Of Single)(0, 0)
                    If gRect.correlation >= task.fCorrThreshold Then
                        DrawRect(dst2, gRect.rect, white)
                        DrawRect(dst3, gRect.rRect, red)
