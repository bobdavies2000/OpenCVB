...
Namespace VBClasses
    Public Class Feature_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskAlg.heartBeat Then labels(2) = strOut
        End Sub
    End Class
    ...
    Public Class Feature_BrickLine : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each pt In taskAlg.feat.features
            ...
            Dim brickLines(taskAlg.lines.lpList.Count - 1) As List(Of cv.Point)
            dst3.SetTo(0)
            features.Clear()
            For Each pt In sortByGrid.Values
                Dim lineIndex = taskAlg.lines.dst1.Get(Of Byte)(pt.Y, pt.X) - 1
    ...
    Public Class Feature_General : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If standaloneTest() Then dst2 = taskAlg.color.Clone

            Dim ptLatest As New List(Of cv.Point2f)
            Dim ptNew As New List(Of cv.Point2f)
            If taskAlg.optionsChanged = False Then
                For Each pt In taskAlg.features
                    Dim val = taskAlg.motionMask.Get(Of Byte)(pt.Y, pt.X)
                    If val = 0 Then ptNew.Add(pt)
                Next
            End If

            strOut = ""
            Select Case taskAlg.featureOptions.FeatureMethod.Text
                Case "GoodFeatures"
                    ptLatest = cv.Cv2.GoodFeaturesToTrack(taskAlg.gray, taskAlg.FeatureSampleSize, options.quality,
            ...
            taskAlg.fpFromGridCellLast = New List(Of Integer)(taskAlg.fpFromGridCell)
            taskAlg.fpLastList = New List(Of fpData)(taskAlg.fpList)

            If taskAlg.optionsChanged Or ptNew.Count = 0 Then
            ...
            taskAlg.featurePoints.Clear()
            taskAlg.fpFromGridCell.Clear()
            For i = 0 To sortByGrid.Count - 1
                Dim pt = sortByGrid.ElementAt(i).Value
                taskAlg.features.Add(pt)
                taskAlg.featurePoints.Add(New cv.Point(pt.X, pt.Y))

                Dim nextIndex = taskAlg.gridMap.Get(Of Integer)(pt.Y, pt.X)
            ...
            strOut += "  " + CStr(taskAlg.features.Count) + " features were found using '" + taskAlg.featureOptions.FeatureMethod.Text +
                  "' method."
            If taskAlg.heartBeat Then labels(2) = strOut
        End Sub
        Public Sub Close()
    ...
    Public Class Feature_Delaunay : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each pt In delaunay.bPoint.ptList
                DrawCircle(dst3, pt, taskAlg.DotSize, white)
            Next
            labels(3) = "There were " + CStr(taskAlg.features.Count) + " Delaunay contours"
        End Sub
    End Class
    ...
    Public Class Feature_LucasKanade : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskAlg.heartBeat Then labels(3) = CStr(stationary) + " features were stationary and " + CStr(motion) + " features had some motion."
    ...
    Public Class Feature_Points : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each pt In taskAlg.features
                DrawCircle(dst2, pt, taskAlg.DotSize, taskAlg.highlight)
            Next
            labels(2) = CStr(taskAlg.features.Count) + " targets were present with " + CStr(taskAlg.FeatureSampleSize) + " requested."
        End Sub
    End Class
    ...
    Public Class Feature_TraceDelaunay : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskAlg.optionsChanged Then goodList.Clear()

            Dim ptList As New List(Of cv.Point2f)(taskAlg.features)
            goodList.Add(ptList)

            If goodList.Count >= taskAlg.frameHistoryCount Then goodList.RemoveAt(0)

            dst2.SetTo(0)
            For Each ptList In goodList
                For Each pt In ptList
                    DrawCircle(taskAlg.color, pt, taskAlg.DotSize, taskAlg.highlight)
                    Dim c = dst3.Get(Of cv.Vec3b)(pt.Y, pt.X)
                    DrawCircle(dst2, pt, taskAlg.DotSize + 1, c)
                Next
            Next
            labels(2) = CStr(taskAlg.features.Count) + " features were identified in the image."
        End Sub
    End Class
    ...
    Public Class Feature_ShiTomasi : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If options.useShiTomasi Then
                dst2 = taskAlg.leftView
                dst3 = taskAlg.rightView
                shiTomasi.Run(taskAlg.leftView)
                dst2.SetTo(cv.Scalar.White, shiTomasi.dst3.CvtColor(cv.ColorConversionCodes.BGR2GRAY))

                shiTomasi.Run(taskAlg.rightView)
                dst3.SetTo(taskAlg.highlight, shiTomasi.dst3.CvtColor(cv.ColorConversionCodes.BGR2GRAY))
            Else
                harris.Run(taskAlg.leftView)
                dst2 = harris.dst2.Clone
                harris.Run(taskAlg.rightView)
                dst3 = harris.dst2
            End If
        End Sub
    End Class
    ...
    Public Class Feature_Generations : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each pt In taskAlg.featurePoints
            ...
            If taskAlg.heartBeat And gens.Count > 0 Then
    ...
    Public Class Feature_History : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            featureHistory.Add(New List(Of cv.Point)(taskAlg.featurePoints))
            ...
            Dim threshold = If(taskAlg.frameHistoryCount = 1, 0, 1)
            features.Clear()
            ...
            For i = 0 To newFeatures.Count - 1
                If gens(i) > threshold Then
                    ...
                    features.Add(pt)
                    If gens(i) < taskAlg.frameHistoryCount Then
            ...
            If featureHistory.Count > taskAlg.frameHistoryCount Then featureHistory.RemoveAt(0)
            If taskAlg.heartBeat Then
                labels(2) = CStr(features.Count) + "/" + CStr(whiteCount) + " present/present on every frame" +
                        ...
                        CStr(taskAlg.frameHistoryCount) + " frames"
            End If
        End Sub
    End Class
    ...
    Public Class Feature_AKaze : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i As Integer = 0 To kazeKeyPoints.Length - 1
                DrawCircle(dst2, kazeKeyPoints(i).Pt, taskAlg.DotSize, taskAlg.highlight)
            Next
        End Sub
        Public Sub Close()
    ...
    Public Class Feature_WithDepth : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each pt In taskAlg.featurePoints
                Dim index = taskAlg.gridMap.Get(Of Integer)(pt.Y, pt.X)
                If taskAlg.bricks.brickList(index).depth > 0 Then
                    DrawCircle(dst2, pt, taskAlg.DotSize, taskAlg.highlight)
                    depthCount += 1
                End If
            Next
            labels(2) = CStr(depthCount) + " features had depth or " + Format(depthCount / taskAlg.features.Count, "0%")
        End Sub
    End Class
    ...
    Public Class Feature_Matching : Inherits TaskParent
        ...
        Dim feat As New Feature_General
        Public Sub New()
            taskAlg.featureOptions.FeatureSampleSize.Value = 150
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each pt In features
                ...
                If val = 0 Then
                    ...
                    Dim r = taskAlg.gridRects(index)
                    match.template = fpLastSrc(r)
                    match.Run(src(r))
                    If match.correlation > taskAlg.fCorrThreshold Then matched.Add(pt)
                Else
                    motionPoints.Add(pt)
                End If
            Next
            ...
            If matched.Count < taskAlg.FeatureSampleSize / 2 Then
                feat.Run(src)
                features = taskAlg.featurePoints
            Else
    ...
    Public Class Feature_SteadyCam : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Static features As New List(Of cv.Point)(taskAlg.featurePoints)
            Static lastSrc As cv.Mat = src.Clone

            Dim resync = features.Count / taskAlg.features.Count < options.resyncThreshold
            If taskAlg.heartBeat Or taskAlg.optionsChanged Or resync Then
                features = New List(Of cv.Point)(taskAlg.featurePoints)
            End If
            ...
            For Each pt In ptList
                ...
                If correlationMat.Get(Of Single)(0, 0) >= taskAlg.fCorrThreshold Then
                    features.Add(pt)
                End If
            Next
    ...
    Public Class Feature_FacetPoints : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            delaunay.inputPoints = taskAlg.features
            delaunay.Run(src)
            ...
            For Each pt In ptList
                Dim index = taskAlg.redList.rcMap.Get(Of Byte)(pt.Y, pt.X)
                If index = 0 Then Continue For
                Dim rc = taskAlg.redList.oldrclist(index)
                Dim val = taskAlg.pcSplit(2).Get(Of Single)(pt.Y, pt.X)
                If val <> 0 Then
                    rc.ptFacets.Add(pt)
                    taskAlg.redList.oldrclist(index) = rc
                End If
            Next
            ...
            If standalone And taskAlg.redList.oldrclist.Count > 0 Then
                taskAlg.color.Rectangle(taskAlg.oldrcD.rect, taskAlg.highlight, taskAlg.lineWidth)
                For Each pt In taskAlg.oldrcD.ptFacets
                    DrawCircle(taskAlg.color, pt, taskAlg.DotSize, taskAlg.highlight)
                Next
            End If
        End Sub
    ...
    Public Class Feature_StableVisual : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            Dim lastFeatures As New List(Of cv.Point)(taskAlg.featurePoints)
    ...
    Public Class Feature_KNN : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To knn.neighbors.Count - 1
                ...
                Dim qPt = taskAlg.features(i)
                If pt.DistanceTo(qPt) > 2 Then knn.trainInput(trainIndex) = taskAlg.features(i)
            Next
            ...
            For Each pt In featurePoints
                DrawCircle(dst2, pt, taskAlg.DotSize + 2, white)
                DrawCircle(dst3, pt, taskAlg.DotSize + 2, white)
            Next
