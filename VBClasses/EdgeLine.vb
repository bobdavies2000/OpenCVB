...
Namespace VBClasses
    Public Class EdgeLine_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To classCount * 4 - 4 Step 4
                ...
                rcList.Add(rc)
                If standaloneTest() Then dst3(rc.rect).SetTo(taskAlg.scalarColors(rc.gIndex), rc.mask)
            Next

            labels(2) = CStr(classCount) + " line segments were found with motion threshold of " +
                        CStr(taskAlg.motionThreshold) + " pixels changed in a grid rect."
        End Sub
        Public Sub Close()
    ...
    Public Class EdgeLine_Motion : Inherits TaskParent
        ...
        Private Sub rcDataDraw(rc As rcData)
            ...
            cv.Cv2.Polylines(dst2(rc.rect), nextList, False, cv.Scalar.All(rc.index), taskAlg.lineWidth, taskAlg.lineType)
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If edgeLine.rcList.Count Then
                ...
                cv.Cv2.CalcHist({dst2}, {0}, taskAlg.motionMask, histogram,
            ...
            cv.Cv2.CalcHist({edgeLine.dst2}, {0}, taskAlg.motionMask, histogram,
            ...
            For Each rc In edgeLine.rcList
                If histarray(rc.index - 1) > 0 And rc.contour.Count > 0 Then
                    ...
                    If rc.contour.Count > 0 Then
                        Dim gIndex = taskAlg.gridMap.Get(Of Integer)(rc.contour(0).Y, rc.contour(0).X)
                        rc.color = taskAlg.vecColors(gIndex Mod 255)
                    End If
                    newList.Add(rc)
    ...
    Public Class EdgeLine_Simple : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim handleSrc = GCHandle.Alloc(cppData, GCHandleType.Pinned)
            Dim imagePtr = EdgeLineSimple_RunCPP(cPtr, handleSrc.AddrOfPinnedObject(), input.Rows, input.Cols, taskAlg.lineWidth * 2)
            handleSrc.Free()
    ...
    Public Class EdgeLine_Segments : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Marshal.Copy(src.Data, cppData, 0, cppData.Length)
            Dim handleSrc = GCHandle.Alloc(cppData, GCHandleType.Pinned)
            Dim imagePtr = EdgeLineRaw_RunCPP(cPtr, handleSrc.AddrOfPinnedObject(), src.Rows, src.Cols, taskAlg.lineWidth)
            handleSrc.Free()
    ...
    Public Class EdgeLine_SimpleMotion : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            Dim input = If(src.Channels() = 1, src.Clone, taskAlg.grayStable.Clone)

            Dim cppData(input.Total - 1) As Byte
            Marshal.Copy(input.Data, cppData, 0, cppData.Length)
            Dim handleSrc = GCHandle.Alloc(cppData, GCHandleType.Pinned)

            Dim maskData(taskAlg.motionMask.Total - 1) As Byte
            Marshal.Copy(taskAlg.motionMask.Data, maskData, 0, maskData.Length)
    ...
    Public Class EdgeLine_LeftRight : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            edges.Run(taskAlg.leftViewStable)
