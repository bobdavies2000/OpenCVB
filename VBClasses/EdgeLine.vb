...
Namespace VBClasses
    Public Class EdgeLine_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To classCount * 4 - 4 Step 4
                ...
                rcList.Add(rc)
                If standaloneTest() Then dst3(rc.rect).SetTo(tsk.scalarColors(rc.gridIndex Mod 255), rc.mask)
            Next

            labels(2) = CStr(classCount) + " line segments were found with motion threshold of " +
                        CStr(tsk.motionThreshold) + " pixels changed in a grid rect."
        End Sub
    ...
    Public Class NR_EdgeLine_Motion : Inherits TaskParent
        ...
        Private Sub rcDataDraw(rc As rcData)
            ...
            cv.Cv2.Polylines(dst2(rc.rect), nextList, False, cv.Scalar.All(rc.index), tsk.lineWidth, tsk.lineType)
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If edgeLine.rcList.Count Then
                ...
                cv.Cv2.CalcHist({dst2}, {0}, tsk.motionRGB.motionMask, histogram,
            ...
            cv.Cv2.CalcHist({edgeLine.dst2}, {0}, tsk.motionRGB.motionMask, histogram,
            ...
            For Each rc In edgeLine.rcList
                If histarray(rc.index - 1) > 0 And rc.contour.Count > 0 Then
                    ...
                    If rc.contour.Count > 0 Then
                        Dim gIndex = tsk.gridMap.Get(Of Integer)(rc.contour(0).Y, rc.contour(0).X)
                        rc.color = tsk.vecColors(gIndex Mod 255)
                    End If
                    newList.Add(rc)
    ...
    Public Class NR_EdgeLine_Simple : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim handleSrc = GCHandle.Alloc(cppData, GCHandleType.Pinned)
            Dim imagePtr = EdgeLineSimple_RunCPP(cPtr, handleSrc.AddrOfPinnedObject(), input.Rows, input.Cols, tsk.lineWidth * 2)
            handleSrc.Free()
    ...
    Public Class NR_EdgeLine_Segments : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Marshal.Copy(src.Data, cppData, 0, cppData.Length)
            Dim handleSrc = GCHandle.Alloc(cppData, GCHandleType.Pinned)
            Dim imagePtr = EdgeLineRaw_RunCPP(cPtr, handleSrc.AddrOfPinnedObject(), src.Rows, src.Cols, tsk.lineWidth)
            handleSrc.Free()
    ...
    Public Class NR_EdgeLine_SimpleMotion : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            Dim input = If(src.Channels() = 1, src.Clone, tsk.grayStable.Clone)

            Dim cppData(input.Total - 1) As Byte
            Marshal.Copy(input.Data, cppData, 0, cppData.Length)
            Dim handleSrc = GCHandle.Alloc(cppData, GCHandleType.Pinned)

            Dim maskData(tsk.motionRGB.motionMask.Total - 1) As Byte
            Marshal.Copy(tsk.motionRGB.motionMask.Data, maskData, 0, maskData.Length)
