...
Namespace VBClasses
    Public Class Brick_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskA.bricks.brickList.Count <> taskA.gridRects.Count Then taskA.bricks.brickList.Clear()

            Dim correlationMat As New cv.Mat
            Dim maxPixels = taskA.brickSize * taskA.brickSize
            ...
            For i = 0 To taskA.gridRects.Count - 1
                ...
                gr.rect = taskA.gridRects(gr.index)
                gr.lRect = gr.rect

                gr.depth = taskA.pcSplit(2)(gr.rect).Mean(taskA.depthmask(gr.rect))
                gr.mmDepth = GetMinMax(taskA.pcSplit(2)(gr.rect), taskA.depthmask(gr.rect))
                ...
                If gr.depth > 0 Then
                    ...
                    If gr.lRect.Width = gr.rRect.Width Then
                        cv.Cv2.MatchTemplate(taskA.leftView(gr.lRect), taskA.rightView(gr.rRect),
            ...
            If standaloneTest() Then
                ...
                Dim index = taskA.gridMap.Get(Of Integer)(taskA.mouseMovePoint.Y, taskA.mouseMovePoint.X)
                Dim br = taskA.bricks.brickList(index)
                dst2.Rectangle(br.lRect, taskA.highlight, taskA.lineWidth + 1)
                taskA.color.Rectangle(br.lRect, taskA.highlight, taskA.lineWidth + 1)
                dst3.Rectangle(br.rRect, taskA.highlight, taskA.lineWidth + 1)
            End If

            If taskA.heartBeat Then
                labels(2) = CStr(taskA.bricks.brickList.Count) + " bricks and " +
    ...
    Public Class NR_Brick_FullDepth : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim col As Integer, bricksPerRow = taskA.bricksPerRow
            Static whiteCol As Integer = bricksPerRow / 2
            If taskA.mouseClickFlag Then
                whiteCol = Math.Round(bricksPerRow * (taskA.clickPoint.X - taskA.brickSize / 2) / dst2.Width)
            End If
            For Each gr In taskA.bricks.brickList
                If gr.depth > 0 Then
                    Dim color = If(col = whiteCol, cv.Scalar.Black, taskA.scalarColors(255 * (col / bricksPerRow)))
                    dst2.Rectangle(gr.rect, color, taskA.lineWidth)
                    dst3.Rectangle(gr.rRect, color, taskA.lineWidth)
                End If
                col += 1
    ...
    Public Class NR_Brick_InstantUpdate : Inherits TaskParent
        Public Sub New()
            If taskA.bricks Is Nothing Then taskA.bricks = New Brick_Basics
            taskA.bricks = New Brick_Basics
            taskA.bricks.instantUpdate = True
            labels(3) = "Pointcloud image for cells with good visibility"
            desc = "Create the grid of bricks with good visibility"
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If taskA.heartBeat Then labels(2) = CStr(taskA.bricks.brickDepthCount) + " bricks have reasonable depth."

            dst2 = taskA.bricks.dst2
            dst3 = taskA.bricks.dst3

            labels(2) = taskA.bricks.labels(2)
        End Sub
    End Class
    ...
    Public Class NR_Brick_MLColorDepth : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To bounds.boundaryCells.Count - 1
                ...
                Dim gr = taskA.gridRects(nList(0))
                ...
                For j = 1 To nList.Count - 1
                    Dim grA = taskA.gridRects(nList(j))
                    Dim x As Integer = Math.Floor(grA.X * taskA.bricksPerRow / taskA.cols)
                    Dim y As Integer = Math.Floor(grA.Y * taskA.bricksPerCol / taskA.rows)
                    Dim val = taskA.lowResColor.Get(Of cv.Vec3f)(y, x)
                    trainRGB.Set(Of cv.Vec3f)(j - 1, 0, val)
                    trainDepth.Set(Of Single)(j - 1, 0, taskA.lowResDepth.Get(Of Single)(y, x))
                ...
                For j = 0 To edgePixels.Rows - 1
                    ...
                    Dim depth = taskA.pcSplit(2)(gr).Get(Of Single)(pt.Y, pt.X)
                    trainDepth.Set(Of Single)(index + j, 0, depth)
                Next

                ml.trainMats = {trainRGB, trainDepth}

                Dim grB = taskA.gridRects(nList(0))
                ml.testMats = {rgb32f(grB), taskA.pcSplit(2)(grB)}
                ml.Run(src)
    ...
    Public Class Brick_CorrelationInput : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If taskA.optionsChanged Then Exit Sub ' settle down first...
            ...
            If index < 0 Or index > taskA.bricks.brickList.Count Then Exit Sub
            taskA.brickD = taskA.bricks.brickList(index)

            Dim corr = taskA.brickD.correlation
            DrawCircle(dst2, taskA.brickD.lRect.TopLeft, taskA.DotSize, 255)
            Dim pt = New cv.Point(taskA.brickD.rect.TopLeft.X, taskA.brickD.rect.TopLeft.Y - 10)
            ...
            DrawRect(dst2, taskA.brickD.lRect, grayScale)
            DrawRect(dst3, taskA.brickD.rRect, grayScale)
            labels(2) = "The correlation coefficient at " + taskA.brickD.rect.TopLeft.ToString + " is " + Format(corr, fmt3)
        End Sub
    End Class
    ...
    Public Class Brick_Info : Inherits TaskParent
        Public Sub New()
            If taskA.bricks Is Nothing Then taskA.bricks = New Brick_Basics
            taskA.clickPoint = New cv.Point(dst2.Width / 2, dst2.Height / 2)
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim gr As brickData = taskA.bricks.brickList(index)
            dst2 = src
    ...
    Public Class NR_Brick_LeftToColor : Inherits TaskParent
        Public Sub New()
            If taskA.bricks Is Nothing Then taskA.bricks = New Brick_Basics
            If taskA.Settings.cameraName.StartsWith("Intel(R) RealSense(TM) Depth Camera") Then taskA.gOptions.gravityPointCloud.Checked = False
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each gr In taskA.bricks.brickList
                If gr.depth > 0 Then
                    count += 1
                    DrawCircle(taskA.color, gr.rect.TopLeft)
    ...
    Public Class Brick_LeftRightMouse : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If standalone And taskA.testAllRunning Then
            ...
            For Each index In myBricks
                ...
                dst0.Rectangle(gr.rect, taskA.highlight, taskA.lineWidth)
                dst2.Rectangle(gr.lRect, taskA.highlight, taskA.lineWidth)
                dst3.Rectangle(gr.rRect, taskA.highlight, taskA.lineWidth)
            Next
            If taskA.mouseClickFlag Then myBricks.Clear()
        End Sub
    End Class
    ...
    Public Class NR_Brick_RGBtoLeft : Inherits TaskParent
        Public Sub New()
            ...
            If taskA.Settings.cameraName.StartsWith("Intel(R) RealSense(TM) Depth Camera") Then
                taskA.gOptions.gravityPointCloud.Checked = False
            End If
            desc = "Translate the RGB to left view - only needed for the Intel RealSense cameras."
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            Dim camInfo = taskA.calibData, correlationMat As New cv.Mat
            Dim index As Integer = taskA.gridMap.Get(Of Integer)(taskA.mouseMovePoint.Y, taskA.mouseMovePoint.X)
            Dim gr As brickData
            If index > 0 And index < taskA.bricks.brickList.Count Then
                gr = taskA.bricks.brickList(index)
            Else
                gr = taskA.bricks.brickList(taskA.bricks.brickList.Count / 2)
            End If
            ...
            dst2.Rectangle(New cv.Rect(irPt.X, irPt.Y, gr.rect.Width, gr.rect.Height), taskA.highlight, taskA.lineWidth)
            dst3.Rectangle(gr.rRect, taskA.highlight, taskA.lineWidth)
        End Sub
    End Class
    ...
    Public Class NR_Brick_RegionLines : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each lp In taskA.lines.lpList
                ...
                If c1 <> c2 Then
                    dst3.Line(lp.p1, lp.p2, white, taskA.lineWidth, taskA.lineWidth)
                Else
                    dst2.Line(lp.p1, lp.p2, white, taskA.lineWidth, taskA.lineWidth)
                End If
            Next
        End Sub
    End Class
    ...
    Public Class NR_Brick_CloudMaxVal : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each gr In taskA.bricks.brickList
                If gr.depth > 0 Then
                    If gr.age < 10 Then
                        Dim split() As cv.Mat = taskA.pointCloud(gr.rect).Split
                        split(2).SetTo(gr.mm.maxVal, taskA.depthmask(gr.rect))

                        cv.Cv2.Multiply(template.dst2(gr.rect), split(2), split(0))
                        split(0) *= cv.Scalar.All(1 / taskA.calibData.leftIntrinsics.fx)

                        cv.Cv2.Multiply(template.dst3(gr.rect), split(2), split(1))
                        split(1) *= cv.Scalar.All(1 / taskA.calibData.leftIntrinsics.fy)
            ...
            labels(2) = CStr(splitCount) + " bricks of " + CStr(taskA.gridRects.Count) + " were modified."
            If standaloneTest() Then dst3 = taskA.pointCloud
        End Sub
    End Class
    ...
    Public Class NR_Brick_CloudMean : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each gr In taskA.bricks.brickList
                If gr.depth > 0 Then
                    Dim split() As cv.Mat = taskA.pointCloud(gr.rect).Split
                    split(2).SetTo(gr.depth, taskA.depthmask(gr.rect))
            ...
            For Each gr In taskA.bricks.brickList
                If gr.depth > 0 And gr.age = 1 Then
                    If gr.mm.range > newRange Then ' if the range within a gr is > 10 cm's, fit it within 10 cm's.
                        ...
                        cv.Cv2.Merge({split(0), split(1), split(2)}, dst2(gr.rect))
                        dst2(gr.rect).SetTo(0, taskA.noDepthMask(gr.rect))
                        splitCount += 1
                    End If
                End If
            Next
    ...
    Public Class Brick_Features : Inherits TaskParent
        ...
        Public Sub New()
            taskA.gOptions.LineWidth.Value = 3
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            dst2 = taskA.feat.dst2

            featureBricks.Clear()
            Dim featList As New List(Of cv.Point)(taskA.feat.features)
            For Each pt In featList
                Dim index As Integer = taskA.gridMap.Get(Of Integer)(pt.Y, pt.X)
                featureBricks.Add(taskA.gridRects(index))
            Next

            If taskA.gOptions.DebugCheckBox.Checked Then
                For Each pt In featList
                    DrawCircle(dst2, pt, taskA.DotSize, cv.Scalar.Black)
                Next
            End If
            ...
            If taskA.heartBeat Then
                Dim flessCount = taskA.gridRects.Count - featureBricks.Count
    ...
    Public Class Brick_FeaturesAndEdges : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            boundaryCells.Clear()
            For Each nabeList In taskA.grid.gridNeighbors
                Dim grA = taskA.gridRects(nabeList(0))
                ...
                If centerType <> 0 Then
                    ...
                    For i = 1 To nabeList.Count - 1
                        Dim grB = taskA.gridRects(nabeList(i))
    ...
    Public Class NR_Brick_CorrelationMap : Inherits TaskParent
        ...
        Public Function ShowPaletteCorrelation(input As cv.Mat) As cv.Mat
            ...
            cv.Cv2.ApplyColorMap(input, output, taskA.correlationColorMap)
            Return output
        End Function
        Public Overrides Sub RunAlg(src As cv.Mat)
            dst1.SetTo(0)
            taskA.depthAndDepthRange = ""
            For Each gr In taskA.bricks.brickList
            ...
            Dim pt = taskA.mouseMovePoint, w = taskA.workRes.Width, h = taskA.workRes.Height
            If pt.X >= 0 And pt.X < w And pt.Y >= 0 And pt.Y < h Then
                Dim index As Integer = taskA.gridMap.Get(Of Integer)(taskA.mouseMovePoint.Y, taskA.mouseMovePoint.X)
                taskA.brickD = taskA.bricks.brickList(index)
                taskA.depthAndDepthRange = "depth = " + Format(taskA.brickD.depth, fmt3) + "m ID=" +
                                      CStr(taskA.brickD.index) + vbCrLf + " range " +
                                      Format(taskA.brickD.mm.minVal, fmt1) + "-" +
                                      Format(taskA.brickD.mm.maxVal, fmt1) + "m, age = " +
                                      CStr(taskA.brickD.age) + vbCrLf +
                                      " correlation = " + Format(taskA.brickD.correlation, fmt3)

                Dim ptTextLoc = taskA.brickD.rect.TopLeft
                If ptTextLoc.X > w * 0.85 Or (ptTextLoc.Y < h * 0.15 And ptTextLoc.X > w * 0.15) Then
                    ptTextLoc.X -= w * 0.15
                Else
                    ptTextLoc.Y -= taskA.brickD.rect.Height * 3
                End If

                SetTrueText(taskA.depthAndDepthRange, ptTextLoc, 2)
                SetTrueText(taskA.depthAndDepthRange, 3)
            End If
            labels(2) = taskA.bricks.labels(2)
        End Sub
    End Class
    ...
    Public Class Brick_LeftRight : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To taskA.bricksPerRow - 1 Step 2
                colorIndex = 0
                For j = i To taskA.gridRects.Count - taskA.bricksPerRow - 1 Step taskA.bricksPerRow
                    Dim gr = taskA.bricks.brickList(j)
                    Dim color = taskA.scalarColors(colorIndex)
                    If gr.depth > 0 Then
                        dst2.Rectangle(gr.lRect, color, taskA.lineWidth)
                        dst3.Rectangle(gr.rRect, color, taskA.lineWidth)
                    End If
                    colorIndex += 1
                Next
    ...
    Public Class Brick_EdgeFlips : Inherits TaskParent
        ...
        Public Sub New()
            ...
            taskA.featureOptions.EdgeMethods.SelectedItem() = "Laplacian"
            desc = "Add edges to features"
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            Static stateList As New List(Of Single)
            Static lastDepth As cv.Mat = taskA.lowResDepth.Clone
            ...
            If taskA.optionsChanged Or stateList.Count = 0 Then
                stateList.Clear()
                For Each n In flist
                    stateList.Add(n)
                Next
            ...
            For Each r In fLessRects
                Dim x = CInt(r.X / taskA.brickSize)
                Dim y = CInt(r.Y / taskA.brickSize)
                taskA.lowResDepth.Set(Of Single)(y, x, lastDepth.Get(Of Single)(y, x))
            Next
            lastDepth = taskA.lowResDepth.Clone
            If taskA.heartBeat Then
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each gr As brickData In taskA.bricks.brickList
                If dst2(gr.lRect).CountNonZero And gr.rRect.Width > 0 And gr.correlation > options.correlation Then
                    taskA.rightView(gr.rRect).CopyTo(dst3(gr.rRect))
                    count += 1
                End If
            Next
            labels(3) = CStr(count) + " of " + CStr(taskA.bricks.brickList.Count) + " rects were identified in dst3"
        End Sub
    End Class
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each gr As brickData In taskA.bricks.brickList
                If dst2(gr.lRect).CountNonZero And gr.rRect.Width > 0 And
                    ...
                    taskA.leftView(gr.lRect).CopyTo(dst3(gr.lRect))
                    count += 1
                End If
            Next
            labels(3) = CStr(count) + " of " + CStr(taskA.bricks.brickList.Count) +
    ...
    Public Class Brick_Plot : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim index As Integer = taskA.gridMap.Get(Of Integer)(taskA.mouseMovePoint.Y, taskA.mouseMovePoint.X)
            If taskA.bricks.brickList.Count = 0 Or taskA.optionsChanged Then Exit Sub

            Dim gr As brickData
            If index < 0 Or index >= taskA.bricks.brickList.Count Then
                gr = taskA.bricks.brickList(taskA.bricks.brickList.Count / 2)
                taskA.mouseMovePoint = New cv.Point(gr.rect.X + gr.rect.Width / 2, gr.rect.Y + gr.rect.Height / 2)
            Else
                gr = taskA.bricks.brickList(index)
            End If

            Dim split() = taskA.pointCloud(gr.rect).Split()
            Dim mm = GetMinMax(split(2))
            If Single.IsInfinity(mm.maxVal) Then Exit Sub

            Static lastMouse As cv.Point = taskA.mouseMovePoint
            If taskA.heartBeat Or lastMouse <> taskA.mouseMovePoint Then
                lastMouse = taskA.mouseMovePoint
    ...
    Public Class Brick_NoDepthLines : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst3.SetTo(0)
            For Each gr In taskA.bricks.brickList
                If dst2(gr.lRect).CountNonZero And taskA.noDepthMask(gr.rect).CountNonZero Then
                    If gr.mm.maxVal - gr.mm.minVal > options.mmRange Then
                        taskA.leftView(gr.lRect).CopyTo(dst3(gr.lRect))
                        count += 1
                    End If
