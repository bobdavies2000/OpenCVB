...
Namespace VBClasses
    ...
    Public Class Gravity_Basics : Inherits TaskParent
        ...
        Public Sub New()
            If standalone Then atask.gOptions.CrossHairs.Checked = True
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            atask.lpHorizon = New lpData(endpoints.p1, endpoints.p2)
            atask.lpGravity = Line_PerpendicularTest.computePerp(atask.lpHorizon)
    ...
    Public Class Gravity_CloudMethod : Inherits TaskParent
        ...
        Public Shared Sub showVectors(dst As cv.Mat)
            dst.Line(atask.lpGravity.pE1, atask.lpGravity.pE2, white, atask.lineWidth, atask.lineType)
            dst.Line(atask.lpHorizon.pE1, atask.lpHorizon.pE2, white, atask.lineWidth, atask.lineType)
            'If atask.lpGravity Is Nothing Then
            '    dst.Line(atask.lines.lpList(0).p1, atask.lines.lpList(0).p2, atask.highlight, atask.lineWidth * 2, atask.lineType)
            '    dst.Line(atask.lines.lpList(0).pE1, atask.lines.lpList(0).pE2, white, atask.lineWidth, atask.lineType)
            'End If
        End Sub
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst3 = atask.pcSplit(0).InRange(-threshold, threshold)
            dst3.SetTo(0, atask.noDepthMask)
            Dim gPoints = dst3.FindNonZero()
            If gPoints.Rows = 0 Then
                ' build a fake gravity vector when we don't have anything so atask.lines.lplist has 1 entry.
            ...
            atask.lpGravity = New lpData(New cv.Point2f(xTop, 0), New cv.Point2f(xBot, dst2.Height))
            If standaloneTest() Then
                dst2 = atask.color
    ...
    Public Class NR_Gravity_RGB : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            Static rotateAngle As Double = atask.verticalizeAngle - 2
            Static rotateCenter As cv.Point2f = New cv.Point2f(dst2.Width / 2, dst2.Height / 2)

            rotateAngle += 0.1
            If rotateAngle >= atask.verticalizeAngle + 2 Then rotateAngle = atask.verticalizeAngle - 2
    ...
    Public Class NR_Gravity_BrickRotate : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            Dim angle = Math.Abs(atask.verticalizeAngle)
    ...
    Public Class NR_Gravity_BasicsOld : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Else
                ...
                atask.lpGravity = New lpData(lp.pE1, lp.pE2)
    ...
    Public Class NR_Gravity_BasicsOriginal : Inherits TaskParent
        ...
        Public Shared Function PrepareDepthInput(index As Integer) As cv.Mat
            If atask.gOptions.gravityPointCloud.Checked Then Return atask.pcSplit(index) ' already oriented to gravity

            ' rebuild the pointcloud so it is oriented to gravity.
            Dim pc = (atask.pointCloud.Reshape(1, atask.pointCloud.Rows * atask.pointCloud.Cols) * atask.gMatrix).ToMat.Reshape(3, atask.pointCloud.Rows)
            Dim split = pc.Split()
            Return split(index)
        End Function
            ...
            For y = startRow To stopRow Step stepRow
                For x = 0 To dst0.Cols - 1
                    ...
                    If val > 0 And lastVal < 0 Then
                        ...
                        ptX.Add(pt.X)
                        ptY.Add(pt.Y)
                        If ptX.Count >= atask.frameHistoryCount Then
