...
Namespace VBClasses
    ...
    Public Class Gravity_Basics : Inherits TaskParent
        ...
        Public Sub New()
            If standalone Then taskA.gOptions.CrossHairs.Checked = True
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            taskA.lpHorizon = New lpData(endpoints.p1, endpoints.p2)
            taskA.lpGravity = Line_PerpendicularTest.computePerp(taskA.lpHorizon)
    ...
    Public Class Gravity_CloudMethod : Inherits TaskParent
        ...
        Public Shared Sub showVectors(dst As cv.Mat)
            dst.Line(taskA.lpGravity.pE1, taskA.lpGravity.pE2, white, taskA.lineWidth, taskA.lineType)
            dst.Line(taskA.lpHorizon.pE1, taskA.lpHorizon.pE2, white, taskA.lineWidth, taskA.lineType)
            'If taskA.lpGravity Is Nothing Then
            '    dst.Line(taskA.lines.lpList(0).p1, taskA.lines.lpList(0).p2, taskA.highlight, taskA.lineWidth * 2, taskA.lineType)
            '    dst.Line(taskA.lines.lpList(0).pE1, taskA.lines.lpList(0).pE2, white, taskA.lineWidth, taskA.lineType)
            'End If
        End Sub
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst3 = taskA.pcSplit(0).InRange(-threshold, threshold)
            dst3.SetTo(0, taskA.noDepthMask)
            Dim gPoints = dst3.FindNonZero()
            If gPoints.Rows = 0 Then
                ' build a fake gravity vector when we don't have anything so taskA.lines.lplist has 1 entry.
            ...
            taskA.lpGravity = New lpData(New cv.Point2f(xTop, 0), New cv.Point2f(xBot, dst2.Height))
            If standaloneTest() Then
                dst2 = taskA.color
    ...
    Public Class NR_Gravity_RGB : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            Static rotateAngle As Double = taskA.verticalizeAngle - 2
            Static rotateCenter As cv.Point2f = New cv.Point2f(dst2.Width / 2, dst2.Height / 2)

            rotateAngle += 0.1
            If rotateAngle >= taskA.verticalizeAngle + 2 Then rotateAngle = taskA.verticalizeAngle - 2
            ...
            Dim incrX = dst1.Width / taskA.brickSize
            Dim incrY = dst1.Height / taskA.brickSize
            For y = 0 To taskA.brickSize - 1
                For x = 0 To taskA.brickSize - 1
    ...
    Public Class NR_Gravity_BrickRotate : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            Dim angle = Math.Abs(taskA.verticalizeAngle)
    ...
    Public Class NR_Gravity_BasicsOld : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Else
                ...
                taskA.lpGravity = New lpData(lp.pE1, lp.pE2)
    ...
    Public Class NR_Gravity_BasicsOriginal : Inherits TaskParent
        ...
        Public Shared Function PrepareDepthInput(index As Integer) As cv.Mat
            If taskA.gOptions.gravityPointCloud.Checked Then Return taskA.pcSplit(index) ' already oriented to gravity

            ' rebuild the pointcloud so it is oriented to gravity.
            Dim pc = (taskA.pointCloud.Reshape(1, taskA.pointCloud.Rows * taskA.pointCloud.Cols) * taskA.gMatrix).ToMat.Reshape(3, taskA.pointCloud.Rows)
            Dim split = pc.Split()
            Return split(index)
        End Function
            ...
            For y = startRow To stopRow Step stepRow
                For x = 0 To dst0.Cols - 1
                    ...
                    If val > 0 And lastVal < 0 Then
                        ...
                        ptX.Add(pt.X)
                        ptY.Add(pt.Y)
                        If ptX.Count >= taskA.frameHistoryCount Then
