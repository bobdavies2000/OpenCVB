...
Namespace VBClasses
    Public Class DCT_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.Subtract(tsk.gray, dst2, dst3)
        End Sub
    End Class
    ...
    Public Class NR_DCT_Surfaces_debug : Inherits TaskParent
        ...
        Public Sub New()
            ...
            If standalone Then tsk.gOptions.displayDst0.Checked = False
            desc = "Find plane equation for a featureless surface - debugging one region for now."
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            ' If tsk.heartBeat Then flow.msgs.Clear()

            mats.mat(0) = src.Clone
            mats.mat(0).SetTo(white, tsk.gridMask)
            ...
            tsk.pcSplit(2).SetTo(0, Not mask) ' remove non-featureless surface depth data.

            ' find the most featureless roi
            Dim maxIndex As Integer
            Dim grCounts(tsk.gridRects.Count - 1)
            For i = 0 To tsk.gridRects.Count - 1
                grCounts(i) = mask(tsk.gridRects(i)).CountNonZero
                If grCounts(i) > grCounts(maxIndex) Then maxIndex = i
            Next

            mats.mat(3) = New cv.Mat(src.Size(), cv.MatType.CV_8UC3, cv.Scalar.All(0))
            src(tsk.gridRects(maxIndex)).CopyTo(mats.mat(3)(tsk.gridRects(maxIndex)), mask(tsk.gridRects(maxIndex)))
            mats.Run(emptyMat)
            dst3 = mats.dst2

            Dim gr = tsk.gridRects(maxIndex) ' this is where the debug comes in.  We just want to look at one region which hopefully is a single plane.
            If gr.X = tsk.gridRects(maxIndex).X And gr.Y = tsk.gridRects(maxIndex).Y Then
                If grCounts(maxIndex) > gr.Width * gr.Height / 4 Then
                    ...
                    For j = 0 To gr.Height - 1
                        For i = 0 To gr.Width - 1
                            Dim nextD = tsk.pcSplit(2)(gr).Get(Of Single)(j, i)
                            If nextD <> 0 Then
