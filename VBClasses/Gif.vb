        ...
        If snapCheck.checked Then
            ...
            Select Case task.gifCaptureIndex
                ...
                Case gifTypes.gifdst2
                    If task.gOptions.ShowGrid.Checked Then Comm.results.dstList(2).SetTo(cv.Scalar.White, task.gridMask)
                    If Comm.results.dstList(2).Channels() = 1 Then
                        Comm.results.dstList(2) = Comm.results.dstList(2).CvtColor(cv.ColorConversionCodes.GRAY2BGR)
                    End If
                    nextBMP = New Bitmap(task.workRes.Width, task.workRes.Height, Imaging.PixelFormat.Format24bppRgb)
                    cvext.BitmapConverter.ToBitmap(Comm.results.dstList(2), nextBMP)
                Case gifTypes.gifdst3
                    If Comm.results.dstList(3).Channels() = 1 Then
                        Comm.results.dstList(3) = Comm.results.dstList(3).CvtColor(cv.ColorConversionCodes.GRAY2BGR)
                    End If
                    nextBMP = New Bitmap(task.workRes.Width, task.workRes.Height, Imaging.PixelFormat.Format24bppRgb)
                    cvext.BitmapConverter.ToBitmap(Comm.results.dstList(3), nextBMP)
