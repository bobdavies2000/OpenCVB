...
Namespace VBClasses
    ...
    Public Class NR_WarpAffine_Captcha : Inherits TaskParent
        ...
        Private Sub addLines(ByRef image As cv.Mat)
            For i = 0 To captchaLength - 1
                ...
                Dim c = New cv.Scalar(rng.Next(0, 255), rng.Next(0, 255), rng.Next(0, 255))
                image.Line(New cv.Point(startX, startY), New cv.Point(endX, endY), c, rng.Next(1, 3), taskA.lineType)
            Next
        End Sub
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To captchaLength - 1
                ...
                cv.Cv2.PutText(charImage, c, New cv.Point(10, charHeight - 10), msRNG.Next(1, 6), msRNG.Next(3, 4),
                           taskA.vecColors(i), msRNG.Next(1, 5), cv.LineTypes.AntiAlias)
            ...
            If taskA.heartBeat Then
                ...
                For j = 0 To 1
                    For i = 0 To triangles(j).Rows - 1
                        ...
                        wideMat.Line(p1, p2, color, taskA.lineWidth + 3, taskA.lineType)
                        If j = 0 Then
                ...
                DrawCircle(wideMat, corner, taskA.DotSize + 5, cv.Scalar.Yellow)
                ...
                For i = 0 To srcPoints1.Count - 1
                    ...
                    DrawCircle(dst3, pt, taskA.DotSize + 2, cv.Scalar.White)
                Next
            End If
    ...
    Public Class NR_WarpAffine_4Points : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If taskA.heartBeat Then
                ...
                For j = 0 To 1
                    For i = 0 To rectangles(j).Points.Length - 1
                        ...
                        dst2.Line(p1, p2, color, taskA.lineWidth + 3, taskA.lineType)
                    Next
                Next
            End If
            ...
            DrawCircle(dst2, center, taskA.DotSize + 5, cv.Scalar.Yellow)
    ...
    Public Class NR_WarpAffine_RepeatedExample8 : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            Dim input = cv.Cv2.ImRead(taskA.homeDir + "Data/8.jpg", cv.ImreadModes.Color)
