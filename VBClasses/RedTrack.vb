...
Namespace VBClasses
    Public Class RedTrack_Basics : Inherits TaskParent
        Public Sub New()
            If New cv.Size(taskAlg.workRes.Width, taskAlg.workRes.Height) <> New cv.Size(168, 94) Then taskAlg.frameHistoryCount = 1
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each rc As oldrcData In taskAlg.redList.oldrclist
                ...
                If rc.index = taskAlg.oldrcD.index Then DrawTour(dst2(rc.rect), rc.contour, white, -1)
            Next
        End Sub
    End Class
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If leftMost = 0 Or rightmost = 0 Or leftMost = rightmost Then
                ...
                While leftCenter.DistanceTo(rightCenter) < dst2.Width / 4
                    ...
                    leftCenter = taskAlg.redList.oldrclist(leftMost).maxDist
                    rightCenter = taskAlg.redList.oldrclist(rightmost).maxDist
                    iterations += 1
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To knn.neighbors.Count - 1
                ...
                If index >= 0 And index < knn.trainInput.Count Then
                    ...
                    DrawCircle(dst3, p1, taskAlg.DotSize, cv.Scalar.Yellow)
                    DrawCircle(dst3, p2, taskAlg.DotSize, cv.Scalar.Yellow)
    ...
    Public Class RedTrack_Points : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim index As Integer
            For Each lp In taskAlg.lines.lpList
                DrawCircle(dst3, lp.p1, taskAlg.DotSize, 255)
                DrawCircle(dst3, lp.p2, taskAlg.DotSize, 255)
                index += 1
                If index >= 10 Then Exit For
            Next
    ...
    Public Class RedTrack_Features : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each pt In taskAlg.features
                DrawCircle(dst2, pt, taskAlg.DotSize, 255)
            Next

            taskAlg.redList.Run(dst2)
            dst3.SetTo(0)
            For Each rc In taskAlg.redList.oldrclist
    ...
    Public Class XO_RedList_TopX : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            taskAlg.redList.inputRemoved = dst1
        End Sub
    End Class
    ...
    Public Class XO_RedList_UnmatchedCount : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To taskAlg.redList.oldrclist.Count - 1
                ...
                If taskAlg.redList.oldrclist(i).depthPixels / taskAlg.redList.oldrclist(i).pixels < 0.5 Then mostlyColor += 1
            ...
            If taskAlg.heartBeat Then
                ...
                labels(2) = CStr(taskAlg.redList.oldrclist.Count) + " cells, unmatched cells = " + CStr(unMatchedCells) + "   " +
                        CStr(mostlyColor) + " cells were mostly color and " + CStr(taskAlg.redList.oldrclist.Count - mostlyColor) + " had depth."
