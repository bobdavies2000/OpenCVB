...
Namespace VBClasses
    Public Class Hist3Dcloud_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.CalcHist({src}, {0, 1, 2}, maskInput, histogram, 3, {bins, bins, bins}, atask.rangesCloud)
            ...
            cv.Cv2.CalcBackProject({src}, {2}, histogram, dst2,
                                   {atask.rangesCloud(atask.rangesCloud.Count - 1)})
    ...
    Public Class NR_Hist3Dcloud_DepthSplit : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            For i = 0 To 2
                hist(i).Run(atask.pcSplit(i))
                mats1.mat(i) = hist(i).dst2.Clone

                If i = 0 Then atask.channels = {0, 1}
                If i = 1 Then atask.channels = {0, 2}
                If i = 2 Then atask.channels = {1, 2}
                hist2d(i).Run(atask.pointCloud)
    ...
    Public Class NR_Hist3Dcloud_Highlights : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If atask.heartBeat Then maskval += 1
