...
Namespace VBClasses
    ...
    Public Class Math_Median_CDF : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            medianVal = computeMedian(src, New cv.Mat, src.Total, tsk.histogramBins, rangeMin, rangeMax)
    ...
    Public Class Math_Intrinsics : Inherits TaskParent
        Public Sub New()
            ...
            dst2 = dst2.Subtract(cv.Scalar.All(tsk.calibData.leftIntrinsics.ppx))
            dst3 = dst3.Subtract(cv.Scalar.All(tsk.calibData.leftIntrinsics.ppx))
    ...
    Public Class Math_ImageAverage : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If tsk.optionsChanged Then images.Clear()
            dst3 = src.Clone
            ...
            If images.Count > tsk.frameHistoryCount Then images.RemoveAt(0)
            ...
            labels(2) = "Average image over previous " + CStr(tsk.frameHistoryCount) + " images"
        End Sub
    End Class
    ...
    Public Class NR_Math_ImageMaskedAverage : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.Multiply(nextImage, cv.Scalar.All(1 / tsk.frameHistoryCount), nextImage)
    ...
    Public Class Math_Stdev : Inherits TaskParent
        ...
        Sub(roi)
            ...
            End If
        End Sub)
            If tsk.gOptions.getShowGrid() Then dst2.SetTo(255, tsk.gridMask)
            dst3.SetTo(0)
            saveFrame.CopyTo(dst3, highStdevMask)
            lastFrame = saveFrame
            Dim stdevPercent = " stdev " + Format(stdevSlider.Value, "0.0")
            labels(2) = CStr(updateCount) + " of " + CStr(tsk.gridRects.Count) + " segments with < " + stdevPercent
            labels(3) = CStr(tsk.gridRects.Count - updateCount) + " out of " + CStr(tsk.gridRects.Count) + " had stdev > " + Format(stdevSlider.Value, "0.0")
        End Sub
    End Class
End Namespace