...
Namespace VBClasses
    Public Class InPaint_Basics : Inherits TaskParent
        ...
        Public Function drawRandomLine(dst As cv.Mat) As cv.Mat
            ...
            vbc.DrawLine(dst2, p1, p2, New cv.Scalar(0, 0, 0))
            Dim mask = New cv.Mat(dst2.Size(), cv.MatType.CV_8UC1)
            mask.SetTo(0)
            vbc.DrawLine(mask, p1, p2, cv.Scalar.All(255))
            Return mask
        End Function
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.Inpaint(dst2, mask, dst3, tsk.lineWidth, If(options.telea, cv.InpaintMethod.Telea, cv.InpaintMethod.NS))
        End Sub
    End Class
    ...
    Public Class NR_InPaint_Depth : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2 = src.Clone
            cv.Cv2.Inpaint(src, tsk.noDepthMask, dst3, 20, If(options.telea, cv.InpaintMethod.Telea, cv.InpaintMethod.NS))
        End Sub
    End Class
    ...
    Public Class NR_InPaint_PointCloud : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To tsk.pcSplit.Count - 1
                split(i) = New cv.Mat
                cv.Cv2.Inpaint(tsk.pcSplit(i), tsk.noDepthMask, split(i), 20,
    ...
    Public Class NR_InPaint_MissingDepth : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.Inpaint(tsk.pcSplit(2), tsk.noDepthMask, dst2, options.radius,
                           If(options.telea, cv.InpaintMethod.Telea, cv.InpaintMethod.NS))
        End Sub
    End Class
