...
Namespace VBClasses
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If inputPoints.Count > 4 Then
                ...
                If standaloneTest() Then
                    ...
                    Next
                    dst2.Ellipse(box, cv.Scalar.Green, atask.lineWidth, atask.lineType)
                End If
            End If
        End Sub
    End Class
    ...
    Public Class NR_FitEllipse_AMS_CPP : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If size.Width < atask.lineWidth + 1 Or size.Height < atask.lineWidth + 1 Then Exit Sub

            Dim box = New cv.RotatedRect(center, size, angle)
            dst2.Ellipse(box, cv.Scalar.Yellow, atask.lineWidth, atask.lineType)
        End Sub
    End Class
    ...
    Public Class NR_FitEllipse_RedCloud : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If atask.oldrcD.contour Is Nothing Then Exit Sub
            ...
            fitE.Run(src)
            dst3.SetTo(0)
            dst3(atask.oldrcD.rect).SetTo(white, atask.oldrcD.mask)
            DrawRect(dst3, atask.oldrcD.rect, white)
            dst3(atask.oldrcD.rect).Ellipse(fitE.box, cv.Scalar.Yellow, atask.lineWidth, atask.lineType)
        End Sub
    End Class
    ...
    Public Class NR_FitEllipse_Rectangle : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If standaloneTest() Then
                If atask.heartBeatLT = False Then
                    Exit Sub
                Else
                    noisyLine.Run(src)
