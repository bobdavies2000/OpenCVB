...
Namespace VBClasses
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If inputPoints.Count > 4 Then
                ...
                If standaloneTest() Then
                    ...
                    Next
                    dst2.Ellipse(box, cv.Scalar.Green, taskA.lineWidth, taskA.lineType)
                End If
            End If
        End Sub
    End Class
    ...
    Public Class NR_FitEllipse_AMS_CPP : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If size.Width < taskA.lineWidth + 1 Or size.Height < taskA.lineWidth + 1 Then Exit Sub

            Dim box = New cv.RotatedRect(center, size, angle)
            dst2.Ellipse(box, cv.Scalar.Yellow, taskA.lineWidth, taskA.lineType)
        End Sub
    End Class
    ...
    Public Class NR_FitEllipse_RedCloud : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskA.oldrcD.contour Is Nothing Then Exit Sub
            ...
            fitE.Run(src)
            dst3.SetTo(0)
            dst3(taskA.oldrcD.rect).SetTo(white, taskA.oldrcD.mask)
            DrawRect(dst3, taskA.oldrcD.rect, white)
            dst3(taskA.oldrcD.rect).Ellipse(fitE.box, cv.Scalar.Yellow, taskA.lineWidth, taskA.lineType)
        End Sub
    End Class
    ...
    Public Class NR_FitEllipse_Rectangle : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If standaloneTest() Then
                If taskA.heartBeatLT = False Then
                    Exit Sub
                Else
                    noisyLine.Run(src)
