...
Public Class XR_FitEllipse_AMS_CPP : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        Dim input As Mat = Mat.FromPixelData(inputPoints.Count, 1, MatType.CV_32FC2, inputPoints.ToArray)
        ...
        Dim box = New RotatedRect(center, size, angle)
...
Public Class XR_FitEllipse_Direct_CPP : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        Dim input As Mat = Mat.FromPixelData(options.srcPoints.Count, 1, MatType.CV_32FC2, options.srcPoints.ToArray)
        Marshal.Copy(input.Data, dataSrc, 0, dataSrc.Length)
        ...
        Dim angle = ellipses(0)
        Dim center As New Point2f(ellipses(1), ellipses(2))
        Dim size As New Size2f(ellipses(3), ellipses(4))
        If size.Width < task.lineWidth + 1 Or size.Height < task.lineWidth + 1 Then Exit Sub
