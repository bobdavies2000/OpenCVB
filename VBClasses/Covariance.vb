...
Public Class Covariance_Basics : Inherits TaskParent
    ...
    Public meanVal As New Mat
    Public covariance As New Mat
    Public Sub New()
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        If standaloneTest() Then
            random.Run(src)
            src = Mat.FromPixelData(random.PointList.Count, 2, MatType.CV_32F, random.PointList.ToArray)
            For i = 0 To random.PointList.Count - 1
                Circle(dst3, random.PointList(i), 3, white, -1, task.lineType)
            Next
        End If
        Dim samples2 = src.Reshape(2)
        CalcCovarMatrix(src, covariance, meanVal, CovarFlags.Cols)
        ...
        Dim overallMean = Cv2.Mean(samples2)
        Dim center = New Point2f(overallMean(0), overallMean(1))
...
Public Class XR_Covariance_Test : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        Dim samples = Mat.FromPixelData(10, 2, MatType.CV_64F, testInput)
        covar.Run(samples)
...
Public Class Covariance_Images : Inherits TaskParent
    Dim covar As New Covariance_Basics
    Public meanVal As Mat
    Public covariance As Mat
    Dim last32f As New Mat
    Public Sub New()
        desc = "Calculate the covariance of 2 images"
    End Sub
    Public Overrides Sub RunAlg(src As cv.Mat)
        If task.optionsChanged Then task.gray.ConvertTo(last32f, MatType.CV_32F)
        dst2 = task.gray
