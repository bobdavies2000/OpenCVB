...
Namespace VBClasses
    Public Class Duster_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 1 To dust.classCount
                ...
                Dim depth = taskAlg.pcSplit(2).Mean(mask)
                taskAlg.pcSplit(2).SetTo(depth(0), mask)
            Next

            cv.Cv2.Merge(taskAlg.pcSplit, dst2)
            dst2.SetTo(0, Not dust.dst0)
            dst2.SetTo(0, taskAlg.maxDepthMask)
    ...
    Public Class Duster_MaskZ : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim src32f = taskAlg.pcSplit(2)
            taskAlg.maxDepthMask = src32f.InRange(taskAlg.MaxZmeters, taskAlg.MaxZmeters).ConvertScaleAbs()
            src32f.SetTo(taskAlg.MaxZmeters, taskAlg.maxDepthMask)

            hist.fixedRanges = {New cv.Rangef(0.001, taskAlg.MaxZmeters)}
            hist.Run(src32f)
            ...
            Dim incr = taskAlg.MaxZmeters / options.bins
            classCount = 0
            ...
            classCount += 1
            dst2.SetTo(classCount, taskAlg.maxDepthMask)

            dst3 = PaletteFull(dst2)
            If taskAlg.heartBeat Then labels(2) = "dst2 = CV_8U version of depth segmented into " + CStr(classCount) + " clusters."
    ...
    Public Class Duster_BasicsY : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 1 To dust.classCount
                Dim mask = dust.dst2.InRange(i, i)
                Dim pcY = taskAlg.pcSplit(1).Mean(mask)
                taskAlg.pcSplit(1).SetTo(pcY(0), mask)
            Next
