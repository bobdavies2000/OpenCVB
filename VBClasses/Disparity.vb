...
Namespace VBClasses
    Public Class Disparity_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If saveIndex <> index Then
                ...
                saveIndex = index
            End If
            rect = taskAlg.gridRects(index)

            match.template = taskAlg.leftView(rect)
            ...
            rightView = taskAlg.rightView

            dst2.Rectangle(rect, black, taskAlg.lineWidth)
            match.Run(rightView(searchRect))
            dst3 = rightView
            matchRect = match.newRect

            dst3.Rectangle(searchRect, black, taskAlg.lineWidth)
            '  dst3.Rectangle(match.newRect, black, taskAlg.lineWidth)
            ...
            End If
            dst3.Rectangle(bestRect, white, taskAlg.lineWidth)
    ...
    Public Class Disparity_Edges : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            edgeline.Run(taskAlg.leftView)
            dst2 = edgeline.dst2.Clone

            edgeline.Run(taskAlg.rightView)
    ...
    Public Class Disparity_Validate : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst3.SetTo(0)
            taskAlg.leftView(r1).CopyTo(dst3(r2))
            disparity.rightView = dst3
            disparity.Run(taskAlg.leftView)
            dst2 = disparity.dst2
            dst3 = disparity.dst3
            labels = disparity.labels
        End Sub
    ...
    Public Class Disparity_RedMask : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            taskAlg.color.Rectangle(disparity.rect, 255, taskAlg.lineWidth)
            dst1.Rectangle(disparity.matchRect, 255, taskAlg.lineWidth)
        End Sub
    End Class
    ...
    Public Class Disparity_Inverse : Inherits TaskParent
        Public Sub New()
            taskAlg.drawRect = New cv.Rect(dst2.Width / 2 - 10, dst2.Height / 2 - 10, 20, 20)
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            ' disparity = B * f / depth
            Dim camInfo = taskAlg.calibData
            If taskAlg.drawRect.Width > 0 Then
                Dim white As New cv.Vec3b(255, 255, 255)
                For y = 0 To taskAlg.drawRect.Height - 1
                    For x = 0 To taskAlg.drawRect.Width - 1
                        Dim depth = taskAlg.pcSplit(2)(taskAlg.drawRect).Get(Of Single)(y, x)
                        If depth > 0 Then
                            Dim disp = camInfo.baseline * camInfo.leftIntrinsics.fx / depth
                            dst3(taskAlg.drawRect).Set(Of cv.Vec3b)(y, x - disp, white)
                        End If
                    Next
                Next
            End If
    ...
    Public Class Disparity_Color8u : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2.Rectangle(rect, 255, taskAlg.lineWidth)
        End Sub
    End Class
