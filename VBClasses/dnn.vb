...
Namespace VBClasses
    Public Class DNN_Basics : Inherits TaskParent
        ...
        Public Sub New()
            ...
            Dim infoText As New FileInfo(taskAlg.homeDir + "Data/MobileNetSSD_deploy.prototxt")
            If infoText.Exists Then
                Dim infoModel As New FileInfo(taskAlg.homeDir + "Data/MobileNetSSD_deploy.caffemodel")
            ...
            If dnnPrepared Then
                ...
                For i = 0 To detectionMat.Rows - 1
                    ...
                    If confidence > options.confidenceThreshold Then
                        ...
                        End If
                        dst3.Rectangle(rect, cv.Scalar.Yellow, taskAlg.lineWidth + 2, taskAlg.lineType)
    ...
    Public Class DNN_SuperRes : Inherits TaskParent
        ...
        Public Sub New()
            taskAlg.drawRect = New cv.Rect(10, 10, 20, 20)
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            End If
            Dim r = taskAlg.drawRect
            If taskAlg.drawRect.Width = 0 Or taskAlg.drawRect.Height = 0 Then Exit Sub
    ...
    '    Public Sub New()
    '        Dim modelFile As New FileInfo(taskAlg.homeDir + "Data/bvlc_googlenet.caffemodel")
    ...
    '        End If
    '        Dim protoTxt = taskAlg.homeDir + "Data/bvlc_googlenet.prototxt"
    '        net = CvDnn.ReadNetFromCaffe(protoTxt, modelFile.FullName)
    '        Dim synsetWords = taskAlg.homeDir + "Data/synset_words.txt"
    ...
    '        Dim image = cv.Cv2.ImRead(taskAlg.homeDir + "Data/space_shuttle.jpg")
