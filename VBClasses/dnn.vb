...
Namespace VBClasses
    Public Class NR_DNN_Basics : Inherits TaskParent
        ...
        Public Sub New()
            ...
            Dim infoText As New FileInfo(taskA.homeDir + "Data/MobileNetSSD_deploy.prototxt")
            If infoText.Exists Then
                Dim infoModel As New FileInfo(taskA.homeDir + "Data/MobileNetSSD_deploy.caffemodel")
            ...
            If dnnPrepared Then
                ...
                For i = 0 To detectionMat.Rows - 1
                    ...
                    If confidence > options.confidenceThreshold Then
                        ...
                        End If
                        dst3.Rectangle(rect, cv.Scalar.Yellow, taskA.lineWidth + 2, taskA.lineType)
    ...
    Public Class NR_DNN_SuperRes : Inherits TaskParent
        ...
        Public Sub New()
            taskA.drawRect = New cv.Rect(10, 10, 20, 20)
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            End If
            Dim r = taskA.drawRect
            If taskA.drawRect.Width = 0 Or taskA.drawRect.Height = 0 Then Exit Sub
    ...
    '    Public Sub New()
    '        Dim modelFile As New FileInfo(taskA.homeDir + "Data/bvlc_googlenet.caffemodel")
    ...
    '        End If
    '        Dim protoTxt = taskA.homeDir + "Data/bvlc_googlenet.prototxt"
    '        net = CvDnn.ReadNetFromCaffe(protoTxt, modelFile.FullName)
    '        Dim synsetWords = taskA.homeDir + "Data/synset_words.txt"
    ...
    '        Dim image = cv.Cv2.ImRead(taskA.homeDir + "Data/space_shuttle.jpg")
