...
Namespace VBClasses
    Public Class Cloud_Basics : Inherits TaskParent
        Public Shared ppx = algTask.calibData.rgbIntrinsics.ppx
        Public Shared ppy = algTask.calibData.rgbIntrinsics.ppy
        Public Shared fx = algTask.calibData.rgbIntrinsics.fx
        Public Shared fy = algTask.calibData.rgbIntrinsics.fy
        Public Sub New()
        ...
        Public Function invert(x As Integer, y As Integer, depth As Single) As cv.Point3f
            ...
            If u >= 0 And u < algTask.workRes.Width And v >= 0 And v < algTask.workRes.Height Then
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For y = 0 To dst2.Height - 1
                For x = 0 To dst2.Width - 1
                    dst2.Set(Of cv.Point3f)(y, x, worldCoordinates(x, y, algTask.pcSplit(2).Get(Of Single)(y, x)))
                Next
            Next

            dst3 = algTask.pointCloud
        End Sub
    End Class
    ...
    Public Class Cloud_Display : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            pcHistory.Add(algTask.pointCloud)
            If pcHistory.Count >= algTask.frameHistoryCount Then pcHistory.RemoveAt(0)
    ...
    Public Class Cloud_SetupSide : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            DrawCircle(dst2, algTask.sideCameraPoint, algTask.DotSize, cv.Scalar.BlueViolet)
            For i = 1 To algTask.MaxZmeters
                Dim xmeter = CInt(dst2.Width * i / algTask.MaxZmeters * distanceRatio)
            ...
            Dim cam = algTask.sideCameraPoint
            Dim marker As New cv.Point2f(dst2.Width / (algTask.MaxZmeters * distanceRatio), 0)
            marker.Y = marker.X * Math.Tan((algTask.vFov / 2) * cv.Cv2.PI / 180)
            Dim markerLeft = New cv.Point(marker.X, cam.Y - marker.Y)
            Dim markerRight = New cv.Point(marker.X, cam.Y + marker.Y)

            Dim offset = Math.Sin(algTask.accRadians.X) * marker.Y
            If algTask.gOptions.gravityPointCloud.Checked Then
                If algTask.accRadians.X > 0 Then
                ...
                markerLeft = New cv.Point(markerLeft.X * Math.Cos(algTask.accRadians.Z) - markerLeft.Y * Math.Sin(algTask.accRadians.Z), ' rotate around x-axis using accRadians.Z
                                              markerLeft.Y * Math.Cos(algTask.accRadians.Z) + markerLeft.X * Math.Sin(algTask.accRadians.Z))
                ...
                markerRight = New cv.Point((markerRight.X - cam.X) * Math.Cos(algTask.accRadians.Z) - (markerRight.Y - cam.Y) * Math.Sin(algTask.accRadians.Z) + cam.X,
                                               (markerRight.Y - cam.Y) * Math.Cos(algTask.accRadians.Z) + (markerRight.X - cam.X) * Math.Sin(algTask.accRadians.Z) + cam.Y)
            End If
            If standaloneTest() = False Then
                DrawCircle(dst2, markerLeft, algTask.DotSize, cv.Scalar.Red)
                DrawCircle(dst2, markerRight, algTask.DotSize, cv.Scalar.Red)
            End If

            ' draw the arc enclosing the camera FOV
            Dim startAngle = (180 - algTask.vFov) / 2
            ...
            dst2.Line(cam, fovTop, white, 1, algTask.lineType)
            dst2.Line(cam, fovBot, white, 1, algTask.lineType)

            DrawCircle(dst2, markerLeft, algTask.DotSize + 3, cv.Scalar.Red)
            DrawCircle(dst2, markerRight, algTask.DotSize + 3, cv.Scalar.Red)
            dst2.Line(cam, markerLeft, cv.Scalar.Red, 1, algTask.lineType)
            dst2.Line(cam, markerRight, cv.Scalar.Red, 1, algTask.lineType)
    ...
    Public Class Cloud_SetupTop : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If standaloneTest() Then dst2.SetTo(0) Else src.CopyTo(dst2)
            DrawCircle(dst2, algTask.topCameraPoint, algTask.DotSize, cv.Scalar.BlueViolet)
            For i = 1 To algTask.MaxZmeters
                Dim ymeter = CInt(dst2.Height - dst2.Height * i / (algTask.MaxZmeters * distanceRatio))
            ...
            Dim cam = algTask.topCameraPoint
            Dim marker As New cv.Point2f(cam.X, dst2.Height / algTask.MaxZmeters)
            Dim topLen = marker.Y * Math.Tan((algTask.hFov / 2) * cv.Cv2.PI / 180)
            Dim sideLen = marker.Y * Math.Tan((algTask.vFov / 2) * cv.Cv2.PI / 180)
            Dim markerLeft = New cv.Point(cam.X - topLen, marker.Y)
            Dim markerRight = New cv.Point(cam.X + topLen, marker.Y)

            Dim offset = Math.Sin(algTask.accRadians.Z) * topLen
            If algTask.gOptions.gravityPointCloud.Checked Then
                If algTask.accRadians.Z > 0 Then
            ...
            ' draw the arc enclosing the camera FOV
            Dim startAngle = (180 - algTask.hFov) / 2
            Dim x = dst2.Height / Math.Tan(startAngle * cv.Cv2.PI / 180)

            Dim fovRight = New cv.Point(algTask.topCameraPoint.X + x, 0)
            Dim fovLeft = New cv.Point(algTask.topCameraPoint.X - x, fovRight.Y)

            dst2.Line(algTask.topCameraPoint, fovLeft, white, 1, algTask.lineType)
            ...
            dst2.Line(algTask.topCameraPoint, fovRight, white, algTask.lineWidth, algTask.lineWidth)
        End Sub
    End Class
    ...
    Public Class Cloud_Solo : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            heat.Run(src)
            dst2 = heat.dst0.InRange(algTask.frameHistoryCount, algTask.frameHistoryCount).ConvertScaleAbs
            dst3 = heat.dst1.InRange(algTask.frameHistoryCount, algTask.frameHistoryCount).ConvertScaleAbs
        End Sub
    End Class
    ...
    Public Class Cloud_SurfaceH : Inherits TaskParent
        ...
        Public Sub New()
            OptionParent.FindCheckBox("Top View (Unchecked Side View)").Checked = True
            labels(3) = "Histogram Of Each Of " + CStr(algTask.histogramBins) + " bins aligned With the sideview"
            desc = "Find the horizontal surfaces With a projects Of the SideView histogram."
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            heat.Run(src)
            ...
            Dim ratio = algTask.mouseMovePoint.Y / dst2.Height
            Dim offset = ratio * dst3.Height
            dst2.Line(New cv.Point(0, algTask.mouseMovePoint.Y), New cv.Point(dst2.Width, algTask.mouseMovePoint.Y), yellow, algTask.lineWidth, algTask.lineWidth)
            dst3.Line(New cv.Point(0, offset), New cv.Point(dst3.Width, offset), cv.Scalar.Yellow, algTask.lineWidth)
        End Sub
    End Class
    ...
    Public Class Cloud_GridInspector : Inherits TaskParent
        Public Sub New()
            ...
            algTask.mouseMovePoint.X = dst2.Width / 2
            desc = "Inspect x, y, and z values by brick"
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            Dim cLine = algTask.mouseMovePoint.X
            ...
            dst2.Line(topPt, botPt, 255, algTask.lineWidth, algTask.lineWidth)

            SetTrueText("Values show brick.pt3d values at the blue line.", New cv.Point(dst2.Width / 2, 0), 3)
            For i = 0 To dst2.Height - 1 Step algTask.brickSize
                Dim pt = New cv.Point2f(cLine, i)
                Dim index = algTask.gridMap.Get(Of Integer)(pt.Y, pt.X)
                Dim center = algTask.bricks.brickList(index).center
                Dim xyz = algTask.pointCloud.Get(Of cv.Vec3f)(center.Y, center.X)
    ...
    Public Class Cloud_ReduceSplit2 : Inherits TaskParent
        ...
        Public Sub New()
            desc = "Reduce the algTask.pcSplit(2) for use in several algorithms."
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            dst2 = algTask.pcSplit(2) * 1000
            ...
            Else
                ...
                dst1 *= algTask.MaxZmeters / mm.maxVal
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For y = 0 To input.Height - 1
                For x = 1 To input.Width - 1
                    ...
                    If Math.Abs(p1 - p2) <= algTask.depthDiffMeters Then dst2.Set(Of Byte)(y, x, 255) Else dst3.Set(Of Byte)(y, x, 255)
                Next
            Next

            dst3.SetTo(0, algTask.noDepthMask)
            dst2.SetTo(0, algTask.noDepthMask)
            labels(2) = "White pixels: Z-values within " + CStr(algTask.depthDiffMeters) + " meters of X neighbor"
            labels(3) = "Mask showing discontinuities > " + CStr(algTask.depthDiffMeters) + " meters of X neighbor"
        End Sub
    End Class
    ...
    Public Class Cloud_Continuous_GridX : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2.SetTo(0)
            dst3.SetTo(0)
            Dim gcPrev = algTask.bricks.brickList(0)
            For Each brick In algTask.bricks.brickList
                If brick.rect.X > 0 Then
                    If Math.Abs(brick.depth - gcPrev.depth) <= algTask.depthDiffMeters Then
            ...
            labels(2) = "White pixels: Z-values within " + CStr(algTask.depthDiffMeters) + " meters of neighbor in X direction"
            labels(3) = "Mask showing discontinuities > " + CStr(algTask.depthDiffMeters) + " meters of neighbor in X direction"
        End Sub
    End Class
    ...
    Public Class Cloud_Continuous_GridXY : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2.SetTo(0)
            Dim gcPrev = algTask.bricks.brickList(0)
            Dim cellMat As New cv.Mat(algTask.brickSize, algTask.brickSize, cv.MatType.CV_8U, cv.Scalar.All(127))
            For Each brick In algTask.bricks.brickList
                Dim gcAbove = algTask.bricks.brickList(CInt(brick.index Mod algTask.bricksPerRow))
                If brick.correlation > algTask.fCorrThreshold Then
                    If brick.rect.Y = 0 Or brick.rect.X = 0 Then Continue For
                    If Math.Abs(brick.depth - gcPrev.depth) <= algTask.depthDiffMeters Then dst2(brick.rect).SetTo(128)
                    If Math.Abs(brick.depth - gcAbove.depth) <= algTask.depthDiffMeters And
            ...
            labels(2) = "White pixels: Z-values within " + CStr(algTask.depthDiffMeters) + " meters of neighbor in X and Y direction"
        End Sub
    End Class
    ...
    Public Class Cloud_Templates : Inherits TaskParent
        ...
        Public Sub New()
            templateX = New cv.Mat(algTask.workRes, cv.MatType.CV_32F)
            templateY = New cv.Mat(algTask.workRes, cv.MatType.CV_32F)
            ...
            Next
            templateX -= algTask.calibData.rgbIntrinsics.ppx
            templateY -= algTask.calibData.rgbIntrinsics.ppy
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If standalone Then
                ...
                For Each contour In contours.contourList
                    ...
                    contour.depth = algTask.pcSplit(2)(contour.rect).Mean(contour.mask)
                    src(contour.rect).SetTo(contour.depth, contour.mask)
                Next
            End If
            Dim fxTemplate = algTask.calibData.rgbIntrinsics.fx
            Dim fyTemplate = algTask.calibData.rgbIntrinsics.fy
