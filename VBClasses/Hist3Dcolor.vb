...
Namespace VBClasses
    Public Class Hist3Dcolor_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If src.Type <> cv.MatType.CV_8UC3 Then src = atask.color
            If atask.heartBeat Or alwaysRun Or histogram.Width = 0 Then
                Dim bins = options.histogram3DBins
                cv.Cv2.CalcHist({src}, {0, 1, 2}, inputMask, histogram, 3, {bins, bins, bins}, atask.rangesBGR)
            ...
            cv.Cv2.CalcBackProject({src}, {0, 1, 2}, histogram, dst2, atask.rangesBGR)
    ...
    Public Class Hist3Dcolor_UniqueRGBPixels : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            pixels.Clear()
            counts.Clear()
            Dim bins As Integer = binSlider.Value
            For z = 0 To bins - 1
                For y = 0 To bins - 1
                    For x = 0 To bins - 1
    ...
    Public Class NR_Hist3Dcolor_ZeroGroups : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If src.Channels() <> 3 Then src = atask.color

            If atask.optionsChanged Then
                Dim bins = options.histogram3DBins
                Dim hBins() As Integer = {bins, bins, bins}
                cv.Cv2.CalcHist({src}, {0, 1, 2}, maskInput, histogram, 3, hBins, atask.rangesBGR)
    ...
    Public Class NR_Hist3Dcolor_Select : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim selection = atask.gOptions.DebugSlider.Value
    ...
    Public Class NR_Hist3Dcolor_Diff : Inherits TaskParent
        ...
        Public Sub New()
            atask.featureOptions.ColorDiffSlider.Value = 0
    ...
    Public Class Hist3Dcolor_Vector : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If src.Channels() <> 3 Then src = atask.color
            If atask.optionsChanged Or binArray Is Nothing Then
                binArray = {options.histogram3DBins, options.histogram3DBins, options.histogram3DBins}
            End If

            cv.Cv2.CalcHist({src}, {0, 1, 2}, inputMask, histogram, 3, binArray, atask.rangesBGR)
