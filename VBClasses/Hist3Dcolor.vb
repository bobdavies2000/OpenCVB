...
Namespace VBClasses
    Public Class Hist3Dcolor_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If src.Type <> cv.MatType.CV_8UC3 Then src = taskA.color
            If taskA.heartBeat Or alwaysRun Or histogram.Width = 0 Then
                Dim bins = options.histogram3DBins
                cv.Cv2.CalcHist({src}, {0, 1, 2}, inputMask, histogram, 3, {bins, bins, bins}, taskA.rangesBGR)
            ...
            cv.Cv2.CalcBackProject({src}, {0, 1, 2}, histogram, dst2, taskA.rangesBGR)
    ...
    Public Class NR_Hist3Dcolor_ZeroGroups : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If src.Channels() <> 3 Then src = taskA.color

            If taskA.optionsChanged Then
                Dim bins = options.histogram3DBins
                Dim hBins() As Integer = {bins, bins, bins}
                cv.Cv2.CalcHist({src}, {0, 1, 2}, maskInput, histogram, 3, hBins, taskA.rangesBGR)
    ...
    Public Class NR_Hist3Dcolor_Select : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim selection = taskA.gOptions.DebugSlider.Value
    ...
    Public Class NR_Hist3Dcolor_Diff : Inherits TaskParent
        ...
        Public Sub New()
            taskA.featureOptions.ColorDiffSlider.Value = 0
    ...
    Public Class Hist3Dcolor_Vector : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If src.Channels() <> 3 Then src = taskA.color
            If taskA.optionsChanged Or binArray Is Nothing Then
                binArray = {options.histogram3DBins, options.histogram3DBins, options.histogram3DBins}
            End If

            cv.Cv2.CalcHist({src}, {0, 1, 2}, inputMask, histogram, 3, binArray, taskA.rangesBGR)
