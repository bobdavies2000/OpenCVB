...
Namespace VBClasses
    Public Class Projection_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If atask.heartBeat Then
                ...
                Dim depthCount = atask.pcSplit(2).CountNonZero
            ...
            If showRectangles Then
                For i = 0 To rclist.Count - 1
                    dst2.Rectangle(rclist(i).rect, atask.highlight, atask.lineWidth)
                Next
            End If
    ...
    Public Class Projection_ObjectIsolate : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If options.index < top.objects.objectList.Count Then
                ...
                dst0 = atask.pointCloud.InRange(lower, upper)

                dst1.SetTo(0)
                atask.pointCloud.CopyTo(dst1, dst0)
                side.Run(dst1)
    ...
    Public Class Projection_Object : Inherits TaskParent
        ...
        Public Sub New()
            atask.gOptions.DebugSlider.Value = 0 ' pick the biggest object...
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim index = atask.gOptions.DebugSlider.Value
            If index < top.objects.objectList.Count Then
                ...
                Dim mask = atask.pointCloud.InRange(lower, upper)

                Dim rc = top.objects.rclist(atask.gOptions.DebugSlider.Value + 1) ' the biggest by default...
                dst0.SetTo(0)
                ...
                dst1.SetTo(0)
                atask.pointCloud.CopyTo(dst1, mask)
                side.Run(dst1)
                dst2 = side.dst2
                labels(2) = side.labels(2)
            End If
    ...
    Public Class Projection_Cell : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst0.SetTo(0)
            atask.pointCloud(rc.rect).CopyTo(dst0(rc.rect), rc.mask)
    ...
    Public Class NR_Projection_Derivative : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            heat.Run(atask.pointCloud)
            dst2 = heat.dst2
            dst3 = heat.dst3
            labels(2) = heat.labels(2)
            labels(3) = heat.labels(3)

            Dim pc As New cv.Mat(atask.pointCloud.Size, cv.MatType.CV_32FC3, 0)
            atask.pointCloud.CopyTo(pc, dst1)
    ...
    Public Class Projection_HistSide : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.CalcHist({src}, atask.channelsSide, New cv.Mat, histogram, 2, atask.bins2D, atask.rangesSide)
    ...
    Public Class Projection_HistTop : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If src.Type <> cv.MatType.CV_32FC3 Then src = atask.pointCloud
            cv.Cv2.CalcHist({src}, atask.channelsTop, New cv.Mat, histogram, 2, atask.bins2D, atask.rangesTop)
