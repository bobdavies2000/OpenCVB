...
Namespace VBClasses
    Public Class HeatMap_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.CalcHist({src}, taskAlg.channelsTop, New cv.Mat, histogramTop, 2, taskAlg.bins2D, taskAlg.rangesTop)
            histogramTop.Row(0).SetTo(0)

            cv.Cv2.CalcHist({src}, taskAlg.channelsSide, New cv.Mat, histogramSide, 2, taskAlg.bins2D, taskAlg.rangesSide)
            ...
            labels(2) = "Top view of heat map with the last " + CStr(taskAlg.frameHistoryCount) + " frames"
            labels(3) = "Side view of heat map with the last " + CStr(taskAlg.frameHistoryCount) + " frames"
        End Sub
    End Class
    ...
    Public Class HeatMap_Grid : Inherits TaskParent
        ...
        Public Sub New()
            taskAlg.gOptions.GridSlider.Value = 5
    ...
    Public Class HeatMap_Hot : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskAlg.heartBeat Then labels(2) = CStr(mmTop.maxVal) + " max count " + CStr(dst2.CountNonZero) + " pixels in the top down view"
            If taskAlg.heartBeat Then labels(3) = CStr(mmSide.maxVal) + " max count " + CStr(dst3.CountNonZero) + " pixels in the side view"
        End Sub
    End Class
    ...
    Public Class HeatMap_Cell : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            taskAlg.pointCloud(taskAlg.oldrcD.rect).CopyTo(dst0(taskAlg.oldrcD.rect), taskAlg.oldrcD.mask)
