...
Namespace VBClasses
    Public Class CamShift_Basics : Inherits TaskParent
        ...
        Public Shared Function Show_HSV_Hist(hist As cv.Mat) As cv.Mat
            Dim img As New cv.Mat(New cv.Size(tsk.workRes.Width, tsk.workRes.Height), cv.MatType.CV_8UC3, cv.Scalar.All(0))
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim hsize() As Integer = {tsk.histogramBins}
            tsk.drawRect = ValidateRect(tsk.drawRect)
            cv.Cv2.CalcHist({hue(tsk.drawRect)}, {0}, mask(tsk.drawRect), histogram, 1, hsize, ranges)
            ...
            If trackBox.Size.Width > 0 Then
                dst2.Ellipse(trackBox, white, tsk.lineWidth + 1, tsk.lineType)
            End If
        End Sub
    End Class
