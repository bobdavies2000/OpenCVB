...
Namespace VBClasses
    Public Class Corner_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            Dim lastFeatures = New HashSet(Of Point2f)(fast.features)
            fast.Run(src)
            ...
            If newPts.Count < threshold Then
                features = New List(Of Point2f)(fast.features)
            Else
                features.Clear()
    ...
    Public Class Corner_Core : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2 = src.Clone
            Dim kpoints() As KeyPoint = FAST(task.gray, options.FASTthreshold, options.useNonMax)

            features.Clear()
            For Each kp As KeyPoint In kpoints
                features.Add(kp.Pt)
            Next
            ...
            If standaloneTest() Then
                dst3.SetTo(0)
                For Each kp As KeyPoint In kpoints
                    Circle(dst2, kp.Pt, task.DotSize, Scalar.Yellow, -1, task.lineType)
    ...
    Public Class XR_Corner_Harris : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            mc = New Mat(task.gray.Size(), MatType.CV_32FC1, 0)
            dst2 = New Mat(task.gray.Size(), MatType.CV_8U, Scalar.All(0))
            CornerEigenValsAndVecs(task.gray, dst2, options.blockSize, options.aperture, BorderTypes.Default)

            For y = 0 To task.gray.Rows - 1
                For x = 0 To task.gray.Cols - 1
                    Dim lambda_1 = dst2.Get(Of Vec6f)(y, x)(0)
                    Dim lambda_2 = dst2.Get(Of Vec6f)(y, x)(1)
            ...
            For y As Integer = 0 To task.gray.Rows - 1
                For x As Integer = 0 To task.gray.Cols - 1
            ...
            Dim McNormal As New Mat
            Normalize(mc, McNormal, 127, 255, NormTypes.MinMax)
    ...
    Public Class XR_Corner_PreCornerDetect : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Normalize(prob, prob, 0, 255, NormTypes.MinMax)
            prob.ConvertTo(task.gray, MatType.CV_8U)
            median.Run(task.gray.Clone())
            CvtColor(task.gray, dst2, ColorConversionCodes.GRAY2BGR)
            CvtColor(task.gray, dst3, ColorConversionCodes.GRAY2BGR)
            Threshold(dst3, dst3, 160, 255, ThresholdTypes.BinaryInv)
    ...
    Public Class Corner_ShiTomasi_CPP : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Threshold(dst3, dst3, options.threshold, 255, ThresholdTypes.Binary)
        End Sub
    End Class
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To task.gridRects.Count - 1
                ...
                If tmp.Rows > 0 Then
                    ...
                    fastCenters(i) = New Point2f(r.X + meanVal(0), r.Y + meanVal(1))
                End If
            Next
    ...
    Public Class XR_Corner_Harris_CPP : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim gray32f = Mat.FromPixelData(src.Rows, src.Cols, MatType.CV_32F, imagePtr)
    ...
    Public Class Corner_HarrisDetector_CPP : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If ptCount > 1 Then
                Dim ptMat = Mat.FromPixelData(ptCount, 2, MatType.CV_32S, imagePtr).Clone
                features.Clear()
                For i = 0 To ptCount - 1
                    features.Add(New Point2f(ptMat.Get(Of Integer)(i, 0), ptMat.Get(Of Integer)(i, 1)))
                    Circle(dst2, features(i), task.DotSize, Scalar.Yellow, -1, task.lineType)
                Next
            End If
        End Sub
        Protected Overrides Sub Finalize()
