...
Namespace VBClasses
    Public Class Classifier_Basics_CPP : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskAlg.optionsChanged Then taskAlg.gOptions.DebugCheckBox.Checked = True
            Dim imagePtr = OEX_Points_Classifier_RunCPP(cPtr, options.sampleCount, options.methodIndex,
                                                        dst2.Rows, dst2.Cols,
                                                        If(taskAlg.gOptions.DebugCheckBox.Checked, 1, 0))
            ...
            imagePtr = OEX_ShowPoints(cPtr, dst2.Rows, dst2.Cols, taskAlg.DotSize)
    ...
    Public Class Classifier_BayesianTest : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            'nabs.oldrclist = taskAlg.redList.oldrclist
            'nabs.Run(taskAlg.redList.rcMap)
            ...
            'dst1.SetTo(0)
            'For Each index In nabs.nabList(taskAlg.oldrcD.index)
            ...
            'responseList(taskAlg.oldrcD.index) = 1
            ...
            '    Marshal.Copy(resultsPtr, results, 0, taskAlg.results.Length)
            'End If
            'dst3.SetTo(0)
            'Dim zeroOutput As Boolean = True
            'For i = 0 To maskList.Count - 1
            '    If results(i) > 0 Then
            '        Dim rc = taskAlg.redList.oldrclist(maskList(i))
