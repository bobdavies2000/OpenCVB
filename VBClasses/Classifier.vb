...
Namespace VBClasses
    Public Class Classifier_Basics_CPP : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If tsk.optionsChanged Then tsk.gOptions.DebugCheckBox.Checked = True
            Dim imagePtr = OEX_Points_Classifier_RunCPP(cPtr, options.sampleCount, options.methodIndex,
                                                        dst2.Rows, dst2.Cols,
                                                        If(tsk.gOptions.DebugCheckBox.Checked, 1, 0))
            ...
            imagePtr = OEX_ShowPoints(cPtr, dst2.Rows, dst2.Cols, tsk.DotSize)
    ...
    Public Class Classifier_BayesianTest : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            'nabs.oldrclist = tsk.redList.oldrclist
            'nabs.Run(tsk.redList.rcMap)
            ...
            'dst1.SetTo(0)
            'For Each index In nabs.nabList(tsk.oldrcD.index)
            ...
            'responseList(tsk.oldrcD.index) = 1
            ...
            '    Marshal.Copy(resultsPtr, results, 0, tsk.results.Length)
            'End If
            'dst3.SetTo(0)
            'Dim zeroOutput As Boolean = True
            'For i = 0 To maskList.Count - 1
            '    If results(i) > 0 Then
            '        Dim rc = tsk.redList.oldrclist(maskList(i))
