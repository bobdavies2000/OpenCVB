...
Namespace VBClasses
    Public Class Clone_Basics : Inherits TaskParent
        ...
        Public Sub New()
            ...
            atask.drawRect = New cv.Rect(dst2.Width / 4, dst2.Height / 4, dst2.Width / 2, dst2.Height / 2)
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If atask.drawRect = New cv.Rect Then
                mask.SetTo(255)
            Else
                cv.Cv2.Rectangle(mask, atask.drawRect, cv.Scalar.White, -1)
            End If
            dst3 = mask.CvtColor(cv.ColorConversionCodes.GRAY2BGR)

            If standaloneTest() And atask.frameCount Mod 10 = 0 Then cloneSpec += 1
    ...
    Public Class NR_Clone_Eagle : Inherits TaskParent
        ...
        Dim options As New Options_Clone
        Public Sub New()
            sourceImage = cv.Cv2.ImRead(atask.homeDir + "Data/CloneSource.png")
            ...
            mask = cv.Cv2.ImRead(atask.homeDir + "Data/Clonemask.png")
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If atask.mouseClickFlag Then
                pt = atask.clickPoint  ' pt corresponds To the center Of the source image.  Roi can't be outside image boundary.
    ...
    Public Class NR_Clone_Seamless : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Else
                cv.Cv2.Rectangle(dst3, atask.drawRect, cv.Scalar.White, -1)
            End If

            dst2 = src.Clone()
            cv.Cv2.SeamlessClone(atask.depthRGB, src, dst3, center, dst2, options.cloneFlag)
