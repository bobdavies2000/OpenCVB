...
Namespace VBClasses
    Public Class History_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If taskAlg.frameHistoryCount = 1 Then
                dst2 = src
                Exit Sub
            End If
            ...
            If dst1.Type <> input.Type Or dst1.Channels() <> input.Channels() Or taskAlg.optionsChanged Then
                dst1 = input
                saveFrames.Clear()
            End If
    ...
    Public Class History_Cloud : Inherits TaskParent
        ...
        Public Sub New()
            desc = "Create a frame history and sum the last X taskAlg.pointcloud's"
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If src.Type <> cv.MatType.CV_32FC3 Or src.Channels() <> 3 Then src = taskAlg.pointCloud

            If taskAlg.optionsChanged Or dst3.Type <> cv.MatType.CV_32FC3 Then
                saveFrames.Clear()
                dst3 = New cv.Mat(dst2.Size(), cv.MatType.CV_32FC3, 0)
            End If

            If saveFrames.Count >= taskAlg.frameHistoryCount Then
    ...
    Public Class History_Basics8U : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskAlg.settings.algorithm = traceName Then
    ...
    Public Class History_ReliableDepth : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If saveFrames.Count > 0 Then
                If taskAlg.optionsChanged Or saveFrames(0).Size <> src.Size Then saveFrames.Clear()
            End If
            ...
            If taskAlg.optionsChanged Then saveFrames.Clear()
