...
Namespace VBClasses
    Public Class Triangle_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If atask.redList.oldrclist.Count <= 1 Then Exit Sub
            atask.oldrcD = atask.redList.oldrclist(1)

            dst3.SetTo(0)
            Dim pt3D As New List(Of cv.Point3f)
            For Each pt In atask.oldrcD.contour
                pt = New cv.Point(pt.X + atask.oldrcD.rect.X, pt.Y + atask.oldrcD.rect.Y)
                Dim vec = atask.pointCloud.Get(Of cv.Point3f)(pt.Y, pt.X)
                If vec.Z = 0 Then
                    vec = Cloud_Basics.worldCoordinates(New cv.Point3f(pt.X, pt.Y, atask.oldrcD.depth))
                End If
                DrawCircle(dst3, pt, atask.DotSize, cv.Scalar.Yellow)
                pt3D.Add(vec)
            Next

            Dim c3D = atask.pointCloud.Get(Of cv.Point3f)(atask.oldrcD.maxDist.Y, atask.oldrcD.maxDist.X)
            triangles.Clear()
            Dim color3D As New cv.Point3f(atask.oldrcD.color(0), atask.oldrcD.color(1), atask.oldrcD.color(2))
    ...
    Public Class Triangle_HullContour : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each pt In rc.hull
                pt = New cv.Point(pt.X + rc.rect.X, pt.Y + rc.rect.Y)
                DrawCircle(dst1, pt, atask.DotSize, cv.Scalar.Yellow)
            Next

            RedCloud_Cell.selectCell(hulls.rcMap, hulls.rclist)
            rc = If(atask.rcD Is Nothing, hulls.rclist(0), atask.rcD)
        End Sub
    End Class
    ...
    Public Class NR_Triangle_Cell : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Else
                ...
                yFactor = dst2.Height
            End If
            dst3.Rectangle(cellRect, white, atask.lineWidth)
            ...
            Dim c3D = atask.pointCloud.Get(Of cv.Point3f)(rc.maxDist.Y, rc.maxDist.X)
            triangles.Clear()
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For y = 0 To rc.rect.Height - 1
                For x = 0 To rc.rect.Width - 1
                    ...
                    Dim vec = atask.pointCloud(rc.rect).Get(Of cv.Point3f)(y, x)
            ...
            DrawCircle(dst3, newMaxDist, atask.DotSize + 2, cv.Scalar.Red)
