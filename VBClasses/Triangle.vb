...
Namespace VBClasses
    Public Class Triangle_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If tsk.redList.oldrclist.Count <= 1 Then Exit Sub
            tsk.oldrcD = tsk.redList.oldrclist(1)

            dst3.SetTo(0)
            Dim pt3D As New List(Of cv.Point3f)
            For Each pt In tsk.oldrcD.contour
                pt = New cv.Point(pt.X + tsk.oldrcD.rect.X, pt.Y + tsk.oldrcD.rect.Y)
                Dim vec = tsk.pointCloud.Get(Of cv.Point3f)(pt.Y, pt.X)
                If vec.Z = 0 Then
                    vec = Cloud_Basics.worldCoordinates(New cv.Point3f(pt.X, pt.Y, tsk.oldrcD.depth))
                End If
                DrawCircle(dst3, pt, tsk.DotSize, cv.Scalar.Yellow)
                pt3D.Add(vec)
            Next

            Dim c3D = tsk.pointCloud.Get(Of cv.Point3f)(tsk.oldrcD.maxDist.Y, tsk.oldrcD.maxDist.X)
            triangles.Clear()
            Dim color3D As New cv.Point3f(tsk.oldrcD.color(0), tsk.oldrcD.color(1), tsk.oldrcD.color(2))
    ...
    Public Class Triangle_HullContour : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each pt In rc.hull
                pt = New cv.Point(pt.X + rc.rect.X, pt.Y + rc.rect.Y)
                DrawCircle(dst1, pt, tsk.DotSize, cv.Scalar.Yellow)
            Next

            RedCloud_Cell.selectCell(hulls.rcMap, hulls.rclist)
            rc = If(tsk.rcD Is Nothing, hulls.rclist(0), tsk.rcD)
        End Sub
    End Class
    ...
    Public Class NR_Triangle_Cell : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Else
                ...
                yFactor = dst2.Height
            End If
            dst3.Rectangle(cellRect, white, tsk.lineWidth)
            ...
            Dim c3D = tsk.pointCloud.Get(Of cv.Point3f)(rc.maxDist.Y, rc.maxDist.X)
            triangles.Clear()
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For y = 0 To rc.rect.Height - 1
                For x = 0 To rc.rect.Width - 1
                    ...
                    Dim vec = tsk.pointCloud(rc.rect).Get(Of cv.Point3f)(y, x)
            ...
            DrawCircle(dst3, newMaxDist, tsk.DotSize + 2, cv.Scalar.Red)
