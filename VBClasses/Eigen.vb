...
Public Class XR_Eigen_Basics : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        End If
        Dim mat As Mat = Mat.FromPixelData(5, 5, MatType.CV_64FC1, inputData)
        Dim eigenVal As New Mat, eigenVec As New Mat
...
Public Class XR_Eigen_Fitline : Inherits TaskParent
    ...
    Dim eigenVec As New Mat(2, 2, MatType.CV_32F, Scalar.All(0)), eigenVal As New Mat(2, 2, MatType.CV_32F, Scalar.All(0))
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        Dim lineX = FitLine(noisyLine.PointList, DistanceTypes.L2, 1, 0.01, 0.01)
        ...
        Dim vec4f As New List(Of Point2f)
        vec4f.Add(New Point2f(eigenInput(0), eigenInput(1)))
        vec4f.Add(New Point2f(eigenInput(2), eigenInput(3)))

        Dim D = Mat.FromPixelData(2, 2, MatType.CV_32FC1, vec4f.ToArray)
        ...
        p1 = New Point2f(meanVal.Val0 - Math.Cos(theta) * len / 2, meanVal.Val1 - Math.Sin(theta) * len / 2)
        p2 = New Point2f(meanVal.Val0 + Math.Cos(theta) * len / 2, meanVal.Val1 + Math.Sin(theta) * len / 2)
        ...
        Else
            ...
            p2 = New Point2f(meanVal.Val0 + Math.Cos(-theta) * len / 2, meanVal.Val1 + Math.Sin(-theta) * len / 2)
            m2 = (p2.Y - p1.Y) / (p2.X - p1.X)
            Line(dst2, p1, p2, Scalar.Yellow, task.lineWidth + 1, task.lineType)
        End If

        p1 = New cv.Point(0, noisyLine.bb)
        p2 = New cv.Point(src.Width, noisyLine.m * src.Width + noisyLine.bb)
        Cv2.Line(dst2, p1, p2, Scalar.Blue, task.lineWidth, task.lineType)
...
Public Class Eigen_Input3D : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        Dim p1 As Point3f, p2 As Point3f
        If Rnd() * 2 - 1 >= 0 Then
            p1 = New Point3f(Rnd() * width, 0, Rnd() * depth)
            p2 = New Point3f(Rnd() * width, height, Rnd() * depth)
        Else
            p1 = New Point3f(0, Rnd() * height, Rnd() * depth)
            p2 = New Point3f(width, Rnd() * height, Rnd() * depth)
        End If
        ...
        For i = 0 To options.noisyPointCount - 1
            ...
            Dim pt = New Point3f(startx + i * incr + noiseOffsetX,
                                        Math.Max(0, Math.Min(m * (startx + i * incr) + bb + noiseOffsetY, height)), Rnd() * depth)
            PointList.Add(pt)
            Circle(dst2, New Point2f(pt.X, pt.Y), task.DotSize + 1, highLight, -1, task.lineType)
        Next
    End Sub
End Class
