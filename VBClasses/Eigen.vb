...
Namespace VBClasses
    ...
    Public Class Eigen_Fitline : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2.Line(lp.p1, lp.p2, cv.Scalar.Red, algTask.lineWidth + 4, algTask.lineType)
            ...
            Else
                ...
                p2 = New cv.Point2f(mean.Val0 + Math.Cos(-theta) * len / 2, mean.Val1 + Math.Sin(-theta) * len / 2)
                m2 = (p2.Y - p1.Y) / (p2.X - p1.X)
                dst2.Line(p1, p2, cv.Scalar.Yellow, algTask.lineWidth + 1, algTask.lineType)
            End If

            p1 = New cv.Point(0, noisyLine.bb)
            p2 = New cv.Point(src.Width, noisyLine.m * src.Width + noisyLine.bb)
            dst2.Line(p1, p2, cv.Scalar.Blue, algTask.lineWidth, algTask.lineType)
    ...
    Public Class Eigen_Input3D : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To options.noisyPointCount - 1
                ...
                PointList.Add(pt)
                DrawCircle(dst2, New cv.Point2f(pt.X, pt.Y), algTask.DotSize + 1, highLight)
            Next
        End Sub
    End Class
End Namespace