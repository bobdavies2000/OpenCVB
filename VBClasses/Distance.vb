...
Namespace VBClasses
    Public Class Distance_Basics : Inherits TaskParent
        ...
        Public Shared Function GetMaxDistDepth(ByRef maskInput As cv.Mat, rect As cv.Rect) As cv.Point
            ...
            atask.depthmask(rect).CopyTo(depth, maskInput)
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If standalone Then src = atask.depthmask.Clone
            If atask.optionsChanged Then dst1 = src.Clone Else src.CopyTo(dst1, atask.motionRGB.motionMask)
            distance.Run(dst1)
            dst2 = distance.dst2
    ...
    Public Class Distance_Point3D : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If standaloneTest() And atask.heartBeat Then
                ...
                Dim vec1 = atask.pointCloud.Get(Of cv.Point3f)(p1.Y, p1.X)
                Dim vec2 = atask.pointCloud.Get(Of cv.Point3f)(p2.Y, p2.X)
            End If
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If standaloneTest() Then
                inPoint1 = New cv.Vec4f(msRNG.Next(0, dst2.Width), msRNG.Next(0, dst2.Height),
                                    msRNG.Next(0, atask.MaxZmeters), msRNG.Next(0, atask.MaxZmeters))
            ...
            If standalone And atask.heartBeat Then SetTrueText(strOut, New cv.Point(10, 10), 2)
        End Sub
    End Class
    ...
    Public Class NR_Distance_BinaryImage : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If atask.drawRect.Width > 0 Then
                distance.Run(dst2(atask.drawRect))
            Else
                distance.Run(dst2)
            End If
            dst3 = distance.dst2
            dst1 = dst3.Threshold(atask.gOptions.DebugSlider.Value, 255, cv.ThresholdTypes.Binary)
        End Sub
    End Class
    ...
    Public Class NR_Distance_PeakDepth : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If atask.noDepthMask.CountNonZero = atask.noDepthMask.Total Then Exit Sub ' startup issue 
            Dim distance32f = atask.depthMask.DistanceTransform(cv.DistanceTypes.L1, 0)
    ...
    Public Class NR_Distance_PeakNoDepth : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If atask.noDepthMask.CountNonZero = atask.noDepthMask.Total Then Exit Sub ' startup issue 
            Dim distance32f = atask.noDepthMask.DistanceTransform(cv.DistanceTypes.L1, 0)
    ...
    Public Class Distance_Labels : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If standalone Then src = atask.noDepthMask
            ...
            If standalone Then dst3.SetTo(0, atask.depthMask)
        End Sub
    End Class
    ...
    Public Class NR_Distance_LabelsNoDepth : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            labeller.Run(atask.noDepthMask)
            dst2 = labeller.dst2
            dst3 = labeller.dst3
    ...
    Public Class Distance_Depth : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst3 = dst2.Threshold(atask.gOptions.DebugSlider.Value, 255, cv.ThresholdTypes.Binary)
            mm = GetMinMax(dst2)
    ...
    Public Class NR_Distance_DepthPeaks : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim threshold = Math.Abs(atask.gOptions.DebugSlider.Value)
    ...
    Public Class NR_Distance_ClickPoint : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst1 = atask.grayStable.InRange(0, 0)

            atask.grayStable.SetTo(255, dst1)
            atask.grayStable.Set(Of Byte)(atask.ClickPoint.Y, atask.ClickPoint.X, 0)
            dst2 = atask.grayStable.DistanceTransform(options.distanceType, 0)
            dst3 = 255 - dst2
        End Sub
    End Class
    ...
    Public Class NR_Distance_DepthBricks : Inherits TaskParent
        ...
        Public Sub New()
            ...
            atask.gOptions.DebugSlider.Value = 20
    ...
    Public Class NR_Distance_Contour : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst3 = src.Clone
            dst3(atask.contourD.rect).SetTo(white, atask.contourD.mask)

            dst1.SetTo(255)
            dst1(atask.contourD.rect).SetTo(0, atask.contourD.mask)
