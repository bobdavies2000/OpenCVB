...
Public Class Distance_Basics : Inherits TaskParent
    ...
    Public Shared Function GetMaxDistDepth(ByRef maskInput As cv.Mat, rect As cv.Rect) As cv.Point
        ...
        Rectangle(depth, New cv.Rect(0, 0, depth.Width, depth.Height), cv.Scalar.All(0), 1)
        Dim distance32f As New cv.Mat
        DistanceTransform(depth, distance32f, cv.DistanceTypes.L1, cv.DistanceTransformMasks.Precise, cv.MatType.CV_32F)
        Dim mm As mmData = GetMinMax(distance32f)
        mm.maxLoc.X += rect.X
        mm.maxLoc.Y += rect.Y
...
Public Class Distance_Foreground : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        DistanceTransform(dst0, dst0, DistanceType, cv.DistanceTransformMasks.Precise, cv.MatType.CV_32F)
        Dim dist32f As New cv.Mat
        Normalize(dst0, dist32f, 0, 255, cv.NormTypes.MinMax)
        dist32f.ConvertTo(dst1, cv.MatType.CV_8UC1)
        CvtColor(dst1, dst2, cv.ColorConversionCodes.GRAY2BGR)
    End Sub
End Class
...
Public Class XR_Distance_BinaryImage : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        End If
        dst3 = distance.dst2
        Threshold(dst3, dst1, task.gOptions.DebugSlider.Value, 255, cv.ThresholdTypes.Binary)
    End Sub
End Class
...
If CountNonZero(task.noDepthMask) = task.noDepthMask.Total Then Exit Sub ' startup issue 
        Dim distance32f As New cv.Mat
        DistanceTransform(task.depthmask, distance32f, cv.DistanceTypes.L1, cv.DistanceTransformMasks.Precise, cv.MatType.CV_32F)
...
If CountNonZero(task.noDepthMask) = task.noDepthMask.Total Then Exit Sub ' startup issue 
        Dim distance32f As New cv.Mat
        DistanceTransform(task.noDepthMask, distance32f, cv.DistanceTypes.L1, cv.DistanceTransformMasks.Precise, cv.MatType.CV_32F)
...
Public Class Distance_Labels : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        DistanceTransformWithLabels(src, dst0, dst1, options.distanceType, cv.DistanceTransformMasks.Precise)
        Normalize(dst0, dst2, 0, 255, cv.NormTypes.MinMax)
...
Public Class Distance_Instant : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        DistanceTransform(src, dst0, options.distanceType, cv.DistanceTransformMasks.Precise, cv.MatType.CV_32F)
...
Public Class Distance_Depth : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        DistanceTransform(dst1, dst2, options.distanceType, cv.DistanceTransformMasks.Precise, cv.MatType.CV_32F)
        Threshold(dst2, dst3, task.gOptions.DebugSlider.Value, 255, cv.ThresholdTypes.Binary)
        mm = GetMinMax(dst2)
...
Public Class XR_Distance_DepthPeaks : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        Threshold(dst2, mask, thresholdVal, 255, cv.ThresholdTypes.Binary)
...
Public Class XR_Distance_ClickPoint : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        DistanceTransform(task.gray, dst2, options.distanceType, cv.DistanceTransformMasks.Precise, cv.MatType.CV_32F)
        dst3 = 255 - dst2
    End Sub
End Class
...
Public Class XR_Distance_Contour : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        DistanceTransform(dst1, dst0, options.distanceType, cv.DistanceTransformMasks.Precise, cv.MatType.CV_32F)
    End Sub
End Class
