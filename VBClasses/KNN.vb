...
Public Class KNN_Basics : Inherits TaskParent
    ...
    Public Shared Function displayResults(result(,) As Integer, queries As List(Of cv.Point2f), trainInput As List(Of cv.Point2f)) As cv.Mat
        ...
        For i = 0 To queries.Count - 1
            ...
            Circle(dst, pt, task.DotSize + 4, cv.Scalar.Yellow, -1, task.lineType)
            Line(dst, pt, nn, cv.Scalar.Yellow, task.lineWidth, task.lineType)
        Next

        For Each pt In trainInput
        Circle(dst, pt, task.DotSize + 4, cv.Scalar.Red, -1, task.lineType)
        Next
        Return dst
    End Function
...
Public Class KNN_Minimal : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        Normalize(trainMat, trainNormalized, 1, 0, cv.NormTypes.MinMax)
        knn.Train(trainNormalized, cv.ML.SampleTypes.RowSample, response)

        Normalize(queryMat, queryNormalized, 1, 0, cv.NormTypes.MinMax)
...
Public Class KNN_Farthest : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        For i = 0 To knn.result.GetUpperBound(0) - 1
            ...
            Circle(dst2, lp.p1, task.DotSize + 4, cv.Scalar.Yellow, -1, task.lineType)
            Circle(dst2, lp.p2, task.DotSize + 4, cv.Scalar.Yellow, -1, task.lineType)
        ...
        Circle(dst3, pt, task.DotSize + 4, cv.Scalar.Red, -1, task.lineType)
        Next
...
Public Class KNN_OneToOne : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        Circle(dst2, pt, task.DotSize + 4, cv.Scalar.Red, -1, task.lineType)
        Next
        ...
        For i = 0 To queries.Count - 1
            Dim pt = queries(i)
            Circle(dst2, pt, task.DotSize + 4, cv.Scalar.Yellow, -1, task.lineType)
            If nearest(i) = -1 Then
                noMatch.Add(pt)
            Else
...
Public Class KNN_MinDistance : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        Circle(dst3, pt, task.DotSize, cv.Scalar.White, -1, task.lineType)
        Next
...
Public Class KNN_Grid : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        For Each r In task.gridRects
            ...
            If val > 0 Then
                trainInput.Add(New cv.Vec3f(r.TopLeft.X, r.TopLeft.Y, Mean(task.gray(r))(0)))
                clusters.Add(val)
            Else
                For y = 0 To r.Height - 1
                    For x = 0 To r.Width - 1
                        queries.Add(New cv.Vec3f(r.TopLeft.X + x, r.TopLeft.Y + y, Mean(task.gray(r))(0)))
                    Next
                Next
            End If
        Next
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        For i = 0 To queries.Count - 1
            ...
            For j = 0 To Math.Min(2, trainInput.Count) - 1
                ...
                Line(dst2, pt, nn, cv.Scalar.Yellow, task.lineWidth, task.lineType)
