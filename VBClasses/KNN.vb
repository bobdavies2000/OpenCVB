...
Public Class KNN_Basics : Inherits TaskParent
    ...
    Public trainInput As New List(Of Point2f)
    Public queries As New List(Of Point2f)
    Public result(,) As Integer ' Get results here...
    Public Sub New()
        desc = "Default normalized KNN with dimension 2"
    End Sub
    Public Shared Function displayResults(result(,) As Integer, queries As List(Of Point2f), trainInput As List(Of Point2f)) As Mat
        Dim dst = New Mat(task.workRes, MatType.CV_8UC3, 0)
        For i = 0 To queries.Count - 1
            Dim pt = queries(i)
            Dim nn = trainInput(result(i, 0))
            Circle(dst, pt, task.DotSize + 4, Scalar.Yellow, -1, task.lineType)
            Line(dst, pt, nn, Scalar.Yellow, task.lineWidth, task.lineType)
        Next

        For Each pt In trainInput
        Circle(dst, pt, task.DotSize + 4, Scalar.Red, -1, task.lineType)
        Next
        Return dst
    End Function
    Public Shared Function getResults(neighbors As Mat, rows As Integer, cols As Integer) As Integer(,)
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        If standalone Then
            ...
            If task.heartBeatLT Then
                random.Run(src)
                trainInput = New List(Of Point2f)(random.PointList)
            End If
            random.Run(src)
            queries = New List(Of Point2f)(random.PointList)
        End If

        If ptListQuery.Count > 0 Then
            trainInput.Clear()
            For Each pt In ptListTrain
                trainInput.Add(New Point2f(pt.X, pt.Y))
            Next

            queries.Clear()
            For Each pt In ptListQuery
                queries.Add(New Point2f(pt.X, pt.Y))
            Next
        End If

        knn2.trainMat = Mat.FromPixelData(trainInput.Count, KNNdimension, MatType.CV_32F, trainInput.ToArray)
        knn2.queryMat = Mat.FromPixelData(queries.Count, KNNdimension, MatType.CV_32F, queries.ToArray)
...
Public Class KNN_Minimal : Inherits TaskParent
    ...
    Public knn As ML.KNearest
    Public result(,) As Integer ' Get results here...
    Public queryMat As New Mat
    Public trainMat As New Mat
    Dim neighbors As New Mat
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        Dim response = Mat.FromPixelData(trainMat.Rows, 1, MatType.CV_32S, Enumerable.Range(start:=0, trainMat.Rows).ToArray)

        Dim trainNormalized As New Mat, queryNormalized As New Mat
        Normalize(trainMat, trainNormalized, 1, 0, NormTypes.MinMax)
        knn.Train(trainNormalized, ML.SampleTypes.RowSample, response)

        Normalize(queryMat, queryNormalized, 1, 0, NormTypes.MinMax)
        knn.FindNearest(queryNormalized, trainMat.Rows, New Mat, neighbors)
...
Public Class KNN_IndividualQuery : Inherits TaskParent
    ...
    Public Sub runTestData()
        knn.FindNearest(queryMat, trainMat.Rows, New Mat, neighbors)
        result = KNN_Basics.getResults(neighbors, queryMat.Rows, trainMat.Rows)
    End Sub

    Public Function runQueryBest(query As Mat) As Integer
        knn.FindNearest(query, trainMat.Rows, New Mat, neighbors)
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        knn.Train(trainMat, ML.SampleTypes.RowSample, response)
    End Sub
    Protected Overrides Sub Finalize()
        If knn IsNot Nothing Then knn.Dispose()
    End Sub
...
Public Class KNN_Farthest : Inherits TaskParent
    ...
    Public Sub New()
        ...
        desc = "Use KNN to find the farthest cv.Point from each query cv.Point."
    End Sub
    Public Overrides Sub RunAlg(src As cv.Mat)
        If standalone Then
            If task.heartBeatLT Then
                ...
                random.Run(src)
                trainInput = New List(Of Point2f)(random.PointList)
                queries = New List(Of Point2f)(trainInput)
            End If
        End If
        ...
        For i = 0 To knn.result.GetUpperBound(0) - 1
            ...
            Circle(dst2, lp.p1, task.DotSize + 4, Scalar.Yellow, -1, task.lineType)
            Circle(dst2, lp.p2, task.DotSize + 4, Scalar.Yellow, -1, task.lineType)
            Line(dst2, lp.p1, lp.p2, Scalar.Yellow, task.lineWidth, task.lineType)
            farthest.Add(lp)
            distances.Add(lp.p1.DistanceTo(lp.p2))
        Next

        For Each pt In queries
        Circle(dst3, pt, task.DotSize + 4, Scalar.Red, -1, task.lineType)
        Next

        Dim maxIndex = distances.IndexOf(distances.Max())
        lpFar = farthest(maxIndex)
        Line(dst3, lpFar.p1, lpFar.p2, white, task.lineWidth, task.lineType)
    End Sub
End Class
...
Public Class KNN_OneToOne : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        If standaloneTest() Then
            If task.heartBeatLT Then
                random.Run(src)
                knn.trainInput = New List(Of Point2f)(random.PointList)
                random.Run(src)
                queries = New List(Of Point2f)(random.PointList)
            End If
        End If
        ...
        knn.queries = queries
        knn.trainInput = New List(Of Point2f)(queries)
        knn.Run(src)
        ...
        Circle(dst2, pt, task.DotSize + 4, Scalar.Red, -1, task.lineType)
        Next
        ...
        For i = 0 To queries.Count - 1
            Dim pt = queries(i)
            Circle(dst2, pt, task.DotSize + 4, Scalar.Yellow, -1, task.lineType)
            If nearest(i) = -1 Then
                noMatch.Add(pt)
            Else
                If nearest(i) < knn.trainInput.Count Then ' there seems like a boundary condition when there is only 1 traininput...
                    ...
                    Line(dst2, nn, pt, white, task.lineWidth, task.lineType)
                End If
            End If
        Next
        If standaloneTest() = False Then knn.trainInput = New List(Of Point2f)(queries)
    End Sub
End Class
...
Public Class XR_KNN_MaxDistance : Inherits TaskParent
    ...
    Public outputPoints As New List(Of (Point2f, Point2f))
    Public options As New Options_KNN
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        Static pairList As New List(Of (Point2f, Point2f))
...
Public Class KNN_MinDistance : Inherits TaskParent
    Dim knn As New KNN_Basics
    Public outputPoints2f As New List(Of Point2f)
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        Circle(dst3, pt, task.DotSize, Scalar.White, -1, task.lineType)
        Next
        labels(3) = "There were " + CStr(feat.features.Count) + " points in the input"

        Dim tooClose As New List(Of (Point2f, Point2f))
        For i = 0 To knn.result.GetUpperBound(0)
        ...
        For Each tuple In tooClose
            Dim p1 = tuple.Item1
            Dim p2 = tuple.Item2
            Dim pt = If(p1.X <= p2.X, p1, p2)  ' trim the cv.Point with lower x to avoid flickering...
...
Public Class KNN_Dimension2 : Inherits TaskParent
    Public knn As New KNN_Minimal
    Dim random As New Random_Basics
    Dim trainInput As New List(Of Point2f)
    Dim queries As New List(Of Point2f)
    Public Sub New()
...
Public Class KNN_Dimension3 : Inherits TaskParent
    ...
    Public queries As New List(Of Point3f)
    Public Sub New()
        labels(2) = "Red=TrainingData, yellow = queries, text shows Euclidean distance to that cv.Point from query cv.Point"
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        For i = 0 To trainInput.Count - 1
            Dim pt = New Point2f(trainInput(i).X, trainInput(i).Y)
        ...
        Next
        For i = 0 To queries.Count - 1
            Dim pt = New Point2f(queries(i).X, queries(i).Y)
            For j = 0 To Math.Min(2, trainInput.Count) - 1
                Dim index = knn.result(i, j)
                Dim nn = New Point2f(trainInput(index).X, trainInput(index).Y)
                Circle(dst2, pt, task.DotSize, Scalar.Yellow, -1, task.lineType)
                Line(dst2, pt, nn, Scalar.Yellow, task.lineWidth, task.lineType)
                Dim midPt = New Point2f((pt.X + nn.X) / 2, (pt.Y + nn.Y) / 2)
...
Public Class KNN_Dimension4 : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        dist.inPoint1 = queries(0)
        For i = 0 To trainInput.Count - 1
            Dim pt = New Point2f(trainInput(i)(0), trainInput(i)(1))
            Circle(dst2, pt, task.DotSize, Scalar.Red, -1, task.lineType)
            dist.inPoint2 = trainInput(i)
            dist.Run(src)
        ...
        Next
        For i = 0 To queries.Count - 1
            Dim pt = New Point2f(queries(i)(0), queries(i)(1))
            For j = knn.result.GetLowerBound(1) To knn.result.GetUpperBound(1)
                Dim index = knn.result(i, j)
                Dim nn = New Point2f(trainInput(index)(0), trainInput(index)(1))
                Circle(dst2, pt, task.DotSize, Scalar.Yellow, -1, task.lineType)
                Line(dst2, pt, nn, task.highlight, task.lineWidth, task.lineType)
                Dim midPt = New Point2f((pt.X + nn.X) / 2, (pt.Y + nn.Y) / 2)
                SetTrueText(CStr(j), midPt)
            Next
        Next
...
Public Class KNN_DimensionN : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        knn.queryMat = Mat.FromPixelData(queries.Count \ knn.options.knnDimension, knn.options.knnDimension, MatType.CV_32F,
                                            queries.ToArray)
        knn.trainMat = Mat.FromPixelData(trainInput.Count \ knn.options.knnDimension, knn.options.knnDimension, MatType.CV_32F,
                                            trainInput.ToArray)
        knn.Run(src)
        dst2.SetTo(0)
        For i = 0 To trainInput.Count \ knn.options.knnDimension - 1 Step knn.options.knnDimension
            Dim pt = New Point2f(trainInput(i), trainInput(i + 1))
            Circle(dst2, pt, task.DotSize, Scalar.Red, -1, task.lineType)
        Next

        For i = 0 To queries.Count \ knn.options.knnDimension - 1 Step knn.options.knnDimension
            Dim pt = New Point2f(queries(i), queries(i + 1))
            Dim index = knn.result(i, 0) * knn.options.knnDimension
            Dim nn = New Point2f(trainInput(index), trainInput(index + 1))
