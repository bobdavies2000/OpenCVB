...
Namespace VBClasses
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskAlg.heartBeat Then
                ...
                For i = 0 To options.drawCount - 1
                    ...
                    Dim nextColor = New cv.Scalar(taskAlg.vecColors(i)(0), taskAlg.vecColors(i)(1), taskAlg.vecColors(i)(2))
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskAlg.heartBeat Then
                ...
                For i = 0 To options.drawCount - 1
                    ...
                    Dim nextColor = New cv.Scalar(taskAlg.vecColors(i)(0), taskAlg.vecColors(i)(1), taskAlg.vecColors(i)(2))
                    dst2.Circle(nPoint, radius, nextColor, options.drawFilled, taskAlg.lineType)
                Next
            End If
        End Sub
    End Class
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskAlg.heartBeat Then
                ...
                For i = 0 To options.drawCount - 1
                    ...
                    Dim nextColor = New cv.Scalar(taskAlg.vecColors(i)(0), taskAlg.vecColors(i)(1), taskAlg.vecColors(i)(2))
                    dst2.Line(nPoint1, nPoint2, nextColor, options.drawFilled, taskAlg.lineType)
                Next
            End If
        End Sub
    End Class
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To options.drawCount - 1
                ...
                If options.drawFilled <> -1 Then
                    cv.Cv2.Polylines(dst3, listOfPoints, True, polyColor, taskAlg.lineWidth + 1, taskAlg.lineType)
                Else
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskAlg.heartBeat Then
                ...
                For i = 0 To options.numPoints - 1
                    ...
                    dst2.Line(p1, p2, taskAlg.scalarColors(i Mod taskAlg.scalarColors.Count), taskAlg.lineWidth + 1, taskAlg.lineType)
                Next
    ...
    Public Class Draw_Arc : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskAlg.heartBeat Then
                ...
                taskAlg.kalman.kInput = {rect.X, rect.Y, rect.Width, rect.Height, angle, startAngle, endAngle}
            End If

            taskAlg.kalman.kInput = {rect.X, rect.Y, rect.Width, rect.Height, angle, startAngle, endAngle}
            taskAlg.kalman.Run(emptyMat)
            Dim r = New cv.Rect(taskAlg.kalman.kOutput(0), taskAlg.kalman.kOutput(1), taskAlg.kalman.kOutput(2), taskAlg.kalman.kOutput(3))
            ...
            Dim color = taskAlg.scalarColors(colorIndex)
            ...
            Else
                ...
                Dim endAngle = taskAlg.kalman.kOutput(6)
                ...
                dst2.Ellipse(New cv.Point(rr.Center.X, rr.Center.Y), New cv.Size(r1.Width, r1.Height),
                         angle, startAngle, endAngle, color, thickness, taskAlg.lineType)
            End If
        End Sub
    End Class
    ...
    Public Class Draw_ClipLine : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            dst3 = src
            taskAlg.kalman.kInput = {pt1.X, pt1.Y, pt2.X, pt2.Y, rect.X, rect.Y, rect.Width, rect.Height}
            taskAlg.kalman.Run(emptyMat)
            Dim p1 = New cv.Point(CInt(taskAlg.kalman.kOutput(0)), CInt(taskAlg.kalman.kOutput(1)))
            Dim p2 = New cv.Point(CInt(taskAlg.kalman.kOutput(2)), CInt(taskAlg.kalman.kOutput(3)))

            If taskAlg.kalman.kOutput(6) < 5 Then taskAlg.kalman.kOutput(6) = 5 ' don't let the width/height get too small...
            If taskAlg.kalman.kOutput(7) < 5 Then taskAlg.kalman.kOutput(7) = 5
            Dim r = New cv.Rect(taskAlg.kalman.kOutput(4), taskAlg.kalman.kOutput(5), taskAlg.kalman.kOutput(6), taskAlg.kalman.kOutput(7))

            Dim clipped = cv.Cv2.ClipLine(r, p1, p2) ' Returns false when the line and the rectangle don't intersect.
            dst3.Line(p1, p2, If(clipped, white, cv.Scalar.Black), taskAlg.lineWidth + 1, taskAlg.lineType)
            dst3.Rectangle(r, If(clipped, cv.Scalar.Yellow, cv.Scalar.Red), taskAlg.lineWidth + 1, taskAlg.lineType)
    ...
    Public Class Draw_Line : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If p1 <> newPoint And p2 <> New cv.Point And taskAlg.ClickPoint <> newPoint Then
                p1 = newPoint
                p2 = newPoint
            End If
            dst2 = src
            If taskAlg.ClickPoint <> New cv.Point Or externalUse Then
                If p1 = New cv.Point Then p1 = taskAlg.ClickPoint Else p2 = taskAlg.ClickPoint
            End If
            If p1 <> newPoint And p2 = newPoint Then DrawCircle(dst2, p1, taskAlg.DotSize, taskAlg.highlight)
    ...
    Public Class Draw_Frustrum : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            src = New cv.Mat(New cv.Size(taskAlg.workRes.Width, taskAlg.workRes.Height), cv.MatType.CV_32F, cv.Scalar.All(0))

            Dim mid = src.Height / 2
            Dim zIncr = taskAlg.MaxZmeters / mid
            dst2 = src.Clone
            ...
            Next
            xyzDepth.Run(dst2)
            dst3 = xyzDepth.dst2.Resize(New cv.Size(taskAlg.workRes.Width, taskAlg.workRes.Height))
        End Sub
    End Class
    ...
    Public Class Draw_RotatedRect : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i As Integer = 0 To vertices.Count - 1
                dst2.Line(vertices(i), vertices((i + 1) Mod 4), cv.Scalar.Green, taskAlg.lineWidth, taskAlg.lineType)
            Next
        End Sub
    End Class
End Namespace