    ...
    Public Overrides sub RunAlg(src As cv.Mat)
        ...
        If algTask.heartBeat Then
            ...
            For i = 0 To options.drawCount - 1
                ...
                Dim nextColor = New cv.Scalar(algTask.vecColors(i)(0), algTask.vecColors(i)(1), algTask.vecColors(i)(2))
                dst2.Circle(nPoint, radius, nextColor, options.drawFilled, algTask.lineType)
            Next
        End If
    End Sub
End Class
    ...
    Public Overrides sub RunAlg(src As cv.Mat)
        ...
        If algTask.heartBeat Then
            ...
            For i = 0 To options.drawCount - 1
                ...
                Dim nextColor = New cv.Scalar(algTask.vecColors(i)(0), algTask.vecColors(i)(1), algTask.vecColors(i)(2))
                dst2.Line(nPoint1, nPoint2, nextColor, options.drawFilled, algTask.lineType)
            Next
        End If
    End Sub
End Class
    ...
    Public Overrides sub RunAlg(src As cv.Mat)
        ...
        For i = 0 To options.drawCount - 1
            ...
            If options.drawFilled <> -1 Then
                cv.Cv2.Polylines(dst3, listOfPoints, True, polyColor, algTask.lineWidth + 1, algTask.lineType)
            Else
    ...
    Public Overrides sub RunAlg(src As cv.Mat)
        ...
        If algTask.heartBeat Then
            ...
            For i = 0 To options.numPoints - 1
                ...
                dst2.Line(p1, p2, algTask.scalarColors(i Mod algTask.scalarColors.Count), algTask.lineWidth + 1, algTask.lineType)
            Next
...
Public Class Draw_Arc : Inherits TaskParent
    ...
    Public Overrides sub RunAlg(src As cv.Mat)
        ...
        If algTask.heartBeat Then
            ...
            algTask.kalman.kInput = {rect.X, rect.Y, rect.Width, rect.Height, angle, startAngle, endAngle}
        End If

        algTask.kalman.kInput = {rect.X, rect.Y, rect.Width, rect.Height, angle, startAngle, endAngle}
        algTask.kalman.Run(emptyMat)
        Dim r = New cv.Rect(algTask.kalman.kOutput(0), algTask.kalman.kOutput(1), algTask.kalman.kOutput(2), algTask.kalman.kOutput(3))
        ...
        Dim color = algTask.scalarColors(colorIndex)

        dst2.SetTo(white)
        If options.drawFull Then
            dst2.Ellipse(rr, color, thickness, algTask.lineType)
            DrawRotatedOutline(rr, dst2, algTask.scalarColors(colorIndex))
        Else
            Dim angle = algTask.kalman.kOutput(4)
            Dim startAngle = algTask.kalman.kOutput(5)
            Dim endAngle = algTask.kalman.kOutput(6)
            ...
            dst2.Ellipse(New cv.Point(rr.Center.X, rr.Center.Y), New cv.Size(r1.Width, r1.Height),
                         angle, startAngle, endAngle, color, thickness, algTask.lineType)
        End If
    End Sub
End Class
...
Public Class Draw_ClipLine : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        dst3 = src
        algTask.kalman.kInput = {pt1.X, pt1.Y, pt2.X, pt2.Y, rect.X, rect.Y, rect.Width, rect.Height}
        algTask.kalman.Run(emptyMat)
        Dim p1 = New cv.Point(CInt(algTask.kalman.kOutput(0)), CInt(algTask.kalman.kOutput(1)))
        Dim p2 = New cv.Point(CInt(algTask.kalman.kOutput(2)), CInt(algTask.kalman.kOutput(3)))

        If algTask.kalman.kOutput(6) < 5 Then algTask.kalman.kOutput(6) = 5 ' don't let the width/height get too small...
        If algTask.kalman.kOutput(7) < 5 Then algTask.kalman.kOutput(7) = 5
        Dim r = New cv.Rect(algTask.kalman.kOutput(4), algTask.kalman.kOutput(5), algTask.kalman.kOutput(6), algTask.kalman.kOutput(7))

        Dim clipped = cv.Cv2.ClipLine(r, p1, p2) ' Returns false when the line and the rectangle don't intersect.
        dst3.Line(p1, p2, If(clipped, white, cv.Scalar.Black), algTask.lineWidth + 1, algTask.lineType)
        dst3.Rectangle(r, If(clipped, cv.Scalar.Yellow, cv.Scalar.Red), algTask.lineWidth + 1, algTask.lineType)
...
Public Class Draw_Line : Inherits TaskParent
    ...
    End Sub
    Public Overrides Sub RunAlg(src As cv.Mat)
        If p1 <> newPoint And p2 <> New cv.Point And algTask.ClickPoint <> newPoint Then
            p1 = newPoint
            p2 = newPoint
        End If
        dst2 = src
        If algTask.ClickPoint <> New cv.Point Or externalUse Then
            If p1 = New cv.Point Then p1 = algTask.ClickPoint Else p2 = algTask.ClickPoint
        End If
        If p1 <> newPoint And p2 = newPoint Then DrawCircle(dst2, p1, algTask.DotSize, algTask.highlight)
...
Public Class Draw_Frustrum : Inherits TaskParent
    ...
    End Sub
    Public Overrides sub RunAlg(src As cv.Mat)
        src = New cv.Mat(New cv.Size(algTask.workRes.Width, algTask.workRes.Height), cv.MatType.CV_32F, cv.Scalar.All(0))

        Dim mid = src.Height / 2
        Dim zIncr = algTask.MaxZmeters / mid
        dst2 = src.Clone
        ...
        Next
        xyzDepth.Run(dst2)
        dst3 = xyzDepth.dst2.Resize(New cv.Size(algTask.workRes.Width, algTask.workRes.Height))
    End Sub
End Class
...
Public Class Draw_RotatedRect : Inherits TaskParent
    ...
    Public Overrides sub RunAlg(src As cv.Mat)
        ...
        For i As Integer = 0 To vertices.Count - 1
            dst2.Line(vertices(i), vertices((i + 1) Mod 4), cv.Scalar.Green, algTask.lineWidth, algTask.lineType)
        Next
    End Sub
End Class
