...
Namespace VBClasses
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If atask.heartBeat Then
                ...
                For i = 0 To options.drawCount - 1
                    ...
                    Dim nextColor = New cv.Scalar(atask.vecColors(i)(0), atask.vecColors(i)(1), atask.vecColors(i)(2))
                    dst2.Circle(nPoint, radius, nextColor, options.drawFilled, atask.lineType)
                Next
            End If
        End Sub
    End Class
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If atask.heartBeat Then
                ...
                For i = 0 To options.drawCount - 1
                    ...
                    Dim nextColor = New cv.Scalar(atask.vecColors(i)(0), atask.vecColors(i)(1), atask.vecColors(i)(2))
                    dst2.Line(nPoint1, nPoint2, nextColor, options.drawFilled, atask.lineType)
                Next
            End If
        End Sub
    End Class
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To options.drawCount - 1
                ...
                If options.drawFilled <> -1 Then
                    cv.Cv2.Polylines(dst3, listOfPoints, True, polyColor, atask.lineWidth + 1, atask.lineType)
                Else
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If atask.heartBeat Then
                ...
                For i = 0 To options.numPoints - 1
                    ...
                    dst2.Line(p1, p2, atask.scalarColors(i Mod atask.scalarColors.Count), atask.lineWidth + 1, atask.lineType)
                Next
    ...
    Public Class Draw_Arc : Inherits TaskParent
        ...
        Public Shared Sub DrawRotatedOutline(rotatedRect As cv.RotatedRect, dst2 As cv.Mat, color As cv.Scalar)
            ...
            For i = 1 To pts.Length
                ...
                vbc.DrawLine(dst2, pt, lastPt, atask.highlight)
                lastPt = pt
            Next
        End Sub
        Public Shared Function InitRandomRect(margin As Integer) As cv.Rect
            Return New cv.Rect(msRNG.Next(margin, atask.workRes.Width - 2 * margin), msRNG.Next(margin, atask.workRes.Height - 2 * margin),
                               msRNG.Next(margin, atask.workRes.Width - 2 * margin), msRNG.Next(margin, atask.workRes.Height - 2 * margin))
        End Function
        Public Overrides Sub RunAlg(src As cv.Mat)
            options.Run()
            If atask.heartBeat Then
                ...
                atask.kalman.kInput = {rect.X, rect.Y, rect.Width, rect.Height, angle, startAngle, endAngle}
            End If

            atask.kalman.kInput = {rect.X, rect.Y, rect.Width, rect.Height, angle, startAngle, endAngle}
            atask.kalman.Run(emptyMat)
            Dim r = New cv.Rect(atask.kalman.kOutput(0), atask.kalman.kOutput(1), atask.kalman.kOutput(2), atask.kalman.kOutput(3))
            ...
            If options.drawFull Then
                dst2.Ellipse(rr, color, thickness, atask.lineType)
                Draw_Arc.DrawRotatedOutline(rr, dst2, atask.scalarColors(colorIndex))
            Else
                Dim angle = atask.kalman.kOutput(4)
                Dim startAngle = atask.kalman.kOutput(5)
                Dim endAngle = atask.kalman.kOutput(6)
                ...
                dst2.Ellipse(New cv.Point(rr.Center.X, rr.Center.Y), New cv.Size(r1.Width, r1.Height),
                         angle, startAngle, endAngle, color, thickness, atask.lineType)
            End If
        End Sub
    End Class
    ...
    Public Class NR_Draw_ClipLine : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            dst3 = src
            atask.kalman.kInput = {pt1.X, pt1.Y, pt2.X, pt2.Y, rect.X, rect.Y, rect.Width, rect.Height}
            atask.kalman.Run(emptyMat)
            Dim p1 = New cv.Point(CInt(atask.kalman.kOutput(0)), CInt(atask.kalman.kOutput(1)))
            Dim p2 = New cv.Point(CInt(atask.kalman.kOutput(2)), CInt(atask.kalman.kOutput(3)))

            If atask.kalman.kOutput(6) < 5 Then atask.kalman.kOutput(6) = 5 ' don't let the width/height get too small...
            If atask.kalman.kOutput(7) < 5 Then atask.kalman.kOutput(7) = 5
            Dim r = New cv.Rect(atask.kalman.kOutput(4), atask.kalman.kOutput(5), atask.kalman.kOutput(6), atask.kalman.kOutput(7))

            Dim clipped = cv.Cv2.ClipLine(r, p1, p2) ' Returns false when the line and the rectangle don't intersect.
            dst3.Line(p1, p2, If(clipped, white, cv.Scalar.Black), atask.lineWidth + 1, atask.lineType)
            dst3.Rectangle(r, If(clipped, cv.Scalar.Yellow, cv.Scalar.Red), atask.lineWidth + 1, atask.lineType)
    ...
    Public Class Draw_Line : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If p1 <> newPoint And p2 <> New cv.Point And atask.ClickPoint <> newPoint Then
                p1 = newPoint
                p2 = newPoint
            End If
            dst2 = src
            If atask.ClickPoint <> New cv.Point Or externalUse Then
                If p1 = New cv.Point Then p1 = atask.ClickPoint Else p2 = atask.ClickPoint
            End If
            If p1 <> newPoint And p2 = newPoint Then DrawCircle(dst2, p1, atask.DotSize, atask.highlight)
    ...
    Public Class Draw_Frustrum : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            src = New cv.Mat(New cv.Size(atask.workRes.Width, atask.workRes.Height), cv.MatType.CV_32F, cv.Scalar.All(0))

            Dim mid = src.Height / 2
            Dim zIncr = atask.MaxZmeters / mid
            dst2 = src.Clone
            ...
            Next
            xyzDepth.Run(dst2)
            dst3 = xyzDepth.dst2.Resize(New cv.Size(atask.workRes.Width, atask.workRes.Height))
        End Sub
    End Class
    ...
    Public Class Draw_RotatedRect : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i As Integer = 0 To vertices.Count - 1
                dst2.Line(vertices(i), vertices((i + 1) Mod 4), cv.Scalar.Green, atask.lineWidth, atask.lineType)
            Next
        End Sub
    End Class
End Namespace