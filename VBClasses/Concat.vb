...
Namespace VBClasses
    Public Class Concat_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.HConcat(src, taskAlg.depthRGB, tmp)
            dst2 = tmp.Resize(src.Size())
            cv.Cv2.VConcat(src, taskAlg.depthRGB, tmp)
    ...
    Public Class Concat_4way : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If standaloneTest() Then
                img(0) = src
                img(1) = taskAlg.depthRGB
                img(2) = If(taskAlg.leftView.Channels() = 1, taskAlg.leftView.CvtColor(cv.ColorConversionCodes.GRAY2BGR), taskAlg.leftView)
                img(3) = If(taskAlg.rightView.Channels() = 1, taskAlg.rightView.CvtColor(cv.ColorConversionCodes.GRAY2BGR), taskAlg.rightView)
            End If
