...
Namespace VBClasses
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To outputMat.Rows - 1
                ...
                If pt.X >= 0 And pt.X <= src.Cols And pt.Y >= 0 And pt.Y <= src.Rows Then
                    If status.Get(Of Byte)(i) Then
                        DrawCircle(dst2, pt, taskAlg.DotSize + 1, circleColor)
                    End If
                Else
    ...
    Public Class KLT_OpticalFlow : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            klt.Run(src)
            If taskAlg.frameCount > 0 And lastpoints IsNot Nothing And klt.ptInput IsNot Nothing Then
                dst2 = klt.dst2
                src.CopyTo(dst3)
                For i = 0 To klt.ptInput.Length - 1
                    If klt.status.Get(Of Byte)(i) And i < lastpoints.Length And i < klt.ptInput.Length Then
                        ' DrawLine(dst2,lastpoints(i), klt.inputPoints(i), cv.Scalar.Yellow, taskAlg.lineWidth + 1, taskAlg.lineType)
                        'Static lastFlowPoints() As cv.Point2f = klt.inputPoints
                        ' DrawLine(dst3, lastFlowPoints(i), klt.inputPoints(i), cv.Scalar.Yellow, taskAlg.lineWidth + 1, taskAlg.lineType)
                        'If taskAlg.heartBeat Then lastFlowPoints = klt.inputPoints
                    End If
                Next
            End If
