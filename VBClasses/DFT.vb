...
Namespace VBClasses
    ...
    Public Class DFT_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            rows = cv.Cv2.GetOptimalDFTSize(atask.gray.Rows)
            cols = cv.Cv2.GetOptimalDFTSize(atask.gray.Cols)
            Dim padded = New cv.Mat(atask.gray.Width, atask.gray.Height, cv.MatType.CV_8UC3)
            cv.Cv2.CopyMakeBorder(atask.gray, padded, 0, rows - atask.gray.Rows, 0, cols - atask.gray.Cols, cv.BorderTypes.Constant, cv.Scalar.All(0))
    ...
    Public Class NR_DFT_Inverse : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.Absdiff(atask.gray, dst2, diff)
    ...
    Public Class NR_DFT_ButterworthDepth : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            bfilter.Run(atask.depthRGB.CvtColor(cv.ColorConversionCodes.BGR2GRAY))
            dst2 = bfilter.dst2
            dst3 = bfilter.dst3
        End Sub
    End Class
