...
Namespace VBClasses
    ...
    Public Class BrickPoint_Core : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            'For Each pt In ptList
            '    If taskAlg.motionMask.Get(Of Byte)(pt.Y, pt.X) = 0 Then noMotionList.Add(pt)
            'Next
            ...
            For Each brick In taskAlg.bricks.brickList
                ...
                'If taskAlg.motionMask.Get(Of Byte)(brick.pt.Y, brick.pt.X) Then
            ...
            labels(2) = "Of the " + CStr(taskAlg.gridRects.Count) + " candidates, " + CStr(ptList.Count) +
    ...
    Public Class BrickPoint_Plot : Inherits TaskParent
        ...
        Public Sub New()
            taskAlg.gOptions.setHistogramBins(3)
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim incr = (plotHist.maxRange - plotHist.minRange) / taskAlg.histogramBins
            Dim histIndex = Math.Floor(taskAlg.mouseMovePoint.X / (dst2.Width / taskAlg.histogramBins))
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each brick In taskAlg.bricks.brickList
                If brick.rect.Y = 0 Then
                    lpList.Add(lpZero)
                Else
                    Dim gc1 = taskAlg.bricks.brickList(brick.index - taskAlg.bricksPerRow)
            ...
            If maxLen = taskAlg.brickSize And minLen = taskAlg.brickSize Then Exit Sub

            plotHist.Run(cv.Mat.FromPixelData(lengths.Count, 1, cv.MatType.CV_32F, lengths.ToArray))
            dst2 = plotHist.dst2

            Dim brickRange = (maxLen - minLen) / taskAlg.histogramBins
            ...
            For Each brick In taskAlg.bricks.brickList
                ...
                dst3.Line(lp.p1, lp.p2, taskAlg.highlight, taskAlg.lineWidth, taskAlg.lineWidth)
            Next
        End Sub
    End Class
    ...
    Public Class BrickPoint_Busiest : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each pt In bPoint.ptList
                ...
                If brick.correlation > 0.9 And brick.depth < taskAlg.MaxZmeters Then sortedBricks.Add(bPoint.sobel.dst2(brick.rect).CountNonZero, brick.rect)
            Next

            dst3 = bPoint.sobel.dst2
            For i = 0 To sortedBricks.Count - 1
                Dim ele = sortedBricks.ElementAt(i)
                dst2.Rectangle(ele.Value, taskAlg.highlight, taskAlg.lineWidth)
                dst3.Rectangle(ele.Value, 255, taskAlg.lineWidth)
            Next
            labels(2) = CStr(sortedBricks.Count) + " bricks had max Sobel values with high left/right correlation and depth < " + CStr(CInt(taskAlg.MaxZmeters)) + "m"
        End Sub
    End Class
    ...
    Public Class BrickPoint_PopulationSurvey : Inherits TaskParent
        ...
        Public Sub New()
            ...
            taskAlg.mouseMovePoint = New cv.Point(0, 0) ' this brick is often the most populated.
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            ReDim results(taskAlg.brickSize - 1, taskAlg.brickSize - 1)
            For Each pt In bPoint.ptList
                Dim index = taskAlg.gridMap.Get(Of Integer)(pt.Y, pt.X)
                Dim brick = taskAlg.bricks.brickList(index)
            ...
            Dim row = Math.Floor(taskAlg.mouseMovePoint.Y / incrY)
            Dim col = Math.Floor(taskAlg.mouseMovePoint.X / incrX)

            dst2 = cv.Mat.FromPixelData(taskAlg.brickSize, taskAlg.brickSize, cv.MatType.CV_32F, results)
    ...
    Public Class BrickPoint_KNN : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To knn.neighbors.Count - 1
                ...
                dst3.Line(p1, p2, 255, taskAlg.lineWidth, taskAlg.lineWidth)
    ...
    Public Class BrickPoint_EndPoints : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each index In lineList
                Dim lp = lplist(index)
                dst2.Line(lp.pE1, lp.pE2, taskAlg.highlight, taskAlg.lineWidth, taskAlg.lineWidth)
            Next
        End Sub
    End Class
    ...
    Public Class BrickPoint_Minimum : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            labels(2) = "Of the " + CStr(taskAlg.gridRects.Count) + " candidates, " + CStr(features.Count) +
