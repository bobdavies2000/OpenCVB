...
Namespace VBClasses
    ...
    Public Class BrickPoint_Core : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            labels(2) = "Of the " + CStr(taskA.gridRects.Count) + " candidates, " + CStr(ptList.Count) +
    ...
    Public Class NR_BrickPoint_Plot : Inherits TaskParent
        ...
        Public Sub New()
            taskA.gOptions.setHistogramBins(3)
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim incr = (plotHist.maxRange - plotHist.minRange) / taskA.histogramBins
            Dim histIndex = Math.Floor(taskA.mouseMovePoint.X / (dst2.Width / taskA.histogramBins))
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each gr In taskA.bricks.brickList
                If gr.rect.Y = 0 Then
                    lpList.Add(lpZero)
                Else
                    Dim gc1 = taskA.bricks.brickList(gr.index - taskA.bricksPerRow)
            ...
            If maxLen = taskA.brickSize And minLen = taskA.brickSize Then Exit Sub

            plotHist.Run(cv.Mat.FromPixelData(lengths.Count, 1, cv.MatType.CV_32F, lengths.ToArray))
            dst2 = plotHist.dst2

            Dim brickRange = (maxLen - minLen) / taskA.histogramBins
            ...
            For Each gr In taskA.bricks.brickList
                ...
                dst3.Line(lp.p1, lp.p2, taskA.highlight, taskA.lineWidth, taskA.lineWidth)
            Next
        End Sub
    End Class
    ...
    Public Class NR_BrickPoint_Busiest : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each pt In bPoint.ptList
                ...
                If gr.correlation > 0.9 And gr.depth < taskA.MaxZmeters Then sortedBricks.Add(bPoint.sobel.dst2(gr.rect).CountNonZero, gr.rect)
            Next

            dst3 = bPoint.sobel.dst2
            For i = 0 To sortedBricks.Count - 1
                Dim ele = sortedBricks.ElementAt(i)
                dst2.Rectangle(ele.Value, taskA.highlight, taskA.lineWidth)
                dst3.Rectangle(ele.Value, 255, taskA.lineWidth)
            Next
            labels(2) = CStr(sortedBricks.Count) + " bricks had max Sobel values with high left/right correlation and depth < " + CStr(CInt(taskA.MaxZmeters)) + "m"
        End Sub
    End Class
    ...
    Public Class BrickPoint_PopulationSurvey : Inherits TaskParent
        ...
        Public Sub New()
            ...
            taskA.mouseMovePoint = New cv.Point(0, 0) ' this gr is often the most populated.
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            ReDim results(taskA.brickSize - 1, taskA.brickSize - 1)
            For Each pt In bPoint.ptList
                Dim index = taskA.gridMap.Get(Of Integer)(pt.Y, pt.X)
                Dim gr = taskA.bricks.brickList(index)
            ...
            Dim row = Math.Floor(taskA.mouseMovePoint.Y / incrY)
            Dim col = Math.Floor(taskA.mouseMovePoint.X / incrX)

            dst2 = cv.Mat.FromPixelData(taskA.brickSize, taskA.brickSize, cv.MatType.CV_32F, results)
    ...
    Public Class BrickPoint_KNN : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To knn.neighbors.Count - 1
                ...
                dst3.Line(p1, p2, 255, taskA.lineWidth, taskA.lineWidth)
    ...
    Public Class NR_BrickPoint_EndPoints : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each index In lineList
                Dim lp = lplist(index)
                dst2.Line(lp.pE1, lp.pE2, taskA.highlight, taskA.lineWidth, taskA.lineWidth)
            Next
        End Sub
    End Class
    ...
    Public Class BrickPoint_Minimum : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            labels(2) = "Of the " + CStr(taskA.gridRects.Count) + " candidates, " + CStr(features.Count) +
