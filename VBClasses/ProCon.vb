...
Namespace VBClasses
    ...
    Public Class ProCon_Basics : Inherits TaskParent
        ...
        Public Sub New()
            If task.testAllRunning = False Then
                flow.parentData = Me
                p = New Thread(AddressOf Producer) With {.Name = "Producer"}
                p.Start()

                c = New Thread(AddressOf Consumer) With {.Name = "Consumer"}
                c.Start()
            End If
    ...
    Public Class ProCon_Variation : Inherits TaskParent
        ...
        Public Sub New()
            procon = New ProCon_Basics() With {.terminateProducer = True} ' we don't need a 2 producer task.  RunVB below provides the second thread.
