...
Namespace VBClasses
    ...
    Public Class Fractal_MandelbrotZoom : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If taskAlg.drawRect.Width <> 0 Then
                Dim newStartX = mandel.startX + (mandel.endX - mandel.startX) * taskAlg.drawRect.X / src.Width
                mandel.endX = mandel.startX + (mandel.endX - mandel.startX) * (taskAlg.drawRect.X + taskAlg.drawRect.Width) / src.Width
                mandel.startX = newStartX

                Dim newStartY = mandel.startY + (mandel.endY - mandel.startY) * taskAlg.drawRect.Y / src.Height
                Dim Height = taskAlg.drawRect.Width * src.Height / src.Width ' maintain aspect ratio across zooms...
                mandel.endY = mandel.startY + (mandel.endY - mandel.startY) * (taskAlg.drawRect.Y + Height) / src.Height
            ...
            If taskAlg.optionsChanged Or saveDrawRect <> taskAlg.drawRect Then
    ...
    Public Class Fractal_Julia : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If savedMouse <> taskAlg.mouseMovePoint Or resetCheck.Checked Then
                savedMouse = taskAlg.mouseMovePoint
                mandel.Run(src)
                ...
                rt = m.startX + (m.endX - m.startX) * taskAlg.mouseMovePoint.X / src.Width
                mt = m.startY + (m.endY - m.startY) * taskAlg.mouseMovePoint.Y / src.Height
    ...
    Public Class Fractal_Dimension : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Static rect = New cv.Rect(0, 0, taskAlg.oldrcD.rect.Width, taskAlg.oldrcD.rect.Height)
            If taskAlg.optionsChanged Or taskAlg.mouseClickFlag Then
                rect = New cv.Rect(0, 0, taskAlg.oldrcD.rect.Width, taskAlg.oldrcD.rect.Height)
            End If

            If taskAlg.oldrcD.rect.Width = 0 Or taskAlg.oldrcD.rect.Height = 0 Then Exit Sub
            taskAlg.oldrcD.mask.CopyTo(dst3(New cv.Rect(0, 0, taskAlg.oldrcD.rect.Width, taskAlg.oldrcD.rect.Height)))
