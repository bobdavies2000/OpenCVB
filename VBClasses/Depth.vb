...
Namespace VBClasses
    Public Class Depth_Basics : Inherits TaskParent
        Public Sub New()
            desc = "Colorize the depth data into tsk.depthRGB"
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            dst2 = tsk.pcSplit(2)

            tsk.pcSplit(2) = tsk.pcSplit(2).Threshold(tsk.MaxZmeters, tsk.MaxZmeters, cv.ThresholdTypes.Trunc)
            tsk.maxDepthMask = tsk.pcSplit(2).ConvertScaleAbs().InRange(tsk.MaxZmeters, 1000)
            SetTrueText(tsk.gravityMatrix.strOut, 3)
        End Sub
    End Class
    ...
    Public Class NR_Depth_Display : Inherits TaskParent
        Public Sub New()
            ...
            If standalone Then tsk.gOptions.displayDst1.Checked = True
            labels = {"tsk.pcSplit(2)", "tsk.pointcloud", "tsk.depthMask", "tsk.noDepthMask"}
            desc = "Display the tsk.pcSplit(2), tsk.pointcloud, tsk.depthMask, and tsk.noDepthMask"
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            dst0 = tsk.pcSplit(2)
            dst1 = tsk.pointCloud
            dst2 = tsk.depthMask
            dst3 = tsk.noDepthMask
        End Sub
    End Class
    ...
    Public Class NR_Depth_FirstLastDistance : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            Dim mm As mmData = GetMinMax(tsk.pcSplit(2), tsk.depthMask)
            tsk.depthRGB.CopyTo(dst2)
    ...
    Public Class NR_Depth_HolesRect : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To Math.Min(sortContours.Count, 10) - 1
                ...
                Dim nextColor = New cv.Scalar(tsk.vecColors(i Mod 256)(0), tsk.vecColors(i Mod 256)(1), tsk.vecColors(i Mod 256)(2))
                Rectangle_Basics.DrawRotatedRect(minRect, dst2, nextColor)
                DrawTour(dst3, contour.ToList, white, tsk.lineWidth)
            Next
            cv.Cv2.AddWeighted(dst2, 0.5, tsk.depthRGB, 0.5, 0, dst2)
        End Sub
    End Class
    ...
    Public Class NR_Depth_MeanStdevPlot : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim depthMask As cv.Mat = tsk.depthMask
            cv.Cv2.MeanStdDev(tsk.pcSplit(2), mean, stdev, depthMask)
    ...
    Public Class NR_Depth_LocalMinMax_MT : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If minPoint.Length <> tsk.gridRects.Count Then
                ReDim minPoint(tsk.gridRects.Count - 1)
                ReDim maxPoint(tsk.gridRects.Count - 1)
            End If
        ...
        Sub(i)
            ...
            DrawCircle(dst2(gr), mm.minLoc, tsk.DotSize, tsk.highlight)
            DrawCircle(dst2(gr), mm.maxLoc, tsk.DotSize, cv.Scalar.Red)

            Dim p1 = New cv.Point(mm.minLoc.X + gr.X, mm.minLoc.Y + gr.Y)
            Dim p2 = New cv.Point(mm.maxLoc.X + gr.X, mm.maxLoc.Y + gr.Y)
            DrawCircle(dst3, p1, tsk.DotSize, tsk.highlight)
            DrawCircle(dst3, p2, tsk.DotSize, cv.Scalar.Red)
        End Sub)
        End Sub
    End Class
    ...
    Public Class Depth_ColorMap : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.ConvertScaleAbs(tsk.pcSplit(2) * 1000, dst1, options.alpha, options.beta)
            dst1 += 1
    ...
    Public Class NR_Depth_Median : Inherits TaskParent
        ...
        Public Sub New()
            median.rangeMax = tsk.MaxZmeters
            median.rangeMin = 0
            desc = "Divide the depth image ahead and behind the median."
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            median.Run(tsk.pcSplit(2))

            Dim mask As cv.Mat
            mask = tsk.pcSplit(2).LessThan(median.medianVal)
            tsk.pcSplit(2).CopyTo(dst2, mask)
            ...
            dst3.SetTo(0)
            tsk.depthRGB.CopyTo(dst3, Not mask)
            dst3.SetTo(0, tsk.noDepthMask)
    ...
    Public Class Depth_SmoothingMat : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Static lastDepth = tsk.pcSplit(2)

            If standaloneTest() Then src = tsk.pcSplit(2)
            Dim rect = If(tsk.drawRect.Width <> 0, tsk.drawRect, New cv.Rect(0, 0, src.Width, src.Height))

            cv.Cv2.Subtract(lastDepth, tsk.pcSplit(2), dst2)
            ...
            cv.Cv2.Add(tsk.pcSplit(2), dst2, dst3)
            lastDepth = tsk.pcSplit(2)

            labels(2) = "Smoothing Mat: range to " + CStr(tsk.MaxZmeters) + " meters"
        End Sub
    End Class
    ...
    Public Class NR_Depth_HolesOverTime : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If images.Count >= tsk.frameHistoryCount Then
                dst0 -= images(0)
                images.RemoveAt(0)
            End If
        End Sub
    ...
    Public Class Depth_Holes : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2 = tsk.pcSplit(2).Threshold(0.01, 255, cv.ThresholdTypes.BinaryInv).ConvertScaleAbs(255)
            dst2 = dst2.Dilate(element, Nothing, holeSlider.Value)
            dst3 = dst2.Dilate(element, Nothing, borderSlider.Value)
            dst3 = dst3 Xor dst2
            If standaloneTest() Then tsk.depthRGB.CopyTo(dst3, dst3)
        End Sub
    End Class
    ...
    Public Class NR_Depth_ForegroundHead : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If fgnd.dst2.CountNonZero And fgnd.maxIndex >= 0 Then
                ...
                tsk.kalman.kInput = {xx, yy, rectSize, rectSize}
                tsk.kalman.Run(emptyMat)
                Dim nextRect = New cv.Rect(xx, yy, rectSize, rectSize)
                Dim kRect = New cv.Rect(tsk.kalman.kOutput(0), tsk.kalman.kOutput(1), tsk.kalman.kOutput(2), tsk.kalman.kOutput(3))
    ...
    Public Class NR_Depth_BGSubtract : Inherits TaskParent
        ...
        Public Sub New()
            labels = {"", "", "Latest tsk.noDepthMask", "BGSubtract output for the tsk.noDepthMask"}
            desc = "Create a mask for the missing depth across multiple frame"
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            dst2 = tsk.noDepthMask
    ...
    Public Class NR_Depth_MaxMask : Inherits TaskParent
        ...
        Public Sub New()
            ...
            desc = "Display the tsk.maxDepthMask and its contour containing depth that is greater than maxdepth (global setting)"
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            dst2 = src

            dst2.SetTo(white, tsk.maxDepthMask)
            contour.Run(tsk.maxDepthMask)
            dst3.SetTo(0)
    ...
    Public Class NR_Depth_ForegroundOverTime : Inherits TaskParent
        ...
        Public Sub New()
            ...
            tsk.frameHistoryCount = 5
            desc = "Create a fused foreground mask over x number of frames (tsk.frameHistoryCount)"
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            options.Run()

            If tsk.optionsChanged Then lastFrames.Clear()
            ...
            For Each m In lastFrames
                dst2 += m
            Next
            If lastFrames.Count >= tsk.frameHistoryCount Then lastFrames.RemoveAt(0)
    ...
    Public Class Depth_ForegroundBlob : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.InRange(tsk.pcSplit(2), 0.01, options.maxForegroundDepthInMeters, dst2)
            dst3 = dst2.Clone
    ...
    Public Class Depth_Foreground : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim dst1 = tsk.pcSplit(2).Threshold(options.maxForegroundDepthInMeters, 255, cv.ThresholdTypes.BinaryInv).ConvertScaleAbs()
    ...
    Public Class Depth_InRange : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To options.numberOfRegions - 1
                ...
                regMats.Add(tsk.pcSplit(2).InRange(i * options.depthPerRegion, upperBound))
                If i = 0 Then regMats(0).SetTo(0, tsk.noDepthMask)
            Next
            ...
            If tsk.heartBeat Then labels(2) = Format(classCount, "000") + " regions were found"
        End Sub
    End Class
    ...
    Public Class NR_Depth_Regions : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            dst1 = tsk.pcSplit(2).Threshold(tsk.MaxZmeters, tsk.MaxZmeters, cv.ThresholdTypes.Binary)
            dst0 = (tsk.pcSplit(2) / tsk.MaxZmeters) * 255 / classCount
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For y = 0 To src.Rows - 1
                For x = 0 To src.Cols - 1
                    ...
                    If pixel > 0 And pixel <= tsk.MaxZmeters Then
                        Dim t = pixel / tsk.MaxZmeters
    ...
    Public Class Depth_PunchDecreasing : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            fore.Run(src)
            dst1.SetTo(0)
            tsk.pcSplit(2).CopyTo(dst1, fore.dst2)
    ...
    Public Class NR_Depth_Contour : Inherits TaskParent
        ...
        Public Sub New()
            dst2 = New cv.Mat(dst2.Size(), cv.MatType.CV_8U, cv.Scalar.All(0))
            labels(2) = "tsk.depthMask contour"
            desc = "Create and display the tsk.depthMask output as a contour."
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            contour.Run(tsk.depthMask)
    ...
    Public Class Depth_Outline : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If standaloneTest() Then src = tsk.depthMask
            contour.Run(src)

            dst2.SetTo(0)
            For Each tour In contour.contourList
                DrawTour(dst2, tour.ToList, 255, tsk.lineWidth)
            Next
    ...
    Public Class Depth_StableMin : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If src.Type <> cv.MatType.CV_32FC1 Then src = tsk.pcSplit(2)

            If tsk.heartBeat Then
                stableMin = src.Clone
                dst3.SetTo(0)
            Else
                src.CopyTo(stableMin, tsk.motionRGB.motionMask)
                If src.Type <> stableMin.Type Then src.ConvertTo(src, stableMin.Type)
                stableMin.CopyTo(src, tsk.noDepthMask)
    ...
    Public Class Depth_StableMinMax : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If src.Type <> cv.MatType.CV_32FC1 Then src = tsk.pcSplit(2)
            If tsk.optionsChanged Then dst3 = tsk.pcSplit(2)
    ...
    Public Class NR_Depth_Flatland : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2 = tsk.depthRGB / options.reductionFactor
    ...
    Public Class Depth_StableMax : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If tsk.heartBeat Then
                stableMax = src.Clone
            Else
                src.CopyTo(stableMax, tsk.motionRGB.motionMask)
                If src.Type <> stableMax.Type Then src.ConvertTo(src, stableMax.Type)
                stableMax.CopyTo(src, tsk.noDepthMask)
    ...
    Public Class NR_Depth_InfinityCheck : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If tsk.gOptions.DebugCheckBox.Checked Then
                Dim mask = tsk.pcSplit(plane).InRange(-100, 100)
                tsk.pcSplit(plane).SetTo(0, Not mask)
            End If

            Dim infCount As Integer
            For y = 0 To tsk.pcSplit(plane).Rows - 1
                For x = 0 To tsk.pcSplit(plane).Cols - 1
                    Dim val = tsk.pcSplit(plane).Get(Of Single)(y, x)
                    If Single.IsInfinity(val) Or Single.IsNegativeInfinity(val) Then infCount += 1
                Next
            Next
            dst2 = tsk.pcSplit(plane)
            Dim planeName = Choose(plane + 1, "X ", "Y ", "Z ")
            labels(2) = CStr(infCount) + " infinite values encountered in the " + planeName + " plane"

            Dim mm = GetMinMax(tsk.pcSplit(plane))
    ...
    Public Class NR_Depth_ColorizerOld : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            ' couldn't do this in the constructor because it uses Gradient_Color and is called in tsk.
            ...
            End If
            If src.Type <> cv.MatType.CV_32F Then src = tsk.pcSplit(2)
            Dim depthNorm As cv.Mat = (src * 255 / tsk.MaxZmeters)
    ...
    Public Class Depth_TierCount : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If kValues.Count > tsk.frameHistoryCount * 10 Then kValues.RemoveAt(0)
    ...
    Public Class NR_Depth_CellTiers : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 1 To Math.Min(10, tsk.redList.oldrclist.Count - 1)
                Dim rc = tsk.redList.oldrclist(i)
                Dim depthData = tsk.pcSplit(2)(rc.rect).Clone
                depthData.SetTo(0, Not rc.mask)

                valley.Run(depthData)
                If i = tsk.gOptions.DebugSlider.Value And standalone Then
                    dst3 = valley.dst2.Clone
                    labels(3) = valley.strOut
                    tsk.clickPoint = rc.maxDist
                    Swarm_Flood.oldSelectCell()
                End If
                If tsk.heartBeat Then SetTrueText(CStr(valley.classCount) + " classes", rc.maxDist)
            Next

            Static saveTrueText As New List(Of TrueText)
            If tsk.heartBeat Then saveTrueText = New List(Of TrueText)(trueData)
    ...
    Public Class NR_Depth_ErrorEstimate : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If tsk.brickD Is Nothing Then Exit Sub
            SetTrueText(Format(ErrorEstimate(tsk.brickD.depth), fmt3) + " estimated error" + vbCrLf + Format(tsk.brickD.depth, fmt3) + "m",
                    tsk.mouseMovePoint, 3)
        End Sub
    End Class
    ...
    Public Class NR_Depth_MinMaxToVoronoi : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each gr In tsk.bricks.brickList
                ...
                DrawCircle(dst1(gr.rect), gr.mm.minLoc, tsk.DotSize, cv.Scalar.Red)
                DrawCircle(dst1(gr.rect), gr.mm.maxLoc, tsk.DotSize, cv.Scalar.Blue)
            Next
    ...
    Public Class NR_Depth_WorldXYMT : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim multX = tsk.pointCloud.Width / src.Width
            Dim multY = tsk.pointCloud.Height / src.Height
    ...
    Public Class Depth_World : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst0 *= cv.Scalar.All(1 / tsk.calibData.leftIntrinsics.fx)

            cv.Cv2.Multiply(template.dst3, src, dst1)
            dst1 *= cv.Scalar.All(1 / tsk.calibData.leftIntrinsics.fy)
    ...
    Public Class Depth_ReliableLines : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If tsk.Settings.cameraName.StartsWith("StereoLabs") = False Then
                SetTrueText("The " + traceName + " algorithm is currently only working for StereoLabs cameras.")
                Exit Sub
            End If
            dst2 = src
            dst3 = tsk.rightView.CvtColor(cv.ColorConversionCodes.GRAY2BGR) ' so we can show the red line...

            Dim count As Integer
            Dim lastPoints As New List(Of cv.Point)(rightPoints)
            rightPoints.Clear()
            For Each lp In tsk.lines.lpList
                Dim brick1 = tsk.bricks.brickList(lp.p1GridIndex)
                Dim brick2 = tsk.bricks.brickList(lp.p2GridIndex)
                ...
                If brick1.depth > 0 And brick2.depth > 0 Then
                    p1.X -= tsk.calibData.baseline * tsk.calibData.leftIntrinsics.fx / brick1.depth
                    p2.X -= tsk.calibData.baseline * tsk.calibData.leftIntrinsics.fx / brick2.depth
                    ...
                    If found1 And lastPoints.Contains(pt1) And found2 And lastPoints.Contains(pt2) Then
                        dst3.Line(p1, p2, lp.color, tsk.lineWidth + 1, tsk.lineType)
                        rightPoints.Add(p1)
                        rightPoints.Add(p2)
                    Else
                        count += 1
