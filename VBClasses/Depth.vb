...
Namespace VBClasses
    Public Class Depth_Basics : Inherits TaskParent
        Public Sub New()
            desc = "Colorize the depth data into algTask.depthRGB"
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            dst2 = algTask.pcSplit(2)

            algTask.pcSplit(2) = algTask.pcSplit(2).Threshold(algTask.MaxZmeters, algTask.MaxZmeters, cv.ThresholdTypes.Trunc)
            algTask.maxDepthMask = algTask.pcSplit(2).ConvertScaleAbs().InRange(algTask.MaxZmeters, 1000)
            If standalone Then dst3 = algTask.maxDepthMask
            SetTrueText(algTask.gmat.strOut, 3)
        End Sub
    End Class
    ...
    Public Class Depth_Display : Inherits TaskParent
        Public Sub New()
            ...
            If standalone Then algTask.gOptions.displayDst1.Checked = True
            labels = {"algTask.pcSplit(2)", "algTask.pointcloud", "algTask.depthMask", "algTask.noDepthMask"}
            desc = "Display the algTask.pcSplit(2), algTask.pointcloud, algTask.depthMask, and algTask.noDepthMask"
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            dst0 = algTask.pcSplit(2)
            dst1 = algTask.pointCloud
            dst2 = algTask.depthMask
            dst3 = algTask.noDepthMask
        End Sub
    End Class
    ...
    Public Class Depth_FirstLastDistance : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            Dim mm As mmData = GetMinMax(algTask.pcSplit(2), algTask.depthMask)
            algTask.depthRGB.CopyTo(dst2)
    ...
    Public Class Depth_HolesRect : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To Math.Min(sortContours.Count, 10) - 1
                ...
                Dim nextColor = New cv.Scalar(algTask.vecColors(i Mod 256)(0), algTask.vecColors(i Mod 256)(1), algTask.vecColors(i Mod 256)(2))
                DrawRotatedRect(minRect, dst2, nextColor)
                DrawTour(dst3, contour.ToList, white, algTask.lineWidth)
            Next
            cv.Cv2.AddWeighted(dst2, 0.5, algTask.depthRGB, 0.5, 0, dst2)
        End Sub
    End Class
    ...
    Public Class Depth_MeanStdevPlot : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim depthMask As cv.Mat = algTask.depthMask
            cv.Cv2.MeanStdDev(algTask.pcSplit(2), mean, stdev, depthMask)
    ...
    Public Class Depth_LocalMinMax_MT : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If minPoint.Length <> algTask.gridRects.Count Then
                ReDim minPoint(algTask.gridRects.Count - 1)
                ReDim maxPoint(algTask.gridRects.Count - 1)
            End If
        ...
        Sub(i)
            ...
            DrawCircle(dst2(roi), mm.minLoc, algTask.DotSize, algTask.highlight)
            DrawCircle(dst2(roi), mm.maxLoc, algTask.DotSize, cv.Scalar.Red)
    ...
    Public Class Depth_ColorMap : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.ConvertScaleAbs(algTask.pcSplit(2) * 1000, dst1, options.alpha, options.beta)
            dst1 += 1
    ...
    Public Class Depth_Median : Inherits TaskParent
        ...
        Public Sub New()
            median.rangeMax = algTask.MaxZmeters
            median.rangeMin = 0
            desc = "Divide the depth image ahead and behind the median."
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            median.Run(algTask.pcSplit(2))

            Dim mask As cv.Mat
            mask = algTask.pcSplit(2).LessThan(median.medianVal)
            algTask.pcSplit(2).CopyTo(dst2, mask)
            ...
            dst3.SetTo(0)
            algTask.depthRGB.CopyTo(dst3, Not mask)
            dst3.SetTo(0, algTask.noDepthMask)
    ...
    Public Class Depth_SmoothingMat : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Static lastDepth = algTask.pcSplit(2)

            If standaloneTest() Then src = algTask.pcSplit(2)
            Dim rect = If(algTask.drawRect.Width <> 0, algTask.drawRect, New cv.Rect(0, 0, src.Width, src.Height))

            cv.Cv2.Subtract(lastDepth, algTask.pcSplit(2), dst2)
            ...
            cv.Cv2.Add(algTask.pcSplit(2), dst2, dst3)
            lastDepth = algTask.pcSplit(2)

            labels(2) = "Smoothing Mat: range to " + CStr(algTask.MaxZmeters) + " meters"
        End Sub
    End Class
    ...
    Public Class Depth_Smoothing : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            reduction.Run(algTask.pcSplit(2))
    ...
    Public Class Depth_HolesOverTime : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If images.Count >= algTask.frameHistoryCount Then
                dst0 -= images(0)
                images.RemoveAt(0)
            End If
        End Sub
    ...
    Public Class Depth_Holes : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2 = algTask.pcSplit(2).Threshold(0.01, 255, cv.ThresholdTypes.BinaryInv).ConvertScaleAbs(255)
            dst2 = dst2.Dilate(element, Nothing, holeSlider.Value)
            dst3 = dst2.Dilate(element, Nothing, borderSlider.Value)
            dst3 = dst3 Xor dst2
            If standaloneTest() Then algTask.depthRGB.CopyTo(dst3, dst3)
        End Sub
    End Class
    ...
    Public Class Depth_ForegroundHead : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If fgnd.dst2.CountNonZero And fgnd.maxIndex >= 0 Then
                ...
                algTask.kalman.kInput = {xx, yy, rectSize, rectSize}
                algTask.kalman.Run(emptyMat)
                Dim nextRect = New cv.Rect(xx, yy, rectSize, rectSize)
                Dim kRect = New cv.Rect(algTask.kalman.kOutput(0), algTask.kalman.kOutput(1), algTask.kalman.kOutput(2), algTask.kalman.kOutput(3))
    ...
    Public Class Depth_BGSubtract : Inherits TaskParent
        ...
        Public Sub New()
            labels = {"", "", "Latest algTask.noDepthMask", "BGSubtract output for the algTask.noDepthMask"}
    ...
    Public Class Depth_MaxMask : Inherits TaskParent
        ...
        Public Sub New()
            ...
            desc = "Display the algTask.maxDepthMask and its contour containing depth that is greater than maxdepth (global setting)"
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            dst2 = src

            dst2.SetTo(white, algTask.maxDepthMask)
            contour.Run(algTask.maxDepthMask)
            dst3.SetTo(0)
    ...
    Public Class Depth_ForegroundOverTime : Inherits TaskParent
        ...
        Public Sub New()
            ...
            algTask.frameHistoryCount = 5
            desc = "Create a fused foreground mask over x number of frames (algTask.frameHistoryCount)"
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            options.Run()

            If algTask.optionsChanged Then lastFrames.Clear()
            ...
            For Each m In lastFrames
                dst2 += m
            Next
            If lastFrames.Count >= algTask.frameHistoryCount Then lastFrames.RemoveAt(0)
    ...
    Public Class Depth_ForegroundBlob : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.InRange(algTask.pcSplit(2), 0.01, options.maxForegroundDepthInMeters, dst2)
            dst3 = dst2.Clone
    ...
    Public Class Depth_Foreground : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim dst1 = algTask.pcSplit(2).Threshold(options.maxForegroundDepthInMeters, 255, cv.ThresholdTypes.BinaryInv).ConvertScaleAbs()
    ...
    Public Class Depth_InRange : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To options.numberOfRegions - 1
                ...
                regMats.Add(algTask.pcSplit(2).InRange(i * options.depthPerRegion, upperBound))
                If i = 0 Then regMats(0).SetTo(0, algTask.noDepthMask)
            Next
            ...
            If algTask.heartBeat Then labels(2) = Format(classCount, "000") + " regions were found"
        End Sub
    End Class
    ...
    Public Class Depth_Regions : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            dst1 = algTask.pcSplit(2).Threshold(algTask.MaxZmeters, algTask.MaxZmeters, cv.ThresholdTypes.Binary)
            dst0 = (algTask.pcSplit(2) / algTask.MaxZmeters) * 255 / classCount
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For y = 0 To src.Rows - 1
                For x = 0 To src.Cols - 1
                    ...
                    If pixel > 0 And pixel <= algTask.MaxZmeters Then
                        Dim t = pixel / algTask.MaxZmeters
    ...
    Public Class Depth_PunchDecreasing : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            fore.Run(src)
            dst1.SetTo(0)
            algTask.pcSplit(2).CopyTo(dst1, fore.dst2)
    ...
    Public Class Depth_Contour : Inherits TaskParent
        ...
        Public Sub New()
            dst2 = New cv.Mat(dst2.Size(), cv.MatType.CV_8U, cv.Scalar.All(0))
            labels(2) = "algTask.depthMask contour"
            desc = "Create and display the algTask.depthMask output as a contour."
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            contour.Run(algTask.depthMask)
    ...
    Public Class Depth_Outline : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If standaloneTest() Then src = algTask.depthMask
            contour.Run(src)

            dst2.SetTo(0)
            For Each tour In contour.contourList
                DrawTour(dst2, tour.ToList, 255, algTask.lineWidth)
            Next
    ...
    Public Class Depth_StableMin : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If src.Type <> cv.MatType.CV_32FC1 Then src = algTask.pcSplit(2)

            If algTask.heartBeat Then
                stableMin = src.Clone
                dst3.SetTo(0)
            Else
                src.CopyTo(stableMin, algTask.motionMask)
                If src.Type <> stableMin.Type Then src.ConvertTo(src, stableMin.Type)
                stableMin.CopyTo(src, algTask.noDepthMask)
    ...
    Public Class Depth_StableMinMax : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If src.Type <> cv.MatType.CV_32FC1 Then src = algTask.pcSplit(2)
            If algTask.optionsChanged Then dst3 = algTask.pcSplit(2)
            ...
            ElseIf options.useNone Then
                dst3 = algTask.pcSplit(2)
                dst2 = algTask.depthRGB
            End If
        End Sub
    End Class
    ...
    Public Class Depth_Flatland : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2 = algTask.depthRGB / options.reductionFactor
    ...
    Public Class Depth_StableMax : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If algTask.heartBeat Then
                stableMax = src.Clone
            Else
                src.CopyTo(stableMax, algTask.motionMask)
                If src.Type <> stableMax.Type Then src.ConvertTo(src, stableMax.Type)
                stableMax.CopyTo(src, algTask.noDepthMask)
    ...
    Public Class Depth_InfinityCheck : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If algTask.gOptions.DebugCheckBox.Checked Then
                Dim mask = algTask.pcSplit(plane).InRange(-100, 100)
                algTask.pcSplit(plane).SetTo(0, Not mask)
            End If

            Dim infCount As Integer
            For y = 0 To algTask.pcSplit(plane).Rows - 1
                For x = 0 To algTask.pcSplit(plane).Cols - 1
                    Dim val = algTask.pcSplit(plane).Get(Of Single)(y, x)
                    If Single.IsInfinity(val) Or Single.IsNegativeInfinity(val) Then infCount += 1
                Next
            Next
            dst2 = algTask.pcSplit(plane)
            Dim planeName = Choose(plane + 1, "X ", "Y ", "Z ")
            labels(2) = CStr(infCount) + " infinite values encountered in the " + planeName + " plane"

            Dim mm = GetMinMax(algTask.pcSplit(plane))
    ...
    Public Class Depth_ColorizerOld : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            ' couldn't do this in the constructor because it uses Gradient_Color and is called in algTask.
            ...
            End If
            If src.Type <> cv.MatType.CV_32F Then src = algTask.pcSplit(2)
            Dim depthNorm As cv.Mat = (src * 255 / algTask.MaxZmeters)
    ...
    Public Class Depth_TierCount : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If kValues.Count > algTask.frameHistoryCount * 10 Then kValues.RemoveAt(0)
    ...
    Public Class Depth_CellTiers : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 1 To Math.Min(10, algTask.redList.oldrclist.Count - 1)
                Dim rc = algTask.redList.oldrclist(i)
                Dim depthData = algTask.pcSplit(2)(rc.rect).Clone
                depthData.SetTo(0, Not rc.mask)

                valley.Run(depthData)
                If i = algTask.gOptions.DebugSlider.Value And standalone Then
                    dst3 = valley.dst2.Clone
                    labels(3) = valley.strOut
                    algTask.ClickPoint = rc.maxDist
                    algTask.setSelectedCell()
                End If
                If algTask.heartBeat Then SetTrueText(CStr(valley.classCount) + " classes", rc.maxDist)
            Next

            Static saveTrueText As New List(Of TrueText)
            If algTask.heartBeat Then saveTrueText = New List(Of TrueText)(trueData)
    ...
    Public Class Depth_ErrorEstimate : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If algTask.brickD Is Nothing Then Exit Sub
            SetTrueText(Format(ErrorEstimate(algTask.brickD.depth), fmt3) + " estimated error" + vbCrLf + Format(algTask.brickD.depth, fmt3) + "m",
                    algTask.mouseMovePoint, 3)
        End Sub
    End Class
    ...
    Public Class Depth_MinMaxToVoronoi : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each brick In algTask.bricks.brickList
                ...
                DrawCircle(dst1(brick.rect), brick.mm.minLoc, algTask.DotSize, cv.Scalar.Red)
                DrawCircle(dst1(brick.rect), brick.mm.maxLoc, algTask.DotSize, cv.Scalar.Blue)
            Next
    ...
    Public Class Depth_WorldXYMT : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim multX = algTask.pointCloud.Width / src.Width
            Dim multY = algTask.pointCloud.Height / src.Height
    ...
    Public Class Depth_World : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst0 *= 1 / algTask.calibData.rgbIntrinsics.fx

            cv.Cv2.Multiply(template.dst3, src, dst1)
            dst1 *= 1 / algTask.calibData.rgbIntrinsics.fy
