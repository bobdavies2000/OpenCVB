...
Namespace VBClasses
    Public Class Foreground_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To classCount - 1
                ...
                Dim depth = taskAlg.pcSplit(2).Mean(tmp)(0)
            ...
            fg = taskAlg.pcSplit(2).Threshold(fgDepth, 255, cv.ThresholdTypes.BinaryInv).ConvertScaleAbs()
            dst0 = fg

            fg.SetTo(0, taskAlg.noDepthMask)
            bg = Not fg
    ...
    Public Class Foreground_KMeans : Inherits TaskParent
        ...
        Public Sub New()
            ...
            dst2 = New cv.Mat(New cv.Size(taskAlg.workRes.Width, taskAlg.workRes.Height), cv.MatType.CV_8U, cv.Scalar.All(0))
            dst3 = New cv.Mat(New cv.Size(taskAlg.workRes.Width, taskAlg.workRes.Height), cv.MatType.CV_8U, cv.Scalar.All(0))
            desc = "Separate foreground and background using Kmeans with k=2."
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            taskAlg.optionsChanged = True

            src = taskAlg.pcSplit(2).Threshold(1, 255, cv.ThresholdTypes.BinaryInv).ConvertScaleAbs()
            km.Run(src)
    ...
    Public Class Foreground_Hist3D : Inherits TaskParent
        ...
        Public Sub New()
            hcloud.maskInput = taskAlg.noDepthMask
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2.SetTo(0)
            dst2 = hcloud.dst2.InRange(1, 1) Or taskAlg.noDepthMask
            dst3 = Not dst2
        End Sub
    End Class
    ...
    Public Class Foreground_CellsBack : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            runRedList(src, labels(2))
            fore.Run(src)
            dst3 = Not fore.dst2 And taskAlg.depthMask
            dst2.SetTo(0)
            For Each rc In taskAlg.redList.oldrclist
                Dim tmp As cv.Mat = dst3(rc.rect) And rc.mask
                If tmp.CountNonZero Then dst2(rc.rect).SetTo(rc.color, rc.mask)
