Namespace VBClasses
    Public Class CPUTime
        ...
        Public Sub measureStartRun(algName As String)
            ...
            ' Credit elapsed time to the parent currently on the stack (if any).
            If algorithmStack.Count > 0 Then
                Dim parentIndex = CInt(algorithmStack.Peek)
                Dim elapsedTicks = nextTime.Ticks - algorithmTimes(parentIndex).Ticks
                Dim span = New TimeSpan(elapsedTicks)
                algorithm_ms(parentIndex) += span.Ticks / TimeSpan.TicksPerMillisecond
            End If

            ' Always push so measureEndRun has a matching frame to pop.
            Dim index = algorithmNames.IndexOf(algName)
            If index < 0 Then Return
            algorithmTimes(index) = nextTime
            algorithmStack.Push(index)
        End Sub
        Public Sub measureEndRun()
            If algorithmStack.Count = 0 Then Return

            Dim nextTime = Now
            Dim index = CInt(algorithmStack.Pop())
            Dim elapsedTicks = nextTime.Ticks - algorithmTimes(index).Ticks
            Dim span = New TimeSpan(elapsedTicks)
            algorithm_ms(index) += span.Ticks / TimeSpan.TicksPerMillisecond

            ' Resume timing the parent frame when one remains.
            If algorithmStack.Count > 0 Then
                algorithmTimes(CInt(algorithmStack.Peek)) = nextTime
            End If
        End Sub
        ...
        Public Shared Function PrepareReport(treeData As List(Of String)) As String
            ...
            For i = 0 To PercentTimes.Count - 1
                If PercentTimes.ElementAt(i).Key > 0.01 Then
                    ...
                    Dim index = treeData.IndexOf(str.AsSpan(6).ToString())
                    percentTime += str + vbCrLf
                    If index >= 0 Then timeDataTree(index) = String.Concat(str.AsSpan(0, 5), " ".AsSpan(), timeDataTree(index).AsSpan())
                End If
            Next
