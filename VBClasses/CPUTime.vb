...
Namespace VBClasses
    Public Class CPUTime
        ...
        Public Function PrepareReport(treeData As List(Of String)) As String
            ...
            Dim algorithm_ms = New List(Of Single)(taskA.cpu.algorithm_ms)
            ...
            For i = 0 To algorithm_ms.Count - 1
                ...
                If i >= taskA.cpu.algorithmNames.Count Then Exit For
                Dim str = Format(algorithm_ms(i), "00.0%") + " " + taskA.cpu.algorithmNames(i)
                If taskA.cpu.displayObjectName IsNot Nothing Then
                    If taskA.cpu.displayObjectName.Length > 0 Then
                        If str.Contains(taskA.cpu.displayObjectName) Then percentStr = str
                    End If
                End If
                If taskA.cpu.algorithmNames(i).Contains("Wait For Input") Then
                    saveWaitTime = str
                Else
                    PercentTimes.Add(algorithm_ms(i), str)
                End If
            Next
            Dim paintStr = Format(taskA.cpu.paintTime, fmt1) + " ms paint time"
            taskA.cpu.paintTime = 0
            ...
            percentTime += "Algorithm FPS = " + Format(taskA.fpsAlgorithm, fmt0) + vbCrLf
            percentTime += "Camera FPS = " + Format(taskA.fpsCamera, fmt0) + vbCrLf
            percentTime += "Target Display FPS = " + CStr(taskA.Settings.FPSPaintTarget) + vbCrLf + vbCrLf
