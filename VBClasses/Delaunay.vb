...
Namespace VBClasses
    Public Class Delaunay_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If atask.heartBeat And standalone Then
    ...
    Public Class NR_Delaunay_SubDiv : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If standaloneTest() Then If Not atask.heartBeat Then Exit Sub
            ...
            For Each pt In random.PointList
                ...
                For i = 0 To edgeList.Length - 1
                    ...
                    dst2.Line(p0, p1, white, atask.lineWidth, atask.lineWidth)
                Next
            Next
            ...
            For i = 0 To facets.Length - 1
                ...
                Next
                ifacets(0) = ifacet
                dst3.FillConvexPoly(ifacet, atask.scalarColors(i Mod atask.scalarColors.Length), cv.LineTypes.Link4)
                cv.Cv2.Polylines(dst3, ifacets, True, cv.Scalar.Black, atask.lineWidth, cv.LineTypes.Link4, 0)
            Next
        End Sub
    End Class
    ...
    Public Class NR_Delaunay_Subdiv2D : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If Not atask.heartBeat Then Exit Sub ' too fast otherwise...
            dst2.SetTo(0)
            ...
            For Each p In points
                DrawCircle(dst2, p, atask.DotSize + 1, cv.Scalar.Red)
            Next
            dst3 = dst2.Clone()
            ...
            For Each edge In edgelist
                ...
                dst2.Line(p1, p2, cv.Scalar.Green, atask.lineWidth, atask.lineWidth)
            Next
        End Sub
    End Class
    ...
    Public Class Delaunay_Generations : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If standaloneTest() Then
                If atask.heartBeatLT Then random.Run(src)
                inputPoints = New List(Of cv.Point2f)(random.PointList)
            End If
    ...
    Public Class Delaunay_ConsistentColor : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst1.SetTo(0)
            For Each pt In inputPoints
                dst1.Circle(New cv.Point(pt.X, pt.Y), atask.DotSize, atask.highlight, -1, cv.LineTypes.Link4)
            Next
    ...
    Public Class Delaunay_LineSelect : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim facet = delaunay.dst1.Get(Of Byte)(atask.mouseMovePoint.Y, atask.mouseMovePoint.X)
            Dim facetIndex = facetList.IndexOf(facet)
            If facetIndex = -1 Then Exit Sub
            atask.lpD = atask.lines.lpList(ptList(facetIndex))

            Static saveID As Integer = atask.lpD.p1GridIndex
            If atask.lpD.p1GridIndex <> saveID Then
                saveID = atask.lpD.p1GridIndex
                atask.optionsChanged = True
            End If

            Dim index1 As Integer, index2 As Integer, index3 As Integer
            index1 = delaunay.dst1.Get(Of Byte)(atask.lpD.p1.Y, atask.lpD.p1.X)
            index2 = delaunay.dst1.Get(Of Byte)(atask.lpD.ptCenter.Y, atask.lpD.ptCenter.X)
            index3 = delaunay.dst1.Get(Of Byte)(atask.lpD.p2.Y, atask.lpD.p2.X)

            dst3.SetTo(0)
            dst3.FillConvexPoly(delaunay.facetList(index1), atask.lpD.color, cv.LineTypes.Link4)
            dst3.FillConvexPoly(delaunay.facetList(index2), atask.lpD.color, cv.LineTypes.Link4)
            dst3.FillConvexPoly(delaunay.facetList(index3), atask.lpD.color, cv.LineTypes.Link4)
            dst3.Line(atask.lpD.p1, atask.lpD.p2, cv.Scalar.Green, atask.lineWidth, atask.lineWidth)

            If atask.lpD Is Nothing Then atask.lpD = atask.lines.lpList(0)
            strOut = atask.lpD.displayCell(dst2)
