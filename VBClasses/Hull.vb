...
Namespace VBClasses
    Public Class Hull_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If (standaloneTest() And atask.heartBeat) Or (useRandomPoints And atask.heartBeat) Then
                random.Run(src)
                dst2.SetTo(0)
    ...
    Public Class Hull_Contour : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            contours1.Run(src)
            atask.contourD = contours1.selectContour()

            dst2.SetTo(0)
            dst2(atask.contourD.rect).SetTo(255, atask.contourD.mask)
            contours2.Run(dst2)
            ...
            If contours1.sortContours.allContours.Count > 0 Then
                If contours1.sortContours.allContours(0).Count > 0 Then
                    ...
                    DrawTour(dst3, hull, white, atask.lineWidth)
                End If
            End If
        End Sub
    End Class
