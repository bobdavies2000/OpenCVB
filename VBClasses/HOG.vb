...
Public Class HOG_Basics : Inherits TaskParent
    ...
    Public Sub New()
        ...
        cv.Cv2.Resize(Image, dst3, dst3.Size)
    End Sub
    Private Sub drawFoundRectangles(dst2 As cv.Mat, found() As cv.Rect)
        For Each rect As cv.Rect In found
            ...
            Dim r As cv.Rect = New cv.Rect With
                    ...
                    cv.Cv2.Rectangle(dst2, r.TopLeft, r.BottomRight, cv.Scalar.Red, 3, cv.LineTypes.Link8, 0)
        Next rect
    End Sub
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        If src.Height = 94 Then cv.Cv2.Resize(src, src, New cv.Size(src.Width * 2, src.Height * 2))
        ...
        If dst2.Height = 94 Then cv.Cv2.Resize(src, dst2, dst2.Size) Else src.CopyTo(dst2)
        drawFoundRectangles(dst2, found)

        If ImageProcessed = False Then
            If dst3.Height = 94 Then cv.Cv2.Resize(dst3, dst3, New cv.Size(dst3.Width * 2, dst3.Height * 2))
