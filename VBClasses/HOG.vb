...
Namespace VBClasses
    Public Class HOG_Basics : Inherits TaskParent
        ...
        Public Sub New()
            ...
            If Image Is Nothing Then Image = ImRead(task.homeDir + "Data/Asahiyama.jpg", ImreadModes.Color)
            Resize(Image, dst3, dst3.Size)
        End Sub
        Private Shared Sub drawFoundRectangles(dst2 As Mat, found() As cv.Rect)
            For Each rect As cv.Rect In found
                ...
                Rectangle(dst2, r.TopLeft, r.BottomRight, Scalar.red, 3, LineTypes.Link8, 0)
            Next rect
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If src.Height = 94 Then Resize(src, src, New Size(src.Width * 2, src.Height * 2))
            Dim found() As cv.Rect = hog.DetectMultiScale(src, options.thresholdHOG, New Size(options.strideHOG, options.strideHOG), New Size(24, 16), options.scaleHOG, 2)
            ...
            If ImageProcessed = False Then
                If dst3.Height = 94 Then Resize(dst3, dst3, New Size(dst3.Width * 2, dst3.Height * 2))
                found = hog.DetectMultiScale(dst3, options.thresholdHOG, New Size(options.strideHOG, options.strideHOG), New Size(24, 16), options.scaleHOG, 2)
