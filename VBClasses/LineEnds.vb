...
Namespace VBClasses
    Public Class LineEnds_Basics : Inherits TaskParent
        ...
        Public Sub New()
            algTask.featureOptions.MatchCorrSlider.Value = 90
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each lp In algTask.lines.lpList
                ...
                If match.p1Correlation > algTask.fCorrThreshold And match.p2Correlation > algTask.fCorrThreshold Then
    ...
    Public Class LineEnds_Concat : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim nabeRect1 = algTask.gridNabeRects(algTask.gridMap.Get(Of Integer)(lpInput.p1.Y, lpInput.p1.X))
            Dim nabeRect2 = algTask.gridNabeRects(algTask.gridMap.Get(Of Integer)(lpInput.p2.Y, lpInput.p2.X))
    ...
    Public Class LineEnds_Correlation : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If standalone Then lpInput = algTask.lineLongest
            Static lastImage = algTask.gray.Clone

            Dim rect = algTask.gridRects(lpInput.p1GridIndex)
            match.template = algTask.gray(rect)
            match.Run(lastImage(algTask.gridNabeRects(lpInput.p1GridIndex)))
            p1Correlation = match.correlation

            rect = algTask.gridRects(lpInput.p2GridIndex)
            match.template = algTask.gray(rect)
            match.Run(lastImage(algTask.gridNabeRects(lpInput.p2GridIndex)))
            p2Correlation = match.correlation

            lastImage = algTask.gray.Clone

            If standaloneTest() Then
                dst2 = src.Clone
                DrawLine(dst2, lpInput, algTask.highlight)
            End If
