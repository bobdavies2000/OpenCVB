...
Namespace VBClasses
    Public Class LineEnds_Basics : Inherits TaskParent
        ...
        Public Sub New()
            taskAlg.featureOptions.MatchCorrSlider.Value = 90
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each lp In taskAlg.lines.lpList
                ...
                If match.p1Correlation > taskAlg.fCorrThreshold And match.p2Correlation > taskAlg.fCorrThreshold Then
    ...
    Public Class LineEnds_Concat : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim nabeRect1 = taskAlg.gridNabeRects(taskAlg.gridMap.Get(Of Integer)(lpInput.p1.Y, lpInput.p1.X))
            Dim nabeRect2 = taskAlg.gridNabeRects(taskAlg.gridMap.Get(Of Integer)(lpInput.p2.Y, lpInput.p2.X))
    ...
    Public Class LineEnds_Correlation : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If standalone Then lpInput = taskAlg.lineLongest
            Static lastImage = taskAlg.gray.Clone

            Dim rect = taskAlg.gridRects(lpInput.p1GridIndex)
            match.template = taskAlg.gray(rect)
            match.Run(lastImage(taskAlg.gridNabeRects(lpInput.p1GridIndex)))
            p1Correlation = match.correlation

            rect = taskAlg.gridRects(lpInput.p2GridIndex)
            match.template = taskAlg.gray(rect)
            match.Run(lastImage(taskAlg.gridNabeRects(lpInput.p2GridIndex)))
            p2Correlation = match.correlation

            lastImage = taskAlg.gray.Clone

            If standaloneTest() Then
                dst2 = src.Clone
                DrawLine(dst2, lpInput, taskAlg.highlight)
            End If
