...
Namespace VBClasses
    ...
    Public Class NR_PCA_CellMask : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If rc.mmZ.maxVal > 0 Then
                pcaPrep.Run(atask.pointCloud(rc.rect).Clone)
    ...
    Public Class PCA_Reconstruct : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim index = atask.frameCount Mod images.Length
            ...
            If atask.frameCount >= images.Length Then
    ...
    Public Class NR_PCA_DrawImage : Inherits TaskParent
        ...
        Public Sub New()
            image = cv.Cv2.ImRead(atask.homeDir + "opencv/Samples/Data/pca_test1.jpg")
        ...
        Sub drawAxis(img As cv.Mat, p As cv.Point, q As cv.Point, color As cv.Scalar, scale As Single)
            ...
            img.Line(p, q, color, atask.lineWidth, atask.lineType)
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To contours.Length - 1
                ...
                cv.Cv2.DrawContours(dst3, contours, i, cv.Scalar.Red, atask.lineWidth, atask.lineType)
                ...
                DrawCircle(dst3, cntr, atask.DotSize + 1, cv.Scalar.BlueViolet)
...
#End Region
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If standaloneTest() Then
                ...
                labels(3) = "dst2 is palettized using global palette option: " + atask.gOptions.Palettes.Text
            End If
    ...
    Public Class PCA_NColor_CPP : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If atask.heartBeat Then palettize.Run(src) ' get the palette in VB.Net
            Marshal.Copy(src.Data, rgb, 0, rgb.Length)
    ...
    Public Class NR_PCA_NColorPalettize : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If atask.heartBeat Then palettize.Run(src) ' get the palette in VB.Net which is very fast.
    ...
    Public Class PCA_LineMask : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim lp = If(standalone, atask.lines.lpList(0), atask.lpD)
            ...
            If findLine3D.vecMat.Rows > 0 Then
                ...
                dst3.Circle(lp.ptCenter, atask.DotSize * 2, 255, -1, atask.lineType)
            End If
