...
Public Class PCA_CellMask : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        If rc.mmZ.maxVal > 0 Then
            pcaPrep.Run(algTask.pointCloud(rc.rect).Clone)
...
Public Class PCA_Reconstruct : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        Dim index = algTask.frameCount Mod images.Length
        ...
        If algTask.frameCount >= images.Length Then
...
Public Class PCA_DrawImage : Inherits TaskParent
    ...
    Public Sub New()
        image = cv.Cv2.ImRead(algTask.homeDir + "opencv/Samples/Data/pca_test1.jpg")
    ...
    Sub drawAxis(img As cv.Mat, p As cv.Point, q As cv.Point, color As cv.Scalar, scale As Single)
        ...
        img.Line(p, q, color, algTask.lineWidth, algTask.lineType)
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        For i = 0 To contours.Length - 1
            ...
            cv.Cv2.DrawContours(dst3, contours, i, cv.Scalar.Red, algTask.lineWidth, algTask.lineType)
            ...
            DrawCircle(dst3, cntr, algTask.DotSize + 1, cv.Scalar.BlueViolet)
...
#End Region
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        If standaloneTest() Then
            ...
            labels(3) = "dst2 is palettized using global palette option: " + algTask.gOptions.Palettes.Text
        End If
...
Public Class PCA_NColor_CPP : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        If algTask.heartBeat Then palettize.Run(src) ' get the palette in VB.Net
        Marshal.Copy(src.Data, rgb, 0, rgb.Length)
...
Public Class PCA_NColorPalettize : Inherits TaskParent
    ...
    End Sub
    Public Overrides Sub RunAlg(src As cv.Mat)
        If algTask.heartBeat Then palettize.Run(src) ' get the palette in VB.Net which is very fast.
...
Public Class PCA_LineMask : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        Dim lp = If(standalone, algTask.lineLongest, algTask.lpD)
        ...
        If findLine3D.vecMat.Rows > 0 Then
            ...
            dst3.Circle(lp.ptCenter, algTask.DotSize * 2, 255, -1, algTask.lineType)
        End If
