...
Namespace VBClasses
    Public Class Grid_Basics : Inherits TaskParent
        ...
        Public Sub New()
            tsk.gridMap = New cv.Mat(dst2.Size, cv.MatType.CV_32S, 0)
            tsk.gridMask = New cv.Mat(dst2.Size(), cv.MatType.CV_8U)
            desc = "Create a grid of squares covering the entire image."
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If tsk.mouseClickFlag And Not tsk.firstPass Then
                tsk.gridROIclicked = tsk.gridMap.Get(Of Single)(tsk.clickPoint.Y, tsk.clickPoint.X)
            End If

            If tsk.optionsChanged Then
                Dim bricksPerCol As Integer, bricksPerRow As Integer
                tsk.gridNabeRects.Clear()
                gridNeighbors.Clear()

                tsk.gridRects.Clear()
                Dim index As Integer
                For y = 0 To dst2.Height - 1 Step tsk.brickSize
                    For x = 0 To dst2.Width - 1 Step tsk.brickSize
                        Dim roi = ValidateRect(New cv.Rect(x, y, tsk.brickSize, tsk.brickSize))
                        ...
                        If roi.Width > 0 And roi.Height > 0 Then
                            ...
                            tsk.gridRects.Add(roi)
                            index += 1
                        End If
                    Next
                Next

                tsk.gridMask.SetTo(0)
                For x = tsk.brickSize To dst2.Width - 1 Step tsk.brickSize
                    Dim p1 = New cv.Point(x, 0), p2 = New cv.Point(x, dst2.Height)
                    tsk.gridMask.Line(p1, p2, 255, 1)
                Next
                For y = tsk.brickSize To dst2.Height - 1 Step tsk.brickSize
                    Dim p1 = New cv.Point(0, y), p2 = New cv.Point(dst2.Width, y)
                    tsk.gridMask.Line(p1, p2, 255, 1)
                Next

                For i = 0 To tsk.gridRects.Count - 1
                    tsk.gridMap.Rectangle(tsk.gridRects(i), i, -1)
                Next
                ...
                For i = 0 To tsk.gridRects.Count - 1
                    ...
                    If tsk.motionLinkType = 4 Or tsk.motionLinkType = 8 Then
                    ...
                    If tsk.motionLinkType = 8 Then
                        ...
                        If p2.X < dst2.Width And p2.Y < dst2.Height Then
                            If i + bricksPerRow + 1 < tsk.gridRects.Count Then nextList.Add(i + bricksPerRow + 1)
                        End If
                    End If
                ...
                For Each nabeList In gridNeighbors
                    ...
                    If r.Width < tsk.brickSize * 3 Then
                        If r.X + r.Width >= dst2.Width Then r.X = dst2.Width - tsk.brickSize * 3
                        r.Width = tsk.brickSize * 3
                    End If
                    If r.Height < tsk.brickSize * 3 Then
                        If r.Y + r.Height >= dst2.Height Then r.Y = dst2.Height - tsk.brickSize * 3
                        r.Height = tsk.brickSize * 3
                    End If
                    If r.Width <> tsk.brickSize * 3 Then r.Width = tsk.brickSize * 3
                    If r.Height <> tsk.brickSize * 3 Then r.Height = tsk.brickSize * 3
                    tsk.gridNabeRects.Add(r)
                Next

                tsk.brickSize = tsk.brickSize
                tsk.bricksPerCol = bricksPerCol
                tsk.bricksPerRow = bricksPerRow
            End If
            If standaloneTest() Then
                dst2 = New cv.Mat(dst2.Size(), cv.MatType.CV_8U)
                tsk.color.CopyTo(dst2)
                dst2.SetTo(white, tsk.gridMask)
                labels(2) = "Grid_Basics " + CStr(tsk.gridRects.Count) + " (" + CStr(tsk.bricksPerCol) + "X" + CStr(tsk.bricksPerRow) + ") " +
                                             CStr(tsk.brickSize) + "X" + CStr(tsk.brickSize) + " regions"
            End If
        End Sub
    End Class
    ...
    Public Class NR_Grid_BasicsTest : Inherits TaskParent
        Public Sub New()
            If standalone Then tsk.gOptions.GridSlider.Value = 16
    ...
    Public Class Grid_FPS : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim fps = tsk.fpsAlgorithm \ desiredFPS
            If fps = 0 Then fps = 1
            heartBeat = (tsk.frameCount Mod fps) = 0
            If heartBeat Then
                saveSkip = skipCount
                skipCount = 0
    ...
    Public Class NR_Grid_ValidateLocation : Inherits TaskParent
        Public Sub New()
            ...
            tsk.clickPoint = New cv.Point(msRNG.Next(0, dst2.Width), msRNG.Next(0, dst2.Height))
            desc = "Click any grid element to see its neighbors"
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            dst2 = src
            labels(2) = "Clickpoint is at (X, Y): " + CStr(tsk.clickPoint.X) + ", " + CStr(tsk.clickPoint.Y)

            SetTrueText("Click any grid entry to see its neighbors", 3)
            dst2.SetTo(white, tsk.gridMask)

            Dim grIndex As Integer = tsk.gridMap.Get(Of Integer)(tsk.clickPoint.Y, tsk.clickPoint.X)
            If tsk.gridRects(grIndex).Contains(tsk.clickPoint) Then
            ...
            End If
            dst3.SetTo(0)
            For Each index In tsk.grid.gridNeighbors(grIndex)
                Dim gr = tsk.gridRects(index)
                dst2.Rectangle(gr, white, tsk.lineWidth)
                dst3.Rectangle(gr, 255, tsk.lineWidth)
            Next
        End Sub
    End Class
    ...
    Public Class NR_Grid_MinMaxDepth : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If minMaxLocs.Count <> tsk.gridRects.Count Then ReDim minMaxLocs(tsk.gridRects.Count - 1)
            If minMaxVals.Count <> tsk.gridRects.Count Then ReDim minMaxVals(tsk.gridRects.Count - 1)
            Dim mm As mmData
            For i = 0 To minMaxLocs.Count - 1
                Dim gr = tsk.gridRects(i)
                tsk.pcSplit(2)(gr).MinMaxLoc(mm.minVal, mm.maxVal, mm.minLoc, mm.maxLoc, tsk.depthmask(gr))
            ...
            If standaloneTest() Then
                ...
                For i = 0 To minMaxLocs.Count - 1
                    Dim lp = minMaxLocs(i)
                    DrawCircle(dst2(tsk.gridRects(i)), lp.p2, tsk.DotSize, cv.Scalar.Red)
                    DrawCircle(dst2(tsk.gridRects(i)), lp.p1, tsk.DotSize, white)
                Next
                dst2.SetTo(white, tsk.gridMask)
            End If
        End Sub
    ...
    Public Class NR_Grid_TrackCenter : Inherits TaskParent
        ...
        Public Sub New()
            If standalone Then tsk.gOptions.ShowGrid.Checked = True
            desc = "Track a cell near the center of the grid"
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If match.correlation < tsk.fCorrThreshold Or tsk.gOptions.DebugCheckBox.Checked Then
                tsk.gOptions.DebugCheckBox.Checked = False
                Dim index As Integer = tsk.gridMap.Get(Of Integer)(dst2.Height / 2, dst2.Width / 2)
                Dim gr = tsk.gridRects(index)
            ...
            Dim searchRect = ValidateRect(New cv.Rect(center.X - pad, center.Y - pad, tsk.brickSize, tsk.brickSize))
            match.Run(src(searchRect))
            center = match.newCenter

            If standaloneTest() Then
                dst2 = src
                dst2.Rectangle(match.newRect, tsk.highlight, tsk.lineWidth + 1, tsk.lineType)
                DrawCircle(dst2, center, tsk.DotSize, white)

                If tsk.heartBeat Then dst3.SetTo(0)
                DrawCircle(dst3, center, tsk.DotSize, tsk.highlight)
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If tsk.optionsChanged Then
                ...
                For x = gridWidth To dst2.Width - 1 Step gridWidth
                    Dim p1 = New cv.Point(x, 0), p2 = New cv.Point(x, dst2.Height)
                    gridMask.Line(p1, p2, 255, tsk.lineWidth)
                Next
