...
Public Class XO_Gravity_HorizonRawOld : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        If standaloneTest() Then
            ...
            dst2.SetTo(0)
            DrawLine(dst2, lineGravity.p1, lineGravity.p2, algTask.highlight)
...
Public Class XO_Horizon_FindNonZero : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        If pointsMat.Rows > 0 Then
            ...
            DrawLine(dst2, algTask.lineHorizon.p1, algTask.lineHorizon.p2, 255)
        End If
        ...
        If pointsMat.Rows > 0 Then
            ...
            End If
            DrawLine(dst2, algTask.lineGravity.p1, algTask.lineGravity.p2, 255)
        End If
    End Sub
End Class
...
Public Class XO_Horizon_Perpendicular : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        dst2 = src
        DrawLine(dst2, algTask.lineHorizon.p1, algTask.lineHorizon.p2, white)
...
Public Class XO_Gravity_Basics : Inherits TaskParent
    ...
    Public Sub displayResults(p1 As cv.Point, p2 As cv.Point)
        ...
        DrawLine(dst2, algTask.lineGravity.p1, algTask.lineGravity.p2, white)
        DrawLine(dst3, algTask.lineGravity.p1, algTask.lineGravity.p2, white)
    End Sub
    Public Overrides Sub RunAlg(src As cv.Mat)
...
Public Class XO_Rotate_Horizon : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        DrawLine(dst2, lineHorizon.p1, lineHorizon.p2, algTask.highlight)
        DrawLine(dst2, algTask.lineHorizon.p1, algTask.lineHorizon.p2, white)
            ...
            For j = 0 To 1
                ...
                If Math.Abs(mp.slope - lp.slope) < tolerance Then
                    ...
                    If lpNew IsNot Nothing Then
                        addList.Add(lpNew)
                        DrawLine(dst2, lpNew.p1, lpNew.p2, algTask.highlight)
...
Public Class XO_KNN_ClosestVertical : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        dst3 = lines.dst3
        DrawLine(dst2, knn.lastP1, knn.lastP2, algTask.highlight)
    End Sub
End Class
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        For Each lp In algTask.lines.lpList
            ...
            If Math.Abs(d1 - d2) <= pixelDiff Then
                DrawLine(dst2, lp.p1, lp.p2, algTask.highlight)
            End If
        Next
...
Public Class XO_Sides_Basics : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        For i = 0 To corners.Count - 1
            ...
            DrawLine(dst3, algTask.oldrcD.maxDist, corners(i), white)
...
Public Class XO_TrackLine_Basics : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        dst3.SetTo(0)
        For Each lp In algTask.lines.lpList
            DrawLine(dst3, lp.p1, lp.p2, white, algTask.lineWidth)
            Dim center = New cv.Point(CInt((lp.p1.X + lp.p2.X) / 2), CInt((lp.p1.Y + lp.p2.Y) / 2))
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        For i = 0 To fGrid.stable.basics.facetGen.facet.facetList.Count - 1
            ...
            If absDiff < fGrid.threshold Then
                hist(CInt(absDiff)) += 1
                DrawLine(dst3, fGrid.anchor, pt, algTask.highlight)
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        For i = 0 To stable.basics.facetGen.facet.facetList.Count - 1
            ...
            If absDiff < threshold Or threshold = 0 Then
                ...
                DrawLine(dst3, anchor, pt, algTask.highlight)
...
Public Class XO_FPoly_Line : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        For i = 0 To pts.Count - 2
            DrawLine(dst2, pts(i), pts(i + 1), algTask.highlight)
        ...
        If distances.Count Then
            ...
            dst3 = src
            DrawLine(dst3, lp.p1, lp.p2, algTask.highlight)
        End If
    End Sub
End Class
...
Public Class XO_Line_ExtendLineTest : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        If algTask.heartBeat Then
            ...
            Dim lp = New lpData(p1, p2)
            dst2 = src
            DrawLine(dst2, lp.pE1, lp.pE2, algTask.highlight)
...
Public Class XO_Line_ExtendAll : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        lpList.Clear()
        For Each lp In algTask.lines.lpList
            DrawLine(dst3, lp.pE1, lp.pE2, algTask.highlight)
        ...
        For Each lp In algTask.lines.lpList
            If Math.Abs(lp.angle) > 45 And Math.Sign(algTask.lineGravity.slope) = Math.Sign(lp.slope) Then
                ...
                If Math.Abs(gravityDelta - delta) < algTask.gravityBasics.options.pixelThreshold Then
                    ...
                    If standalone Then DrawLine(dst3, lp.p1, lp.p2, algTask.highlight)
                End If
            End If
        Next
                ...
                If distance1 < distanceMid * 2 And distance2 < distanceMid * 2 Then
                    ...
                    If checkList.Contains(cp.p1) = False And checkList.Contains(cp.p2) = False And checkList.Contains(cp.p3) = False And checkList.Contains(cp.p4) = False Then
                        If (cp.p1 = cp.p3 Or cp.p1 = cp.p4) And (cp.p2 = cp.p3 Or cp.p2 = cp.p4) Then
                            ' duplicate points...
                        Else
                            DrawLine(dst2, cp.p1, cp.p2, algTask.highlight)
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        For Each lp In algTask.lines.lpList
            If Math.Abs(lp.angle - degrees) < algTask.angleThreshold Then
                DrawLine(dst2, lp.p1, lp.p2, algTask.highlight, algTask.lineWidth * 2)
                count += 1
            Else
                DrawLine(dst2, lp, algTask.highlight)
            End If
        Next
...
Public Class XO_Line_LeftRightMatch : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        DrawLine(dst3, lpOutput.p1, lpOutput.p2, algTask.highlight, algTask.lineWidth + 1)
        DrawLine(dst2, lp.p1, lp.p2, algTask.highlight, algTask.lineWidth + 1)
    End Sub
End Class
...
Public Class XO_Line_LeftRightMatch3 : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        For i = 0 To lplist.Count - 1
            ...
            lpOutput.Add(lp)
            DrawLine(dst3, lpOut.p1, lpOut.p2, algTask.highlight, algTask.lineWidth + 1)
            DrawLine(dst2, lp.p1, lp.p2, algTask.highlight, algTask.lineWidth + 1)
        Next
...
Public Class XO_Gravity_Basics2 : Inherits TaskParent
    ...
    Public Shared Sub showVectors(dst As cv.Mat)
        ...
        If algTask.lineLongest IsNot Nothing Then
            ...
            DrawLine(dst, algTask.lineLongest.pE1, algTask.lineLongest.pE2, white)
        End If
    End Sub
    Public Overrides Sub RunAlg(src As cv.Mat)
        options.Run()
        ...
        If standaloneTest() Then
            ...
            dst3.SetTo(0)
            For Each lp In algTask.lines.lpList
                If Math.Abs(algTask.lineGravity.angle - lp.angle) < algTask.angleThreshold Then DrawLine(dst3, lp, white)
            Next
            labels(3) = algTask.lines.labels(3)
        End If
    End Sub
...
Public Class XO_Gravity_BasicsKalman : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        If standaloneTest() Then
            dst2.SetTo(0)
            DrawLine(dst2, algTask.lineGravity.p1, algTask.lineGravity.p2, algTask.highlight)
            DrawLine(dst2, algTask.lineHorizon.p1, algTask.lineHorizon.p2, cv.Scalar.Red)
        End If
    End Sub
End Class
...
Public Class XO_Line_Trace : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        DrawLine(dst2, algTask.lineLongest, algTask.highlight)
...
Public Class XO_Line_TraceCenter : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        DrawLine(dst2, linePerp, algTask.highlight)
...
Public Class XO_KNN_ClosestLine : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        DrawLine(dst2, lastP1, lastP2, algTask.highlight)
        trainInput.Clear()
    End Sub
End Class
...
Public Class XO_MatchLine_Gravity : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        matchLine.Run(src)
        dst2 = matchLine.dst2
        DrawLine(dst2, algTask.lineGravity.p1, algTask.lineGravity.p2, cv.Scalar.Red)
