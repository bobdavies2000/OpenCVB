    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        If standaloneTest() Then
            ...
            For x = 1 To src.Height - 1
                ...
                cv.Cv2.Line(dst3, New cv.Point2f(x - 1, y1), New cv.Point2f(x, y2), white, task.lineWidth, task.lineType)
            Next
        End If
    End Sub
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        For i = 0 To options.sampleCount - 1
            ...
            If y > inputFunction(x) Then
                responses.Add(1)
                cv.Cv2.Circle(dst2, New cv.Point(x, y), 2, cv.Scalar.Red, -1, task.lineType)
            Else
                responses.Add(-1)
                cv.Cv2.Circle(dst2, New cv.Point(x, y), 3, cv.Scalar.GreenYellow, -1, task.lineType)
            End If
        Next
    End Sub
End Class
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        For y = 0 To dst2.Height - 1 Step options.granularity
            For x = 0 To dst2.Width - 1 Step options.granularity
                ...
                cv.Cv2.Circle(dst3, New cv.Point(CInt(x), CInt(y)), task.DotSize + 1, color, -1, task.lineType)
            Next
        Next
        ...
        For i = 0 To trainMat.Rows - 1
            ...
            cv.Cv2.Circle(dst2, pt, task.DotSize + 2, color, -1, task.lineType)
            cv.Cv2.Circle(dst3, pt, task.DotSize + 2, color, -1, task.lineType)
        Next
    End Sub
    Protected Overrides Sub Finalize()
        If svm IsNot Nothing Then svm.Dispose()
...
Public Class XR_SVM_ReuseBasics : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        For i = 0 To svm.points.Count - 1
            ...
            cv.Cv2.Circle(dst2, svm.points(i), task.DotSize, color, -1, task.lineType)
            cv.Cv2.Circle(dst3, svm.points(i), task.DotSize, color, -1, task.lineType)
        Next
    End Sub
End Class
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        If task.heartBeat Then
            ...
            For i = 0 To svm.options.sampleCount - 1
                ...
                cv.Cv2.Circle(dst2, pt, task.DotSize, If(res = 1, cv.Scalar.Blue, cv.Scalar.Green), -1, task.lineType)
            Next
        ...
        cv.Cv2.Rectangle(dst2, rect, cv.Scalar.Black, 2)
cv.Cv2.Rectangle(dst3, rect, cv.Scalar.Black, 2)
        End If
    End Sub
End Class
