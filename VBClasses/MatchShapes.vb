...
Namespace VBClasses
    Public Class MatchShapes_Basics : Inherits TaskParent
        ...
        Public Sub New()
            ...
            dst0 = cv.Cv2.ImRead(taskA.homeDir + "Data/star1.png", cv.ImreadModes.Color).CvtColor(cv.ColorConversionCodes.BGR2GRAY)
            dst1 = cv.Cv2.ImRead(taskA.homeDir + "Data/star2.png", cv.ImreadModes.Color).CvtColor(cv.ColorConversionCodes.BGR2GRAY)
        ...
        Public Function findBiggestHull(hull As cv.Point()(), maxLen As Integer, maxIndex As Integer, dst As cv.Mat) As Integer
            ...
            For Each p In hull(maxIndex)
                DrawCircle(dst, p, taskA.DotSize, cv.Scalar.Yellow)
            Next
            Return maxIndex
        End Function
    ...
    Public Class MatchShapes_Nearby : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskA.heartBeat And myStandalone Then dst3.SetTo(0)
            similarCells.Clear()

            If taskA.gOptions.displayDst0.Checked Then
                dst0 = taskA.color.Clone
                DrawTour(dst0(rc.rect), rc.contour, taskA.highlight)
            End If
