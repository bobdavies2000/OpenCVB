...
Namespace VBClasses
    Public Class MatchShapes_Basics : Inherits TaskParent
        ...
        Public Sub New()
            ...
            dst0 = cv.Cv2.ImRead(taskAlg.homeDir + "Data/star1.png", cv.ImreadModes.Color).CvtColor(cv.ColorConversionCodes.BGR2GRAY)
            dst1 = cv.Cv2.ImRead(taskAlg.homeDir + "Data/star2.png", cv.ImreadModes.Color).CvtColor(cv.ColorConversionCodes.BGR2GRAY)
        ...
        Public Function findBiggestHull(hull As cv.Point()(), maxLen As Integer, maxIndex As Integer, dst As cv.Mat) As Integer
            ...
            For Each p In hull(maxIndex)
                DrawCircle(dst, p, taskAlg.DotSize, cv.Scalar.Yellow)
            Next
            Return maxIndex
        End Function
    ...
    Public Class MatchShapes_NearbyHull : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each rc2 In taskAlg.redList.oldrclist
    ...
    Public Class MatchShapes_Nearby : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If myStandalone Then
                dst2 = runRedList(taskAlg.color, labels(2)).Clone
                If taskAlg.redList.oldrclist.Count = 0 Then Exit Sub
                addTour.oldrclist = New List(Of oldrcData)(taskAlg.redList.oldrclist)
                addTour.Run(src)
                rc = taskAlg.oldrcD
            End If

            If taskAlg.heartBeat And myStandalone Then dst3.SetTo(0)
            similarCells.Clear()

            If taskAlg.gOptions.displayDst0.Checked Then
                dst0 = taskAlg.color.Clone
                DrawTour(dst0(rc.rect), rc.contour, taskAlg.highlight)
            End If
