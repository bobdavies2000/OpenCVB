...
Namespace VBClasses
    Public Class NR_Quad_RightView : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each rect In taskA.gridRects
                cv.Cv2.MeanStdDev(taskA.rightView(rect), colorMean, colorStdev)
    ...
    Public Class NR_Quad_MinMax : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If taskA.optionsChanged Then
                ...
                For i = 0 To taskA.gridRects.Count
            ...
            Dim depth32f As cv.Mat = taskA.pcSplit(2) * 1000, depth32s As New cv.Mat
            ...
            For i = 0 To taskA.gridRects.Count - 1
                ...
                Dim index = taskA.redList.rcMap.Get(Of Byte)(center.Y, center.X)

                If index <= 0 Or index >= taskA.redList.oldrclist.Count Then
                    depthList1(i).Clear()
                    depthList2(i).Clear()
                    colorList(i) = black
                    Continue For
                ...
                Dim mm = GetMinMax(depth32s(gr), taskA.depthmask(gr))
                ...
                For j = 0 To depthCount - 1
                    ...
                    Dim topLeft = Cloud_Basics.worldCoordinates(New cv.Point3f(gr.X, gr.Y, depth))
                    Dim botRight = Cloud_Basics.worldCoordinates(New cv.Point3f(gr.X + gr.Width, gr.Y + gr.Height, depth))
                ...
                Next
                SetTrueText(Format(d1, fmt1) + vbCrLf + Format(d2, fmt1), New cv.Point(gr.X, gr.Y), 3)
    ...
    Public Class NR_Quad_Bricks : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To taskA.gridRects.Count - 1
                Dim gr = taskA.gridRects(i)
                Dim center = New cv.Point(gr.X + gr.Width / 2, gr.Y + gr.Height / 2)
                Dim index = taskA.redList.rcMap.Get(Of Byte)(center.Y, center.X)
                Dim depthMin As Single = 0, depthMax As Single = 0, minLoc As cv.Point, maxLoc As cv.Point
                If index >= 0 And taskA.redList.oldrclist.Count > 0 Then
                    taskA.pcSplit(2)(gr).MinMaxLoc(depthMin, depthMax, minLoc, maxLoc, taskA.depthmask(gr))
                    Dim rc = taskA.redList.oldrclist(index)
                    depthMin = If(depthMax > rc.depth, rc.depth, depthMin)

                    If depthMin > 0 And depthMax > 0 And depthMax < taskA.MaxZmeters Then
                        ...
                        For j = 0 To 4 - 1
                            Dim x = Choose(j + 1, gr.X, gr.X + gr.Width, gr.X + gr.Width, gr.X)
                            Dim y = Choose(j + 1, gr.Y, gr.Y, gr.Y + gr.Height, gr.Y + gr.Height)
                        ...
                        SetTrueText(Format(depthMin, fmt1) + vbCrLf + Format(depthMax, fmt1), New cv.Point(gr.X, gr.Y))
                ...
                End If
            Next
            labels(2) = traceName + " completed: " + Format(taskA.gridRects.Count, fmt0) + " gr's produced " +
    ...
    Public Class NR_Quad_Boundaries : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2 = taskA.bricks.dst2.Clone
            Dim width = dst2.Width / taskA.brickSize
            Dim height = dst2.Height / taskA.brickSize
            For i = 0 To taskA.bricks.brickList.Count - width Step width
                For j = i + 1 To i + width - 1
                    Dim d1 = taskA.bricks.brickList(j).depth
                    Dim d2 = taskA.bricks.brickList(j - 1).depth
                    If Math.Abs(d1 - d2) > taskA.depthDiffMeters Then
                        dst2.Rectangle(taskA.bricks.brickList(j).rect, taskA.highlight, -1)
                    End If
                Next
            Next
            ...
            For i = 0 To width - 1
                For j = 1 To height - 1
                    Dim d1 = taskA.bricks.brickList(j * width).depth
                    Dim d2 = taskA.bricks.brickList((j - 1) * width).depth
                    If Math.Abs(d1 - d2) > taskA.depthDiffMeters Then
                        dst2.Rectangle(taskA.bricks.brickList(j).rect, taskA.highlight, -1)
                    End If
                Next
