...
Public Class Quad_RightView : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        For Each rect In algTask.gridRects
            cv.Cv2.MeanStdDev(algTask.rightView(rect), colorMean, colorStdev)
...
Public Class Quad_MinMax : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        If algTask.optionsChanged Then
            ...
            For i = 0 To algTask.gridRects.Count
        ...
        Dim depth32f As cv.Mat = algTask.pcSplit(2) * 1000, depth32s As New cv.Mat
        ...
        For i = 0 To algTask.gridRects.Count - 1
            ...
            Dim index = algTask.redList.rcMap.Get(Of Byte)(center.Y, center.X)

            If index <= 0 Or index >= algTask.redList.oldrclist.Count Then
                depthList1(i).Clear()
                depthList2(i).Clear()
                colorList(i) = black
                Continue For
            ...
            Dim mm = GetMinMax(depth32s(roi), algTask.depthMask(roi))
...
Public Class Quad_Bricks : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        For i = 0 To algTask.gridRects.Count - 1
            ...
            If index >= 0 And algTask.redList.oldrclist.Count > 0 Then
                algTask.pcSplit(2)(roi).MinMaxLoc(depthMin, depthMax, minLoc, maxLoc, algTask.depthMask(roi))
                Dim rc = algTask.redList.oldrclist(index)
                depthMin = If(depthMax > rc.depth, rc.depth, depthMin)

                If depthMin > 0 And depthMax > 0 And depthMax < algTask.MaxZmeters Then
            ...
            End If
        Next
        labels(2) = traceName + " completed: " + Format(algTask.gridRects.Count, fmt0) + " ROI's produced " +
...
Public Class Quad_Boundaries : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        dst2 = algTask.bricks.dst2.Clone
        Dim width = dst2.Width / algTask.brickSize
        Dim height = dst2.Height / algTask.brickSize
        For i = 0 To algTask.bricks.brickList.Count - width Step width
            For j = i + 1 To i + width - 1
                Dim d1 = algTask.bricks.brickList(j).depth
                Dim d2 = algTask.bricks.brickList(j - 1).depth
                If Math.Abs(d1 - d2) > algTask.depthDiffMeters Then
                    dst2.Rectangle(algTask.bricks.brickList(j).rect, algTask.highlight, -1)
                End If
            Next
        Next
        ...
        For i = 0 To width - 1
            For j = 1 To height - 1
                Dim d1 = algTask.bricks.brickList(j * width).depth
                Dim d2 = algTask.bricks.brickList((j - 1) * width).depth
                If Math.Abs(d1 - d2) > algTask.depthDiffMeters Then
                    dst2.Rectangle(algTask.bricks.brickList(j).rect, algTask.highlight, -1)
                End If
            Next
