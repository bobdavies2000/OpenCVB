...
Namespace VBClasses
    Public Class NR_Quad_RightView : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each rect In tsk.gridRects
                cv.Cv2.MeanStdDev(tsk.rightView(rect), colorMean, colorStdev)
    ...
    Public Class NR_Quad_MinMax : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If tsk.optionsChanged Then
                ...
                For i = 0 To tsk.gridRects.Count
            ...
            Dim depth32f As cv.Mat = tsk.pcSplit(2) * 1000, depth32s As New cv.Mat
            ...
            For i = 0 To tsk.gridRects.Count - 1
                ...
                Dim index = tsk.redList.rcMap.Get(Of Byte)(center.Y, center.X)

                If index <= 0 Or index >= tsk.redList.oldrclist.Count Then
                    depthList1(i).Clear()
                    depthList2(i).Clear()
                    colorList(i) = black
                    Continue For
                ...
                Dim mm = GetMinMax(depth32s(gr), tsk.depthmask(gr))
    ...
    Public Class NR_Quad_Bricks : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To tsk.gridRects.Count - 1
                ...
                If index >= 0 And tsk.redList.oldrclist.Count > 0 Then
                    tsk.pcSplit(2)(gr).MinMaxLoc(depthMin, depthMax, minLoc, maxLoc, tsk.depthmask(gr))
                    Dim rc = tsk.redList.oldrclist(index)
                    depthMin = If(depthMax > rc.depth, rc.depth, depthMin)

                    If depthMin > 0 And depthMax > 0 And depthMax < tsk.MaxZmeters Then
                ...
                End If
            Next
            labels(2) = traceName + " completed: " + Format(tsk.gridRects.Count, fmt0) + " gr's produced " +
    ...
    Public Class NR_Quad_Boundaries : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2 = tsk.bricks.dst2.Clone
            Dim width = dst2.Width / tsk.brickSize
            Dim height = dst2.Height / tsk.brickSize
            For i = 0 To tsk.bricks.brickList.Count - width Step width
                For j = i + 1 To i + width - 1
                    Dim d1 = tsk.bricks.brickList(j).depth
                    Dim d2 = tsk.bricks.brickList(j - 1).depth
                    If Math.Abs(d1 - d2) > tsk.depthDiffMeters Then
                        dst2.Rectangle(tsk.bricks.brickList(j).rect, tsk.highlight, -1)
                    End If
                Next
            Next
            ...
            For i = 0 To width - 1
                For j = 1 To height - 1
                    Dim d1 = tsk.bricks.brickList(j * width).depth
                    Dim d2 = tsk.bricks.brickList((j - 1) * width).depth
                    If Math.Abs(d1 - d2) > tsk.depthDiffMeters Then
                        dst2.Rectangle(tsk.bricks.brickList(j).rect, tsk.highlight, -1)
                    End If
                Next
