    ...
    Public Class XR_Quad_MinMax : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To task.gridRects.Count - 1
                ...
                Dim index = redc.rcIndexMap.Get(Of Single)(center.Y, center.X)
    ...
    Public Class Quad_Hulls : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To task.gridRects.Count - 1
                ...
                Dim center = New cv.Point(CInt(gRect.X + gRect.Width / 2), CInt(gRect.Y + gRect.Height / 2))
                Dim index = hulls.rcIndexMap.Get(Of Single)(center.Y, center.X)
                ...
                If colorList(i) <> task.scalarColors(rc.index Mod 255) Then depthList(i).Clear()
                ...
                If depthList(i).Count > 0 Then
                    ...
                    quadData.Add(New Point3f(colorList(i)(0), colorList(i)(1), colorList(i)(2)))
                    quadData.Add(New Point3f(topLeft.X + shift.X, topLeft.Y + shift.Y, depth + shift.Z))
                    quadData.Add(New Point3f(botRight.X + shift.X, topLeft.Y + shift.Y, depth + shift.Z))
                    quadData.Add(New Point3f(botRight.X + shift.X, botRight.Y + shift.Y, depth + shift.Z))
                    quadData.Add(New Point3f(topLeft.X + shift.X, botRight.Y + shift.Y, depth + shift.Z))
            ...
            For i = 0 To bricks.brickList.Count - width Step width
                For j = i + 1 To i + width - 1
                    ...
                    If Math.Abs(brick1 - brick2) > task.depthDiffMeters Then
                        Rectangle(dst2, bricks.brickList(j).rect, task.highlight, -1)
                    End If
                Next
            Next
