    ...
    Public Class NR_Entropy_SubDivisions : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To task.gridRects.Count - 1
                ...
                cv.Cv2.CalcHist({dst1(gRect)}, {0}, New cv.Mat(), hist, 1, dimensions, ranges)
                hist = hist.Normalize(0, hist.Rows, cv.NormTypes.MinMax)

                Dim nextEntropy = entropy.channelEntropy(dst1(gRect).Total, hist) * 1000

                entropies(subDivisions(i)).Add(nextEntropy)
                eROI(subDivisions(i)).Add(gRect)
            Next
