...
Namespace VBClasses
    Public Class Entropy_Basics : Inherits TaskParent
        ...
        Private Function validatePreserve(ByVal r As cv.Rect) As cv.Rect
            ...
            If r.X < 0 Then r.X = 0
            If r.Y < 0 Then r.Y = 0
            If r.X + r.Width >= taskAlg.workRes.Width Then r.X = taskAlg.workRes.Width - r.Width - 1
            If r.Y + r.Height >= taskAlg.workRes.Height Then r.Y = taskAlg.workRes.Height - r.Height - 1
            Return r
        End Function
        Public Overrides Sub RunAlg(src As cv.Mat)
            Dim stdSize = 30
            If taskAlg.drawRect = New cv.Rect Then
                taskAlg.drawRect = New cv.Rect(30, 30, stdSize, stdSize) ' arbitrary rectangle
            End If
            If taskAlg.mouseClickFlag Then
                taskAlg.drawRect = validatePreserve(New cv.Rect(taskAlg.clickPoint.X, taskAlg.clickPoint.Y, stdSize, stdSize))
            End If
            taskAlg.drawRect = ValidateRect(taskAlg.drawRect)
            If src.Channels() = 3 Then
                entropy.Run(src.CvtColor(cv.ColorConversionCodes.BGR2GRAY)(taskAlg.drawRect))
            Else
                entropy.Run(src(taskAlg.drawRect))
            End If
            dst2 = entropy.dst2
            dst2.Rectangle(taskAlg.drawRect, white, taskAlg.lineWidth)
            If taskAlg.heartBeat Then strOut = "Click anywhere to measure the entropy with rect(pt.x, pt.y, " +
    ...
    Public Class Entropy_Highest : Inherits TaskParent
        ...
        Public Sub New()
            ...
            If standalone Then
                ...
                If taskAlg.gOptions.GridSlider.Maximum < val Then taskAlg.gOptions.GridSlider.Maximum = val
                taskAlg.gOptions.GridSlider.Value = dst2.Width \ 10
            End If
            desc = "Find the highest entropy section of the color image."
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            Dim entropyList(taskAlg.gridRects.Count - 1) As Single
            ...
            For Each roi In taskAlg.gridRects
                If roi.Width = roi.Height Then
                    ...
                    If entropy.entropyVal > maxEntropy Or taskAlg.optionsChanged Then
                        maxEntropy = entropy.entropyVal
                        eMaxRect = roi
                    End If
                    If entropy.entropyVal < minEntropy Then minEntropy = entropy.entropyVal
                    If standaloneTest() And taskAlg.brickSize > 16 Then
            ...
            If standaloneTest() Then
                dst2.Rectangle(eMaxRect, 255, taskAlg.lineWidth)
                dst3.SetTo(0)
                dst3.Rectangle(eMaxRect, white, taskAlg.lineWidth)
            End If
    ...
    Public Class Entropy_FAST : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2 = entropy.dst2
            dst3 = entropy.dst2
            dst3.Rectangle(entropy.eMaxRect, taskAlg.highlight, taskAlg.lineWidth)
        End Sub
    End Class
    ...
    Public Class Entropy_Rectangle : Inherits TaskParent
        ...
        End Function
        Public Overrides Sub RunAlg(src As cv.Mat)
            Dim dimensions() = New Integer() {taskAlg.histogramBins}
            ...
            If standalone Then
                If taskAlg.drawRect.Width = 0 Or taskAlg.drawRect.Height = 0 Then
                    taskAlg.drawRect = New cv.Rect(10, 10, 50, 50) ' arbitrary template to match
                End If
                src = src(taskAlg.drawRect)
            End If
    ...
    Public Class Entropy_SubDivisions : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim str = If(taskAlg.toggleOn, "minimum", "maximum")
            labels(3) = "The " + str + " entropy values in each subdivision"
            For i = 0 To entropies.Count - 1
                Dim val = If(taskAlg.toggleOn, entropies(i).Min, entropies(i).Max)
