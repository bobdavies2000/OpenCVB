...
Namespace VBClasses
    ...
    Public Class NR_FitLine_Lines : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If Not atask.heartBeatLT Then Exit Sub
            options.Run()
    ...
    Public Class NR_FitLine_Simple3D : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If standalone Then
                ...
                If atask.firstPass Then OptionParent.FindSlider("Random Pixel Count").Value = 200
                random.Run(src)
                dst2.SetTo(0)
                ptList.Clear()
            ...
            dst2.Circle(center, atask.DotSize + 2, cv.Scalar.Blue, -1)
        End Sub
    End Class
    ...
    Public Class NR_FitLine_Example2D : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If standalone And atask.heartBeat = False Then Exit Sub
            noisyLine.Run(src)
            dst2 = noisyLine.dst2
            ...
            dst2.Circle(fitLine.center, atask.DotSize + 2, cv.Scalar.Blue, -1)
        End Sub
    End Class
    ...
    Public Class NR_FitLine_Basics3D : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If standalone Then
                ...
                ptList.Clear()
                For Each pt In noisyLine.PointList
                    DrawCircle(dst2, New cv.Point2f(pt.X, pt.Y), atask.DotSize, atask.highlight)
                    ptList.Add(pt)
                Next
            End If
            ...
            dst2.Circle(New cv.Point2f(line.X1, line.Y1), atask.DotSize + 2, cv.Scalar.Blue, -1)
        End Sub
    End Class
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For Each rect In atask.gridRects
                If dst2(rect).CountNonZero >= 5 Then
                    ...
                    dst3(rect).Line(fitline.lp.p1, fitline.lp.p2, 255, atask.lineWidth, atask.lineType)
                End If
            Next
        End Sub
    End Class
