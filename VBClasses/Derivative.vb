...
Namespace VBClasses
    Public Class Derivative_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            subD.Run(tsk.pcSplit(2))

            Dim ranges = {New cv.Rangef(-subD.options.mmThreshold - 0.00001, subD.options.mmThreshold + 0.00001)}
            Dim histogram As New cv.Mat
            cv.Cv2.CalcHist({subD.dst2}, {0}, New cv.Mat, histogram, 1, {tsk.histogramBins}, ranges)
            ...
            Dim histIndex = Math.Truncate(tsk.mouseMovePoint.X / brickWidth)

            Dim index As Integer = 1
            Dim bars = subD.options.histBars
            Dim center As Integer = tsk.histogramBins / 2
            Dim centerAdjust As Integer = If(tsk.histogramBins Mod 2 = 0, 1, 0)
    ...
    Public Class Derivative_Sobel : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If src.Type <> cv.MatType.CV_32F Then src = tsk.pcSplit(options.channel)
            ...
            Dim histogram As New cv.Mat
            cv.Cv2.CalcHist({src}, {0}, tsk.depthMask, histogram, 1, {tsk.histogramBins}, ranges)
    ...
    Public Class NR_Derivative_Sobel1 : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            src = tsk.pcSplit(channel).Sobel(cv.MatType.CV_32F, 1, 0, kern)
            deriv.Run(src)
            ...
            src = tsk.pcSplit(channel).Sobel(cv.MatType.CV_32F, 0, 1, kern)
            deriv.Run(src)
            dst2 = deriv.dst2
            dst3 = deriv.dst3
    ...
    Public Class NR_Derivative_Laplacian : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst1 = tsk.pcSplit(channel).GaussianBlur(gausskern, 0, 0)
    ...
    Public Class NR_Derivative_Classes : Inherits TaskParent
        ...
        Private Function derivClassCount(ByRef dst As cv.Mat) As Integer
            ...
            dst.SetTo(0, tsk.noDepthMask)
            Return derivClassCount
        End Function
        Public Overrides Sub RunAlg(src As cv.Mat)
            deriv.Run(tsk.pcSplit(deriv.options.channel).Sobel(cv.MatType.CV_32F, 1, 0, deriv.options.kernelSize))
            classCountX = derivClassCount(dst2)
            labels(2) = $"Backprojection of X dimension of tsk.pcSplit({deriv.options.channel})"

            deriv.Run(tsk.pcSplit(deriv.options.channel).Sobel(cv.MatType.CV_32F, 0, 1, deriv.options.kernelSize))
            classCountY = derivClassCount(dst3)
            labels(3) = $"Backprojection of Y dimension of tsk.pcSplit({deriv.options.channel})"
        End Sub
    End Class
End Namespace