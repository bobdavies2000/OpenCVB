...
Namespace VBClasses
    Public Class Derivative_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            subD.Run(taskA.pcSplit(2))

            Dim ranges = {New cv.Rangef(-subD.options.mmThreshold - 0.00001, subD.options.mmThreshold + 0.00001)}
            Dim histogram As New cv.Mat
            cv.Cv2.CalcHist({subD.dst2}, {0}, New cv.Mat, histogram, 1, {taskA.histogramBins}, ranges)
            ...
            Dim histIndex = Math.Truncate(taskA.mouseMovePoint.X / brickWidth)

            Dim index As Integer = 1
            Dim bars = subD.options.histBars
            Dim center As Integer = taskA.histogramBins / 2
            Dim centerAdjust As Integer = If(taskA.histogramBins Mod 2 = 0, 1, 0)
            ...
            dst2.Rectangle(New cv.Rect(CInt((center - bars) * brickWidth), 0,
                           ...
                           taskA.highlight, taskA.lineWidth)
    ...
    Public Class Derivative_Sobel : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If src.Type <> cv.MatType.CV_32F Then src = taskA.pcSplit(options.channel)
            ...
            Dim histogram As New cv.Mat
            cv.Cv2.CalcHist({src}, {0}, taskA.depthMask, histogram, 1, {taskA.histogramBins}, ranges)
    ...
    Public Class NR_Derivative_Sobel1 : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            src = taskA.pcSplit(channel).Sobel(cv.MatType.CV_32F, 1, 0, kern)
            deriv.Run(src)
            ...
            src = taskA.pcSplit(channel).Sobel(cv.MatType.CV_32F, 0, 1, kern)
            deriv.Run(src)
            dst2 = deriv.dst2
            dst3 = deriv.dst3
    ...
    Public Class NR_Derivative_Laplacian : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst1 = taskA.pcSplit(channel).GaussianBlur(gausskern, 0, 0)
    ...
    Public Class NR_Derivative_Classes : Inherits TaskParent
        ...
        Private Function derivClassCount(ByRef dst As cv.Mat) As Integer
            ...
            dst.SetTo(0, taskA.noDepthMask)
            Return derivClassCount
        End Function
        Public Overrides Sub RunAlg(src As cv.Mat)
            deriv.Run(taskA.pcSplit(deriv.options.channel).Sobel(cv.MatType.CV_32F, 1, 0, deriv.options.kernelSize))
            classCountX = derivClassCount(dst2)
            labels(2) = $"Backprojection of X dimension of taskA.pcSplit({deriv.options.channel})"

            deriv.Run(taskA.pcSplit(deriv.options.channel).Sobel(cv.MatType.CV_32F, 0, 1, deriv.options.kernelSize))
            classCountY = derivClassCount(dst3)
            labels(3) = $"Backprojection of Y dimension of taskA.pcSplit({deriv.options.channel})"
        End Sub
    End Class
End Namespace