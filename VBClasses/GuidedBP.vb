...
Namespace VBClasses
        ...
        Private Sub runKNN(knn As KNN_Basics, rectList As List(Of cv.Rect), dst As cv.Mat, dstindex As Integer)
            ...
            For i = 0 To knn.queries.Count - 1
                ...
                If dist < r.Width / 2 And dist < r.Height / 2 Then
                    dst.Rectangle(r, white, atask.lineWidth)
    ...
    Public Class GuidedBP_Depth : Inherits TaskParent
        ...
        Public Sub New()
            atask.gOptions.setHistogramBins(16)
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.CalcBackProject({src}, atask.channels, hist.histogram, dst2, atask.ranges)
            dst2.ConvertTo(dst2, cv.MatType.CV_8U)

            labels(3) = "Use atask.gOptions.PointCloudReduction to select different cloud combinations."
            If standaloneTest() Then dst3 = PaletteFull(dst2 + 1)

            Dim depthCount = atask.depthmask.CountNonZero
    ...
    Public Class GuidedBP_HotPoints : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If atask.heartBeat Then labels(2) = "Top " + CStr(topRects.Count) + " objects identified in the top view."
            If atask.heartBeat Then labels(3) = "Top " + CStr(sideRects.Count) + " objects identified in the side view."
        End Sub
    End Class
    ...
    Public Class GuidedBP_MultiSlice : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For x = 0 To histTop.dst3.Height - stepSize Step stepSize
                ...
                End If
            Next
            cv.Cv2.CalcBackProject({atask.pointCloud}, atask.channelsTop, histTop.histogram, dst0, atask.rangesTop)
            ...
            For y = 0 To histSide.dst3.Height - stepSize Step stepSize
                ...
                End If
            Next
            cv.Cv2.CalcBackProject({atask.pointCloud}, atask.channelsSide, histSide.histogram, dst1, atask.rangesSide)
    ...
    Public Class NR_GuidedBP_Points : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.CalcBackProject({atask.pointCloud}, atask.channelsTop, histogramTop, backP,
                                atask.rangesTop)
            ...
            cv.Cv2.CalcBackProject({atask.pointCloud}, atask.channelsSide, histogramSide, dst3, atask.rangesSide)

            dst3 = PaletteFull(dst3)

            classCount = topRects.Count + sideRects.Count

            If atask.mouseClickFlag Then selectedPoint = atask.clickPoint
            If atask.heartBeat Then labels(2) = CStr(topRects.Count) + " objects were identified in the top view."
            If atask.heartBeat Then labels(3) = CStr(sideRects.Count) + " objects were identified in the side view."
        End Sub
    End Class
    ...
    Public Class GuidedBP_TopView : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            hotPoints.ptHot.histTop.dst3.ConvertTo(histogramTop, cv.MatType.CV_32F)
            cv.Cv2.CalcBackProject({src}, atask.channelsTop, histogramTop, backP,
                                atask.rangesTop)
