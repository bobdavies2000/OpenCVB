...
Namespace VBClasses
    Public Class HistPeak2D_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.CalcBackProject({src}, taskAlg.channels, histogram, backProjection, ranges)
    ...
    Public Class HistPeak2D_TopAndSide : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            If taskAlg.toggleOn Then
                histSide.Run(src)
                peak.ranges = taskAlg.rangesSide
                taskAlg.channels = taskAlg.channelsSide
                peak.histogram = histSide.histogram
            Else
                histTop.Run(src)
                taskAlg.channels = taskAlg.channelsTop
                peak.ranges = taskAlg.rangesTop
                peak.histogram = histTop.histogram
            End If
            peak.Run(taskAlg.pointCloud)
            dst1 = peak.dst2
    ...
    Public Class HistPeak2D_Edges : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst3 = histTop.histogram.Threshold(taskAlg.projectionThreshold, 255, cv.ThresholdTypes.Binary)
            peak.histogram = histTop.histogram
            peak.Run(taskAlg.pointCloud)
            dst2 = peak.dst2
    ...
    Public Class HistPeak2D_HotSide : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To peak.auto.clusterPoints.Count - 1
                ...
                DrawCircle(dst3, pt, taskAlg.DotSize * 3, white)
            Next
