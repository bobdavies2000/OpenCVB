...
Namespace VBClasses
    ...
    Public Class Blur_TopoMap : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst3 = ShowAddweighted(dst3, algTask.color, labels(3))

            labels(2) = "Blur = " + CStr(options.nextPercent) + "% Reduction Factor = " + CStr(options.reduction)
            If algTask.frameCount Mod options.frameCycle = 0 Then options.nextPercent -= 1
    ...
    Public Class Blur_Detection : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            If standaloneTest() Then
                If algTask.drawRect <> New cv.Rect Then r = algTask.drawRect
                ' deliberately blur a small region to test the algorithm
                If algTask.frameCount Mod 2 Then
                    blur.Run(src(r))
                    src(r) = blur.dst2
                End If
            End If
            ...
            If standaloneTest() Then dst2.Rectangle(r, white, algTask.lineWidth)
        End Sub
    End Class
    ...
    Public Class Blur_Depth : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            dst3 = algTask.depthRGB.CvtColor(cv.ColorConversionCodes.BGR2GRAY).Threshold(0, 255, cv.ThresholdTypes.Binary)
    ...
    Public Class Blur_PlusHistogram : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            myhist.Run(algTask.grayStable)

            mat2to1.mat(0) = myhist.dst2.Clone

            blur.Run(algTask.grayStable)
    ...
    Public Class Blur_Histogram : Inherits TaskParent
        ...
        End Sub
        Public Overrides Sub RunAlg(src As cv.Mat)
            If algTask.drawRect <> New cv.Rect Then src = algTask.grayStable(algTask.drawRect) Else src = algTask.grayStable
