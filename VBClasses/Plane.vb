...
Namespace VBClasses
    Public Class Plane_Basics : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.CalcHist({taskAlg.pointCloud}, taskAlg.channelsTop, New cv.Mat, topHist, 2,
                            {dst2.Height, dst2.Width}, taskAlg.rangesTop)
            topHist.Row(0).SetTo(0)
            cv.Cv2.InRange(topHist, taskAlg.projectionThreshold, topHist.Total, dst1)
            dst1.ConvertTo(dst1, cv.MatType.CV_32F)
            cv.Cv2.CalcBackProject({taskAlg.pointCloud}, taskAlg.channelsTop, dst1, topBackP, taskAlg.rangesTop)
    ...
    Public Class Plane_FlatSurfaces : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2 = plane.dst2
            If taskAlg.heartBeat Then addW.src2.SetTo(0)
                    ...
                    For x = 0 To rc.rect.Width - 1
                        ...
                        If val > 0 Then
                            If msRNG.Next(100) < 10 Then
                                Dim pt = taskAlg.pointCloud(rc.rect).Get(Of cv.Point3f)(y, x)
    ...
    Public Class Plane_OnlyPlanes : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Next
            If plane.options.reuseRawDepthData Then dst3 = taskAlg.pointCloud
    ...
    Public Class Plane_CellColor : Inherits TaskParent
        ...
        Public Function buildContourPoints(rc As oldrcData) As List(Of cv.Point3f)
            ...
            For Each pt In rc.contour
                ...
                fitPoints.Add(taskAlg.pointCloud(rc.rect).Get(Of cv.Point3f)(pt.Y, pt.X)) ' each contour point is guaranteed to be in the mask and have depth.
            Next
            Return fitPoints
        End Function
    ...
    Public Class Plane_Points : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To rc.contour.Count - 1
                pt = taskAlg.pointCloud.Get(Of cv.Point3f)(rc.contour(i).Y, rc.contour(i).X)
                If pt.Z > 0 Then
                    ptList.Add(pt)
            ...
            If taskAlg.heartBeat Or needOutput Then
                ptList2D.Clear()
                equations.Clear()
                needOutput = False
                strOut = ""
    ...
    Public Class Plane_Histogram : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To points.Rows - 1
                ...
                Dim yVal = taskAlg.pcSplit(1).Get(Of Single)(pt.Y, pt.X)
            ...
            dst2 = hist.dst2
            Dim binWidth As Single = dst2.Width / taskAlg.histogramBins
            Dim rangePerBin = (hist.mm.maxVal - hist.mm.minVal) / taskAlg.histogramBins

            Dim midHist = taskAlg.histogramBins / 2
            ...
            dst2.Rectangle(New cv.Rect(rX, 0, binWidth, dst2.Height), cv.Scalar.Black, taskAlg.lineWidth)
            ...
            dst2.Rectangle(New cv.Rect(rX, 0, binWidth, dst2.Height), cv.Scalar.Yellow, taskAlg.lineWidth)
    ...
    Public Class Plane_Verticals : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            solo.Run(src)
            dst3 = solo.heat.topframes.dst2.InRange(taskAlg.projectionThreshold * taskAlg.frameHistoryCount, dst2.Total)

            dst1 = New cv.Mat(dst1.Size(), cv.MatType.CV_32FC1, 0)
            solo.heat.dst0.CopyTo(dst1, dst3)
            dst1.ConvertTo(dst1, cv.MatType.CV_32FC1)

            cv.Cv2.CalcBackProject({taskAlg.pointCloud}, taskAlg.channelsTop, dst1, dst2, taskAlg.rangesTop)

            frames.Run(dst2)
            frames.dst2.ConvertTo(dst2, cv.MatType.CV_8U)
            dst2 = frames.dst2.Threshold(0, 255, cv.ThresholdTypes.Binary)
            dst2.ConvertTo(dst0, cv.MatType.CV_8U)
            taskAlg.color.SetTo(white, dst0)
        End Sub
    End Class
    ...
    Public Class Plane_Horizontals : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            solo.Run(src)
            dst3 = solo.heat.sideframes.dst2.InRange(taskAlg.projectionThreshold * taskAlg.frameHistoryCount, dst2.Total)

            dst1 = New cv.Mat(dst1.Size(), cv.MatType.CV_8U, cv.Scalar.All(0))
            solo.heat.dst1.CopyTo(dst1, dst3)
            dst1.ConvertTo(dst1, cv.MatType.CV_32FC1)

            cv.Cv2.CalcBackProject({taskAlg.pointCloud}, taskAlg.channelsSide, dst1, dst2, taskAlg.rangesSide)
    ...
    Public Class Plane_FloorStudy : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim thicknessCMs = taskAlg.metersPerPixel * 1000 / 100, rect As cv.Rect, nextY As Single
            For y = dst0.Height - 2 To 0 Step -1
                ...
                If count > options.countThreshold Then
                    nextY = -taskAlg.yRange * (taskAlg.sideCameraPoint.Y - y) / taskAlg.sideCameraPoint.Y - thicknessCMs / 2.5 ' narrow it down to about 1 cm
                    labels(2) = "Y = " + Format(planeY, fmt3) + " separates the floor."
                    SetTrueText(labels(2), 3)
                    Dim sliceMask = taskAlg.pcSplit(1).InRange(cv.Scalar.All(planeY), cv.Scalar.All(3.0))
                    dst2 = src
            ...
            dst1.Line(New cv.Point(0, rect.Y), New cv.Point(dst2.Width, rect.Y), cv.Scalar.Yellow, slice.options.sliceSize, taskAlg.lineType)
        End Sub
    End Class
End Namespace