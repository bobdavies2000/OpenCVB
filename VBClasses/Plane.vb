...
Namespace VBClasses
    Public Class Plane_Basics : Inherits TaskParent
        ...
        Public Shared Function build3PointEquation(rc As rcData) As cv.Vec4f
            ...
            Dim v1 = tsk.pointCloud(rc.rect).Get(Of cv.Point3f)(p1.Y, p1.X)
            Dim v2 = tsk.pointCloud(rc.rect).Get(Of cv.Point3f)(p2.Y, p2.X)
            Dim v3 = tsk.pointCloud(rc.rect).Get(Of cv.Point3f)(p3.Y, p3.X)
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.CalcHist({tsk.pointCloud}, tsk.channelsTop, New cv.Mat, topHist, 2,
                            {dst2.Height, dst2.Width}, tsk.rangesTop)
            topHist.Row(0).SetTo(0)
            cv.Cv2.InRange(topHist, tsk.projectionThreshold, topHist.Total, dst1)
            dst1.ConvertTo(dst1, cv.MatType.CV_32F)
            cv.Cv2.CalcBackProject({tsk.pointCloud}, tsk.channelsTop, dst1, topBackP, tsk.rangesTop)
    ...
    Public Class NR_Plane_FlatSurfaces : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2 = plane.dst2
            If tsk.heartBeat Then addW.src2.SetTo(0)
                    ...
                    For x = 0 To rc.rect.Width - 1
                        ...
                        If val > 0 Then
                            If msRNG.Next(100) < 10 Then
                                Dim pt = tsk.pointCloud(rc.rect).Get(Of cv.Point3f)(y, x)
    ...
    Public Class Plane_OnlyPlanes : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Next
            If plane.options.reuseRawDepthData Then dst3 = tsk.pointCloud
    ...
    Public Class Plane_CellColor : Inherits TaskParent
        ...
        Public Function buildContourPoints(rc As rcData) As List(Of cv.Point3f)
            ...
            For Each pt In rc.contour
                ...
                fitPoints.Add(tsk.pointCloud(rc.rect).Get(Of cv.Point3f)(pt.Y, pt.X)) ' each contour point is guaranteed to be in the mask and have depth.
            Next
            Return fitPoints
        End Function
    ...
    Public Class Plane_Points : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To rc.contour.Count - 1
                pt = tsk.pointCloud.Get(Of cv.Point3f)(rc.contour(i).Y, rc.contour(i).X)
                If pt.Z > 0 Then
                    ptList.Add(pt)
            ...
            If tsk.heartBeat Or needOutput Then
                ptList2D.Clear()
                equations.Clear()
                needOutput = False
                strOut = ""
    ...
    Public Class NR_Plane_Histogram : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To points.Rows - 1
                ...
                Dim yVal = tsk.pcSplit(1).Get(Of Single)(pt.Y, pt.X)
            ...
            dst2 = hist.dst2
            Dim binWidth As Single = dst2.Width / tsk.histogramBins
            Dim rangePerBin = (hist.mm.maxVal - hist.mm.minVal) / tsk.histogramBins

            Dim midHist = tsk.histogramBins / 2
            ...
            dst2.Rectangle(New cv.Rect(rX, 0, binWidth, dst2.Height), cv.Scalar.Black, tsk.lineWidth)
            ...
            dst2.Rectangle(New cv.Rect(rX, 0, binWidth, dst2.Height), cv.Scalar.Yellow, tsk.lineWidth)
    ...
    Public Class Plane_Equation : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For j = 0 To offset - 1
                ...
                Dim v2 = tsk.pointCloud(rc.rect).Get(Of cv.Point3f)(p2.Y, p2.X)
                Dim v3 = tsk.pointCloud(rc.rect).Get(Of cv.Point3f)(p3.Y, p3.X)
                Dim v4 = tsk.pointCloud(rc.rect).Get(Of cv.Point3f)(p4.Y, p4.X)
    ...
    Public Class NR_Plane_Horizontals : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            solo.Run(src)
            dst3 = solo.heat.sideframes.dst2.InRange(tsk.projectionThreshold * tsk.frameHistoryCount, dst2.Total)

            dst1 = New cv.Mat(dst1.Size(), cv.MatType.CV_8U, cv.Scalar.All(0))
            solo.heat.dst1.CopyTo(dst1, dst3)
            dst1.ConvertTo(dst1, cv.MatType.CV_32FC1)

            cv.Cv2.CalcBackProject({tsk.pointCloud}, tsk.channelsSide, dst1, dst2, tsk.rangesSide)
    ...
    Public Class NR_Plane_FloorStudy : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim thicknessCMs = tsk.metersPerPixel * 1000 / 100, rect As cv.Rect, nextY As Single
            For y = dst0.Height - 2 To 0 Step -1
                ...
                If count > options.countThreshold Then
                    nextY = -tsk.yRange * (tsk.sideCameraPoint.Y - y) / tsk.sideCameraPoint.Y - thicknessCMs / 2.5 ' narrow it down to about 1 cm
                    labels(2) = "Y = " + Format(planeY, fmt3) + " separates the floor."
                    SetTrueText(labels(2), 3)
                    Dim sliceMask = tsk.pcSplit(1).InRange(cv.Scalar.All(planeY), cv.Scalar.All(3.0))
                    dst2 = src
            ...
            dst1.Line(New cv.Point(0, rect.Y), New cv.Point(dst2.Width, rect.Y), cv.Scalar.Yellow, slice.options.sliceSize, tsk.lineType)
        End Sub
    End Class
End Namespace