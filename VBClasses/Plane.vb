...
Namespace VBClasses
    Public Class Plane_Basics : Inherits TaskParent
        ...
        Public Shared Function build3PointEquation(rc As rcData) As cv.Vec4f
            ...
            Dim v1 = taskA.pointCloud(rc.rect).Get(Of cv.Point3f)(p1.Y, p1.X)
            Dim v2 = taskA.pointCloud(rc.rect).Get(Of cv.Point3f)(p2.Y, p2.X)
            Dim v3 = taskA.pointCloud(rc.rect).Get(Of cv.Point3f)(p3.Y, p3.X)
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            cv.Cv2.CalcHist({taskA.pointCloud}, taskA.channelsTop, New cv.Mat, topHist, 2,
                            {dst2.Height, dst2.Width}, taskA.rangesTop)
            topHist.Row(0).SetTo(0)
            cv.Cv2.InRange(topHist, taskA.projectionThreshold, topHist.Total, dst1)
            dst1.ConvertTo(dst1, cv.MatType.CV_32F)
            cv.Cv2.CalcBackProject({taskA.pointCloud}, taskA.channelsTop, dst1, topBackP, taskA.rangesTop)
    ...
    Public Class NR_Plane_FlatSurfaces : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            dst2 = plane.dst2
            If taskA.heartBeat Then addW.src2.SetTo(0)
                    ...
                    For x = 0 To rc.rect.Width - 1
                        ...
                        If val > 0 Then
                            If msRNG.Next(100) < 10 Then
                                Dim pt = taskA.pointCloud(rc.rect).Get(Of cv.Point3f)(y, x)
    ...
    Public Class Plane_OnlyPlanes : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Next
            If plane.options.reuseRawDepthData Then dst3 = taskA.pointCloud
    ...
    Public Class Plane_CellColor : Inherits TaskParent
        ...
        Public Function buildContourPoints(rc As rcData) As List(Of cv.Point3f)
            ...
            For Each pt In rc.contour
                ...
                fitPoints.Add(taskA.pointCloud(rc.rect).Get(Of cv.Point3f)(pt.Y, pt.X)) ' each contour point is guaranteed to be in the mask and have depth.
            Next
            Return fitPoints
        End Function
    ...
    Public Class Plane_Points : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To rc.contour.Count - 1
                pt = taskA.pointCloud.Get(Of cv.Point3f)(rc.contour(i).Y, rc.contour(i).X)
                If pt.Z > 0 Then
                    ptList.Add(pt)
            ...
            If taskA.heartBeat Or needOutput Then
                ptList2D.Clear()
                equations.Clear()
                needOutput = False
                strOut = ""
    ...
    Public Class NR_Plane_Histogram : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For i = 0 To points.Rows - 1
                ...
                Dim yVal = taskA.pcSplit(1).Get(Of Single)(pt.Y, pt.X)
            ...
            dst2 = hist.dst2
            Dim binWidth As Single = dst2.Width / taskA.histogramBins
            Dim rangePerBin = (hist.mm.maxVal - hist.mm.minVal) / taskA.histogramBins

            Dim midHist = taskA.histogramBins / 2
            ...
            dst2.Rectangle(New cv.Rect(rX, 0, binWidth, dst2.Height), cv.Scalar.Black, taskA.lineWidth)
            ...
            dst2.Rectangle(New cv.Rect(rX, 0, binWidth, dst2.Height), cv.Scalar.Yellow, taskA.lineWidth)
    ...
    Public Class Plane_Equation : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            For j = 0 To offset - 1
                ...
                Dim v2 = taskA.pointCloud(rc.rect).Get(Of cv.Point3f)(p2.Y, p2.X)
                Dim v3 = taskA.pointCloud(rc.rect).Get(Of cv.Point3f)(p3.Y, p3.X)
                Dim v4 = taskA.pointCloud(rc.rect).Get(Of cv.Point3f)(p4.Y, p4.X)
    ...
    Public Class NR_Plane_Horizontals : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            solo.Run(src)
            dst3 = solo.heat.sideframes.dst2.InRange(taskA.projectionThreshold * taskA.frameHistoryCount, dst2.Total)

            dst1 = New cv.Mat(dst1.Size(), cv.MatType.CV_8U, cv.Scalar.All(0))
            solo.heat.dst1.CopyTo(dst1, dst3)
            dst1.ConvertTo(dst1, cv.MatType.CV_32FC1)

            cv.Cv2.CalcBackProject({taskA.pointCloud}, taskA.channelsSide, dst1, dst2, taskA.rangesSide)
    ...
    Public Class NR_Plane_FloorStudy : Inherits TaskParent
        ...
        Public Overrides Sub RunAlg(src As cv.Mat)
            ...
            Dim thicknessCMs = taskA.metersPerPixel * 1000 / 100, rect As cv.Rect, nextY As Single
            For y = dst0.Height - 2 To 0 Step -1
                ...
                If count > options.countThreshold Then
                    nextY = -taskA.yRange * (taskA.sideCameraPoint.Y - y) / taskA.sideCameraPoint.Y - thicknessCMs / 2.5 ' narrow it down to about 1 cm
                    labels(2) = "Y = " + Format(planeY, fmt3) + " separates the floor."
                    SetTrueText(labels(2), 3)
                    Dim sliceMask = taskA.pcSplit(1).InRange(cv.Scalar.All(planeY), cv.Scalar.All(3.0))
                    dst2 = src
            ...
            dst1.Line(New cv.Point(0, rect.Y), New cv.Point(dst2.Width, rect.Y), cv.Scalar.Yellow, slice.options.sliceSize, taskA.lineType)
        End Sub
    End Class
End Namespace