...
Public Class Flood_PrepDataFull : Inherits TaskParent
    Dim prep As New RedPrep_Basics
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        dst3.SetTo(0)
        prepList.Clear()
        Dim minCount = dst2.Total * 0.001, maxCount = dst2.Total * 3 / 4
        For y = 0 To dst2.Height - 1
            For x = 0 To dst2.Width - 1
                Dim pt = New cv.Point(x, y)
                Dim val = dst2.Get(Of Byte)(pt.Y, pt.X) ' skip the regions with no depth
                If val > 0 Then
                    ...
                    If val = 0 Then
                        ...
                        If count >= minCount And count < maxCount Then
                            ...
                            dst3(rect).SetTo(pd.color, mask(rect))
                            prepList.Add(pd)
                        End If
                    End If
                End If
        ...
        For Each pd In prepList
            dst3.Circle(pd.center, task.DotSize, task.highlight, -1)
        Next
        labels(2) = CStr(index) + " regions were identified"
    End Sub
End Class
