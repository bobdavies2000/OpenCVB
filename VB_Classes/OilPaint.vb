...
Public Class OilPaint_Pointilism : Inherits TaskParent
    ...
    Dim saveDrawRect As New cvb.Rect
    Public Sub New()
    ...
    Public Sub RunAlg(src As cvb.Mat)
        ...
        If saveDrawRect <> task.drawRect Then
            ...
            randomMask = New cvb.Mat(img.Size(), cvb.MatType.CV_32SC2)
            Dim nPt As New cvb.Point
            For y = 0 To randomMask.Height - 1
                For x = 0 To randomMask.Width - 1
                    ...
                    randomMask.Set(Of cvb.Point)(y, x, nPt)
                Next
            Next
            cvb.Cv2.RandShuffle(randomMask, 1.0, myRNG) ' the RNG is not optional.
        End If
        Dim rand = randomMask.Resize(img.Size())
        Dim gray = src.CvtColor(cvb.ColorConversionCodes.BGR2GRAY)

        Dim fieldx As New cvb.Mat, fieldy As New cvb.Mat
        cvb.Cv2.Scharr(gray, fieldx, cvb.MatType.CV_32FC1, 1, 0, 1 / 15.36)
        cvb.Cv2.Scharr(gray, fieldy, cvb.MatType.CV_32FC1, 0, 1, 1 / 15.36)

        cvb.Cv2.GaussianBlur(fieldx, fieldx, New cvb.Size(options.smoothingRadius, options.smoothingRadius), 0, 0)
        cvb.Cv2.GaussianBlur(fieldy, fieldy, New cvb.Size(options.smoothingRadius, options.smoothingRadius), 0, 0)

        For y = 0 To img.Height - 1
            For x = 0 To img.Width - 1
                Dim nPt = rand.Get(Of cvb.Point)(y, x)
                Dim nextColor = src.Get(Of cvb.Vec3b)(saveDrawRect.Y + nPt.Y, saveDrawRect.X + nPt.X)
                Dim fx = fieldx(saveDrawRect).Get(Of Single)(nPt.Y, nPt.X)
                Dim fy = fieldy(saveDrawRect).Get(Of Single)(nPt.Y, nPt.X)
                Dim nPoint = New cvb.Point2f(nPt.X, nPt.Y)
                ...
                Dim eSize = New cvb.Size2f(slen, options.strokeSize)
                Dim direction = Math.Atan2(fx, fy)
                Dim angle = direction * 180.0 / Math.PI + 90

                Dim rotatedRect = New cvb.RotatedRect(nPoint, eSize, angle)
    ...
    Public Sub RunAlg(src As cvb.Mat)
        ...
        For y = filterKern To roi.Height - filterKern - 1
            For x = filterKern To roi.Width - filterKern - 1
                ...
                result1.Set(Of cvb.Vec3b)(y, x, vec)
            Next
        Next
