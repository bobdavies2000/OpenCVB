...
Public Class IntrinsicsLeft_Basics : Inherits VBparent
    ...
    Public Sub Run(src As cv.Mat) ' Rank = 1
        ...
        dst2.SetTo(0)
        dst2.Line(New cv.Point(src.Width / 2, 0), New cv.Point(src.Width / 2, src.Height), cv.Scalar.White, task.lineThickness)
        dst2.Line(New cv.Point(0, src.Height / 2), New cv.Point(src.Width, src.Height / 2), cv.Scalar.White, task.lineThickness)
        ...
        cv.Cv2.PutText(dst2, nextline, ttLocation, task.font, task.fontSize, cv.Scalar.White, task.lineThickness, task.lineType)

        Dim ptLoc = New cv.Point(src.Width / 2 + 4, src.Height / 2 + 4)
        dst2.Line(ptLoc, ttLocation, cv.Scalar.Red, task.lineThickness + 1, task.lineType)
    End Sub
End Class
