...
Public Class Depth_FirstLastDistance : Inherits VB_Algorithm
    ...
    End Sub
    Public Sub RunVB(src As cv.Mat)
        Dim mm = vbMinMax(task.pcSplit(2), task.depthMask)
...
Public Class Depth_MeanStdev_MT : Inherits VB_Algorithm
    ...
    Public Sub RunVB(src As cv.Mat)
        ...
        If task.frameCount >= task.historyCount Then
            ...
            Dim mm = vbMinMax(means, meanmask)
...
Public Class Depth_LocalMinMax_MT : Inherits VB_Algorithm
    ...
    Public Sub RunVB(src As cv.Mat)
        ...
        Sub(i)
            Dim roi = task.gridList(i)
            Dim mm = vbMinMax(task.pcSplit(2)(roi), task.depthMask(roi))
...
Public Class Depth_MinMaxToVoronoi : Inherits VB_Algorithm
    ...
    Public Sub RunVB(src As cv.Mat)
        ...
        Sub(i)
            Dim roi = task.gridList(i)
            Dim mm = vbMinMax(task.pcSplit(2)(roi), depthmask(roi))
            If mm.minLoc.X < 0 Or mm.minLoc.Y < 0 Then mm.minLoc = New cv.Point2f(0, 0)
...
Public Class Depth_Far : Inherits VB_Algorithm
    ...
    End Sub
    Public Sub RunVB(src As cv.Mat)
        dst2 = task.pcSplit(2).Threshold(gOptions.MaxDepth.Value, 255, cv.ThresholdTypes.Binary).ConvertScaleAbs()
...
Public Class Depth_Colorizer_VB : Inherits VB_Algorithm
    ...
    Public Sub RunVB(src As cv.Mat)
        If src.Type <> cv.MatType.CV_32F Then src = task.pcSplit(2)
        If src.Size <> task.minRes Then src = src.Resize(task.minRes, cv.InterpolationFlags.Nearest)

        dst2 = New cv.Mat(task.minRes, cv.MatType.CV_8UC3, 0)
