...
Public Class Depth_Colorizer_CPP : Inherits VBparent
    ...
    Public Sub Run(src As cv.Mat) ' Rank = 1
        If src.Type <> cv.MatType.CV_32F Then
            If standalone Or task.intermediateActive Then src = task.depth32f Else dst2 = New cv.Mat(src.Size(), cv.MatType.CV_8UC3)
        End If
...
Public Class Depth_ColorizerFastFade_CPP : Inherits VBparent
    ...
    Public Sub Run(src As cv.Mat) ' Rank = 1
        ...
        If imagePtr <> 0 Then
            ...
            If standalone Or task.intermediateActive Then dst2.SetTo(0, dst3)
        End If
    End Sub
    Public Sub Close()
...
Public Class Depth_ColorizerVB_MT : Inherits VBparent
    ...
    Public Sub Run(src As cv.Mat) ' Rank = 1
        ...
        If standalone Or task.intermediateActive Then src = task.depth32f
...
Public Class Depth_NotMissing : Inherits VBparent
    ...
    End Sub
    Public Sub Run(src As cv.Mat) ' Rank = 1
        If standalone Or task.intermediateActive Then src = task.RGBDepth
        mog.RunClass(src)
        dst2 = mog.dst2
...
Public Class Depth_Smoothing : Inherits VBparent
    ...
    Public Sub Run(src As cv.Mat) ' Rank = 1
        ...
        reduction.RunClass(task.depth32f)
        reduction.dst2.ConvertTo(reducedDepth, cv.MatType.CV_32F)
        colorize.RunClass(reducedDepth)
        dst2 = colorize.dst2
        mats.RunClass(src)
        dst3 = mats.dst2
...
Public Class Depth_WorldXYZ : Inherits VBparent
    ...
    Public Sub Run(src As cv.Mat) ' Rank = 1
        ...
        For xy.Y = 0 To dst3.Height - 1
            ...
            Next
        Next
        If standalone Or task.intermediateActive Then setTrueText("OpenGL data prepared.")
    End Sub
End Class
...
Public Class Depth_PunchBlob : Inherits VBparent
    ...
    End Sub
    Public Sub Run(src As cv.Mat) ' Rank = 1
        depthInc.RunClass(src)
        dst2 = depthInc.dst2

        contours.RunClass(dst2)
        dst3 = contours.dst3
...
Public Class Depth_AveragingStable : Inherits VBparent
    ...
    Public Sub Run(src As cv.Mat) ' Rank = 1
        ...
        If src.Type <> cv.MatType.CV_32F Then src = task.depth32f
        extrema.RunClass(src)

        If noAvgRadio.checked = False Then
            dAvg.RunClass(extrema.dst3)
            dst2 = dAvg.dst2
            dst3 = dAvg.dst3
        Else
            dst2 = extrema.dst2
