...
Public Class XO_Line_LeftRight : Inherits TaskParent
    ...
    End Sub
    Public Overrides Sub RunAlg(src As cv.Mat)
        dst2 = task.hullLines.dst2.Clone
        labels(2) = "Left view" + task.hullLines.labels(2)
...
Public Class XO_Line_Matching : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        For i = 0 To task.hullLines.lpList.Count - 1
        ...
        For i = 0 To removeList.Count - 1
            task.hullLines.lpList.RemoveAt(removeList.ElementAt(i).Value)
        Next

        For Each lp In addList
            task.hullLines.lpList.Add(lp)
        Next
        lpList = New List(Of lpData)(task.hullLines.lpList)
        lineMap.SetTo(0)
        ...
        If task.heartBeat Then
            labels(2) = CStr(task.hullLines.lpList.Count) + " lines were input and " + CStr(combineCount) +
...
Public Class XO_Line_TimeView : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        Dim nextMpList = New List(Of lpData)(task.hullLines.lpList)
...
Public Class XO_Line_Basics : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        task.hullLines.lpList = New List(Of lpData)(lineCore.lpList)
...
Public Class XO_FCSLine_Basics : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        Dim lastCount = task.hullLines.lpList.Count
        ...
        task.lpD = task.hullLines.lpList(index)
...
Public Class XO_FeatureLine_Finder3D : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        Else
            ...
            Next
        End If
        labels(2) = "Starting with " + Format(task.hullLines.lpList.Count, "000") + " lines, there are " +
...
Public Class XO_FeatureLine_Tutorial2 : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        If task.heartBeat Then labels(2) = "Starting with " + Format(task.hullLines.lpList.Count, "000") +
...
Public Class XO_Line_VerticalHorizontal1 : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        dst2 = src.Clone
        If standaloneTest() Then dst3 = task.hullLines.dst2
...
Public Class XO_BrickPoint_FeatureLessOld2 : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        dst0.SetTo(0)
        Dim brickPrev = task.bricks.brickList(0)
        Dim fLessCount As Integer = 1
        For Each brick In task.bricks.brickList
            If brick.rect.X = 0 Or brick.rect.Y = 0 Then Continue For

            Dim brickAbove = task.bricks.brickList(brick.index - task.cellsPerRow)
        ...
        For i = task.bricks.brickList.Count - 1 To 1 Step -1
            Dim brick = task.bricks.brickList(i)
            If brick.contourFull > 0 Then
                brickPrev = task.bricks.brickList(i - 1)
                If brickPrev.contourFull > 0 And brickPrev.contourFull <> 0 And brickPrev.contourFull <> brick.contourFull And
                    ...
                    task.bricks.brickList(i - 1) = brickPrev
                End If
            End If
        Next
...
Public Class XO_LongLine_BasicsEx : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        labels(2) = $"{task.hullLines.lpList.Count} lines found, longest {lpList.Count} displayed."
    End Sub
End Class
...
Public Class XO_LongLine_Basics : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        For i = 1 To task.hullLines.lpList.Count - 1
            Dim lp = task.hullLines.lpList(i)
            lp.index = lpList.Count
        ...
        labels(2) = CStr(lpList.Count - 1) + " longest lines in the image in " + CStr(task.hullLines.lpList.Count) + " total lines."
        labels(3) = labels(2)
    End Sub
End Class
...
Public Class XO_tructured_MouseSlice : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        If task.hullLines.lpList.Count > 0 Then
            dst3 = lines.dst2
            For Each lp In task.hullLines.lpList
