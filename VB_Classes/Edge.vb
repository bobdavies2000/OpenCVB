...
Public Class Edge_CameraMotion : Inherits VB_Algorithm
    Public translation As Single
    Public rotation As Single
    Public center As cv.Point2f
    Dim gravityVec = task.gravityVec
    Dim horizonVec = task.horizonVec
    Public Sub New()
        desc = "Create edges from the camera motion"
    End Sub
    Public Sub translateRotate(y1 As Integer, y2 As Integer)
        rotation = Math.Atan(Math.Abs((y1 - y2)) / dst2.Width)
        If y1 > 0 And y2 > 0 Then
            translation = If(y1 > y2, y2, y1)
            center = horizonVec.p2
        ElseIf y1 < 0 And y2 < 0 Then
            translation = If(y1 > y2, y1, y2)
            center = horizonVec.p1
        ElseIf y1 < 0 And y2 > 0 Then
            translation = 0
            rotation *= -1
            center = horizonVec.p1
    ...
    Public Sub RunVB(src As cv.Mat)
        ...
        translateRotate(y1, y2)

        gravityVec = task.gravityVec
        horizonVec = task.horizonVec

        labels(3) = "Translation = " + Format(translation) + " rotation = " + Format(rotation * 57.2958, fmt1) +
                    " center of rotation = " + center.ToString()
    End Sub
End Class
