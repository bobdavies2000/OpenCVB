    ...
    Public Sub RunVB(src As cv.Mat)
        ...
        For Each rc In plane.redC.redCells
            ...
            If RMSerror / pixelCount <= plane.options.rmsThreshold Then
                addW.src2(rc.rect).SetTo(cv.Scalar.White, rc.mask)
                flatCount += 1
            End If
        Next
...
Public Class Plane_FloorStudy : Inherits VB_Algorithm
    ...
    Public Sub RunVB(src As cv.Mat)
        ...
        dst1.Line(New cv.Point(0, rect.Y), New cv.Point(dst2.Width, rect.Y), cv.Scalar.Yellow, slice.options.cushion, task.lineType)
    End Sub
End Class
