...
Public Class Homography_Basics : Inherits VB_Parent
    ...
    Public Sub RunAlg(src As cvb.Mat)
        ...
        If standaloneTest() And task.heartBeat And options.hMethod = cvb.HomographyMethods.None Then
            random.Run(empty)
            corners1 = New List(Of cvb.Point2d)(random.PointList)
            random.Run(empty)
            corners2 = New List(Of cvb.Point2d)(random.PointList)
        End If

        ' cannot find a homography when less than 4...
        If corners1.Count >= 4 Or corners2.Count >= 4 Then
            Dim H = cvb.Cv2.FindHomography(corners1, corners2, options.hMethod)
...
Public Class Homography_FPoly : Inherits VB_Parent
    ...
    Public Sub RunAlg(src As cvb.Mat)
        ...
        For i = 0 To fPoly.fPD.currPoly.Count - 1
            ...
            hGraph.corners1.Add(New cvb.Point2d(p1.X, p1.Y))
            hGraph.corners2.Add(New cvb.Point2d(p2.X, p2.Y))
        Next
