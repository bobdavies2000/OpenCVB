...
Public Class Coherent_FloodFill
    ...
    Public Sub Run()
        ...
        For i = 0 To lastPoints.Count - 1
            For j = i + 1 To lastPoints.Count - 1
                If lastPoints(i) = lastPoints(j) Then
                    removeList.Add(j)
                    Exit For
                End If
            Next
        ...
        ' remove an duplicates
        For i = removeList.Count - 1 To 0 Step -1
            Dim index = removeList(i)
            lastPoints.RemoveAt(index)
            lastMasks.RemoveAt(index)
            lastRects.RemoveAt(index)
            lastSizes.RemoveAt(index)
        Next
        ...
        For i = 0 To sortedMasks.Count - 1
            ...
            dst1(rect).SetTo(index, mask(rect))
        Next

        label1 = CStr(lastPoints.Count) + " regions identified"

        If basics.rects.Count > 0 Then
            Dim incr = 155 / lastPoints.Count
            dst1 = dst1 * incr + 100
            palette.src = dst1
            palette.Run()
            dst2 = palette.dst1
