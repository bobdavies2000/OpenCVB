    ...
    Public Sub Run()
        ...
        If keypoints IsNot Nothing Then
            For Each kpt As cv.KeyPoint In keypoints
                ...
                cv.Cv2.Circle(dst1, kpt.Pt, CInt(Math.Truncate(r)), New cv.Scalar(0, 255, 0), 1, task.lineType, 0)
                cv.Cv2.Line(dst1, New cv.Point(kpt.Pt.X + r, kpt.Pt.Y + r), New cv.Point(kpt.Pt.X - r, kpt.Pt.Y - r), New cv.Scalar(0, 255, 0), 1, task.lineType, 0)
                cv.Cv2.Line(dst1, New cv.Point(kpt.Pt.X - r, kpt.Pt.Y + r), New cv.Point(kpt.Pt.X + r, kpt.Pt.Y - r), New cv.Scalar(0, 255, 0), 1, task.lineType, 0)
            Next kpt
        End If
    End Sub
