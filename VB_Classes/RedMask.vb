...
Public Class RedMaskNew_Basics : Inherits TaskParent
    Public inputRemoved As cv.Mat
    Public mdList As New List(Of maskData)
    Public classCount As Integer
    Public Sub New()
        cPtr = RedMaskNew_Open()
        inputRemoved = New cv.Mat(dst2.Size(), cv.MatType.CV_8U, cv.Scalar.All(0))
        desc = "Run the C++ RedMaskNew to create a list of mask, rect, and other info about image"
    End Sub
    Public Overrides Sub RunAlg(src As cv.Mat)
        src = srcMustBe8U(src)
        ...
        Dim imagePtr = RedMaskNew_Run(cPtr, handleInput.AddrOfPinnedObject(), handleMask.AddrOfPinnedObject(),
                                   src.Rows, src.Cols, task.rcPixelThreshold)
        handleMask.Free()
        handleInput.Free()
        dst2 = cv.Mat.FromPixelData(src.Rows, src.Cols, cv.MatType.CV_8U, imagePtr).Clone

        classCount = RedMaskNew_Count(cPtr)
        If classCount <= 1 Then Exit Sub ' no data to process.

        Dim rectData = cv.Mat.FromPixelData(classCount, 1, cv.MatType.CV_32SC4, RedMaskNew_Rects(cPtr))
        Dim rects(classCount * 4) As Integer
        Marshal.Copy(rectData.Data, rects, 0, rects.Length)
        Dim rectlist As New List(Of cv.Rect)
        For i = 0 To rects.Count - 4 Step 4
    ...
    End Sub
    Public Sub Close()
        If cPtr <> 0 Then cPtr = RedMaskNew_Close(cPtr)
    End Sub
End Class