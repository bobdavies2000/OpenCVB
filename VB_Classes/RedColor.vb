...
Public Class RedColor_Basics : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        labels(3) = "The " + CStr(task.redOptions.identifySlider.Value) + " largest cells shown below " +
...
Public Class RedColor_Equations : Inherits TaskParent
    ...
    Public Sub New()
        task.redOptions.identifySlider.Value = 5
...
Public Class RedColor_GenCellContains : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        dst2.SetTo(0)
        Dim count = Math.Min(task.redOptions.identifySlider.Value, task.rcList.Count)
        For i = 0 To count - 1
            Dim rc = task.rcList(i)
        ...
        For i = task.redOptions.identifySlider.Value To task.rcList.Count - 1
            Dim rc = task.rcList(i)
...
Public Class RedColor_Flippers : Inherits TaskParent
    ...
    Public Sub New()
        task.redOptions.identifySlider.Value = 255
...
Public Class RedColor_TopX : Inherits TaskParent
    ...
    Public Overrides Sub RunAlg(src As cv.Mat)
        ...
        If task.heartBeat Or task.optionsChanged Then
            topXcells.Clear()
            For i = 1 To Math.Min(task.redOptions.identifySlider.Value + 1, task.rcList.Count) - 1
                Dim rc = task.rcList(i)
                dst2(rc.rect).SetTo(rc.color, rc.mask)
