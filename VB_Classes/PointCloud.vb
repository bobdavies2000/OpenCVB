...
Public Class PointCloud_Basics : Inherits VB_Parent
    ...
    Public allPointsV As New List(Of cvb.Point3f)

    Public hList As New List(Of List(Of cvb.Point3f))
    Public xyHList As New List(Of List(Of cvb.Point))

    Public vList As New List(Of List(Of cvb.Point3f))
    Public xyVList As New List(Of List(Of cvb.Point))
            ...
            For x = 0 To task.pointCloud.Width - 1 Step task.gridList(0).Width - 1
                ...
                If vec.Z = 0 Or jumpZ Then
                    If vecList.Count > 1 Then
                        ptlist.Add(New List(Of cvb.Point3f)(vecList))
                        xyList.Add(New List(Of cvb.Point)(xyVec))
                    End If
                    vecList.Clear()
                    xyVec.Clear()
                End If
...
Public Class PointCloud_SetupSide : Inherits VB_Parent
    ...
    Public Sub RunAlg(src As cvb.Mat)
        ...
        DrawCircle(dst2, task.sideCameraPoint, task.DotSize, cvb.Scalar.BlueViolet)
        For i = 1 To task.MaxZmeters
            Dim xmeter = CInt(dst2.Width * i / task.MaxZmeters * distanceRatio)
            dst2.Line(New cvb.Point(xmeter, 0), New cvb.Point(xmeter, dst2.Height), cvb.Scalar.AliceBlue, 1)
            SetTrueText(CStr(i) + "m", New cvb.Point(xmeter - src.Width / 24, dst2.Height - 10))
        Next

        Dim cam = task.sideCameraPoint
        Dim marker As New cvb.Point2f(dst2.Width / (task.MaxZmeters * distanceRatio), 0)
        marker.Y = marker.X * Math.Tan((task.vFov / 2) * cvb.Cv2.PI / 180)
        Dim markerLeft = New cvb.Point(marker.X, cam.Y - marker.Y)
        Dim markerRight = New cvb.Point(marker.X, cam.Y + marker.Y)
        ...
        If task.useGravityPointcloud Then
            ...
            markerLeft = New cvb.Point(markerLeft.X - cam.X, markerLeft.Y - cam.Y) ' Change the origin
            markerLeft = New cvb.Point(markerLeft.X * Math.Cos(task.accRadians.Z) - markerLeft.Y * Math.Sin(task.accRadians.Z), ' rotate around x-axis using accRadians.Z
                                              markerLeft.Y * Math.Cos(task.accRadians.Z) + markerLeft.X * Math.Sin(task.accRadians.Z))
            markerLeft = New cvb.Point(markerLeft.X + cam.X, markerLeft.Y + cam.Y) ' Move the origin to the side camera location.

            ' Same as above for markerLeft but consolidated algebraically.
            markerRight = New cvb.Point((markerRight.X - cam.X) * Math.Cos(task.accRadians.Z) - (markerRight.Y - cam.Y) * Math.Sin(task.accRadians.Z) + cam.X,
        ...
        If standaloneTest() = False Then
            DrawCircle(dst2, markerLeft, task.DotSize, cvb.Scalar.Red)
            DrawCircle(dst2, markerRight, task.DotSize, cvb.Scalar.Red)
        End If

        ' draw the arc enclosing the camera FOV
        Dim startAngle = (180 - task.vFov) / 2
        Dim y = dst2.Width / Math.Tan(startAngle * cvb.Cv2.PI / 180)
        ...
        dst2.Line(cam, markerLeft, cvb.Scalar.Red, 1, task.lineType)
        dst2.Line(cam, markerRight, cvb.Scalar.Red, 1, task.lineType)

        Dim labelLocation = New cvb.Point(src.Width * 0.02, src.Height * 7 / 8)
        SetTrueText("vFOV=" + Format(180 - startAngle * 2, "0.0") + " deg.", New cvb.Point(4, dst2.Height * 3 / 4))
    End Sub
End Class
...
Public Class PointCloud_SetupTop : Inherits VB_Parent
    ...
    Public Sub RunAlg(src As cvb.Mat)
        ...
        If standaloneTest() Then dst2.SetTo(0) Else src.CopyTo(dst2)
        DrawCircle(dst2, task.topCameraPoint, task.DotSize, cvb.Scalar.BlueViolet)
        For i = 1 To task.MaxZmeters
            Dim ymeter = CInt(dst2.Height - dst2.Height * i / (task.MaxZmeters * distanceRatio))
            dst2.Line(New cvb.Point(0, ymeter), New cvb.Point(dst2.Width, ymeter), cvb.Scalar.AliceBlue, 1)
            SetTrueText(CStr(i) + "m", New cvb.Point(10, ymeter))
        Next

        Dim cam = task.topCameraPoint
        Dim marker As New cvb.Point2f(cam.X, dst2.Height / task.MaxZmeters)
        Dim topLen = marker.Y * Math.Tan((task.hFov / 2) * cvb.Cv2.PI / 180)
        Dim sideLen = marker.Y * Math.Tan((task.vFov / 2) * cvb.Cv2.PI / 180)
        Dim markerLeft = New cvb.Point(cam.X - topLen, marker.Y)
        Dim markerRight = New cvb.Point(cam.X + topLen, marker.Y)
        ...
        Dim x = dst2.Height / Math.Tan(startAngle * cvb.Cv2.PI / 180)
...
Public Class PointCloud_Raw_CPP_VB : Inherits VB_Parent
    ...
    Public Sub RunAlg(src As cvb.Mat)
        ...
        dst2 = cvb.Mat.FromPixelData(task.pcSplit(2).Rows, task.pcSplit(2).Cols, cvb.MatType.CV_8U, imagePtr).CvtColor(cvb.ColorConversionCodes.GRAY2BGR)
        dst3 = cvb.Mat.FromPixelData(task.pcSplit(2).Rows, task.pcSplit(2).Cols, cvb.MatType.CV_8U, SimpleProjectionSide(cPtr)).CvtColor(cvb.ColorConversionCodes.GRAY2BGR)
                 ...
                 For y = roi.Y To roi.Y + roi.Height - 1
                     For x = roi.X To roi.X + roi.Width - 1
                         ...
                         If m > 0 Then
                             ...
                             If dy < src.Height And dy > 0 Then dst2.Set(Of cvb.Vec3b)(src.Height - dy, x, black)
                             Dim dx = CInt(src.Width * depth / range)
                             If dx < src.Width And dx > 0 Then dst3.Set(Of cvb.Vec3b)(y, dx, black)
                         End If
                     Next
                 Next
             End Sub)
...
Public Class PointCloud_SurfaceH_CPP_VB : Inherits VB_Parent
    ...
    Public Sub RunAlg(src As cvb.Mat)
        ...
        For i = 0 To dst2.Height - 1
            plot.srcX.Add(i)
            If dst2.Channels() = 1 Then plot.srcY.Add(dst2.Row(i).CountNonZero) Else plot.srcY.Add(dst2.Row(i).CvtColor(cvb.ColorConversionCodes.BGR2GRAY).CountNonZero)
...
Public Class PointCloud_SurfaceH : Inherits VB_Parent
    ...
    Public Sub RunAlg(src As cvb.Mat)
        heat.Run(src)
        dst2 = heat.dst2
        Dim hist = New cvb.Mat(dst2.Height, 1, cvb.MatType.CV_32F, cvb.Scalar.All(0))
        ...
        peakRow = 0
        Dim peakVal As Integer
        If dst2.Channels() <> 1 Then dst1 = dst2.CvtColor(cvb.ColorConversionCodes.BGR2GRAY)
        ...
        plot.Run(hist)
        dst3 = plot.dst2.Transpose()
        dst3 = dst3.Flip(cvb.FlipMode.Y)(New cvb.Rect(0, 0, dst0.Height, dst0.Height)).Resize(dst0.Size)
        labels(2) = "Top row = " + CStr(topRow) + " peak row = " + CStr(peakRow) + " bottom row = " + CStr(botRow)

        Dim ratio = task.mouseMovePoint.Y / dst2.Height
        Dim offset = ratio * dst3.Height
        DrawLine(dst2, New cvb.Point(0, task.mouseMovePoint.Y), New cvb.Point(dst2.Width, task.mouseMovePoint.Y), cvb.Scalar.Yellow)
        dst3.Line(New cvb.Point(0, offset), New cvb.Point(dst3.Width, offset), cvb.Scalar.Yellow, task.lineWidth)
    End Sub
End Class
...
Public Class PointCloud_NeighborV : Inherits VB_Parent
    ...
    Public Sub RunAlg(src As cvb.Mat)
        ...
        Dim r1 = New cvb.Rect(options.pixels, 0, dst2.Width - options.pixels, dst2.Height)
        Dim r2 = New cvb.Rect(0, 0, dst2.Width - options.pixels, dst2.Height)
        ...
        dst2.SetTo(0, task.noDepthMask)
        dst2(New cvb.Rect(0, dst2.Height - options.pixels, dst2.Width, options.pixels)).SetTo(0)
...
Public Class PointCloud_PCpointsMask : Inherits VB_Parent
    ...
    End Sub
    Public Sub RunAlg(src As cvb.Mat)
        If tInfo.optionsChanged Then pcPoints = New cvb.Mat(task.gridRows, task.gridCols, cvb.MatType.CV_32FC3, cvb.Scalar.All(0))
...
Public Class PointCloud_PCPointsPlane : Inherits VB_Parent
    ...
    Dim white32 = New cvb.Point3f(1, 1, 1)
    Public Sub New()
        setPointCloudGrid()
...
Public Class PointCloud_Inspector : Inherits VB_Parent
    ...
    Public Sub RunAlg(src As cvb.Mat)
        ...
        Dim topPt = New cvb.Point2f(cLine, 0)
        Dim botPt = New cvb.Point2f(cLine, dst2.Height)
        dst2 = task.depthRGB
        ...
        For i = 1 To yLines - 1
            ...
            SetTrueText("Row " + CStr(i) + vbTab + Format(xyz(0), fmt2) + vbTab + Format(xyz(1), fmt2) + vbTab + Format(xyz(2), fmt2), New cvb.Point(5, pt.Y), 3)
        Next
...
Public Class PointCloud_Histograms : Inherits VB_Parent
    ...
    Public Sub RunAlg(src As cvb.Mat)
        ...
        cvb.Cv2.CalcHist({task.pointCloud}, task.redOptions.channels, New cvb.Mat(), histogram, task.redOptions.channelCount,
        ...
        Select Case task.redOptions.PointCloudReduction
            ...
            Case 6 ' "XYZ Reduction"
                If dst2.Type <> cvb.MatType.CV_8U Then dst2 = New cvb.Mat(dst2.Size(), cvb.MatType.CV_8U)
...
Public Class PointCloud_ReduceSplit2 : Inherits VB_Parent
    ...
    Public Sub RunAlg(src As cvb.Mat)
        ...
        dst2.ConvertTo(dst2, cvb.MatType.CV_32S)
        reduction.Run(dst2)
        reduction.dst2.ConvertTo(dst1, cvb.MatType.CV_32F)
        dst1 *= 0.001
        ...
        Else
            ...
            cvb.Cv2.Merge({task.pcSplit(0), task.pcSplit(1), dst1}, dst3)
        End If
    End Sub
End Class
...
Public Class PointCloud_ReducedTopView : Inherits VB_Parent
    ...
    Public Sub RunAlg(src As cvb.Mat)
        ...
        cvb.Cv2.CalcHist({split2.dst3}, task.channelsTop, New cvb.Mat, dst1, 2, task.bins2D, task.rangesTop)

        dst1 = dst1.Flip(cvb.FlipMode.X)
...
Public Class PointCloud_ReducedSideView : Inherits VB_Parent
    ...
    Public Sub RunAlg(src As cvb.Mat)
        ...
        cvb.Cv2.CalcHist({split2.dst3}, task.channelsSide, New cvb.Mat, dst1, 2, task.bins2D, task.rangesSide)
