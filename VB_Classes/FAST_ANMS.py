...
parser.add_argument("--num_ret_points", type=int, default=750)
...
try:
    ...
    cv2.drawKeypoints(img2, keypoints, img2, color=(255, 0, 0))
    ...
    selected_keypoints = ssc(keypoints, args.num_ret_points, args.tolerance, img2.shape[1], img2.shape[0])

    cv2.drawKeypoints(img3, selected_keypoints, img3, color=(0, 255, 0))

    CombinedImages = cv2.hconcat([img, img2])
    CombinedImagesAll = cv2.hconcat([CombinedImages, img3])

    cv2.imshow("Original, FAST keypoints, and Selected keypoints", CombinedImagesAll)
    cv2.waitKey(0)
