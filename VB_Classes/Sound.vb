...
Public Class Sound_Display
    ...
    Public Sub Run()
        ...
        If useGenerated <> check.Box(0).Checked Or soundSource Is Nothing Then
            useGenerated = check.Box(0).Checked
            If soundSource IsNot Nothing Then soundSource.dispose()
            If check.Box(0).Checked Then
                soundSource = New Sound_SignalGenerator
                sliderPercent = 0
            Else
                soundSource = New Sound_Basics
            End If
            starttime = Now
        End If

        Static fileStarted As Boolean
        If check.Box(0).Checked Or fileStarted = False Then
            fileStarted = True
            soundSource.Run()
            ...
            Dim samplesPerLine = If(soundSource.stereo, totalSamples / 2 / dst1.Width, totalSamples / dst1.Width)
        ...
        If sliderPercent > 0.99 Or fileStarted = False Then
            soundSource.close()
            soundSource = Nothing ' this will restart the audio
        End If
...
Public Class Sound_DisplayOld
    ...
    Public Sub Run()
        ...
        If check.Box(0).Checked Or task.fileStarted Then
            ...
            Dim formatIndex As Integer
            Static frm = findfrm("Sound_DisplayOld Radio Options")
            For i = 0 To frm.check.length - 1
                If frm.check(i).Checked Then formatIndex = i
            Next
