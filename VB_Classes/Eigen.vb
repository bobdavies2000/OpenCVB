...
Public Class Eigen_Basics : Inherits VB_Parent
    ...
    Public Sub RunVB(src as cvb.Mat)
        ...
        Dim mat As cvb.Mat = cvb.Mat.FromPixelData(5, 5, cvb.MatType.CV_64FC1, a)
        Dim eigenVal As New cvb.Mat, eigenVec As New cvb.Mat
        cvb.Cv2.Eigen(mat, eigenVal, eigenVec)
    ...
    Public Sub RunVB(src as cvb.Mat)
        ...
        If task.heartBeat Then
            ...
            For i = 0 To options.randomCount - 1
                Dim pt = New cvb.Point2f(Rnd() * width, Rnd() * height)
            ...
            If Rnd() * 2 - 1 >= 0 Then
                p1 = New cvb.Point(Rnd() * width, 0)
                p2 = New cvb.Point(Rnd() * width, height)
            Else
                p1 = New cvb.Point(0, Rnd() * height)
                p2 = New cvb.Point(width, Rnd() * height)
            End If
            ...
            For i = 0 To options.linePairCount - 1
                ...
                Dim pt = New cvb.Point(startx + i * incr + noiseOffsetX, Math.Max(0, Math.Min(m * (startx + i * incr) + bb + noiseOffsetY, height)))
                If pt.X < 0 Then pt.X = 0
                If pt.X > width Then pt.X = width
                If pt.Y < 0 Then pt.Y = 0
                If pt.Y > height Then pt.Y = height
...
Public Class Eigen_Fitline : Inherits VB_Parent
    ...
    Public Sub RunVB(src as cvb.Mat)
        ...
        Dim line = cvb.Cv2.FitLine(noisyLine.points, cvb.DistanceTypes.L2, 1, 0.01, 0.01)
        Dim m = line.Vy / line.Vx
        Dim bb = line.Y1 - m * line.X1
        Dim p1 = New cvb.Point(0, bb)
        Dim p2 = New cvb.Point(width, m * width + bb)
        dst2.Line(p1, p2, cvb.Scalar.Red, 20, task.lineType)

        Dim pointMat = cvb.Mat.FromPixelData(noisyLine.options.randomCount, 1, cvb.MatType.CV_32FC2, noisyLine.points.ToArray)
        Dim mean = pointMat.Mean()
        Dim split() = pointMat.Split()
        Dim mmX = GetMinMax(split(0))
        Dim mmY = GetMinMax(split(1))

        Dim eigenInput As New cvb.Vec4f
        ...
        Dim vec4f As New List(Of cvb.Point2f)
        vec4f.Add(New cvb.Point2f(eigenInput(0), eigenInput(1)))
        vec4f.Add(New cvb.Point2f(eigenInput(1), eigenInput(3)))

        Dim D = cvb.Mat.FromPixelData(2, 2, cvb.MatType.CV_32FC1, vec4f.ToArray)
        cvb.Cv2.Eigen(D, eigenVal, eigenVec)
        ...
        p1 = New cvb.Point2f(mean.Val0 - Math.Cos(theta) * len / 2, mean.Val1 - Math.Sin(theta) * len / 2)
        p2 = New cvb.Point2f(mean.Val0 + Math.Cos(theta) * len / 2, mean.Val1 + Math.Sin(theta) * len / 2)
        ...
        Else
            ...
            p2 = New cvb.Point2f(mean.Val0 + Math.Cos(-theta) * len / 2, mean.Val1 + Math.Sin(-theta) * len / 2)
            m2 = (p2.Y - p1.Y) / (p2.X - p1.X)
            dst2.Line(p1, p2, cvb.Scalar.Yellow, 10, task.lineType)
        End If
        p1 = New cvb.Point(0, noisyLine.bb)
        p2 = New cvb.Point(width, noisyLine.m * width + noisyLine.bb)
        dst2.Line(p1, p2, cvb.Scalar.Blue, task.lineWidth + 2, task.lineType)
