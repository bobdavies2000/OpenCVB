...
Public Class Correlation_Basics : Inherits VB_Parent
    ...
    Public Sub RunAlg(src As cvb.Mat)
        ...
        If row = 0 Then SetTrueText("Move mouse across image to see the relationship between X and Z" + vbCrLf +
                                    "A linear relationship is a useful correlation", New cvb.Point(0, 10), 3)

        Dim dataX As New cvb.Mat(New cvb.Size(src.Width, src.Height), cvb.MatType.CV_32F, cvb.Scalar.All(0))
        Dim dataY As New cvb.Mat(New cvb.Size(src.Width, src.Height), cvb.MatType.CV_32F, cvb.Scalar.All(0))
        Dim dataZ As New cvb.Mat(New cvb.Size(src.Width, src.Height), cvb.MatType.CV_32F, cvb.Scalar.All(0))

        Dim mask = kFlood.dst3.CvtColor(cvb.ColorConversionCodes.BGR2Gray)
        ...
        dst2.Line(New cvb.Point(0, row), New cvb.Point(dst2.Width, row), cvb.Scalar.Yellow, task.lineWidth + 1)

        Dim correlationmat As New cvb.Mat
        cvb.Cv2.MatchTemplate(row1, row2, correlationmat, options.matchOption)
        ...
        If plotX.Count > 0 Then
            ...
            For i = 0 To plotX.Count - 1
                ...
                DrawCircle(dst3,New cvb.Point(x, y), task.DotSize, cvb.Scalar.Yellow)
            Next
            SetTrueText("Z-min " + Format(minZ, fmt2), New cvb.Point(10, 5), 3)
            SetTrueText("Z-max " + Format(maxZ, fmt2) + vbCrLf + vbTab + "X-min " + Format(minx, fmt2), New cvb.Point(0, dst3.Height - 20), 3)
            SetTrueText("X-max " + Format(maxx, fmt2), New cvb.Point(dst3.Width - 40, dst3.Height - 10), 3)
        End If
    End Sub
End Class