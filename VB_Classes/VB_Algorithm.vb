...
Public Class VB_Algorithm : Implements IDisposable
    ...
    Public Sub identifyCellRects(redCells As List(Of rcData), Optional dstSetting As Integer = 2)
        For Each rc In redCells
            '  If rc.index > 0 And rc.index <= redOptions.identifyCount Then setTrueText(CStr(rc.index), rc.maxDStable, dstSetting)
            If rc.index > 0 Then setTrueText(CStr(rc.index), rc.maxDStable, dstSetting)
        Next
    End Sub
