...
Public Class Mat_Repeat : Inherits TaskParent
    ...
    Public Overrides sub runAlg(src As cvb.Mat)
        Dim small = src.Resize(New cvb.Size(src.Cols / 10, src.Rows / 10))
        dst2 = small.Repeat(10, 10)
        small = task.depthRGB.Resize(New cvb.Size(src.Cols / 10, src.Rows / 10))
...
Public Class Mat_PointToMat : Inherits TaskParent
    ...
    Public Overrides sub runAlg(src As cvb.Mat)
        ...
        Dim pMat = cvb.Mat.FromPixelData(rows, 1, cvb.MatType.CV_32FC2, random.PointList.ToArray)
        Dim indexer = pMat.GetGenericIndexer(Of cvb.Vec2f)()
        dst3.SetTo(0)
        Dim white = New cvb.Vec3b(255, 255, 255)
        For i = 0 To rows - 1
            dst3.Set(Of cvb.Vec3b)(indexer(i)(1), indexer(i)(0), white)
        Next
    End Sub
End Class
...
Public Class Mat_MatToPoint : Inherits TaskParent
    ...
    End Sub
    Public Overrides sub runAlg(src As cvb.Mat)
        Dim points(src.Total - 1) As cvb.Vec3b
        Dim vec As New cvb.Vec3b
        Dim index As Integer = 0
        Dim m3b = src.Clone()
        Dim indexer = m3b.GetGenericIndexer(Of cvb.Vec3b)()
        For y = 0 To src.Rows - 1
            For x = 0 To src.Cols - 1
                vec = indexer(y, x)
                points(index) = New cvb.Vec3b(vec(0), vec(1), vec(2))
                index += 1
            Next
        Next
        dst2 = cvb.Mat.FromPixelData(src.Rows, src.Cols, cvb.MatType.CV_8UC3, points)
    End Sub
End Class
...
Public Class Mat_Transpose : Inherits TaskParent
    ...
    Public Overrides sub runAlg(src As cvb.Mat)
        Dim trColor = src.T()
        dst2 = trColor.ToMat.Resize(New cvb.Size(src.Cols, src.Rows))
        Dim trBack = dst2.T()
...
Public Class Mat_Tricks : Inherits TaskParent
    ...
    End Sub
    Public Overrides sub runAlg(src As cvb.Mat)
        Dim mat = src.Resize(New cvb.Size(src.Height, src.Height))
        Dim roi = New cvb.Rect(0, 0, mat.Width, mat.Height)
        dst2(roi) = mat
        dst3(roi) = mat.T
    End Sub
End Class
...
Public Class Mat_RowColRange : Inherits TaskParent
    ...
    Public Overrides sub runAlg(src As cvb.Mat)
        ...
        dst2 = src
        cvb.Cv2.BitwiseNot(dst2.RowRange(midY - 25, midY + 25), dst2.RowRange(midY - 25, midY + 25))
        cvb.Cv2.BitwiseNot(dst2.ColRange(midX - 25, midX + 25), dst2.ColRange(midX - 25, midX + 25))
    End Sub
End Class
...
Public Class Mat_Managed : Inherits TaskParent
    ...
    Public Sub New()
        labels(2) = "Color change is in the managed cvb.vec3b array"
        desc = "There is a limited ability to use Mat data in Managed code directly."
    End Sub
    Public Overrides sub runAlg(src As cvb.Mat)
        dst2 = cvb.Mat.FromPixelData(src.Rows, src.Cols, cvb.MatType.CV_8UC3, img)
        If task.heartBeat Then
            If nextColor = New cvb.Vec3b(0, 0, 255) Then nextColor = New cvb.Vec3b(0, 255, 0) Else nextColor = New cvb.Vec3b(0, 0, 255)
        End If
        For i = 0 To img.Length - 1
            img(i) = nextColor
        Next
        Dim rect As New cvb.Rect(autoRand.Next(0, src.Width - 50), autoRand.Next(0, src.Height - 50), 50, 50)
        dst2(rect).SetTo(0)
    End Sub
End Class
...
Public Class Mat_Inverse : Inherits TaskParent
    ...
    Public Overrides sub runAlg(src As cvb.Mat)
        ...
        Dim input = cvb.Mat.FromPixelData(3, 3, cvb.MatType.CV_32F, matrix)
        cvb.Cv2.Invert(input, inverse, options.decompType)
...
Public Class Mat_Inverse_4D : Inherits TaskParent
    ...
    Public Sub New()
        input = cvb.Mat.FromPixelData(4, 4, cvb.MatType.CV_64F, defaultInput)
        desc = "Use OpenCV to invert a matrix"
    End Sub
    Private Function printMatrixResults(src As cvb.Mat, dst2 As cvb.Mat) As String
    ...
    Public Overrides sub runAlg(src As cvb.Mat)
        ...
        Dim result As New cvb.Mat
        cvb.Cv2.Invert(input, result, cvb.DecompTypes.LU)
...
'        If src.Type = cvb.MatType.CV_8U Then
...
'            dst2 = New cvb.Mat(dGray.Rows, dGray.Columns, cvb.MatType.CV_8U, cvb.Scalar.All(0))
'            Marshal.Copy(dGray.ToBytes, 0, dst2.Data, dst2.Total)
'        End If

'        If dRGB IsNot Nothing Then
'            dst3 = New cvb.Mat(dRGB.Rows, dRGB.Columns, cvb.MatType.CV_8UC3)
...
Public Class Mat_2to1 : Inherits TaskParent
    ...
    Public Sub New()
        mat1 = New cvb.Mat(New cvb.Size(dst2.Rows, dst2.Cols), cvb.MatType.CV_8UC3, cvb.Scalar.All(0))
        mat2 = mat1.Clone()
        mat = {mat1, mat2}
    ...
    Public Overrides sub runAlg(src As cvb.Mat)
        ...
        If mat(0) IsNot Nothing Then
            If dst2.Type <> mat(0).Type Then dst2 = New cvb.Mat(dst2.Size(), mat(0).Type)
            For i = 0 To 1
...
Public Class Mat_4to1 : Inherits TaskParent
    ...
    End Sub
    Public Sub defaultMats(src As cvb.Mat)
        Dim tmpLeft = If(task.leftView.Channels() = 1, task.leftView.CvtColor(cvb.ColorConversionCodes.GRAY2BGR),
                         task.leftView)
        Dim tmpRight = If(task.rightView.Channels() = 1, task.rightView.CvtColor(cvb.ColorConversionCodes.GRAY2BGR),
                          task.rightView)
        mat = {task.color.Clone, task.depthRGB.Clone, tmpLeft, tmpRight}
    End Sub
    Public Overrides sub runAlg(src As cvb.Mat)
        Dim nSize = New cvb.Size(dst2.Width / 2, dst2.Height / 2)
        Dim roiTopLeft = New cvb.Rect(0, 0, nSize.Width, nSize.Height)
        Dim roiTopRight = New cvb.Rect(nSize.Width, 0, nSize.Width, nSize.Height)
        Dim roibotLeft = New cvb.Rect(0, nSize.Height, nSize.Width, nSize.Height)
        Dim roibotRight = New cvb.Rect(nSize.Width, nSize.Height, nSize.Width, nSize.Height)
        If standalone Then defaultMats(src)

        dst2 = New cvb.Mat(dst2.Size(), cvb.MatType.CV_8UC3)
        For i = 0 To 4 - 1
            Dim tmp = mat(i).Clone
            If tmp.Channels() = 1 Then tmp = mat(i).CvtColor(cvb.ColorConversionCodes.GRAY2BGR)
...
Public Class Mat_ToList : Inherits TaskParent
    ...
    Public Overrides sub runAlg(src As cvb.Mat)
        ...
        dst2 = histTop.histogram.Threshold(task.projectionThreshold, 255, cvb.ThresholdTypes.Binary).ConvertScaleAbs

        Dim ptList As New List(Of cvb.Point)
        If task.gOptions.DebugChecked Then
            For y = 0 To dst2.Height - 1
                For x = 0 To dst2.Width - 1
                    If dst2.Get(Of Byte)(y, x) <> 0 Then ptList.Add(New cvb.Point(x, y))
                Next
            Next
        Else
            Dim points = dst2.FindNonZero()
            For i = 0 To points.Rows - 1
                ptList.Add(points.Get(Of cvb.Point)(i, 0))
            Next
        End If
