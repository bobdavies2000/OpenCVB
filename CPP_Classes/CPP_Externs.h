                   ...
                   int rectX, int rectY, int rectWidth, int rectHeight, bool displayDst0, bool displayDst1)
{
    ...
    case fDelaunay_Basics:
    {task->alg = new CPP_Delaunay_Basics(rows, cols);break;}
    case fDelaunay_GenerationsNoKNN:
...
int* cppTask_Close(cppTask * task)
{
    ...
    if (task->cppFunction != fDelaunay_GenerationsNoKNN) delete task;  // why does fDelaunay_GenerationsNoKNN fail.  Skipping it for now.
    return (int*)0;
}
                    ...
                    bool optionsChanged, bool heartBeat, bool displayDst0, bool displayDst1, float addWeightedPercent)
{
    ...
    if (task->alg->dst0.type() == CV_8U) cvtColor(task->alg->dst0, task->xdst0, COLOR_GRAY2BGR);
    if (task->alg->dst1.type() == CV_8U) cvtColor(task->alg->dst1, task->xdst1, COLOR_GRAY2BGR);
    if (task->alg->dst2.type() == CV_8U) cvtColor(task->alg->dst2, task->xdst2, COLOR_GRAY2BGR);
    if (task->alg->dst3.type() == CV_8U) cvtColor(task->alg->dst3, task->xdst3, COLOR_GRAY2BGR);
